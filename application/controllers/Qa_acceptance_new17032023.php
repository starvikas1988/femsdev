...
// die;
				   ...
				   if($ol['accept_in_24']!=0 || $ol['approved_audit']!=0){
					...
					$not_accept_per = number_format(($ol['total_not_accept']/$ol['approved_audit'])*100,2);
					 ...
					 if($ttl['total_in_24']==0 || $ttl['approved_audit']==0){
					...
					if($ttl['total_accept']==0 || $ttl['approved_audit']==0){
					...
					if($ttl['total_not_accept']==0 || $ttl['approved_audit']==0){
		...
		if(check_logged_in()){
			...
			if(!empty($rowDefect)){
				...
				foreach($rowDefect as $defect){
					...
					//echo $overall_sql;die;

					$overall_data[] = $this->Common_model->get_query_row_array($overall_sql);

					// $campaign_wise_sql = "select count(t.id) as total_feedback,t.lob_campaign,
					// sum(case when t.audit_status = 1 then 1 else 0 end) as approved_audit,
					// sum(case when t.agnt_fd_acpt = 'Accepted' then 1 else 0 end) as accept_count,
					// sum(case when t.agnt_fd_acpt = 'Not Accepted' then 1 else 0 end) as rebuttal_count,
					// sum(case when t.agnt_fd_acpt is null and t.audit_status = 1 then 1 else 0 end) as not_accepted_count,
					...
					// $campaign_wise_sql[] = $this->Common_model->get_query_result_array($campaign_wise_sql);
				}
			}	
		// echo"<pre>";
		// print_r($overall_data);
...
$row_gt_agent++;
	...
	public function get_campaign_by_process(){
		...
		if(check_logged_in()){
			...
			$sql = "SELECT qd.id as campaign_id,REPLACE(REPLACE(REPLACE(qd.table_name,'qa_',''),'_feedback',''),'_',' ') as campaign_name FROM qa_defect qd WHERE qd.id <> 0 AND qd.is_active = 1 AND qd.client_id = $client_id";
