...
//////////////// management filtering data /source///////////////////////
	...
	public function qaod_management_sorting_feedback()
	{
		if(check_logged_in()){
			...
			$data["od_business_direct_list"] = $this->Qa_od_model->get_business_direct_data($field_array);
	...
	public function add_edit_od_business_direct($od_business_id){
		if(check_logged_in())
		{
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_od/add_edit_od_business_direct.php";
			$data["content_js"] = "qa_universal_js.php";
			$data['od_business_id']=$od_business_id;
			$tl_mgnt_cond='';
			if(get_role_dir()=='manager' && get_dept_folder()=='operations'){
				$tl_mgnt_cond=" and (assigned_to='$current_user' OR assigned_to in (SELECT id FROM signin where assigned_to ='$current_user'))";
			}else if(get_role_dir()=='tl' && get_dept_folder()=='operations'){
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name
				from qa_od_business_direct_feedback where id='$od_business_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["od_business"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($field_array['agent_id']){
				...
				if($od_business_id==0){
					...
					$rowid= data_inserter('qa_od_business_direct_feedback',$field_array);
				///////////
					if(get_login_type()=="client"){
						$add_array = array("client_entryby" => $current_user);
					}else{
					...
					$this->db->where('id', $rowid);
					$this->db->update('qa_od_business_direct_feedback',$add_array);
					...
					$this->db->where('id', $od_business_id);
					$this->db->update('qa_od_business_direct_feedback',$field_array1);
					/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
							"client_rvw_by" => $current_user,
					...
					$this->db->where('id', $od_business_id);
					$this->db->update('qa_od_business_direct_feedback',$edit_array);
...
//////////////////Agent QA OD Feedback///////////////////////
	...
	public function qaod_agent_sorting_feedback()
	{
		if(check_logged_in())
		{
			...
			if($campaign=='old_chat'){
				...
				$data["total_review_needed"] =  $this->Common_model->get_single_value($qSql);
			}else if($campaign=='business_direct_call'){
				$qSql="Select count(id) as value from qa_od_business_direct_feedback where agent_id = '$current_user' And audit_type not in ('Calibration', 'Pre-Certificate Mock Call', 'Certification Audit')";
				$data["total_feedback"] =  $this->Common_model->get_single_value($qSql);
				$qSql="Select count(id) as value from qa_od_business_direct_feedback where agent_id='$current_user' And audit_type not in ('Calibration', 'Pre-Certificate Mock Call', 'Certification Audit') and agent_rvw_date is Null";
		...
		if($this->input->get('btnView')=='View')
		{
			...
			if($campaign=='od_voice'){
				...
				$data["get_agent_review_list"] = $this->Qa_od_model->get_agent_business_direct_data($field_array);
			}	
	...
	public function qaod_agent_business_direct_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			$data["aside_template"] = "qa/aside.php";
			//$data["content_template"] = "qa_od/qaod_agent_voice_rvw.php";
			$data["content_template"] = "qa_od/qaod_agent_business_direct_rvw.php";
			$data["content_js"] = "qa_universal_js.php";
			$data["agentUrl"] = "qa_od/qaod_agent_sorting_feedback";
			...
			$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_od_business_direct_feedback where id='$id') xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["od_business"] = $this->Common_model->get_query_row_array($qSql);
			
			$data["od_business_id"]=$id;
			
			if($this->input->post('od_business_id'))
			{
				$pnid=$this->input->post('od_business_id');
				$curDateTime=CurrMySqlDate();
				$log=get_logs();
				...
				$this->db->where('id', $pnid);
				$this->db->update('qa_od_business_direct_feedback',$field_array);
