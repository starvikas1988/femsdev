...
class Qa_graph extends CI_Controller {
	...
	public function index()
	{
		if(check_logged_in())
		{
			...
			if(get_dept_folder()=="operations"){
				...
				if(empty($user_process_ids)){ $user_process_ids = '0'; }
			...
			if($search_office == "ALL"){ $search_office = ""; }
			...
			if(!$searchFilter){			
				...
				////========== QA DEFECT - PROGRAMS ===========////
				$sqlTables = "SELECT * from qa_defect";
				...
				foreach($queryTables as $token)
				{
					...
					$officeFilter = "";
					if(!empty($search_office)){ $officeFilter = " AND s.office_id IN ('$search_office')"; } 
					...
					$sql_qaScore = "SELECT count(*) as auditCount, avg(qa.overall_score) as score, p.name as process_name, c.shname as client_name from $currentTable as qa
								  ...
								  WHERE qa.audit_type = 'CQ Audit' AND (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') $officeFilter";
				...
				foreach($allData as $token)
				{
					if($token['score'] > 0){						
						...
						if($counterCheck <= 10){						
							...
							$officeFilter = "";
							if(!empty($search_office)){ $officeFilter = " AND office_id IN ('$search_office')"; } 
							
							// GET TARGET SCORE
							$processID = $token['process_id'];
							$sqlTarget = "SELECT * from target_qa_score WHERE process_id = '$processID' AND month = '$search_month' AND year = '$search_year'  $officeFilter ORDER by id DESC LIMIT 1";
				...
				//echo "<pre>".print_r($qa_table_name, 1)."</pre>";
				//echo "<pre>".print_r($allData, 1)."</pre>";
				//die();
				
				////=========== TOTAL AUDIT =====================/////
				/*$final_audit_count = "0";				
				$sql_defect   = "SELECT table_name from qa_defect";
				$query_defect = $this->Common_model->get_query_result_array($sql_defect);
				foreach($query_defect as $token_defect){
					$tablename = $token_defect['table_name'];
					$count_audit = "SELECT count(*) as value from $tablename";
					$current_count = $this->Common_model->get_single_value($count_audit);
					$final_audit_count = $final_audit_count + $current_count;
				}*/
				
				//$sql_account   = "SELECT count(*) as value from v_qa_score as qa WHERE qa.audit_date >= '$start_date' AND qa.audit_date <= '$end_date' GROUP BY process_id";
				//$query_account = $this->Common_model->get_single_value($sql_account);
				
				/*--
				$sql_audit   = "SELECT count(*) as value from v_qa_score as qa WHERE CAST(qa.audit_date as CHAR(10))  >= '$start_date' AND CAST(qa.audit_date as CHAR(10)) <= '$end_date' $extra_client_filter $extra_process_filter";
				
				$final_audit_count = $this->Common_model->get_single_value($sql_audit);
				
				$data['total_account'] = $query_account;
				$data['total_audit'] = $final_audit_count;
				
				////=========== TOP AVERAGE QA SCORE =====================/////
				
				//-- GET AVERAGE TOTAL TOP QA SCORES
				$sql_qa = "SELECT qa.agent_id, concat(s.fname, ' ', s.lname) as agent_name, c.fullname as client_full, p.name as process_name, qa.client_id, qa.process_id, avg(overall_score) as average_score FROM v_qa_score as qa
				LEFT JOIN client as c ON c.id = qa.client_id
				LEFT JOIN process as p ON p.id = qa.process_id
				LEFT JOIN signin as s ON s.id = qa.agent_id 
				WHERE qa.client_id = c.id AND qa.process_id = p.id AND (CAST(qa.audit_date as CHAR(10)) >= '$start_date' AND CAST(qa.audit_date as CHAR(10)) <= '$end_date') 
				$extra_client_filter $extra_process_filter
				GROUP BY qa.client_id, qa.process_id
				ORDER by average_score DESC LIMIT 10";			
				$query_qa = $this->Common_model->get_query_result_array($sql_qa);
				
				foreach($query_qa as $token)
				{
					// QA SCORE
					$qa_data_value_array[] = round($token['average_score'],2);
					$qa_data_label_array[] = $token['process_name']; 					
					$qa_data_process_array[] = $token['process_id']; 
					
					// GET TARGET SCORE
					$processID = $token['process_id'];
					$sqlTarget = "SELECT * from target_qa_score WHERE process_id = '$processID' AND month = '$search_month' AND year = '$search_year' ORDER by id DESC LIMIT 1";
					$queryTarget = $this->Common_model->get_query_row_array($sqlTarget);
					$target_score = $queryTarget['target_score'];
					if(empty($target_score)){ $target_score = 0; }
					$qa_data_target_array[] = $target_score;
					...
					$current_process_id   = $token['process_id'];
					$current_process_name = $token['process_name'];
					$csatscore = $this->get_avg_csat_process_score_view_based($current_process_id, $office_location, $start_date, $end_date);
					$npsscore = $this->get_avg_nps_process_score_view_based($current_process_id, $office_location, $start_date, $end_date);
					$data_csat_value_array[] = round($csatscore['datas'],2);
					$data_nps_value_array[] = round($npsscore['datas'],2);
					---*/
					...
					$current_process_id   = $token['process_id'];
					$current_process_name = $token['process_name'];
					$current_table_name = $this->Common_model->get_single_value("SELECT table_name as value from qa_defect where FIND_IN_SET($current_process_id,process_id)");
					if($current_table_name == ""){
						$current_table_name = $this->Common_model->get_single_value("SELECT table_name as value from qa_defect where FIND_IN_SET($current_process_id,floor(process_id))");
					}
					
					// VOC SCORE
					if($current_table_name != ""){
						$csatscore = $this->get_avg_csat_process_score($current_table_name, $current_process_name, $office_location, $start_date, $end_date);
						$npsscore = $this->get_avg_nps_process_score($current_table_name, $current_process_name, $office_location, $start_date, $end_date);
						$data_csat_value_array[] = round($csatscore['datas'],2);
						$data_nps_value_array[] = round($npsscore['datas'],2);
					}
					if($current_table_name == ""){
						$data_csat_value_array[] = "0";
						$data_nps_value_array[] = "0";
					}
					*/
				...
				/*
				$csat = NULL;				
				////=========== TOP AVERAGE CSAT - NPS SCORE =====================/////
				$sql_voc_tables = "SELECT table_name, process_id, client_id FROM qa_defect";
				$query_voc_tables = $this->Common_model->get_query_result_array($sql_voc_tables);
				foreach($query_voc_tables as $tokentable)
				{
					$c_table_name = $tokentable['table_name'];
					$c_process_id = $tokentable['process_id'];
					$c_client_id = $tokentable['client_id'];
					
					$c_process_ar = explode(',',$c_process_id);
					$c_process_ar_count = count($c_process_ar);
					for($i=0; $i<$c_process_ar_count; $i++)
					{
						$current_process_id = $c_process_ar[$i];
						$current_process_id_floor = floor($c_process_ar[$i]);
						$current_process_name = $this->Common_model->get_single_value("SELECT name as value from process where id = '$current_process_id_floor'");
						
						$csat[$current_process_id]['name'] = $current_process_name;
						
						// GET PROCESS CSAT SCORE
						$csatscore = $this->get_avg_csat_process_score($c_table_name, $current_process_name, $office_location);
						$csat['csat'][$current_process_id] = $csatscore['datas'];
					
						// GET PROCESS NPS SCORE
						$npsscore = $this->get_avg_nps_process_score($c_table_name, $current_process_name, $office_location);
						$csat['nps'][$current_process_id] = $npsscore['datas'];
					}
				}
				
				//echo "<pre>" .print_r($csat, true) ."</pre>";die();
				$new_csat = $csat['csat'];
				$new_nps  = $csat['nps'];
				$csat_max = arsort($new_csat);
				$nps_max = arsort($new_nps);
				$count = 0;
				foreach($new_csat as $key=>$value)
				{
					$count++;
					if($count<=10){
					$csat_max_values[] = round($value,2);
					$csat_max_keys_name[] = $csat[$key]['name'];
					$csat_max_keys_id[] = $key;
					}
				}
				$data['csat_graph_max_keys_id'] = implode(',',$csat_max_keys_id);
				$data['csat_graph_max_value'] = implode(',',$csat_max_values);
				$data['csat_graph_max_keys_name'] = implode('","',$csat_max_keys_name);
				*/
					...
					if($currentGraphType == 'weekly')
					{
						if(strtotime($search_end_date) > strtotime($search_start_date))
						{
							...
							while(strtotime($checkerDate) <= strtotime($search_end_date))
							{
								...
								$weekArray[$weekCount]['fullname'] = "Week " .$weekCount;
								//." (" .$start_date ." - " .$end_date.")";
	...
	public function program_level()
	{
		if(check_logged_in())
		{
			...
			if(!empty($search_process_id))
			{
				...
				////print_r($paramColumns);
				$paramsArray = explode(',', $paramColumns);
				foreach($paramsArray as $token)
				{
					$keyparam = 'qa.'.$token;
					$flagExecuter = true;
					if($search_process_id == '285' && $token == 'auto_fail'){
						$flagExecuter = false;
					}
					if($flagExecuter == true){
						...
						if($get_defect_columns['process_id'] == "171" || $get_defect_columns['process_id'] == "181"){
						 ...
						 $sqlToken = "SELECT count(*) as auditCount from $c_defect_table as qa 
									...
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND ".$keyparam." IN (".$errorChecker.")  $extraUserFilter";						
						...
						$sqlToken = "SELECT CONCAT(s.fname, ' ', s.lname) as agent_name, s.fusion_id, count(*) as auditCount from $c_defect_table as qa 
						            INNER JOIN signin as s ON s.id = qa.agent_id LEFT JOIN signin as sl ON sl.id = s.assigned_to
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND ".$keyparam." IN (".$errorChecker.")  $extraUserFilter GROUP BY qa.agent_id ORDER BY auditCount DESC LIMIT 5";
	...
	public function defect_level()
	{
		if(check_logged_in())
		{
			...
			if(!empty($search_process_id))
			{
				...
				$get_defect_columns = $this->Common_model->get_query_row_array($sql_defect_columns);
				$c_defect_table  =  trim($get_defect_columns['table_name']);
				$c_client_id  =  $get_defect_columns['client_id'];
				...
				// GET CUSTOM PARAMS
				$errorCheckGrant = true; 
				$errorNoCheck = "'0', 'No', 'no', 'Unacceptable', 'Fail', 'Absent', 'Action needed','Below Expectation','Well below Expectation','AFD','50'";
				$errorYesCheck = "'1', 'Yes', 'yes', 'Acceptable', 'Pass', 'Awesome', 'Average','Above Expectation','Meeting Expectation','100'";
				$errorNACheck = "'N/A', 'NA', 'na', 'n/a'";
				if($search_process_id == '380'){
					$errorNoCheck = "'0','Action needed','Below Expectation','Well below Expectation'";
					$errorYesCheck = "'1','Above Expectation','Meeting Expectation'";
					$errorNACheck = "'N/A', 'NA', 'na', 'n/a'";
				}
				$customParams = array();
				if($get_defect_columns['process_id'] == "171" || $get_defect_columns['process_id'] == "181"){
				...
				$excludeParams  = "product_described,describe_replacement,describe_PP_replacement,not_use_reinstate,identified_signs,good_working_order,brand_not_known_asked,agent_provide_saving_infoconfirm_name_address,total_cost_confirm,confirm_registered_card,payment_frequency_confirm,DD_script_read_customer,payment_unsuccessful,TC_sent_in_3_5_days,cancellation_rights_read,question_asked_statement_read,wording_of_policy_premium,script_for_limited_cover_read,status_accurately_recorded,company_name_stated_3times";
				$excludeParamArray = explode(',', $excludeParams);
				...
				{					
					...
					if($search_process_id == '380'){
						if(in_array($token, $excludeParamArray)){
							$flagExecuter = false;
						}
					}	
					...
					//==== CASABLANCE CLIENTS
					$casablanca_clients = $this->get_casablanca_client();	
					$ideal_living_clients = $this->get_ideal_living_client();
					
					if(in_array($c_client_id, $casablanca_clients['client']))
					{
						...
						$sqlToken = "SELECT count(*) as auditCount from $c_defect_table as qa 
									INNER JOIN signin as s ON s.id = qa.agent_id LEFT JOIN signin as sl ON sl.id = s.assigned_to
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND qa.$token IN (".$errorType.") $extraUserFilter";					
						$c_qaScore = $this->Common_model->get_query_row_array($sqlToken);
						$c_ParamCount = !empty($c_qaScore['auditCount']) ? $c_qaScore['auditCount'] : 0;					
						$paramsPercent = 0;
						if($totalAuditCount > 0){
						...
						$params2['count'][$token][100] = $c_ParamCount;
						$params2['percent'][$token][100] = $paramsPercent;
						...
						$params2['count'][$token][50] = $c_ParamCount;
						$params2['percent'][$token][50] = $paramsPercent;
						...
						$params2['count'][$token]['score'] = $c_ParamCount;
						$params2['percent'][$token]['score'] = $paramsPercent;
						$params2['parameters'][$token] = $paramsPercent;
						...
						$params2['count'][$token][0] = $c_ParamCount;
						$params2['percent'][$token][0] = $paramsPercent;
						...
						$params2['count'][$token]['NIA'] = $c_ParamCount;
						$params2['percent'][$token]['NIA'] = $paramsPercent;
					
					
						$data['showType'] = "casablanca";
					
					
					} else if(in_array($c_client_id, $ideal_living_clients['client'])) {
						...
						$errorsArray = array('Awesome', 'Average', 'Action needed', 'Absent', 'NA');
						foreach($errorsArray as $findCQ){
							...
							$params2['count'][$token][$errorType] = $c_ParamCount;
							$params2['percent'][$token][$errorType] = $paramsPercent;
						
						}

						//=== SCORE PERCENT 
						$errorType = "'Awesome', 'Average'";						
						$sqlToken = "SELECT count(*) as auditCount from $c_defect_table as qa 
									INNER JOIN signin as s ON s.id = qa.agent_id LEFT JOIN signin as sl ON sl.id = s.assigned_to
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND (qa.$token IN (".$errorType.")) $extraUserFilter";							
						...
						$sqlToken = "SELECT count(*) as auditCount from $c_defect_table as qa 
									INNER JOIN signin as s ON s.id = qa.agent_id LEFT JOIN signin as sl ON sl.id = s.assigned_to
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND qa.$token IN (".$errorNACheck.") $extraUserFilter";
						$c_qaScore = $this->Common_model->get_query_row_array($sqlToken);
						$c_ParamCount = !empty($c_qaScore['auditCount']) ? $c_qaScore['auditCount'] : 0;					
						$paramsPercent = 0;
						if($totalAuditCount > 0){
						...
						if($search_process_id == '380'){
							...
							$findValue = array("Above Expectation", "Meeting Expectation", "Below Expectation", "Well below Expectation");
							
							foreach($findValue as $tokenp){
								$sqlToken = "SELECT count(*) as auditCount from $c_defect_table as qa 
										INNER JOIN signin as s ON s.id = qa.agent_id LEFT JOIN signin as sl ON sl.id = s.assigned_to
										WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
										AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND qa.$token IN ('".$tokenp."') $extraUserFilter";
							$c_qaScore = $this->Common_model->get_query_row_array($sqlToken);
							$c_ParamCount = !empty($c_qaScore['auditCount']) ? $c_qaScore['auditCount'] : 0;					
							$paramsPercent = 0;
							if($totalAuditCount > 0){
							...
							$params2['count'][$token][$tokenp] = $c_ParamCount;
							$params2['percent'][$token][$tokenp] = $paramsPercent;
							}
	...
	{ 
		if(check_logged_in())
		{
			...
			$sql_client = "SELECT c.shname as shname, c.id as id from info_assign_client as i
						   ...
						   WHERE i.user_id = '$current_user' AND  c.is_active=1 ORDER BY c.shname";
			$result_client = $this->Common_model->get_query_result_array($sql_client);
			$data['client_list'] = $result_client;
			...
			{	
				...
				$sqlDefectProcess = "SELECT REPLACE(REPLACE(REPLACE(p.table_name, 'qa_',''), '_feedback', ''), '_', ' ') as process_name,
							p.process_id as process_id, c.shname as client_name, c.id as client_id FROM qa_defect as p
							LEFT JOIN client as c ON c.id = p.client_id 
							LEFT JOIN process as ip ON ip.id = p.process_id WHERE 1 $extraCampaignFilter";
				$queryDefectProcess = $this->Common_model->get_query_result_array($sqlDefectProcess);				
			}
		...
		if($process_id != "" && $office_id != ""){
			...
			foreach($queryDefectTable as $token)
			{
				...
				$sql_tlScore = "SELECT count(*) as auditCount, qa.agent_id, avg(qa.overall_score) as score, CONCAT(s.fname, ' ', s.lname) as fullname, s.fusion_id from $c_defect_table as qa
								...
								$extraTLFilter GROUP BY qa.agent_id ORDER by score DESC";
		...
		if(check_logged_in())
		{
			...
			if((get_role_dir() != 'agent' && get_role_dir() != 'tl') || get_global_access())
			{
				...
				$sqlDefectProcess = "SELECT REPLACE(REPLACE(REPLACE(p.table_name, 'qa_',''), '_feedback', ''), '_', ' ') as process_name,
							p.process_id as process_id, c.shname as client_name, c.id as client_id FROM qa_defect as p
							LEFT JOIN client as c ON c.id = p.client_id 
							LEFT JOIN process as ip ON ip.id = p.process_id WHERE 1 $extraQCheck";
				$queryDefectProcess = $this->Common_model->get_query_result_array($sqlDefectProcess);
			}
			...
			if($is_global_access==1 || (get_dept_folder()=="qa" and get_role_dir()=="manager")){
				$data['location_list'] = $this->Common_model->get_office_location_list();
			} else if(get_user_fusion_id()=="FKOL010704"){
				$data['location_list'] = $this->Common_model->get_office_location_list();
			} else {
				$data['location_list'] = $this->Common_model->get_office_location_session_all($current_user);
			}
			...
			if(!empty($search_campaign_id))
			{
				...
				foreach($data['managersList'] as $tokenManagers)
				{
					...
					$sql_tlScore = "SELECT count(*) as auditCount, avg(qa.overall_score) as score, CONCAT(l.fname, ' ', l.lname) as fullname, l.fusion_id, l.id as tl_id from $c_defect_table as qa
									...
									INNER JOIN signin as l ON l.id = s.assigned_to
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND s.assigned_to IN ($userSearchManagerTls) ORDER by score DESC";
				...
				$sql_tlScore = "SELECT count(*) as auditCount, avg(qa.overall_score) as score, CONCAT(l.fname, ' ', l.lname) as fullname, l.fusion_id, l.id as tl_id from $c_defect_table as qa
							    ...
							    WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
								AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
								$extraTLFilter GROUP BY s.assigned_to ORDER by score DESC";
				...
				// OVERALL CQ SCORE
				$sql_qaScoreAll = "SELECT count(*) as auditCount, avg(qa.overall_score) as score from $c_defect_table as qa
							    INNER JOIN signin as s ON s.id = qa.agent_id
							    WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
								AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'";
				$c_qaScoreAll = $this->Common_model->get_query_row_array($sql_qaScoreAll);
				$c_auditCountAll = !empty($c_qaScore['auditCount']) ? $c_qaScore['auditCount'] : 0;
				$totalAuditCountAll = $c_auditCountAll;
				$avgCQScoreAll = sprintf('%.2f', $c_qaScoreAll['score']);
				...
				$sqlAgent = "SELECT qa.agent_id, count(qa.overall_score) as total_count, avg(qa.overall_score) as average_score, s.doj, CONCAT(s.fname, ' ', s.lname) as fullname, s.fusion_id from $c_defect_table as qa
							 INNER JOIN signin as s ON s.id = qa.agent_id
							 WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
							 AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' $extraUserFilter GROUP BY qa.agent_id  ORDER by average_score DESC";
				$queryAgent = $this->Common_model->get_query_result_array($sqlAgent);
				$AgnetList = $queryAgent;				
				...
				$data['overview']['total_count'] = $totalAuditCount;
				$data['overview']['total_count_all'] = $totalAuditCountAll;
				$data['overview']['cq_score'] = $avgCQScore;
				$data['overview']['cq_score_all'] = $avgCQScoreAll;
				$data['overview']['total_agent'] = count($AgnetList);
				$data['overview']['total_tl'] = count($c_tlScoreArray);
				$data['overview']['tl_array'] = $c_tlScoreArray;
				
				$data['agentOverview']['total'] = $queryAgent;
				$data['agentOverview']['fatal'] = $this->array_indexed($c_qaScore_fatal_tl, 'agent_id');
				$data['agentOverview']['autofail'] = $this->array_indexed($c_qaScore_autofail, 'agent_id');
...
//================= ACCEPTANCE DASHBOARD =================================================//
	...
	{ 
		if(check_logged_in())
		{
			...
			$extraCheckFilter = "";
			if(!empty($search_client_id) && $search_client_id != 'ALL'){
				$extraCheckFilter .= " AND is_assign_client(s.id, $search_client_id) ";
			}
			
			if(!empty($search_office) && $search_office != 'ALL'){
				$extraCheckFilter .= " AND s.office_id = '$search_office' ";
			}
			
			$SQLtxt = "SELECT s.id, CONCAT(s.fname, ' ', s.lname) as fullname, s.fusion_id FROM signin as s 
			          LEFT JOIN role as r ON r.id = s.role_id 
					  LEFT JOIN signin as sl ON sl.id = s.assigned_to 
					  where 1 AND r.folder='tl' $extraCheckFilter order by fullname";			
			$resultUser = $this->Common_model->get_query_result_array($SQLtxt);
			$data['tl_list'] = $resultUser;
			...
			$extraTLFilter = "";
			if(!empty($select_tl) && $select_tl != 'ALL'){
				$extraTLFilter .= " AND s.assigned_to = '$select_tl' ";
			}
			...
			if($tableFound){
				...
				$sqlCount = "SELECT count(*) as value from $currentTable as qa 
							 INNER JOIN signin as s ON s.id = qa.agent_id
				             WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
							 AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' $extraTLFilter";
				...
				if($type == "column"){
					...
					$sqlAccepted = "SELECT count(*) as value from $agentTable as qa
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw <> '' AND qa.$agent_column_rvw IS NOT NULL)) $extraTLFilter";
					...
					$sqlTimeAccepted = "SELECT count(*) as value from $agentTable as qa
									...
									AND qa.$agent_column_date <= DATE_ADD(qa.entry_date, INTERVAL 24 HOUR) $extraTLFilter";
					...
					$sqlTimeAccepted = "SELECT count(*) as value from $agentTable as qa
									...
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw <> '' AND qa.$agent_column_rvw IS NOT NULL)) 
									AND qa.$agent_column_date > DATE_ADD(qa.entry_date, INTERVAL 24 HOUR) AND qa.$agent_column_date <= DATE_ADD(qa.entry_date, INTERVAL 48 HOUR) $extraTLFilter";
					$countTimeAccepted48 = $this->Common_model->get_single_value($sqlTimeAccepted);
					
					$countAfterTimeAccepted = $countAccepted - ($countTimeAccepted + $countTimeAccepted48);
					
					$sqlMgAccepted = "SELECT count(*) as value from $mgntTable as qa
									INNER JOIN signin as s ON s.id = qa.agent_id
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$mgnt_column_rvw <> '' AND qa.$mgnt_column_rvw IS NOT NULL)) $extraTLFilter";
				...
				if($type == "table"){
					...
					$sqlAccepted = "SELECT count(*) as value from $agentTable as fd
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw <> '' AND fd.$agent_column_rvw IS NOT NULL)) $extraTLFilter";
					$countAccepted = $this->Common_model->get_single_value($sqlAccepted);
					...
					$sqlTimeAccepted = "SELECT count(*) as value from $agentTable as fd
									...
									AND fd.$agent_column_date <= DATE_ADD(qa.entry_date, INTERVAL 24 HOUR) $extraTLFilter";
					$countTimeAccepted = $this->Common_model->get_single_value($sqlTimeAccepted);	
					...
					$sqlTimeAccepted = "SELECT count(*) as value from $agentTable as fd
									...
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw <> '' AND fd.$agent_column_rvw IS NOT NULL))
									AND fd.$agent_column_date > DATE_ADD(qa.entry_date, INTERVAL 24 HOUR) AND fd.$agent_column_date <= DATE_ADD(qa.entry_date, INTERVAL 48 HOUR) $extraTLFilter";
					$countTime48Accepted = $this->Common_model->get_single_value($sqlTimeAccepted);				
					...
					$sqlMgAccepted = "SELECT count(*) as value from $mgntTable as fd
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$mgnt_column_rvw <> '' AND fd.$mgnt_column_rvw IS NOT NULL)) $extraTLFilter";
					$countMgAccepted = $this->Common_model->get_single_value($sqlAccepted);
				}
				...
				if($type == "custom"){
					...
					$sqlAccepted = "SELECT count(*) as value from $agentTable as fd
									...
									AND ((fd.$agent_column_rvw <> '' AND fd.$agent_column_rvw IS NOT NULL))
									AND fd.$agent_column_date > DATE_ADD(qa.entry_date, INTERVAL 24 HOUR) AND fd.$agent_column_date <= DATE_ADD(qa.entry_date, INTERVAL 48 HOUR) $extraTLFilter";
					$countTimeAccepted48 = $this->Common_model->get_single_value($sqlAccepted);	
				...
				*/
				...
				$qaAcceptance['feedback'][$currentsubProcess]['details'] = array(
					...
					"agent_before2448" => $countTimeAccepted48,
	...
	{ 
		if(check_logged_in())
		{
			...
			// Process Selection
			$sql_process = "SELECT * FROM process WHERE is_active=1 ORDER BY name";
			$result_process = $this->Common_model->get_query_result_array($sql_client);
			$data['process_list'] = $result_process;
			
			$data["aside_template"]   = "qa/aside.php";
			$data["content_template"] = "qa_graph/qa_feedback_dashboard.php";
			$data["content_js"] = "qa_graph/qa_feedback_dashboard_js.php";
	...
	public function testsql(){
		$sql ="SELECT qa.agent_id, count(qa.agent_id) as feedback_count from qa_oyolife_ib_ob_agent_rvw as fd INNER JOIN qa_oyolife_ib_ob_feedback as qa ON qa.id = fd.fd_id INNER JOIN signin as s ON s.id = qa.agent_id WHERE (DATE(qa.audit_date) >= '2021-02-01' AND DATE(qa.audit_date) <='2021-02-04') AND s.office_id = 'KOL' AND qa.audit_type = 'CQ Audit' AND ((fd.note <> '' AND fd.note IS NOT NULL) OR qa.overall_score = 100) AND qa.agent_id IN (6435,9239,9241,9242,9243,9247,9253,9256,9257,9262,9275,9280,9281,9293,9307,9336,9343,10123,10124,10125,10126,10127,10128,10129,10130,10131,10132,10133,10134,10135,10136,10137,10138,10139,10467,10468,10469,10470,10471,10472,10473,10474,10503,10620,10621,10622,10623,10624,10625,10626,10627,10628,10629,10630,10631,10632,10774,10777,10778,11005,11007,11016,11017,11018,11019,11020,11021,11032,11041,11042,11043,11188,11200,11201,11202,11203,11204,11205,11206,11207,11293,11357,11358,11513,11515,11517,11518,11521,11522,11523,11524,11525,11526,11528,11800,11810,11987,12056,12058,12060,12062,12095,12282,12291,12292,12293,12294,12295,12299,12300,12305,12323,12339,12355,12356,12360,12361,12364,12365,12371,12436,12458,12546,12589,12590,12591,12593,12594,12595,12596,12597,12862,12865,12866,12972,12973,12974,13005,13006,13007,13008,13013,13036,13042,13043,13045,13049,13245,13336,13337,13404,13405,13407,13408,13425,13436,13460,13461,13493,13502,13503,13504,13505,13506,13507,13510,13675,13728,13729,13734,13735,13737,13738,13739,13740,13741,13742,13743,13744,13745,13746,13747,13793,14986,14990,14992,14993,14995,14996,14997,14998,15000,15001,15002,15003,15004,15005,15007,15008,15009,15010,15012,15305,15306,15307,15309,15312,15313,15314,15316,15318,15319,15908,15909,15910,15914,15915,15916,15917,15918,15919,15920,15922,15948,21540,21791,21793,21794,21795,22210) GROUP BY qa.agent_id";
		$check = $this->Common_model->get_query_result_array($sql);
		print_r($check);
	}
	
	public function feedback_dashboard_view()
	{ 
		if(check_logged_in())
		{
			...
			$currentUser = get_user_id();
			$currentUserDetails = "SELECT s.*, CONCAT(sl.fname, ' ', sl.lname) as tl_name from signin as s LEFT JOIN signin as sl ON sl.id = s.assigned_to WHERE s.id = '$currentUser'";
			$data['userDetails'] = $this->Common_model->get_query_row_array($currentUserDetails);
			
			$extraSFitler = "";
			if(!empty($office_id) && $office_id!= "ALL"){
				$extraSFitler .= " AND s.office_id = '$office_id'";
			}
			if(!empty($client_id) && $client_id!= "ALL"){
				$extraSFitler .= " AND is_assign_client(s.id, $client_id)";
			}
			if(!empty($process_id) && $process_id!= "ALL"){
				$extraSFitler .= " AND is_assign_process(s.id, $process_id)";
			}
			if(!empty($tl_id) && $tl_id!= "ALL"){
				$extraSFitler .= " AND s.assigned_to IN ($tl_id)";
			}
			if(!empty($agent_id) && $agent_id!= "ALL"){
				$extraSFitler .= " AND s.id IN ($agent_id)";
			}
			
			// GET USER
			$sqlSignin = "SELECT s.* from signin as s LEFT JOIN role as r ON r.id = s.role_id WHERE r.folder = 'agent' $extraSFitler";
			$querySignin = $this->Common_model->get_query_result_array($sqlSignin);
			$userArray = array_column($querySignin, 'id');
			$data['userList'] = $this->array_indexed($querySignin, 'id');
			$signinIds = 0;
			if(!empty($userArray)){
				$signinIds = implode(',', $userArray);
			}
			...
			$sqlQAProcess = "SELECT q.*, p.name as process_name, c.shname as client_name 
								   ...
								   LEFT JOIN process as p ON p.id = q.process_id
								   LEFT JOIN client as c ON c.id = q.client_id 
								   WHERE q.process_id = '$campaign_id'
								   ORDER by q.table_name";
			$data['processQAlist'] = $processQAArray = $this->Common_model->get_query_result_array($sqlQAProcess);
			...
			if($tableFound){
				...
				// COUNT FEEDBACK RAISED
				$sqlCount = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $currentTable as qa 
							 INNER JOIN signin as s ON s.id = qa.agent_id
				             WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
							 AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit' AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
				$countFeedback = $this->Common_model->get_query_result_array($sqlCount);
				...
				if($type == "column"){
					...
					$sqlAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as qa
									INNER JOIN signin as s ON s.id = qa.agent_id
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw <> '' AND qa.$agent_column_rvw IS NOT NULL)) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countAccepted = $this->Common_model->get_query_result_array($sqlAccepted);
					
					$sqlNotAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as qa
									INNER JOIN signin as s ON s.id = qa.agent_id
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw = '' OR qa.$agent_column_rvw IS NULL)) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countNotAccepted = $this->Common_model->get_query_result_array($sqlNotAccepted);
				...
				if($type == "table"){
					...
					$sqlAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as fd
									INNER JOIN $currentTable as qa ON qa.id = fd.fd_id
									INNER JOIN signin as s ON s.id = qa.agent_id
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw <> '' AND fd.$agent_column_rvw IS NOT NULL)) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countAccepted = $this->Common_model->get_query_result_array($sqlAccepted);
					
					$sqlNotAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as fd
									INNER JOIN $currentTable as qa ON qa.id = fd.fd_id
									INNER JOIN signin as s ON s.id = qa.agent_id
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw = '' OR fd.$agent_column_rvw IS NULL)) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countNotAccepted = $this->Common_model->get_query_result_array($sqlNotAccepted);
				...
				$qaAcceptance['feedback']['accepted'] = $this->array_indexed($countAccepted, 'agent_id');
				$qaAcceptance['feedback']['notaccepted'] = $this->array_indexed($countNotAccepted, 'agent_id');
			...
			$data['allAgents'] = $countFeedback;
			$data['acceptance'] = $qaAcceptance;
							
			$this->load->view('qa_graph/qa_feedback_dashboard_view',$data);
...
//================ OTHER DASHBOARDS =============================================================================//
	...
	public function opsview()
	{
		if(check_logged_in())
		{
			...
			///==================== GET DEFECTS TABLE ========================///
			$sql_defect_columns = "SELECT count(*) as countdc, table_name, defect_columns, defect_column_names FROM qa_defect WHERE FIND_IN_SET(".$select_process.",process_id)";
		    $get_defect_columns = $this->Common_model->get_query_row_array($sql_defect_columns);
			...
			*/
			...
			$data["aside_template"]   = "qa/aside.php";
			$data["content_template"] = "qa_graph/qa_ops_view.php";
	...
	public function globalview()
	{
		if(check_logged_in())
		{
			...
			foreach($query_process as $tokenp)
			{
				...
				///==================== GET DEFECTS TABLE ========================///
				$sql_defect_columns = "SELECT count(*) as countdc, table_name, defect_columns, defect_column_names FROM qa_defect WHERE FIND_IN_SET(".$process_id.",process_id)";
				$get_defect_columns = $this->Common_model->get_query_row_array($sql_defect_columns);
				$count_dc = $get_defect_columns['countdc'];
			...
			$data["aside_template"]   = "qa/aside.php";
			$data["content_template"] = "qa_graph/qa_global_view.php";
	...
	public function leadview()
	{
		if(check_logged_in())
		{
			...
			*/
			...
			$data["aside_template"]   = "qa/aside.php";
			$data["content_template"] = "qa_graph/qa_lead_view.php";
...
/////////============= CALCULATE AVG NPS SCORE PROCESS WISE  ==========================///////////////////////////////////
	...
	public function select_tl(){
		...
		if(check_logged_in())
		{
			...
			$client_id = $this->input->get('client_id');
			$process_id = $this->input->get('process_id');
			$extraFilter = " AND r.folder = 'tl'";
			if(!empty($client_id)  && $client_id != 'ALL'){
				$extraFilter .= " AND is_assign_client(s.id, $client_id)";
			}
			if(!empty($process_id)  && $process_id != 'ALL'){
				$extraFilter .= " AND is_assign_process(s.id, $process_id)";
			}
			if(!empty($office_id) && $office_id != 'ALL'){
				$extraFilter .= " AND s.office_id = '$office_id'";
			}
			$SQLtxt = "SELECT CONCAT(s.fname, ' ', s.lname, ' (', s.fusion_id ,')') as fullname, s.id as user_id FROM signin as s LEFT JOIN role as r ON s.role_id = r.id WHERE 1 $extraFilter ORDER by s.fname";		
	...
	public function select_agent(){
		...
		if(check_logged_in())
		{
			...
			if(!empty($tl_id)  && $tl_id != 'ALL'){
				$extraFilter .= " AND s.assigned_to IN ($tl_id)";
			}
			if(!empty($office_id)  && $office_id != 'ALL'){
				$extraFilter .= " AND s.office_id = '$office_id'";
			}
	...
	public function select_process(){
		...
		if(check_logged_in())
		{
			...
			$SQLtxt = "SELECT id,name FROM process WHERE 1 $extraFilter ORDER by name";
	...
	public function select_campaign(){
		...
		if(check_logged_in())
		{
			...
			if(!empty($process_id) && $process_id != 'ALL'){
				$extraFilter = " AND ROUND(process_id) in ($process_id)";
			}
			
			$SQLtxt = "SELECT process_id as id, REPLACE(REPLACE(REPLACE(table_name, 'qa_',''), '_feedback', ''), '_', ' ') as name FROM qa_defect where 1 $extraFilter order by name";
			
			if(get_login_type() == 'client'){
				$my_client_ids = !empty(get_clients_client_id()) ? get_clients_client_id() : -1;
				$my_process_ids = !empty(get_clients_process_id()) ? get_clients_process_id() : -1;
				$SQLtxt = "SELECT process_id as id, REPLACE(REPLACE(REPLACE(table_name, 'qa_',''), '_feedback', ''), '_', ' ') as name FROM qa_defect where 1  AND client_id IN ($my_client_ids) AND FLOOR(process_id) IN ($my_process_ids) $extraFilter order by name";
			}
	...
	public function select_user(){
		if(check_logged_in())
		{
			...
			if(!empty($tl_id) && $tl_id != 'ALL'){
				$extraFilter .= " AND s.assigned_to = '$tl_id' ";
			}
			
			if(!empty($role)){
				$extraFilter .= " AND r.folder = '$role' ";
			}
			
			$SQLtxt = "SELECT s.id, CONCAT(s.fname, ' ', s.lname) as fullname, s.fusion_id FROM signin as s 
			          LEFT JOIN role as r ON r.id = s.role_id 
					  LEFT JOIN signin as sl ON sl.id = s.assigned_to 
					  where 1 $extraFilter order by fullname";			
			$fields = $this->db->query($SQLtxt);			
			$process_data =  $fields->result_array();			  
			echo  json_encode($process_data);
	...
	public function get_casablanca_client()
	{		
		$clientArray = array(172);
		$processArray = array(364,362,363);
		$resultArray['client'] = $clientArray;
		$resultArray['process'] = $processArray;
		return $resultArray;
	}
	
	public function get_ideal_living_client()
	{		
		$clientArray = array(79);
		$processArray = array(99,209);
		$resultArray['client'] = $clientArray;
		$resultArray['process'] = $processArray;
		return $resultArray;
	}
	...
	public function showCheck(){
		//$sql = "SELECT * from email_log WHERE email_to = 'claimsq@aegisfirst.com' AND send_time >= '2021-07-08 00:00:00' AND send_time <= '2021-07-08 23:59:59'";
	...
	{
		// GET EMAIL ID
		//$u_emailID = "delegate@omindtech.com";
		...
		$emailConfig = array();
		$emailConfig = array();		
		$emailConfig['encrypto'] = 'ssl';
		$emailConfig['validate'] = false;
		...
		$emailConfig['expunge_on_disconnect'] = false;
		$emailConfig['cache'] = [
			'active'     => false,
			'adapter'    => 'file',
			'backup'     => 'file',
		...
		$emailConfig['host'] = "outlook.office365.com";
		//$emailConfig['username'] = "travelpartners@jurysinns.com";
		$emailConfig['username'] = "holiday_extras@jurysinns.com";
		$emailConfig['password'] = "Welcome21!";
		
		$emailConfig['authuser'] = "fusion_rpa@jurysinns.com";
		//$emailConfig['shared'] = "travelpartners@jurysinns.com";
		$emailConfig['shared'] = "holiday_extras@jurysinns.com";
		
		//$masterConfig['folder'] = $u_folder;
		
		$masterConfig = $emailConfig;
		$imap = new IMap($masterConfig);
		
		$resultCheck = $imap->get_folders();
		echo "<pre>".print_r($resultCheck, 1)."</pre>";
		
		// GET EMAILS
		$emailsAr = $imap->search($u_search_type);
		arsort($emailsAr);
		echo "<pre>".print_r($emailsAr, 1)."</pre>";
		
		$emailList = array();
		if(!empty($emailsAr))
		{
			$cn=0;
			foreach($emailsAr as $mail){
				$cn++;								
				if($cn>1){ continue; }				
				$emailMessage = $imap->get_message($mail);
				$emailList[] = $emailMessage;				
			}
		}
		
		echo "<pre>".print_r($emailList, 1)."</pre>"; die();
		$data['messageList'] = $emailList;
		
		// GET EMAIL IDS
		if(!empty($emailList)){
			$emailUIDs = array_column($emailList, 'uid');
			//$imap->mark_as_read($emailUIDs);
		}
	}
	...
	public function upload_profile_data()
	{
		if(check_logged_in())
		{
			$current_user     = get_user_id();
			...
			$outputFile = FCPATH ."uploads/avon_image/";
			/*$config = [
				'upload_path'   => $outputFile,
				'allowed_types' => 'xls|xlsx',
				'max_size' => '1024000',
			...
			*/
			...
			$file_name = $outputFile ."Paycom ID to be added.xlsx";
			$this->load->library('excel');
			$inputFileType = PHPExcel_IOFactory::identify($file_name);
			$objReader = PHPExcel_IOFactory::createReader($inputFileType);
			$objReader->setReadDataOnly(true);
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
			{
				...
				for($i=2; $i<=$totalData; $i++)
				{
					...
					$_fusion_id = $currentDataSet['A'];
					$_site_id = $currentDataSet['B'];
					if(!empty($_fusion_id) && !empty($_site_id)){
						$sqlGet = "UPDATE signin SET xpoid='$_site_id' WHERE fusion_id = '$_fusion_id';";
						$generateQuery[] = $sqlGet;
					}
