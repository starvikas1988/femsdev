		...
		if($client_id == 288 && $pro_id== 614){
			...
			if($client_id == 202 && $pro_id== 411){
			 ...
			 "called_no", "out_in", "call_type", "upload_date");
				...
				$header = array("fusion_id","agent_name","tl_name","call_date","call_id","call_type","csat","aht","customer_number","ivr_name","call_disposition","disconnection_party","service_name","ivr_language");
				$rand_table = "qa_randamiser_nurture_farm_data";
			}
			// Cras 24 Pre-booking
			if($client_id == 246 && $pro_id== 529){
	...
	public function create_qa_randamiser_CSV($rr,$client_id,$pro_id,$header,$rand_table)
	{
		...
		foreach($rr as $user)
		{
			for($z=0;$z<$count_for_field;$z++){
				if($client_id == 345 && $pro_id== 719){
					/////////////////////////////////
					if($field_val[$z]==="call_date"){
				 	$row = '"'.$user['call_date'].'",';
					 }
					elseif($field_val[$z]==="aht"){
						$row .= '"'.$user['aht'].'",';
					}
					elseif($field_val[$z]==="fusion_id"){
						$row .= '"'.$user['fusion_id'].'",';
					}
					elseif($field_val[$z]==="caller_no"){
						$row .= '"'.$user['caller_no'].'",';
					}elseif($field_val[$z]==="campaign"){
						$row .= '"'.$user['campaign'].'",';
					}elseif($field_val[$z]==="call_id"){
						$row .= '"'.$user['call_id'].'",';
					}elseif($field_val[$z]==="skill"){
						$row .= '"'.$user['skill'].'",';
					}elseif($field_val[$z]==="status"){
						$row .= '"'.$user['status'].'",';
					}elseif($field_val[$z]==="dial_status"){
						$row .= '"'.$user['dial_status'].'",';
					}elseif($field_val[$z]==="disposition"){
						$row .= '"'.$user['disposition'].'",';
					}elseif($field_val[$z]==="hangup_by"){
						$row .= '"'.$user['hangup_by'].'",';
					}elseif($field_val[$z]==="campaign"){
						$row .= '"'.$user['campaign'].'",';
					}elseif($field_val[$z]==="upload_date"){
						$row .= '"'.$user['upload_date'].'",';
					}elseif($field_val[$z]==="agent_name"){
					 	$row .= '"'.$user['agent_name'].'",';
					 }
					 elseif($field_val[$z]==="tl_name"){
					 	$row .= '"'.$user['tl_name'].'",';
					 }
				...
				if($client_id == 37 && $pro_id== 49){
					if(in_array($field_val[$z], array('id','audit_sheet','compute_status','audit_status','is_distributed','compute_by','non_auditable','non_auditable_comment','non_auditable_by','non_auditable_date','distribution_opend_by','distribution_opened_datetime','log'))) {
						continue;
