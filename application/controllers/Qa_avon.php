...
class Qa_avon extends CI_Controller {
	...
	public function qa_avon_report(){
		if(check_logged_in()){
			...
			if($this->input->get('show')=='Show'){
                ...
                (select concat(fname, ' ', lname) as name from signin_client scx where scx.id=client_rvw_by) as client_rvw_name from qa_avon_".$campaign."_feedback) xx Left Join
    ...
    public function agent_avon_feedback(){
		if(check_logged_in()){
			...
			if($this->input->get('btnView')=='View'){
				...
				$qSql = "SELECT * from
				(Select *, get_user_name(entry_by) as auditor_name, get_user_name(client_entryby) as client_name, get_user_name(tl_id) as tl_name, get_user_name(mgnt_rvw_by) as mgnt_rvw_name
                from qa_avon_".$campaign."_feedback $cond And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
				...
				(Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name,
			 	(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
			 	(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
			 	(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_huda_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
    ...
    public function agent_avon_rvw($campaign, $id){
		if(check_logged_in()){
            ...
            $table="qa_avon_".$campaign."_feedback";
