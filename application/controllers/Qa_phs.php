	...
	public function index(){
		...
		/////////////////////
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_phs_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["phs"] = $this->Common_model->get_query_result_array($qSql);
		/////////////////////
			$qSql = "SELECT * from
				(Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name,
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
...
////////////////////// Email & Chat ///////////////////////
			...
			if($field_array['agent_id']){
				...
				///////////
					...
					if($field_array['overall_score'] == 0){
						$tablename = "qa_phs_chat_v2_feedback";
						...
						$to = 'Gustavo.Diaz@fusionbposervices.com,walter.miranda@fusionbposervices.com,rene.martinez@fusionbposervices.com,Diego.Quezada@fusionbposervices.com,aaron.munguia@fusionbposervices.com,Roxana.Martinez@fusionbposervices.com,amitabh.vartak@fusionbposervices.com,ashish.tere@fusionbposervices.com,Faisal.Anwar@fusionbposervices.com,projectpremier.operations@fusionbposervices.com,projectpremier.teamleaders@fusionbposervices.com,bella.fabricante@fusionbposervices.com,Marenne.Bamba@fusionbposervices.com,Geremae.Nacion@fusionbposervices.com,francis.pono@fusionbposervices.com,sheila.manalastas@fusionbposervices.com,Zephaniah.Satiembre@fusionbposervices.com,bryan.carpio@fusionbposervices.com,Acha.Joseph@fusionbposervices.com';
						$ebody = "Hello ". $result['tl_fullname'].",<br>";
						$ebody .= "<p>Agent Name : ".$result['fullname']."</p>";
						$ebody .= "<p>File Number :  ".$result['file_no']."</p>";
						$ebody .= "<p>Member ID :  ".$result['member_id']."</p>";
		...
		if(check_logged_in())
		{
			...
			if($field_array['agent_id']){
				...
				///////////
					...
					/*******************Fatal Call Email Send functionality added on 12-12-22 START ***********************/
					if($field_array['overall_score'] == 0){
						$tablename = "qa_phs_email_v2_feedback";
						$sql = "SELECT tname.*, ip.email_id_off, ip_tl.email_id_off as tl_email, concat(s.fname, ' ', s.lname) as fullname,
							(SELECT concat(tls.fname, ' ', tls.lname) as tl_fullname FROM signin tls WHERE tls.id=tname.tl_id) as tl_fullname
							FROM $tablename tname
							LEFT JOIN info_personal ip ON ip.user_id=tname.agent_id 
		...
		if(check_logged_in())
		{
			...
			if($field_array['agent_id']){
				...
				///////////
					...
					/*******************Fatal Call Email Send functionality added on 12-12-22 START ***********************/
					if($field_array['overall_score'] == 0){
						$tablename = "qa_phs_voice_v2_feedback";
						$sql = "SELECT tname.*, ip.email_id_off, ip_tl.email_id_off as tl_email, concat(s.fname, ' ', s.lname) as fullname,
							(SELECT concat(tls.fname, ' ', tls.lname) as tl_fullname FROM signin tls WHERE tls.id=tname.tl_id) as tl_fullname
							FROM $tablename tname
							LEFT JOIN info_personal ip ON ip.user_id=tname.agent_id 
	...
	function import_phs_voice2_excel_data(){
		...
		if(check_logged_in())
		{
			...
			$clmarr = array("audit_date","fusion_id","call_date","call_duration","channel","file_no","link","acpt","ztp","member_id","call_type","caller_type","product_agency","language","audit_type","auditor_type","voc","possible_score","earned_score","overall_score","verify_name_address","entraction_notes","csr_policy","accurate_information","appropriate_greeting","provider_call","call_back_number","hold_time","csr_statement","empathy","active_listening","dead_air","transfer_etiquette","control_of_call","speak_without_interrupting","csr_review","products_benefits","probing_questions","reason_for_call","call_recap","additional_assistance","updated_information","offer_help","cmt1","cmt2","cmt3","cmt4","cmt5","cmt6","cmt7","cmt8","cmt9","cmt10","cmt11","cmt12","cmt13","cmt14","cmt15","cmt16","cmt17","cmt18","cmt19","cmt20","cmt21","cmt22","cmt23","call_summary","feedback");
			...
			foreach($object->getWorksheetIterator() as $worksheet){
				...
				$this->Qa_philip_model->phs_voice2_excel_data($user_list);
				redirect('Qa_phs');
			}
		}
	}

	function import_phs_email2_excel_data(){
		...
		if(check_logged_in())
		{
			...
			$path = $_FILES["file"]["tmp_name"];
			$object = PHPExcel_IOFactory::load($path);
			$clmarr = array("audit_date","fusion_id","call_date","call_duration","channel","file_no","link","acpt","ztp","member_id","call_type","caller_type","product_agency","language","audit_type","auditor_type","voc","possible_score","earned_score","overall_score","csr_adhere","entraction_notes","csr_policy","reason_for_email","accurate_information","email_composed","standard_time","active_reading","csr_tone","empathy","csr_review","products_benefits","self_service","additional_assistance","updated_information","appropriate_disposition","offer_help","cmt1","cmt2","cmt3","cmt4","cmt5","cmt6","cmt7","cmt8","cmt9","cmt10","cmt11","cmt12","cmt13","cmt14","cmt15","cmt16","cmt17","call_summary","feedback");
			...
			foreach($object->getWorksheetIterator() as $worksheet){
				...
				$this->Qa_philip_model->phs_email2_excel_data($user_list);
				redirect('Qa_phs');
			}
		}
	}

	function import_phs_chat2_excel_data(){
		...
		if(check_logged_in())
		{
			...
			$path = $_FILES["file"]["tmp_name"];
			$object = PHPExcel_IOFactory::load($path);
			$clmarr = array("audit_date","fusion_id","call_date","call_duration","channel","file_no","link","acpt","ztp","member_id","call_type","caller_type","product_agency","language","audit_type","auditor_type","voc","possible_score","earned_score","overall_score","verify_name_address","entraction_notes","csr_policy","reason_for_chat","accurate_information","csr_security_policy","hold_time","csr_statement","empathy","active_reading","timely_manner","spelling_grammer","control_conservation","members_concern","csr_review","products_benefits","probing_questions","hold_etiquette","offer_self_service","updated_information","appropriate_disposition","offer_help","cmt1","cmt2","cmt3","cmt4","cmt5","cmt6","cmt7","cmt8","cmt9","cmt10","cmt11","cmt12","cmt13","cmt14","cmt15","cmt16","cmt17","cmt18","cmt19","cmt20","cmt21","cmt22","call_summary","feedback");
			...
			foreach($object->getWorksheetIterator() as $worksheet){
				...
				$this->Qa_philip_model->phs_chat2_excel_data($user_list);
				redirect('Qa_phs');
			}
		}
	}
	...
	public function sample_phs_voice2_download(){
		if(check_logged_in()){ 
			$file_name = base_url()."qa_files/qa_vfs/sample_phs_voice2_file.xlsx";
			header("location:".$file_name);	
			exit();
		}
	}

	public function sample_phs_email2_download(){
		if(check_logged_in()){ 
			$file_name = base_url()."qa_files/qa_vfs/sample_phs_email2_file.xlsx";
			header("location:".$file_name);	
			exit();
		}
	}

	public function sample_phs_chat2_download(){
		if(check_logged_in()){ 
			$file_name = base_url()."qa_files/qa_vfs/sample_phs_chat2_file.xlsx";
			header("location:".$file_name);	
			exit();
		}
	}		
