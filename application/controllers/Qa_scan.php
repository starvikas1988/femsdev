<?php
 ...
 class Qa_scan extends CI_Controller{
  ...
  private function scan_upload_files($files,$path)   // this is for file uploaging purpose
  {
...
/////////////////Home Sea World vikas//////////////////
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$data["content_template"] = "qa_scan/qa_scan_feedback.php";
			$data["content_js"] = "qa_scan_js.php";

			$qSql="SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder ='agent') and dept_id=6 and is_assign_client (id,400) and is_assign_process (id,878)  and status=1  order by name";
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_name from qa_scan_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["scan_data"] = $this->Common_model->get_query_result_array($qSql);
	...
	public function add_edit_scan($scan_id){
		if(check_logged_in())
		{
			$current_user=get_user_id();
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_scan/add_edit_scan.php";
			$data["content_js"] = "qa_scan_js.php";
	      ...
	      $data['agentName'] = $this->Common_model->get_query_result_array( $qSql );
			...
			$qSql = "SELECT * from
				...
				from qa_scan_feedback where id='$scan_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["scan_data"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($field_array['agent_id']){
				...
				if($scan_id==0){
					...
					if($_FILES['attach_file']['tmp_name'][0]!=''){
						$a = $this->scan_upload_files($_FILES['attach_file'], $path='./qa_files/qa_scan/');
						$field_array["attach_file"] = implode(',',$a);
					}

					$rowid= data_inserter('qa_scan_feedback',$field_array);
					...
					$this->db->update('qa_scan_feedback',$add_array);
					...
					if($_FILES['attach_file']['tmp_name'][0]!=''){
						if(!file_exists("./qa_files/qa_scan/")){
							mkdir("./qa_files/qa_scan/");
						}
						$a = $this->scan_upload_files( $_FILES['attach_file'], $path = './qa_files/qa_scan/' );
					...
					$this->db->update('qa_scan_feedback',$field_array1);
					/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
					...
					$this->db->where('id', $scan_id);
					$this->db->update('qa_scan_feedback',$edit_array);
					...
					if($rand_id!=0){
						...
						$this->db->where('id', $rowid);
						$this->db->update('qa_scan_feedback',$rand_array);
					}
  ...
  /////////////////Sea World  Agent part//////////////////////////
	...
	public function agent_scan_feedback()
	{
		if(check_logged_in())
		{
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_scan/agent_scan_feedback.php";
			$data["content_js"] = "qa_scan_js.php";
			$data["agentUrl"] = "qa_scan/agent_scan_feedback";


			$qSql="Select count(id) as value from qa_scan_feedback where agent_id='$current_user' and audit_type not in ('Calibration', 'Pre-Certificate Mock Call', 'Certification Audit','QA Supervisor Audit')";
			$data["tot_agent_feedback"] =  $this->Common_model->get_single_value($qSql);

			$qSql="Select count(id) as value from qa_scan_feedback where agent_rvw_date is null and agent_id='$current_user' and audit_type not in ('Calibration', 'Pre-Certificate Mock Call', 'Certification Audit','QA Supervisor Audit')";
			...
			if($this->input->get('btnView')=='View')
			{
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_scan_feedback $cond and agent_id ='$current_user' And audit_type not in ('Calibration', 'Pre-Certificate Mock Call','QA Supervisor Audit', 'Certification Audit')) xx Inner Join
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_scan_feedback where agent_id='$current_user' And audit_type not in ('Calibration', 'Pre-Certificate Mock Call','QA Supervisor Audit', 'Certification Audit')) xx Inner Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_client_names(id) as client, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
				$data["agent_review_list"] = $this->Common_model->get_query_result_array($qSql);
	...
	public function agent_scan_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_scan/agent_scan_rvw.php";
			$data["agentUrl"] = "qa_scan/agent_scan_feedback";
			$data["content_js"] = "qa_scan_js.php";
	    ...
	    $qSql = "SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder ='agent') and dept_id=6 and is_assign_client (id,400) and is_assign_process(id,878) and status=1  order by name";
	      $data['agentName'] = $this->Common_model->get_query_result_array( $qSql );
			...
			if($this->input->post('scan_id'))
			{
				...
				$this->db->where('id', $scan_id);
				$this->db->update('qa_scan_feedback',$field_array);
				
				redirect('Qa_scan/agent_scan_feedback');
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	public function qa_scan_report(){
		if(check_logged_in()){
			...
			$data["content_template"] = "qa_scan/qa_scan_report.php";
			$data["content_js"] = "qa_scan_js.php";
			...
			$campaign = 'scan';
				...
				if(get_role_dir()=='manager' && get_dept_folder()=='operations'){
					...
					$data["qa_scan_list"] = $fullAray;
			 

				$this->create_Qa_scan_CSV($fullAray,$campaign);

				$dn_link = base_url()."Qa_scan/download_qa_scan_CSV/".$campaign;
   ...
   ////////////Norther ///////////////////////////////
	public function download_qa_scan_CSV($campaign)
	{
		$currDate=date("Y-m-d");
	...
	public function create_qa_scan_CSV($rr,$campaign)
	{

		$filename = "./assets/reports/Report".get_user_id().".csv";
		$fopen = fopen($filename,"w+");
		if($campaign=="scan"){
			$header = array("Auditor Name", "Audit Date", "Employee ID", "Agent Name", "L1 Supervisor", "KPI - ACPT", "Call Date/Time", "Call Duration", "Audit Type", "Auditor Type", "VOC","Possible Score", "Earned Score", "Overall Score",
				"HIPAA/PHI","REMARKS",
				"Call Recording Disclosure","REMARKS",
				"Accurate Information","REMARKS",
				"Telephonic Agreement","REMARKS",
				"Greeting","REMARKS",
				"Probing/Paraphrasing","REMARKS",
				"Critical Thinking","REMARKS",
				"Resolution","REMARKS",
				"Offer Additional Assistance","REMARKS",
				"Closing","REMARKS",
				"Proper Category Sub Category","REMARKS",
				"Documentation","REMARKS",
				"Use of Technology and Resources","REMARKS",
				"Active Listening & Avoiding Interruption","REMARKS",
				"Courtesy & Emphaty","REMARKS",
				"Proffesional Language","REMARKS",
				"Hold Time and Dead Air","REMARKS",
				"Concierge: Own the Relationship & Make it Simple / SAL: Make it Simple","REMARKS",
				"Briefcase Management","REMARKS",
