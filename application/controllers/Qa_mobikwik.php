...
/////////////////////////////////////////////////////
    ...
    public function getDesignation(){
		if(check_logged_in()){
			...
			// $qSql = "Select id, assigned_to, fusion_id,get_process_ids(id)as process_id, get_process_names(id) as process_name, office_id, DATEDIFF(CURDATE(), doj) as tenure FROM signin where id = '$aid'";
				//echo $qSql;
			$qSql ="SELECT s.id, concat(s.fname, ' ', s.lname) as name, s.assigned_to, s.fusion_id,dept.description,r.folder as designation,r.name as roleName  FROM `signin` s LEFT JOIN department dept ON s.dept_id=dept.id LEFT JOIN role r ON s.role_id=r.id where role_id in (select id from role where folder in('agent','tl','trainer')) and dept_id=6 and is_assign_process(s.id,'719,720') and status=1 and s.id = $aid order by name";
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$qSql="SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder in('agent','tl','trainer')) and dept_id=6 and is_assign_process(id,'719,720')  and status=1 order by name";
...
/////////////////////////////////// Mobikwik ///////////////////////////////////
	...
	public function add_edit_mobikwik($mobikwik_id){
		if(check_logged_in())
		{
			...
			where role_id in (select id from role where folder in('agent','tl','trainer')) and dept_id=6 and is_assign_process(s.id,'719,720') and status=1 and s.id!=$current_user order by name";
			$data["agentName"] = $this->Common_model->get_query_result_array($qSql);
