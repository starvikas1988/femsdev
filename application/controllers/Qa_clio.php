<?php 
 ...
 class Qa_clio extends CI_Controller{
...
////////////////////// CLIO /////////////////////////	
	public function index(){
		if(check_logged_in())
		{
			...
			$data["content_template"] = "qa_clio/clio.php";
			$data["content_js"] = "qa_clio_js.php";
			
			$qSql="SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder ='agent') and dept_id=6 and is_assign_client (id,221) and is_assign_process (id,453) and status=1  order by name";
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_clio_feedback $cond) xx Left Join
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_clio_spot_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["spot_check"] = $this->Common_model->get_query_result_array($qSql);
    ...
    /////////////////////////add_edit_clio/////////////////////////////////////
	public function add_edit_clio($gds_id){
		...
		if(check_logged_in())
		{
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_clio/add_edit_clio.php";
			$data["content_js"] = "qa_clio_js.php";
			$data['gds_id']=$gds_id;
			$tl_mgnt_cond='';
			if(get_role_dir()=='manager' && get_dept_folder()=='operations'){
			...
			$qSql = "SELECT * from
				...
				from qa_clio_feedback where id='$gds_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			...
			if($field_array['agent_id']){
				...
				///////////
					...
					$this->db->where('id', $gds_id);
					$this->db->update('qa_clio_feedback',$edit_array);
	...
	public function add_edit_clio_spot($gds_id){
		...
		if(check_logged_in())
		{
			...
			$user_office_id=get_user_office_id();
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_clio/add_edit_clio_spot.php";
			$data["content_js"] = "qa_clio_js.php";
			$data['gds_id']=$gds_id;
			$tl_mgnt_cond='';
			if(get_role_dir()=='manager' && get_dept_folder()=='operations'){
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name
				from qa_clio_spot_feedback where id='$gds_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["clio"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($field_array['agent_id']){
				...
				if($gds_id==0){
					...
					$a = $this->mt_upload_files($_FILES['attach_file'], $path='./qa_files/qa_clio/');
					$field_array["attach_file"] = implode(',',$a);
					$rowid= data_inserter('qa_clio_spot_feedback',$field_array);
				///////////
					if(get_login_type()=="client"){
						$add_array = array("client_entryby" => $current_user);
					}else{
					...
					$this->db->where('id', $gds_id);
					$this->db->update('qa_clio_spot_feedback',$field_array1);
					/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
							"client_rvw_by" => $current_user,
					...
					$this->db->where('id', $gds_id);
					$this->db->update('qa_clio_spot_feedback',$edit_array);
...
/////////////////////////////////////////////////////////////////////
/////////////////////////Agent part/////////////////////////////////	
public function agent_clio_feedback()
	{
		if(check_logged_in())
		{
	...
	public function agent_clio_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			$data['gds_id']=$id;
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_clio/agent_clio_rvw.php";
			$data["content_js"] = "qa_clio_js.php";
			$data["agentUrl"] = "qa_clio/agent_clio_feedback";
			...
			$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_clio_feedback where id='$id') xx Left Join
			...
			if($this->input->post('pnid'))
			{
				...
				redirect('qa_clio/agent_clio_feedback');
...
public function agent_clio_rvw_spot($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			$data['gds_id']=$id;
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_clio/agent_clio_rvw_spot.php";
			$data["content_js"] = "qa_clio_js.php";
			$data["agentUrl"] = "qa_clio/agent_clio_feedback";
			...
			$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_clio_spot_feedback where id='$id') xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["clio"] = $this->Common_model->get_query_row_array($qSql);
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	public function qa_clio_report(){
		if(check_logged_in()){
			...
			$data["content_template"] = "qa_clio/qa_clio_report.php";
			$data["content_js"] = "qa_clio_js.php";
			...
			if($this->input->get('show')=='Show')
			{
				...
				$dn_link = base_url()."qa_clio/download_qa_clio_CSV/".$campaign;	
			}
	...
	public function create_qa_clio_CSV($rr,$campaign)
	{
		...
		$fopen = fopen($filename,"w+");
			...
			$header = array("Auditor Name", "Audit Date", "Agent", "Fusion ID", "L1 Super", "Call Date","reference_id", "Query Type", "Query Sub Type", "Total Opportunity","Auto Fail","Audit Type", "Auditor Type", "Voc","Overall Score", "Possible Score", "Earned Score", "Customer Score","Business Score","Compliance Score", "Did the CSR adhere to all Authentication and Security Policies of the account member?", "Properly verifies the account holder information before providing/discussing any account information.", "For account changes and caller should be authorized on the account (ie wife or dependent)", "Obtain verbal permission from an authenticated account owner before providing any account information.", "Did the agent check if the order is STAT?","Did the agent show empathy and acknowledge patients concern?", "Did the agent utilize all needed information before scheduling an appointment?","Did the agent observe proper Hold Procedure and avoid long silences?", "Did the agent maintain a positive interaction during the call?", "Did the agent asked Covid screening questions?","Did the agent provide accurate reminders for the patient?","Did the agent select the correct order status and left account notes?","Did the agent check if CPT and diagnosis are both correct?","Did the agent update the patient demographic information?","Did the agent check if Insurance requires Auth or not? (PPO or HMO)","Did the agent check Demog and Prescription / Authorization has the same referral doctor?","Did the agent check expiration date of the order?","Did the agent verify if it needs to be sent to corrections or pre-cert?","Did the agent make sure to set up the correct Modality for the patient?","Comments 1", "Comments 2","Comments 3","Comments 4","Comments 5","Comments 6","Comments 7","Comments 8","Comments 9","Comments 10","Comments 11","Comments 12", "Comments 13", "Comments 14", "Comments 15", "Comments 16", "Comments 17", "Comments 18", "Comments 19","Call Summary", "Feedback","Agent Feedback Acceptance", "Agent Review Date","Agent Comment","Mgnt Review Date","Mgnt Review By", "Mgnt Comment");
		...
		$searches = array("\r", "\n", "\r\n");
			...
			foreach($rr as $user){	
				...
				$row .= '"'.$user['do_check_expire_date_order'].'",';
