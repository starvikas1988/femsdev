...
// nur_upload_files
	private function revive_rx_upload_files($files,$path)
    {
	...
	public function index(){
		...
		////////////////////////	
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_revive_rx_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
				// qa_revive_rx_feedback
	...
	public function add_edit_revive_rx($revive_rx_id){
		...
		if(check_logged_in())
		{
			...
			$qSql = "SELECT * from
				...
				from qa_revive_rx_feedback where id='$revive_rx_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			...
			if($field_array['agent_id']){
				...
				if($revive_rx_id==0){
					...
					$rowid= data_inserter('qa_revive_rx_feedback',$field_array);
					...
					if($field_array['overall_score'] == 0){
						...
						$sqlParam ="SELECT process_id,params_columns, fatal_param FROM qa_defect where table_name='$tablename'"; 
						...
						foreach($params as $par){
							...
							if($result[$par]=='No'){
								$msgTable .= "<TR><TD>".$i."</TD><TD>". ucwords( str_replace('_', ' ', $par))."</TD> <TD style='color:#FF0000'>".$result[$par]."</TD><TD>".$result['cmt'.$i]."</TD></TR>";
							}else{
								$msgTable .= "<TR><TD>".$i."</TD><TD>". ucwords( str_replace('_', ' ', $par))."</TD> <TD>".$result[$par]."</TD><TD>".$result['cmt'.$i]."</TD></TR>";
							}
					...
					$this->db->update('qa_revive_rx_feedback',$field_array1);
					/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
					...
					$this->db->where('id', $revive_rx_id);
					$this->db->update('qa_revive_rx_feedback',$edit_array);
...
/////////////////Revive Rx Agent part//////////////////////////	
	...
	public function agent_revive_rx_feedback(){
		if(check_logged_in())
		{
			...
			$qSql="Select count(id) as value from qa_revive_rx_feedback where agent_id='$current_user'";
			$data["tot_feedback"] =  $this->Common_model->get_single_value($qSql);
			$qSql="Select count(id) as value from qa_revive_rx_feedback where agent_rvw_date is null and agent_id='$current_user'";
			...
			if($this->input->get('btnView')=='View')
			{
				...
				$qSql = "SELECT * from
					...
					(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
					(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
					(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_revive_rx_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
	...
	public function agent_revive_rx_feedback_rvw($id){
		if(check_logged_in()){
			...
			if($this->input->post('revive_rx_id'))
			{
				...
				$this->db->where('id', $revive_rx_id);
				$this->db->update('qa_revive_rx_feedback',$field_array);
