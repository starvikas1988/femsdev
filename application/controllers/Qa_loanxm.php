...
/*-------------- Report Part ---------------*/
	...
	function import_loanxm_voicemail_excel_data(){
		...
		if(check_logged_in())
		{
			...
			$path = $_FILES["file"]["tmp_name"];
			$object = PHPExcel_IOFactory::load($path);
			$clmarr = array("audit_date","call_date","call_duration","fusion_id","call_type","customer_name","customer_contact","five9_id","disposition","pass_fail","audit_type","auditor_type","voc","overall_score","possible_score","earned_score","customer_score","business_score","compliance_score",
				"adherence_voicemail_script","use_appropiate_tone","agent_use_proper_word_choice","leave_voicemail","disposition_in_livebox","quality",
			...
			foreach($object->getWorksheetIterator() as $worksheet){
				...
				$this->Qa_philip_model->debt_solution_insert_voicemail_excel($user_list);
				redirect('Qa_loanxm');
			}
		}
	}
	...
	public function sample_loanxm_download(){
		if(check_logged_in()){ 
			$file_name = base_url()."qa_files/qa_loanxm/loanxm_updated_file.xlsx";
			header("location:".$file_name);	
			exit();
		}
	}
	public function sample_loanxm_voicemail_download(){
		if(check_logged_in()){ 
			$file_name = base_url()."qa_files/qa_loanxm/loanxm_voicemail_file.xlsx";
			header("location:".$file_name);	
			exit();
		}
	}
