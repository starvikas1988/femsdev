 ...
 class Qa_octafx extends CI_Controller{
	...
	public function agent_octafx_feedback(){
		if(check_logged_in())
		{
			...
			$qSql="Select count(id) as value from qa_octafx_feedback where agent_id='$current_user'";
			$data["tot_feedback"] =  $this->Common_model->get_single_value($qSql);
			$qSql="Select count(id) as value from qa_octafx_feedback where agent_rvw_date is null and agent_id='$current_user'";
			...
			if($this->input->get('btnView')=='View')
			{
				...
				$qSql = "SELECT * from
					...
					(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
					(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
					(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_octafx_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
