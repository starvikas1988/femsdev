...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	public function add_meesho_inbound_feedback()
	{
		if(check_logged_in())
		{
			...
			$data['global_element'] = global_acpt();
			
			
			if($this->input->post('agent_id')){
				
				$field_array['stella_phishing']=!empty($_POST['data']['stella_phishing'])?$_POST['data']['stella_phishing']:"";
	...
	public function mgnt_meesho_inbound_feedback_rvw($id){
		if(check_logged_in()){
			...
			$data['global_element'] = global_acpt_edit($id);
	...
	public function supplier_support(){
		if(check_logged_in())
		{
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name, 
				(select concat(fname, ' ', lname) as name from signin s where s.id=mgnt_rvw_by) as mgnt_rvw_name from qa_meesho_supplier_support_feedback $cond) xx Left Join 
				(Select id as sid, fname, lname, fusion_id, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["supplier_support"] = $this->Common_model->get_query_result_array($qSql);
		///////////////////
			$qSql = "SELECT * from
				(Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name, 
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
...
/*--------------------------------- Supplier Support (NEW) -------------------------------*/
	public function add_edit_supplier_support($ss_id){
		if(check_logged_in())
		{
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_meesho/supplier_support/add_edit_supplier_support.php";
			$data['ss_id']=$ss_id;
			$tl_mgnt_cond='';
			...
			$qSql = "SELECT * from
				...
				from qa_meesho_supplier_support_feedback where id='$ss_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as campaign, DATEDIFF(CURDATE(), doj) as tenure from signin) yy on (xx.agent_id=yy.sid)";
			$data["supplier_support"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($field_array['agent_id']){
				...
				if($ss_id==0){
					...
					$field_array['attach_file'] = implode(',',$a);
					$rowid= data_inserter('qa_meesho_supplier_support_feedback',$field_array);
				///////////
					if(get_login_type()=="client"){
					...
					$this->db->where('id', $rowid);
					$this->db->update('qa_meesho_supplier_support_feedback',$add_array);
					...
					$this->db->update('qa_meesho_supplier_support_feedback',$field_array1);
				/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
							"client_rvw_by" => $current_user,
					...
					$this->db->where('id', $ss_id);
					$this->db->update('qa_meesho_supplier_support_feedback',$edit_array);
...
/////
	public function pop_shop(){
		...
		///////////
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_meesho_popshop_email_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["popshop_email"] = $this->Common_model->get_query_result_array($qSql);
	...
	public function add_edit_popshop_email($pop_id){
		if(check_logged_in())
		{
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_meesho/pop_shop/add_edit_popshop_email.php";
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name
				from qa_meesho_popshop_email_feedback where id='$pop_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process, DATEDIFF(CURDATE(), doj) as tenure from signin) yy on (xx.agent_id=yy.sid)";
			$data["php_shop"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($field_array['agent_id']){
				...
				if($pop_id==0){
					...
					$field_array['entry_date']=$curDateTime;
					$field_array['audit_start_time']=$this->input->post('audit_start_time');
					$a = $this->meesho_upload_files($_FILES['attach_file'], $path='./qa_files/qa_meesho/pop_shop/email/');
					$field_array["attach_file"] = implode(',',$a);
					$rowid= data_inserter('qa_meesho_popshop_email_feedback',$field_array);
				///////////
					if(get_login_type()=="client"){
						$add_array = array("client_entryby" => $current_user);
					}else{
					...
					$this->db->where('id', $rowid);
					$this->db->update('qa_meesho_popshop_email_feedback',$add_array);
					...
					$field_array1['call_date']=mdydt2mysql($this->input->post('call_date'));
					$this->db->where('id', $pop_id);
					$this->db->update('qa_meesho_popshop_email_feedback',$field_array1);
				/////////////
					if(get_login_type()=="client"){
						$edit_array = array(
							"client_rvw_by" => $current_user,
					...
					$this->db->where('id', $pop_id);
					$this->db->update('qa_meesho_popshop_email_feedback',$edit_array);
	...
	public function agent_popshop_feedback(){
		if(check_logged_in())
		{
			...
			$qSql1="Select count(id) as value from qa_meesho_popshop_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')";
			$data["tot_feedback"] =  $this->Common_model->get_single_value($qSql1);
			$qSql2="Select count(id) as value from qa_meesho_popshop_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit') and agent_rvw_date is Null";
			$data["yet_rvw"] =  $this->Common_model->get_single_value($qSql2);
		////////////
			$email_sql1="Select count(id) as value from qa_meesho_popshop_email_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')";
			$data["tot_email_feedback"] =  $this->Common_model->get_single_value($email_sql1);
			$email_sql2="Select count(id) as value from qa_meesho_popshop_email_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit') and agent_rvw_date is Null";
			$data["yet_email_rvw"] =  $this->Common_model->get_single_value($email_sql2);
			...
			///////////
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_meesho_popshop_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as campaign from signin) yy on (xx.agent_id=yy.sid) Where xx.agent_rvw_date is Null";
				$data["agent_rvw_list"] = $this->Common_model->get_query_result_array($qSql);
			/////////////
				$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_meesho_popshop_mail_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')) xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as campaign from signin) yy on (xx.agent_id=yy.sid) Where xx.agent_rvw_date is Null";
				$data["agent_email_rvw_list"] = $this->Common_model->get_query_result_array($qSql);
	...
	public function agent_popshop_email_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_meesho/pop_shop/agent_popshop_email_rvw.php";
			$data["agentUrl"] = "qa_meesho/agent_popshop_feedback";
			...
			$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_meesho_popshop_email_feedback where id='$id') xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as campaign from signin) yy on (xx.agent_id=yy.sid)";
			$data["pop_shop"] = $this->Common_model->get_query_row_array($qSql);
