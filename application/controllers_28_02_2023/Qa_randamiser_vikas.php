...
/*---------------------------------------------------------------------------------------------------*/	
	...
	function import_cdr_excel_data(){ /*-- For BSNL --*/
		...
		if(isset($_FILES["file"]["name"])){
			...
			if($client_id == 133 && $pro_id== 706){
				$rand_table = "qa_randamiser_conduent_data";
				$clmarr = array("call_date","aht","fusion_id","phone_number");
			}
			...
			foreach($object->getWorksheetIterator() as $worksheet){
				...
				//conduent
				if($client_id == 133 && $pro_id== 706){
					 $table_name = "qa_randamiser_conduent_data";
...
/*******************************************************************/
	...
	public function sample_cdr_download(){ /*-- For BSNL --*/
		if(check_logged_in()){ 
			...
			//conduent
			if($client_id == 37 && $pro_id== 49){
				$file_name = base_url()."qa_files/sampling_randamiser/conduent_randamiser_data.xlsx";
			}
			//vikas ends//
...
/*******************************************************************/
	public function data_randamise_compute_freshdesk(){ /*-- For Compute --*/
		if(check_logged_in()){
			...
			if($client_id == 133 && $pro_id== 706){
				$data["content_template"] = "qa_randamiser/randamiser/conduent/data_randamise_compute_freshdesk.php";
				$table = "qa_randamiser_conduent_data";
				$pro_cond .= " and is_assign_process(S.id,706)";
			}
		...
		/////////////// Filter Part Start //////////////
			...
			if($table == 'qa_randamiser_conduent_data'){
				// $call_type_sql="Select call_type from $table where call_type is not Null and call_type!='' and compute_status=1 group by call_type";
				// $data["call_type"]=$this->Common_model->get_query_result_array($call_type_sql);
		...
		/////////////// Filter Part End //////////////
			...
			//////////////////////// Add Compute Logic /////////////////////////
				...
				if($client_id == 133 && $pro_id== 706){
					...
					$comp_table = "qa_randamiser_compute_logic_conduent";
					//$comp_table = "qa_randamiser_compute_logic_blains";
					$rand_table = "qa_randamiser_conduent_data";
					$sel_field = "RCD.id as rand_id, RCD.fusion_id, RCD.aht,
									TIME_TO_SEC(aht) as aht_sec,
	...
	public function data_distribute_freshdesk(){ /*-- For BSNL --*/
		if(check_logged_in()){
			...
			if($this->input->get('submit')=='Submit')
			{
				...
				//Condulent
				if($client_id == 133 && $pro_id== 706){
					$rand_tbl='qa_randamiser_conduent_data';
					...
					$sel_distribute_fields="";
					$compCondTable = "qa_randamiser_compute_logic_conduent";
				}
...
/*******************************************************************/
	...
	public function compute_logiclist_delete($id,$client_id,$pro_id){ /*-- For BSNL --*/
		if(check_logged_in()){
			...
			redirect("Qa_randamiser_vikas/data_randamise_logiclist_freshdesk/".$redURL);
		}
	}
	...
	public function remove_data_cdr_upload_freshdesk(){ /*-- For BSNL --*/
		if(check_logged_in()){
			...
			if($client_id == 133 && $pro_id== 706){
				$tableName = "qa_randamiser_conduent_data";
				$redURL = 'qa_randamiser_vikas/data_upload_freshdesk/'.$client_id.'/'.$pro_id;
			}
