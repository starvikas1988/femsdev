...
function gen_csrf_token()
	...
	{
    	global $CI;
		if($CI->session->userdata('pre_logged_in')) return True;
		else{
			if(!$CI->input->is_cli_request()) redirect(base_url(),"refresh");
		}
	}
	...
	function get_pre_user_id()
	{
		...
		if(check_pre_logged_in()) return $rr["id"];
	}
	...
	function get_pre_user_fusion_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["fusion_id"];
	}
	...
	function get_pre_username()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return ucwords($rr["name"]);
	}
	...
	function get_pre_status()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["status"];
	}
	...
	function get_pre_login_type()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["login_type"];
	}
	...
	function get_pre_enable_2fa()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["is_enable_2fa"];
	}
	...
	function get_pre_email_id_off()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["email_id_off"];
	}
	...
	function get_pre_email_id_per()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["email_id_per"];
	}
	...
	function get_pre_phone()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["phone"];
	}
	...
	function get_pre_varify_email()
	{
    	global $CI;
    	$rr = $CI->session->userdata('pre_logged_in');
		if(check_pre_logged_in()) return $rr["is_varify_email"];
	}
	...
	function get_login_type()
	{
		...
		if(check_logged_in()) return $rr["login_type"];
	}
	...
	function get_clients_client_name()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["client_name"];
	}
	...
	function get_clients_allow_qa_module()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_module"];
	}
	...
	function get_clients_allow_qa_audit()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_audit"];
	}
	...
	function get_clients_allow_qa_review()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_review"];
	}
	...
	function get_clients_allow_mind_faq()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_mind_faq"];
	}
	...
	function get_clients_allow_qa_dashboard()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_dashboard"];
	}
	...
	function get_clients_allow_qa_dipcheck()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_dipcheck"];
	}
	...
	function get_clients_allow_qa_report()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_qa_report"];
	}
	...
	function get_clients_allow_calibration()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_calibration"];
	}
	...
	function is_access_client_qa_feedback(){
		...
		if(get_login_type()=="client"){
			if(get_clients_allow_qa_audit()=='1') return true;
			else return false;
		}else{
			return false;
		}
	...
	function is_access_client_qa_review(){
		...
		if(get_login_type()=="client"){
			if(get_clients_allow_qa_review()=='1') return true;
			else return false;
		}else{
			return false;
		}
	...
	function get_clients_allow_dfr_interview()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_dfr_interview"];
	}
	...
	function get_clients_allow_dfr_report()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_dfr_report"];
	}
	...
	function get_clients_allow_kyt_crm()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_kyt"];
	}
	...
	function get_clients_allow_diy_crm()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_diy"];
	}
	...
	function get_clients_allow_naps()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["allow_naps"];
	}
...
////////////////////
	...
	function get_username($user_id = "")
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) $full_name = ucwords($rr["name"]);
		
		if($user_id != ""){
			$qSQL = "SELECT concat(s.fname , ' ', s.lname) as fullname FROM signin s WHERE s.id = '$user_id'";
			$query = $CI->db->query($qSQL);
			$row = $query->result_array();
			$full_name = $row[0]['fullname'];
	...
	function get_deptname()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return ucwords($rr["dept_name"]);
	}
	...
	function get_deptshname()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return ucwords($rr["dept_shname"]);
	}
	...
	function get_dept_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["dept_id"];
	}
	...
	function get_dept_folder()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["dept_folder"];
	...
	function get_dept_folder_original()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["dept_folder_original"];
	}
	...
	function get_role()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["role"];
	}
	...
	function get_role_dir()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["role_dir"];
	}
	...
	function get_role_dir_original()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["role_dir_original"];
	}
	...
	function get_role_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["role_id"];
	}

	///WRONG FUNCTION. so commenting it
	/*
	function get_org_grade_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["role_id"];
	...
	*/
	...
	function get_global_access()
	{
		...
		if(check_logged_in()){
			...
			if($rr["is_global_access"] == "1") $ret = "1";
			else{
				if(get_role_dir()=="super") $ret = "1";
			}
	...
	function get_site_admin()
	{
		...
		if(check_logged_in()){
			...
			if($rr["is_site_admin"] == "1") $ret = "1";
			else{
				if(get_role_dir()=="admin") $ret = "1";
			}
			return $ret;			
		}
	}
	...
	{
		if(get_global_access()=='1') return "super/aside.php";
		else if(get_dept_folder()=="hr") return "hr/aside.php";
		else if(get_dept_folder()=="wfm") return "wfm/aside.php";
		else if(get_dept_folder()=="rta") return "wfm/aside.php";
		else if(get_site_admin()=="1") return "admin/aside.php";
		else if(get_login_type()=="client") return "user/client_aside.php";
		else if(is_access_cspl_my_team()) return "wfm/aside.php";
    	else return get_role_dir()."/aside.php";
	}
	...
	function get_user_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["id"];
	}
	...
	function get_user_omuid()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["omuid"];
	}
	...
	function get_user_fusion_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["fusion_id"];
	}
	...
	function get_user_site_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["site_id"];
	}
	...
	function get_user_office_id()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["office_id"];
	}
	...
	function get_user_country($user_id = ""){
    	...
    	$qSQL= "SELECT ol.location AS 'Country'
				FROM office_location ol
				WHERE ol.abbr IN (SELECT s.office_id
				  				  FROM signin s
				  				  WHERE s.id = $user_id)";
    ...
    function get_brand_uid($uid){
        
        global $CI;
		$fusion_id = get_user_fusion_id() ;
    	$qSQL= "SELECT brand FROM signin WHERE id = '".$uid."'";
	...
	function get_user_location_name()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["location_name"];
	}
	...
	function get_location_country()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["location_country"];
	}
	...
	function get_user_oth_office()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["oth_office"];
	}
	...
	function get_update_pswd()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["is_update_pswd"];
	}
	...
	function get_accept_consent()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["is_accept_consent"];
	}
	...
	function get_brand()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["brand"];
	}
	...
	function get_status()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["status"];
	}
	...
	function get_enable_2fa()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["is_enable_2fa"];
	}
	...
	function get_email_id_off()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["email_id_off"];
	}
	...
	function get_email_id_per()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["email_id_per"];
	}
	...
	function get_phone()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["phone"];
	}
	...
	function get_country()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["country"];
	}
	...
	function get_varify_email()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["is_varify_email"];
	}
	...
	*/
	...
	function get_client_ids()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["client_ids"];
	}
	...
	function get_client_names()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["client_names"];
	}
	...
	function get_process_ids()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["process_ids"];
	}
	...
	function get_process_names()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["process_names"];
	}
	...
	function get_assigned_to()
	{
    	global $CI;
    	$rr = $CI->session->userdata('logged_in');
		if(check_logged_in()) return $rr["assigned_to"];
	}
	...
	function get_assigned_to_name()
	{
		...
		if(check_logged_in()) return ucwords($res["fname"]." ".$res["lname"]);
		}
	}
	...
	#*******************************************************
	# Pulse And Anonymous Survey Report
	#*******************************************************

	function pulse_anonymous_survey_phil() {
		$ret = false;
		$ses_fusion_id = get_user_fusion_id();   
		$access_ids = array("FCEB006490", "FCEB000093", "FCEB006726", "FCEB005426", "FCEB006474");
	...
	function get_client_id_for_access(){

		global $CI;

		$client_ids = get_client_ids();

		$qSQL = "SELECT client_id FROM signin WHERE client_id = '" . $client_ids . "'";

		$query = $CI->db->query($qSQL);

		$row = $query->result_array();

		if($row[0]['client_id']=='234'){
			return true;
		}
	...
	function get_fems_user_qa_menu(){
		...
		if($client_id=="") $client_id=99999;
		if($process_id=="") $process_id=99999;
	...
	function isDisableFemsLogin($user_login_id){
		...
		if(strpos($disable_fems_ids,"#".$user_login_id."#")== false) $ret=false;
		else $ret=true;
		
		if($ret==false){
			if(strpos($disable_omuid_ids,"#".$user_login_id."#")== false) $ret=false;
			else $ret=true;
		}
	...
	function isDisableModule(){
		...
		if(strpos($disable_ids,"#".$ses_fusion_id."#")> 0) $ret=true;
	...
	function is_enable_coaching_break(){
		...
		*/
		...
		if(get_user_office_id() == "CEB" || get_user_office_id() == "ELS" || get_user_office_id() == "MAN" || get_dept_folder() == "transformation" || get_dept_folder() == "wfm") $ret=true;
		
		if($ret==false){
			 if($client_ids=="") $ret=false;
			 else {
				 //CentreLake Imaging
				 if (in_array('221', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_enable_my_team(){
		...
		$access_ids="##FKOL011173#FKOL005386#FKOL007211#FKOL011173#FJAM005877#FCAS000953#";
		$ret=false;
	...
	function is_enable_search(){
		...
		if(get_site_admin()=='1') $ret=true;
		
		if(get_dept_folder()=="hr" || get_dept_folder()=="Recruitment" || get_dept_folder()=="wfm"  || get_dept_folder()=="rta") $ret=true;

		return $ret;
	}


	function is_access_search_employee(){
		$ses_fusion_id=get_user_fusion_id();
		
		

		$access_ids=" #FELS004207#FKOL015829#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if(get_dept_folder()=="hr" || get_dept_folder()=="wfm"  || get_dept_folder()=="rta") $ret=true;
	...
	function is_access_cj_crm_modules(){
		...
		$access_ids=" #FKOL006359#FKOL006136#FKOL004674#FCEB005643#FCEB005640#FCEB005637#FCEB005638#FCEB005639#FCEB005636#FCEB005635#FCEB005645#FCEB004150#FCEB005633#FCEB003880#FCEB000729#FCEB004062#FCEB003947#FCEB000580#FKOL006714#FKOL001710#FKOL008602#FKOL007078#FCEB004062#FKOL007196#FCEB006165#FCEB006166#FCEB006167#FCEB003975#FCEB003525#FKOL006398#FCEB006802#FCEB006535#FCEB006562#FCEB006560#FCEB006803#FCEB006167#FCEB007129#FCEB007130#FCEB007131#FCEB007132#FCEB007133#FCEB007171#FCEB004933#FCEB000482#FKOL006359#FKOL006136#FKOL004674#FKOL016406#FCEB007416#FCEB007420#FCEB007421#FCEB007422#FKOL007362#FKOL015989#";
	...
	function is_cj_crm_admin(){
		...
		$access_ids="#FKOL006359#FKOL006136#FKOL004674#FCEB003880#FCEB000729#FCEB004062#FCEB003947#FCEB000580#FKOL006714#FKOL001710#FKOL008602#FKOL007078#FKOL007196#FCEB003975#FCEB003525#FCEB006802#FCEB006535#FCEB006562#FCEB006560#FCEB006803#FCEB006167#FCEB000482#FKOL016406#";
	...
	function isDisableFemsChat(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('1', explode(',',$client_ids)) == true) $ret=true;
				 else if (in_array('157', explode(',',$client_ids)) == true) $ret=true;
				 //else if (in_array('17', explode(',',$client_ids)) == true) $ret=true;
				 else if (in_array('19', explode(',',$client_ids)) == true) $ret=true;
	...
	function isKatAvailable(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids=" #FCEB004126#FCEB004264#FCEB000688#FCEB005357#FCEB003525#FCEB000029#FCEB000791#FCEB003423#FCEB004238#FCEB004355#FCEB004009#FCEB004889#FCEB003880#FCEB004073#FCEB004064#FMAN000131#FMAN001057#FMAN000762#FMAN001151#FCEB003365#FCEB000078#FCEB000078#FCEB000029#FCEB004335#FCEB000457#FMAN000410#FCEB000386#FCEB005369#FCEB000340#FCEB000722#FCEB000722#FCEB000340#FCEB003423#FCEB000383#FCEB004264#FCEB003441#FCEB000142#FCEB004126#FCEB004173#FCEB003947#FCEB004217#FCEB005192#FCEB005193#FCEB005430#FCEB000553#FCEB004921#FCEB005356#FCEB005078#FCEB000757#FCEB005921#FCEB005297#FCEB004672#FCEB000114#FCEB003839#FCEB000333#FCEB004974#FCEB005110#FCEB006470#FJAM004938#FJAM005934#FJAM005382#";
	...
	function isAccessKatExam(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FCEB004126#FCEB004264#FCEB000688#FCEB005357#FCEB003525#FKOL000001#FKOL000007#FCEB000004#FCEB000029#FCEB000791#FCEB003423#FCEB004238#FCEB004355#FCEB004009#FCEB004889#FCEB003880#FCEB004073#FCEB004064#FMAN000131#FMAN001057#FMAN000762#FMAN001151#FMAN001074#FCEB003365#FCEB000078#FCEB000078#FCEB000029#FCEB004335#FCEB000457#FMAN000410#FCEB000386#FCEB005369#FCEB000340#FCEB000722#FCEB000722#FCEB000340#FCEB003423#FCEB000383#FCEB004264#FCEB003441#FCEB000142#FCEB004126#FCEB004173#FCEB003947#FCEB004217#FCEB005192#FCEB005193#FCEB005430#FCEB000553#FCEB004921#FCEB005356#FCEB005078#FCEB000757#FCEB005921#FCEB005297#FCEB004672#FCEB000114#FCEB005433#FCEB000155#FCEB005424#FCEB003839#FCEB000333#FCEB004974#FCEB005110#FCEB006470#FJAM004938#FJAM005934#FJAM005382#";
		...
		if(get_dept_folder()=="wfm") $ret=true;
	...
	function is_access_global_mindfaq(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids="##FKOL000001#FKOL000007#FKOL001985#FKOL005388#FKOL005828#FKOL006573#FKOL006998#FCEB004639#FKOL006390#FKOL008587#FKOL008598#FKOL005767#FKOL002702#FKOL007078#FKOL002245#FKOL001854#FKOL004812#FKOL006795#FKOL008559#FKOL006065#FKOL001900#FKOL011685#FKOL001929#FKOL001961#FKOL012753#FKOL011554#FKOL013064#FHIG000013#FKOL012858#FKOL013936#FKOL011883#FKOL011883#";
		...
		if(get_login_type()=='client' && get_user_id() ==319 ) $ret=true;
	...
	function is_access_k2crm_modules(){
		...
		$access_ids="##FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('142', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_zovio_modules(){
		...
		$access_ids=" #FKOL002200#FKOL007078#FKOL005611#FCEB004639#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('185', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_howard_modules(){
		...
		$access_ids=" #FKOL007078#FKOL008610#FKOL007187#FKOL001929#FKOL001961#FHIG000301#FHIG000211#FKOL002200#FKOL005611#FSPI000105#FHIG000211#FHIG000302#FSPI000752#FHIG000301#FHIG000007#FHIG000210#FSPI000005#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if(get_global_access()=='1') $ret=true;
		if($ret==false){
			 if($process_ids=="") $ret=false;
			 else {
				 if (in_array('581', explode(',',$process_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_kyt_modules(){
		...
		if(get_login_type()=='client') $ret=true;
		if(get_global_access()=='1') $ret=true;
	...
	function is_access_qenglish_modules(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('250', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	}
	...
	function is_access_alpha_gas_modules(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('104', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_jurys_inn_crm_report(){
		...
		$access_ids=" #FKOL001893#FCEB004639#FCEB004889#FALB000001#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if(is_access_global_mindfaq()==true) $ret=true;
		
		if($ret==false){
			 if($client_ids=="") $ret=false;
			 else {
				 if (in_array('124', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_mpower_voc_report(){
		...
		$access_ids=" #FCEB000418#FCEB003939#FCEB000413#FCEB000018#FCEB004639#FCEB000005#FCEB004358#FKOL003087#FCEB000409#FKOL007362#FKOL015989#";
		$ret=false;
	...
	function is_access_client_voc(){
		...
		$access_ids=" #FCEB000005#FKOL007362#FKOL015989#";
		$ret=false;
	...
	function is_access_duns_crm(){
		...
		$access_ids=" #FKOL001961#FKOL007078#FCAS000848#FCAS000200#FCAS000143#FCAS000203#FCAS000038#FCAS000117#FCAS000111#FCAS000036#FCAS000037#FCAS000121#";
		$ret=true;
	...
	function is_access_apphelp_modules(){
		...
		*/
		...
		$access_ids=" #FCEB004639#FMON004163#FKOL003616#FSPI000170#FKOL007149#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('7', explode(',',$client_ids)) == true) $ret=true;
				 else if (in_array('12', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_all_locations(){
		
		$client_ids=get_client_ids();
		$ses_fusion_id=get_user_fusion_id();
		
		$access_ids=" #FKOL012753#";
		$ret=false;
	...
	function is_access_affinity_modules(){
		...
		*/
		...
		$access_ids=" #FCEB004639#FKOL003616#FSPI000170#FKOL007149#FSPI000001#FSPI000086#FSPI000127#FSPI000105#FHIG000302#FKOL008598#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($process_ids=="") $ret=false;
				 ...
				 if (in_array('266', explode(',',$process_ids)) == true) $ret=true;
			 }
		}
		...
		*/
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('39', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_jurys_inn_modules(){
		...
		John Christian Dela Peña	FCEB004189
		Charles Bellord Tellidua	FCEB005474
		Yvan Gerald Acuzar	FCEB000385
		Jarrell Kitz Reinante	FCEB004922
		Wendy Joy Wenceslao	FCEB004739
		Remie Jonathan Biñan	FCEB000722
		Rey Mart Fajardo	FCEB000340
		Leann Lesserreh Maluay	FCEB000386
		Syra Jane Basubas	FCEB003423
		...
		*/
		...
		$access_ids=" #FCEB003302#FCEB004265#FCEB004128#FCEB000584#FCEB000766#FCEB005191#FALB000001#FCEB000791#FCEB000965#FCEB004238#FCEB004354#FCEB004355#FCEB003423#FCEB004639#FCEB004889#FCEB004189#FCEB005474#FCEB000385#FCEB004922#FCEB004739#FCEB000722#FCEB000340#FCEB000386#FKOL007362#FKOL015989#";
		$ret=false;
	...
	function is_access_meesho_modules(){
		...
		$access_ids=" #FHWH005204#FHWH005243#FHWH002540#FHWH005281#FHWH005188#FHWH005187#FHWH005220#FHWH005215#FHWH005189#FKOL001985#FKOL006573#FKOL005828#FKOL005388#FKOL006426#FCHE000739#FKOL002880#FKOL008612#FKOL002519#FKOL007362#FKOL015989#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('105', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_kabbage_modules(){
		...
		$access_ids=" #FCEB003365#FMAN000131#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('140', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_brightway_modules(){
		...
		if($ret==false){
			 if($process_ids=="") $ret=false;
				 ...
				 if (in_array('377', explode(',',$process_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_snapdeal_modules(){
		...
		$access_ids=" #FKOL003616#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('223', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_cars24_modules(){
		...
		$access_ids="-#FCHA005320#FCHA005325#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('246', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
    ...
    function is_access_cars24_revision_modules(){
		...
		$access_ids="-#FKOL013064#FKOL005828#FKOL006573#";
		$ret=false;
	...
	function is_access_awareness_modules(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('22', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_paynearby_modules(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('115', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
    ...
    function is_access_cci_modules(){
		...
		if($ret==false){
			 if($process_ids=="") $ret=false;
				 ...
				 if (in_array('264', explode(',',$process_ids)) == true) $ret=true;
				 else if (in_array('263', explode(',',$process_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_phs_modules(){
		...
		$access_ids="##FCEB005078#FKOL006355#FMAN000131#FCEB003516#FCEB004981#FMAN001069#FCEB000739#FCEB004317#FKOL007362#FKOL015989#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('196', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_clio_modules(){
		...
		$access_ids="##FCEB000757#FCEB000688#FCEB004264#FCEB004126#FCEB003880#FCEB006520#FKOL007362#FKOL015989#";
		$ret=false;
	...
	function is_access_att_modules(){
		...
		$access_ids="#*#FMON004394#FFLO000173#FFLO000241#FFLO000164#FFLO000185#";
		$ret=false;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('157', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function is_access_achieve_modules(){
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('209', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function isAccessMindfaqApps(){
		...
		if(is_access_global_mindfaq()==true) $ret=true;
		else if(get_global_access()=='1') $ret=true;
		else if(get_site_admin()=='1') $ret=true;
		else if(is_access_mpower_modules()==true) $ret=true;
		else if(is_access_jurys_inn_modules()==true) $ret=true;
		else if(is_access_affinity_modules()==true) $ret=true;
		else if(is_access_apphelp_modules()==true) $ret=true;
		else if(is_access_meesho_modules()==true) $ret=true;
		else if(is_access_kabbage_modules()==true) $ret=true;
		else if(is_access_brightway_modules()==true) $ret=true;
		else if(is_access_snapdeal_modules()==true) $ret=true;
		else if(is_access_cars24_modules()==true) $ret=true;
		else if(is_access_awareness_modules()==true) $ret=true;
		else if(is_access_paynearby_modules()==true) $ret=true;
		else if(is_access_cci_modules()==true) $ret=true;
		else if(is_access_phs_modules()==true) $ret=true;
		else if(is_access_clio_modules()==true) $ret=true;
		else if(is_access_att_modules()==true) $ret=true;
		else if(is_access_achieve_modules()==true) $ret=true;
	...
	function isAccessMindfaqReports(){
		...
		else if((get_role_dir()!="agent" && is_access_mpower_modules()==true) || (get_dept_folder()!="operations" && is_access_mpower_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_jurys_inn_modules()==true) || (get_dept_folder()!="operations" && is_access_jurys_inn_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_affinity_modules()==true) || (get_dept_folder()!="operations" && is_access_affinity_modules()==true)) $ret=true;
		else if( (get_role_dir()!="agent" && is_access_apphelp_modules()==true) || (get_dept_folder()!="operations" && is_access_apphelp_modules()==true)) $ret=true;
		else if( (get_role_dir()!="agent" && is_access_meesho_modules()==true) || (get_dept_folder()!="operations" && is_access_meesho_modules()==true)) $ret=true;
		else if( (get_role_dir()!="agent" && is_access_kabbage_modules()==true) || (get_dept_folder()!="operations" && is_access_kabbage_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_brightway_modules()==true) || (get_dept_folder()!="operations" && is_access_brightway_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_snapdeal_modules()==true) || (get_dept_folder()!="operations" && is_access_snapdeal_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_cars24_modules()==true) || (get_dept_folder()!="operations" && is_access_cars24_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_awareness_modules()==true) || (get_dept_folder()!="operations" && is_access_awareness_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_paynearby_modules()==true) || (get_dept_folder()!="operations" && is_access_paynearby_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_cci_modules()==true) || (get_dept_folder()!="operations" && is_access_cci_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_phs_modules()==true) || (get_dept_folder()!="operations" && is_access_phs_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_clio_modules()==true) || (get_dept_folder()!="operations" && is_access_clio_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_att_modules()==true) || (get_dept_folder()!="operations" && is_access_att_modules()==true)) $ret=true;
		else if((get_role_dir()!="agent" && is_access_achieve_modules()==true) || (get_dept_folder()!="operations" && is_access_achieve_modules()==true)) $ret=true;
	...
	function isAccessAvonReports(){
		...
		$access_ids=" #FALB000267#FKOL001961#FKOL007078#FKOL002542#FKOL007301#FKOL007614#FKOL007196#FKOL003098#FKOL015417#FKOL010038#FKOL007362#FKOL015989#";
		$ret=true;
	...
	function isAccessReports(){ 
		...
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids=" #FKOL006426#FALB000267#FCHA001040#FKOL007860#FALB0002670#FALB000189#FALB000072#FCAS000898#FELS003768FCAS000868#FCAS001004#FHWH002502#FHWH000010#FHWH002515#FHWH008825#FHWH007150#FKOL012183#FHWH006410#FCEB000333#FMAN001145#FCEB000553#FCEB000078#FCEB003983#FCAS000953#FKOL000007#FELS003427#FHIG000239#FMON004090#FMON004089#FCEB003423#FSPI000109#FSPI000110#FKOL001738#FCEB000791#FMAN001140#FCEB000011#FCEB004485#FCEB004009#FCEB000965#FCEB004354#FALB000026#FCEB004355#FCEB000791#FCEB003423#FCEB004238#FELS003419#FHWH002648#FHWH002648#FALB000001#FCEB004450#FBLR000155#FKOL002619#FKOL006885#FKOL001804#FKOL002896#FKOL007975#FCAS000007#FKOL000006#FKOL005386#FSPI000004#FKOL007906#FKOL001854#FKOL002245#FKOL004812#FSPI000108#FHIG000301#FCHA000264#FCHA000262#FKOL008613#FKOL008629#FKOL009915#FCEB000482#FCEB000526#FCEB000502#FCEB005433#FELS004207#FKOL001900#FKOL011815#FKOL005826#FKOL006742#FCEB004073#FCEB003975#FKOL007860#FALB000079#FALB000085#FALB000167#FALB000188#FALB000189#FALB000101#FKOL003779#FKOL007158#FKOL001899#FCHA005112#FCAS000484#FKOL000162#FKOL007512#FKOL008409#FJAM005481#FJAM005224#FKOL008441#FJAM004099#FJAM004208#FKOL001934#FELS003773#FKOL007304#FJAM004092#FELS003595#FELS003034#FELS003295#FELS005281#FKOL007211#FALB000240#FALB000153#FALB000256#FKOL001269#FALB000082#FALB000111#FKOL003040#FCHE000633#FCAS000434#FCAS000458#FCAS000154#FHIG000005#FCAS000078#FCAS000076#FCAS000075#FCAS000158#FCAS000198#FCAS000157#FCAS000156#FCAS000196#FCAS000200#FCAS000027#FCAS000143#FCAS000152#FCAS000123#FCAS000077#FCAS000203#FELS001565#FKOL005767#FKOL001919#FALB000151#FALB000158#FALB000204#FALB000315#FALB000370#FMAN000410#FJAM005464#FALB000441#FMAN000343#FJAM005935#FKOL006655#FKOL008502#FKOL012215#FCAS000857#FCAS000169#FCAS000484#FKOS000002#FALB000148#FALB000193#FKOL001901#FKOL007187#FCEB006948#FFLO000546#FCHA005435#FCHA002295#FCAS000187#FCAS001003#FKOL015960#FNOI011364#FNOI011335#FNOI011369#FNOI011323#FNOI011403#FNO1011404#FNO1011407#FJAM003056#FJAM005332#FJAM005219#FJAM005482#FJAM005059#FJAM004465#FJAM004104#FJAM005252#FJAM004531#FJAM004103#FJAM005037#FJAM005084#FCHA012589#FCHA002751#FCHA003482#FCHA007392#FCHA003046#FSPI000942#FKOL006563#FHWH005661#FCHA013509#FKOL010590#FELS003456#FELS003172#FCAS000868#FELS003768#FALB000128#FKOL008317#FKOL015829#FELS000380#FMON004037#FNOI011473#FMAN000351#FHWH007704#FCEB006171#FMAN001981#FKOL012621#FCAS000154#FCAS000458#FCAS000953#FCAS001004#FCAS000848#FSPI000753#FCEB006530#FCHE001981#FCEB006520#";
		$ret=false;
		...
		if( get_dept_folder()=="hr" ||  get_dept_folder()=="mis" ||  get_dept_folder()=="wfm" ||  get_dept_folder()=="rta" ||  get_dept_folder()=="accounts" || get_dept_folder()=="qa" || get_dept_folder()=="Recruitment" || (get_dept_folder()=="operations" && get_role_dir()=="manager") ) $ret=true;
		
		
		return $ret;
	}

    function isAccessLetters(){
		
		$client_ids=get_client_ids();
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids=" #FKOL010158#FKOL012755#FKOL005386#FKOL007860#FKOL011173#FKOL008807#FKOL014701#FKOL012020#FKOL007937#FKOL001766#FKOL007911#FKOL007957#FCHA000283#FCHA000327#FCHA000280#FCHA000326#FCHA000262#FCHA003519#FCHA000264#FCHA000278#FCHA000279#FKOL000003#FKOL005837#FKOL015114#FKOL010419#FKOL012897#FKOL006484#FCEB006490#FCEB000093#FCEB005426#FHIG000722#FKOL015827#FKOL002536#FKOL000007#FKOL000001#";
		$ret=false;
	...
	function is_access_downtime_tracker(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();#
		$access_ids=" #FKOL001754#FKOL004711#FKOL000077#FKOL006793#FKOL007362#FKOL015989#";
		$ret=false;
	...
	function is_access_tna_reports(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids=" #FKOL006655#FCEB000013#FKOL014737#FKOL012215#FKOL008502#";
		$ret=false;
	...
	function is_access_t2_capabilities(){
		...
		*/
		...
		if(get_global_access()=='1' || ( get_user_office_id()=="CEB" && $client_id=='30')) $ret=true;
	...
	function is_access_oyo_disposition(){
		...
		*/
		...
		if(($process_id=='171' || $process_id=='181') && $client_id=='90') $ret=true;
	...
	function is_access_clinic_portal(){
		...
		*/
		$ses_fusion_id=get_user_fusion_id();
		$client_id=get_client_ids();
		$access_ids=" #FKOL001961#FCEB003836#FCEB000089#FCEB004453#FMAN000456#FCEB000093#FKOL007362#FKOL015989#";
		$ret=false;
		
		if( get_global_access()=='1' ) $ret=true;
		else if(get_user_office_id()=="CEB" || get_user_office_id()=="MAN") $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
			else $ret=true;
		}
	...
	function is_access_clinic_portal_distro()
	{
		...
		else if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
...
//////////FNF /
	...
	{
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL015908#FKOL001771#FKOL012213#FMUM000051#FKOL005575#FJAM004503#FKOL012897#FDUR000125#FCHE000367#FKOL006366#FKOC000138#FMUM000048#FNOI000140#FJMP000071#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL008638#FKOL010888#FCHA003550#FCHA002016#FKOL008233#FKOL015716#FCHA002102#FKOL013738#FKOL015865#FKOL015979#FKOL017980#FKOL015178#FKOL015739#FKOL015980#FKOL015402#FKOL001919#FKOL001917#FKOL016690#FCEB004473#FKOL007362#FKOL015989#";		
		$ret=false;				
		if(get_global_access()=='1') $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
	...
	function isAccessFNFHr(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FJAM005935#FELS004207#FELS00017#FKOL000003#FKOL003187#FKOL010158#FKOL012897#FNOI000565#FCEB004473#";
		
		$ret=false;
		
				
		if(get_global_access()=='1' || get_dept_folder()=="hr" ) $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
			else $ret=true;
		}
	...
	function isAccessFNFITSecurity(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL001786#FKOL004016#FNOI000140#FJMP000071#FKOL001919#FKOL001917#FCEB004473#";
	...
	function isAccessFNFITHelpdesk(){
		...
		/*
			Mumbai: FMUM007504 Omkar Mhetre
			Kalyani: FKLY000097 Ananda Biswas
			Durgapur and Jamshedpur: FKOL008451 Debojit Ghosh
			Howrah: FKOL006366 Pijush Bank	
		*/

		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKLY000097#FKOL008451#FCHE010691#FMUM007504#FNOI000140#FKOL015908#FKOL013653#FKOL015402#FKOL015980#FKOL015908#FKOL002504#FKOL001803#FKOL001780#FKOL001782#FKOL005575#FKOL001774#FKOL001790#FKOL005557#FKOL002505#FKOL001777#FKOL002501#FKOL001779#FKOL007297#FKOL007405#FKOL007232#FKOL007508#FKOL008233#FKOL006963#FCHE000112#FKOL001788#FKOL008638#FKOL008600#FKOL009320#FKOL008262#FKOL008627#FKOL012213#FJAM004503#FDUR000125#FCHE000367#FKOL006366#FKOC000138#FMUM000048#FNOI000140#FJMP000071#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL008638#FKOL010888#FCHA003550#FCHA002016#FKOL008233#FKOL015716#FCHA002102#FKOL013738#FKOL015865#FKOL015979#FKOL017980#FKOL015178#FKOL015739#FKOL001919#FKOL001917#FCHA003575#FCHA026574#FCHA002017#FCHA001874#FJAM005015#FJAM006235#FJAM006077#FJAM006320#FJAM004148#FCEB004473#";
	...
	function isAccessFNFITNetwork(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKLY000097#FKOL008451#FKOL008451#FCHE010691#FMUM007504#FNOI000140#FKOL015908#FKOL013653#FKOL015402#FKOL015980#FKOL015908#FKOL002504#FKOL001803#FKOL001780#FKOL001782#FKOL005575#FKOL001774#FKOL001790#FKOL005557#FKOL002505#FKOL002501#FKOL001779#FKOL007297#FKOL007405#FKOL007232#FKOL007508#FKOL008233#FKOL006963#FCHE000112#FKOL001788#FKOL008638#FKOL009320#FKOL008262#FKOL008451#FKOL008627#FKOL012213#FJAM004503#FDUR000125#FCHE000367#FKOL006366#FKOC000138#FMUM000048#FNOI000140#FJMP000071#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL008638#FKOL010888#FCHA003550#FCHA002016#FKOL008233#FKOL015716#FCHA002102#FKOL013738#FKOL015865#FKOL015979#FKOL017980#FKOL015178#FKOL015739#FKOL001919#FKOL001917#FCEB004473#";
	...
	function isAccessFNFITGlobalHelpdesk(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKLY000097#FCHE010691#FMUM007504#FNOI000140#FKOL015908#FKOL013653#FKOL015402#FKOL015980#FKOL015908#FKOL002504#FKOL001803#FKOL001780#FKOL001782#FKOL005575#FKOL001774#FKOL001790#FKOL005557#FKOL002505#FKOL001777#FKOL002501#FKOL001779#FKOL007297#FKOL007405#FKOL007232#FKOL007508#FKOL008233#FKOL006963#FCHE000112#FKOL001788#FKOL008638#FKOL009320#FKOL008262#FKOL008451#FKOL008627#FKOL012213#FJAM004503#FDUR000125#FCHE000367#FKOL006366#FKOC000138#FMUM000048#FNOI000140#FJMP000071#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL008638#FKOL010888#FCHA003550#FCHA002016#FKOL008233#FKOL015716#FCHA002102#FKOL013738#FKOL015865#FKOL015979#FKOL017980#FKOL015178#FKOL015739#FKOL001919#FKOL001917#FCEB004473#";
	...
	function isAccessFNFPayroll(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL001766#FKOL001709#FKOL004086#FKOL006124#FKOL007911#FKOL008639#FKOL008807#FCHA000264#FCHA000262#FKOL011173#FKOL007860#FKOL005386#FKOL007211#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FKOL014701#FCEB004473#";
	...
	function isAccessFNFAccounts(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL001725#FKOL005795#FKOL001730#FKOL012213#FJAM004503#FNOI000140#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL001919#FKOL001917#FCEB004473#";
...
///////////////	
	...
	function isAccessEGazeReport(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="-#FCEB000748#FKOL006573#FKOL005828#FKOL013064#FCAS000154#FCAS000458#FCAS000953#FCAS001004#FCAS000848#";
	...
	function is_access_master_entry(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="-#FKOL006359#FALB000189#FCEB000582#FKOL001916#FKOL001899#FMON002590#FJAM006076#FJAM005935#FJAM005464#FELS004207#FHIG000314#FKOL007108#FKOL012897#FKOL007362#FKOL015989#";
		
		$ret=false;
						
		if(get_global_access()=='1' || get_dept_folder() == 'transition') $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
			else $ret=true;
		}
...
function is_access_announcment(){
	
    $ses_fusion_id = get_user_fusion_id();
    $access_ids = "-#FKOL010158#FKOL016146#FKOL010535#FNOI000565#FKOL010321#FMUM007550#FKOL015357#FJAM006076#FJAM005935#FJAM005464#FELS004207#";
	...
	function isDisablePayrollInfo($user_office_id=""){
		...
		if($user_office_id=="")	$user_office_id=get_user_office_id();
	...
	function isUSLocation($user_office_id=""){
		...
		$skipArray = array("ALT","MON", "DRA", "FTK", "HIG", "MIN", "SPI", "TEX", "UTA", "CAM","FLO");
		if($user_office_id=="") $user_office_id=get_user_office_id();  
		return in_array($user_office_id, $skipArray);
	}
	...
	function is_access_neo_chatbot(){
		...
		if(isIndiaLocation(get_user_office_id()) == true ) $ret=true;
	...
	function isUploadKnowledgeBase(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FCEB000093#FALT002335#FALT002330#FALT002329#FHIG000032#FHIG000301#FJAM004099#FELS001494#FKOL001763#FKOL004242#FKOL006266#FKOL006539#FKOL002966#FELS003424#FELS003830#FCHA000004#FCHA000285#FFLO000053#FELS003561#FJAM005382#FJAM004103#FJAM005224#FKOL013736#FCHE000235#FJAM005877#FJAM004938#FJAM005889#FJAM005464#";
		
		$ret=false;
		
		if(get_dept_folder()=="training" || get_global_access()=='1' || get_site_admin()=='1' ) $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
			else $ret=true;
		}
	...
	function is_access_ld_registration(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL000001#FKOL006655#FCEB000013#FCEB004159#FKOL008502#FELS001494#FCEB005596#FFLO000062#";
	...
	function isUpdatePolicy(){
		...
		$access_ids=" #FCEB000766#FKOL000001#FKOL001929#FKOL001961#FCEB000017#FCEB000014#FCEB000748#FKOL001766#FMAN000456#FCEB000582#FCEB000337#FCEB000750#FCEB000308#FCEB000751#FJAM004503#FCEB000092#FALT002330#FHIG000185#FSPI000110#FCEB004452#FKOL011751#FKOL008806#FKOL010158#FKOL015830#FMON004482#FALB000189#FCAS000124#";
		...
		if($ret==false){	
			if(get_global_access()=='1' || get_site_admin()==1) $ret=true;
			else $ret=false;
		}
	...
	function isProcessUpdates(){
		...
		*/
		...
		if(get_global_access()=='1' || get_site_admin()==1 || get_role_dir() == "manager" || get_role_dir() == "tl") $ret=true;
		else $ret=false;
	...
	*/
	...
	function is_access_Personal_Info($pfid=""){
		...
		*/
		...
		$ses_omuid_id=get_user_omuid();
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" ##FCAS000953#FKOL003187#FKOL000003#FKOL005837#FHWH002596#FCHE000227#FNOI000141#FKOL007562#FKOL007099#FKOL003663#FKOL001854#FKOL002245#FKOL004812#FKOL010158#FKOL001900#FKOL015827#";
		
		$ret=false;
		
		if(get_dept_folder()=="hr" || get_global_access()=='1' || get_site_admin()=='1') $ret=true;
		if(isIndiaLocation(get_user_office_id())==true) $ret=false;
	...
	function is_update_ad_office_location($user=""){
		...
		FHIG000302    Terri Peterman
		FKOL008598    Debanjan Guha
		FACS000085    Brittany Bridges
		*/
		
		$ses_omuid_id=get_user_omuid();
		$ses_fusion_id=get_user_fusion_id();
		if(!empty($user)){ $ses_fusion_id= $user; }
		$access_ids=" ##FHIG000142#FHIG000302#FKOL008598#FACS000085#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
	...
	function is_update_office_location_limit($user=""){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		if(!empty($user)){ $ses_fusion_id= $user; }
		$access_ids=" ##FCAS000953#FHIG000170#FHIG000013#FHIG000309#FSPI000019#FSPI000105#FUTA000001#FHIG000314#FKOL002702#FKOL000003#FKOL010158#FKOL002536#FKOL000007#FACS000085#FHIG000142#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
	...
	function is_update_profile($pfid){
		...
		$ses_omuid_id=get_user_omuid();
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" ##FCAS000953#FJAM002325#FCEB000748#FJAM004503#FMAN000342#FBLR000327#FMAN000343#FCEB000236#FCEB000492#FKOL001854#FKOL002245#FKOL004812#FKOL007562#FCEB000746#FKOL001901#FCHA000571#FCHA003182#FCHA002098#FKOL001900#FKOL010158#";
		
		$ret=false;
		
		if(get_dept_folder()=="hr" || get_user_fusion_id()==$pfid || get_global_access()=='1' || get_site_admin()=='1' ) $ret=true;
		else{
		...
		if(is_update_system_data()==true) $ret=true;
	...
	function is_view_profile($pfid){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" ##FCAS000953#FCEB004160#FCEB000492#FCEB000268#FCEB000746#FCHA000571#FCHA003182#FCHA002098#FKOL001900#FKOL011173#";
		...
		if(get_role_dir()!="agent" ) $ret=true;
		if(get_dept_folder()=="training" ) $ret=true;
	...
	function is_view_social_security_info($pfid){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" ##FKOL000003#FKOL002960#FKOL005837#FKOL003187#FKOL006124#FKOL002206#FKOL007099#FKOL008639#FCHA000264#FCHA000262#FKOL010158#FKOL014701#FKOL015827#";
		$ret=false;
	...
	function is_update_payroll_info(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000003#FKOL002960#FKOL005837#FKOL003187#FKOL008643#FKOL008803#FKOL007562#FKOL010158#FKOL002328#FKOL010419#FKOL011477#FKOL011172#FKOL001929#FKOL015827#";
		$ret=false;
	...
	function is_access_operations_module(){
		...
		if(get_dept_folder()=="operations" || get_global_access()=='1' || get_dept_folder()=="hr" ) return true;
		else return false;
		//return true;
	}
	...
	function is_access_qa_agent_module(){
		if( get_dept_folder()== "operations" && get_role_dir()=='agent') return true;
		else if(get_user_fusion_id()=='FMON004368' || get_user_fusion_id()=='FSPI000816'){ return true; }
		else return false;
	}
...
///////////////// Edit Audit Access //////////////////////
	function is_access_qa_edit_feedback(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000001#FKOL000023#FKOL005863#FELS003424#FHIG000301#FKOL003087#FCEB004358#FCEB000757#FCEB000764#FMAN000616#FCEB000525#FMAN000360#FCEB004836#FJAM004102#FJAM004208#FJAM004104#FJAM004103#FJAM003056#FKOL000162#FKOL001932#FKOL002896#FCAS000140#FCHE000795#FELS003363#FKOL006539#FCHE000798#FKOL007975#FKOL007975#FKOL007057#FKOL004242#FKOL008253#FCAS000007#FMAN001310#FCEB004402#FNOI000139#FNOI000412#FCEB003805#FKOL008612#FCEB005630#FCEB005767#FCEB005921#FCHA001660#FCHA001661#FCHA001390#FCHA005930#FCHA005929#FCHA001440#FCHA004790#FCHA001662#FCHA005401#FCHA005580#FCHA005208#FELS003561#FELS004874#FMAN000612#FKOL011817#FKOL010073#FMUM006114#FCEB006380#FNOI010808#FCAS00007#FCEB006797#FHIG00295#FHIG00210#FHIG000186#FHIG000047#FMIN000050#FELS005900#FKOL011897#FCEB004799#FBLR000155#FNOI000567#FKOL001800#FSPI000386#FKOL010196#FHIG000189#FCEB005130#FCEB007466#FELS005904#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
...
/////////////////// Randamiser/Sampling //////////////////////
	function is_access_randamiser(){
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000001#FKOL001800#FMUM007614#FBLR000155#FKOL000023#FCHA005208#FMUM006114#FDUR000199#FCHA002927#FCHA002295#FMUM005923#FMAN000612#FCEB006380#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
		
		//if(get_dept_folder()=="qa" || get_global_access()=='1') $ret=true;
		
		return $ret;
	}
	
/////////////////// Agent Categorisation //////////////////////
	function is_access_agent_categorisation(){
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000001#FKOL001800#FMUM007614#FBLR000155#FKOL000023#FCHA005208#FMUM006114#FDUR000199#FCHA002927#FCHA002295#FMUM005923#FMAN000612#FCEB006380#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
...
/*------------------ BOOMSOURCING --------------------*/
	function is_access_qa_boomsourcing(){
		
		/* $ses_fusion_id=get_user_fusion_id();
		$access_ids="#FKOL000001#FKOL001800#";
		$user_ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $user_ret=false;
		else $user_ret=true; */
		
		$user_ret=false;
		
		if((get_user_office_id()=="LEZ" || get_user_office_id()=="ORT" || get_user_office_id()=="SLG" || get_user_office_id()=="SPI") && get_client_ids()=="275") 
		$user_ret=true;
...
////////////////////////////////////////////////////////
	...
	function is_access_qa_operations_module(){
		...
		if(get_login_type()=="client"){
			...
			if(get_dept_folder()=="cs"  || (get_dept_folder()== "operations" && get_role_dir() !='agent') ) return true;
			else return false;
		}
	...
	function is_access_coach_module(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_coach_ids="##FHIG000001#FKOL002150#FJAM005224#FJAM004099#FCEB005630#FCEB000504#FCEB003609#FCEB000516#FCEB000157#FCEB004328#FELS000376#FJAM002219#FSPI000386#FSPI001057#FELS003510#FELS005680#FELS005755#FFLO000562#FELS005579#FELS004754#FELS005925#FELS003548#FCEB006948#FBLR000155#FELS000380#FCEB000155#FCEB000386#FMON004037#FSPI001020#FSPI000753#FHIG000939#FFLO000757#FCHA005929#";
		$coach_ret=false;
		if(strpos($access_coach_ids,"#".$ses_fusion_id."#")== false) $coach_ret=false;
		else $coach_ret=true;
...
//////////// Traner QUALITY Access (10/11/2020 - Deb) //////////////	
	function is_quality_access_trainer(){
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL002635#FKOL003729#FKOL002157#FKOL002536#FKOL001271#FHWH002527#FHWH002648#FHWH002651#FKOL005826#FKOL003050#FHWH000010#FHWH002502#FCHE000368#FKOL002619#FKOL006742#FKOL006660#FKOL005356#FNOI000320#FKOL006785#FKOL006426#FKOL002536#FKOL003689#FKOL002619#FCHE000798#FNOI000572#FJAM005889#FMAN000343#FJAM005935#FJAM00505#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
...
//////////////////////////////
	...
	function is_access_login_dtl_entry()
	{
		...
		*/
		...
		$access_ids="##FJAM004241#FELS001178#FKOL001709#FELS000328#FELS000025#FELS000939#FELS000445#FALT001942#FJAM002219#FCEB000748#FCEB000582#FCEB000337#FCEB000750#FCEB000308#FCEB000751#FJAM005382#FJAM005374#FHIG000189#FMAN000343#";
	...
	function is_access_mandatory_report(){
		...
		$access_ids="##FCEB003983#FCAS000953#FKOL000003#FKOL001766#FCEB000748#FKOL001732#FJAM004503#FKOL004086#FBLR000003#FKOL006124#FKOL003187#FKOL002206#FKOL007099#FKOL006885#FKOL007911#FKOL008639#FKOL008807#FCHA000264#FCHA000262#FKOL010158#FKOL011173#FKOL011172#FKOL007860#FKOL005386#FKOL007211#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FKOL014701#";
	...
	function is_access_bank_edit()
	{
		...
		*/
		...
		$access_ids="##FKOL016132#FKOL010419#FKOL006484#FKOL010931#FKOL016135#FKOL016137#FKOL001766#FBLR000003#FKOL004086#FKOL003187#FKOL006124#FKOL007911#FCAS000044#FCAS000174#FCAS000171#FKOL008639#FKOL008803#FKOL008807#FCHA000264#FCHA000262#FKOL010158#FKOL010868#FCHE000227#FHWH002596#FMUM000779#FKOL011173#FNOI000141#FKOC000001#FKOL006708#FKOL007055#FKOL006412#FHWH006461#FMUM005964#FMUM005999#FKOL011477#FHWH008383#FKOL011172#FKOL001929#FKOL007860#FKOL005386#FKOL007211#FKOL012897#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FKOC001630#FKOL011290#FKOL007805#FNOI010060#FKOL014701#FDUR009515#FKOL005993#FHWH005313#FKOL010535#FKOL003663#FMUM007523#FKOL015830#FCHE010744#FHWH007172#FHWH005218#FALB000128#FDUR011769#";
	...
	function is_access_ba_design(){
			...
			$ses_fusion_id=get_user_fusion_id();
			$access_ids="##FCHA002317#FKOL011979#FKOL006223#FCHA001659#FCHA003515#FCHA003516#FCHA003505#FCHA000001#FCHA002098#FCHA000406#FKOL000003#FKOL001709#FKOL001716#FKOL002203#FKOL002878#FKOL007975#FCHA000001#FCHA000017#FKOL005386#FKOL007906#FKOL005227#FKOL004013#FKOL005037#FKOL008142#FKOL008609#FKOL007247#FKOL004013#FCHA001659#FKOL007211#FKOL015327#";
			$ret=false;
...
function is_access_ba_mgntview(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FCAS000657#FCHA002317#FKOL011979#FKOL006223#FCHA001659#FCHA003515#FCHA003516#FCHA003505#FCHA002098#FCHA000406#FKOL007975#FCHA000001#FCHA000017#FKOL005386#FMAN000230#FKOL002702#FKOL005227#FKOL004013#FKOL005037#FKOL008142#FKOL008609#FKOL007247#FKOL004013#FCHA001659#FKOL007860#FKOL007211#FKOL015327#FKOL011103#";
		$ret=false;
		...
		if(get_dept_folder()=="mis" || get_dept_folder()=="cs"  || get_dept_folder()=="qa") $ret=true;
...
function is_access_ba_upload(){
		...
		if(get_dept_folder()=="mis") $ret=true;
...
//////////////////////////////////////	
	...
	function is_access_pm_design(){
		...
		$access_ids="##FCHA003518#FCHA002317#FKOL011979#FKOL006223#FCHA001659#FCHA003515#FCHA003516#FCHA003505#FCHA000001#FCHA002098#FCHA000406#FKOL000003#FCEB000748#FKOL002203#FKOL002878#FKOL007975#FCHA000001#FCHA000017#FKOL005386#FKOL007906#FKOL005227#FKOL004013#FKOL005037#FKOL008142#FKOL008609#FCHA001659#FKOL007860#FKOL007211#FKOL015327#";
	...
	function is_access_vrs_bonus(){
		...
		if(get_user_office_id()=="ALT" && (get_role_dir() == "admin" || get_role_dir() == "manager" || get_role_dir() == "trainer" || get_role_dir() == "tl")) $ret=true;
		
		if($ret==false){
			 if($client_ids=="") $ret=false;
			 else {
				 if (in_array('18', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
	...
	function isAccessEmployeeFeedback(){
		...
		$access_ids="##FKOL000003#FKOL000004#FCEB000289#FKOL001801#FCEB000748#CEB000582#FCEB000337#FCEB000750#FCEB000308#FCEB000751#FCEB006490#FCEB006474#";
	...
	//  FCEB000079, FCEB000076, FCEB000078, FMAN000410, FJAM004945
		...
		*/
		...
		if(get_dept_folder()=="training") $ret=true;
		if(global_access_training_module()==true) $ret=true;
	...
	function is_access_training_analytics(){
						
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL014053#FCHA002108#FCHA003182#FKOL007163#FKOL000014#";
	...
	function is_access_break_identifier(){
		...
		if(get_dept_folder() == "wfm" || get_dept_folder() == "transformation") $ret=true ;
    ...
    function is_access_progression_report(){
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL001901#FKOL005837#FKOL010158#FKOL006175#FKOL011477#FKOL000003#FKOL015827#";
		
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;

		if(get_dept_folder() == "wfm" || get_global_access() == '1') $ret=true ;
...
//IJP
	...
	function is_Spl_Access_IJP_Interview()
	{
		...
		$access_ids=" #FKOL004517#FCEB003573#FMAN000382#FCHA000407#FCHA005251#";
	...
	function is_create_progression()
	{
		...
		*/
		...
		$access_ids=" #FKOL001901#FCHA000724#FCHA005931#FCEB000018#FCEB000015#FCEB000008#FCEB000033#FCEB000004#FCEB000730#FCEB000383#FCEB000288#FCEB004161#FCEB000005#FCEB004358#FKOL003187#FKOL010158#FKOL011172#FKOL008881#FKOL006355#FKOL011103#FMAN000382#FJAM004503#FKOL001919#FCHA005231#FCHA005381#FCHA005222#FCHA000407#FCHA005251#FKOL007187#";
	...
	{		
		...
		*/		
		$ses_fusion_id=get_user_fusion_id();		
		$access_ids="##FKOL001901#FKOL003187#FKOL010158#FMAN000342#FMAN000382#FJAM004503#FKOL001919#FCHA005231#FKOL007187#FKOL011477#FKOL000003#FKOL007187#";
		if( get_global_access()=='1' ) $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
			else $ret=true;
...
/////////////	
	...
	function is_force_close_dfr_requisition()
	{
		...
		*/
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL001929#FKOL001961#FKOL000005#FKOL011172#FMUM006573#FJAM005935#FKOL013936#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;
	...
	function is_resign_hr_access()
	{
		
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL010535#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;
	...
	function is_disable_add_requisition()
	{
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="###FCHA002023#FCHA003524#FCHA002279#FCHA005440#FKOL005721#";
	...
	function is_access_dfr_module()
	{
		...
		*/
		...
		if( get_dept_folder()=="hr" || get_dept_folder()=="wfm" || get_site_admin()=='1' || get_role_dir()=="manager" || get_role_dir_original()=='am' || isAssignInterview()>0) $ret=true;
	...
	function is_approve_requisition()
	{
		...
		$access_ids="##FKOL001901#FKOS000001#FCAS000953#FKOL007362#FKOL000005#FMAN000342#FCHA002093#FCHA003450#FCEB000748#FHIG000014#FKOL001929#FKOL007158#FCAS000174#FKOL010321#FKOL001919#FKOL001961#FKOL000007#FKOL012897#FJAM005935#FKOL006359#FKOL007362#FCHA000724#FCHA005931#FKOL013936#FKOL011751#";
	...
	function is_assign_trainer_dfr()
	{
		...
		*/
		...
		$access_ids=" ##FKOS000001#FKOL004410#FKOL002880#FKOL001961#FCEB000013#FCEB000748#FELS003492#FKOL003644#FKOL005837#FKOL003187#FKOL006426#FMIN000011#FMIN000029#FSPI000348#FSPI000020#FSPI000022#FHIG000294#FSPI000107#FHIG000014#FHIG000142#FHIG000170#FUTA000001#FHIG000309#FKOL002328#FKOL002157#FKOL005356#FKOL003050#FMAN000338#FSPI000019#FKOL010158#FKOL002536#FCEB000078#FKOL010419#FMAN000410#FMUM006573#FJAM005935#FKOL015827#FELS005708#FMON004089#FMAN001933#FFLO000757#FCEB006198#";
    ...
    function is_hr_permission_id_card(){
		...
		$access_ids=" #FKOL005837#FKOL010158#FKOL012897#FKOL006412#FKOL007055#FKOL003663#FNOI000565#FKOL015404#FKOL015827#FKOL015830#FCHE010744#FKOL016146#FKOL010931#FKOL010535#FKOL006708#FKOL016146#FKOL010931#FKOL006708#FKOL016146#FKOL010931#FKOL006708#FKOL010931#";
	...
	function is_access_add_user()
	{
	   ...
	   // FCHA000327 - BALAM SINGH
		...
		//FKOL015517	Amarjeet Singh
		
		
		$ses_fusion_id=get_user_fusion_id();

		$access_ids=" #FKOL012897#FKOL007362#FKOL000001#FKOL001961#FKOL003186#FKOL001916#FALB000001#FKOL005767#FKOL002702#FSPI000170#FHIG000301#FMIN000029#FHIG000142#FHIG000142#FHIG000170#FUTA000001#FHIG000309#FSPI000022#FHIG000013#FSPI000105#FKOL007078#FMON004019#FKOL005837#FKOL003187#FHIG000302#FKOL007643#FKOL002877#FSPI000019#FKOL001854#FKOL002245#FKOL004812#FKOL007158#FKOL003616#FCHA000280#FKOL001901#FCHA003182#FKOL010158#FALB000144#FKOL001900#FKOL008610#FALB000189#FFLO000172#FKOS000002#FKOL015517#FSPI000005#FELS005708#";
	...
	function is_access_term_module(){
		...
		if( get_global_access()=='1' || get_role_dir()=="super" || get_dept_folder()=="hr" || get_dept_folder()=="wfm" || get_role_dir()=="admin" || get_role_dir()=="manager" || get_role_dir()=="tl" || get_role_dir()=="trainer"  || is_access_cspl_my_team()  ) return true;
		else return false;
		//return true;
	}

	function isAccessFurlough(){
		
		$ses_fusion_id=get_user_fusion_id();

		$access_ids=" #FKOL011477#FKOL016266#FKOL010158#FKOL000005#FKOL005837#";
	...
	function is_create_shout_box_msg()
	{
		...
		$ses_fusion_id=get_user_fusion_id();
		$access_ids=" #FKOL003186#FCEB000748#FJAM004144#FJAM004148#FJAM004503#FJAM004635#FJAM004203#FJAM005935#FJAM004134#FJAM004099#FJAM002222#FJAM002227#FJAM002219#FJAM004577#FJAM002214#FJAM004132#FJAM002790#FJAM002325#FJAM004139#FCEB000791#FJAM005160#FJAM005935#FKOL015517#";
	...
	function is_access_l1_map()
	{
		...
		*/
		...
		$ret=false;
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="-###FJAM002219#FELS000376#FJAM005935#FJAM005464#FKOS000001#FKOL000007#FKOL001961#FKOL002880#FKOL004517#FKOL004553#FKOL003012#FKOL004368#FKOL001269#FKOL003644#FKOL004480#FKOL003087#FKOL004410#FKOL002156#FKOL002157#FKOL001916#FKOL002879#FKOL002494#FKOL002523#FBLR000005#FKOL001365#FKOL002494#FKOL005348#FKOL005015#FKOL003187#FMAN000342#FBLR000213#FHWH002650#FHWH002649#FCEB000582#FCEB000748#FMAN000230#FJAM005935#FJAM004503#FKOL002497#FBLR000017#FMAN000343#FCEB000236#FKOL003616#FKOL005767#FKOL002702#FMAN000131#FCEBB003880#FALB000001#FCHE000227#FKOL006426#FCHE000263#FMON004086#FCHE000739#FKOL002877#FKOL000023#FCHA000005#FCHA000018#FHWH002605#FHWH002561#FJAM004099#FMAN000131#FCEB003880#FKOL008803#FKOL001854#FKOL002245#FKOL004812#FCHA000777#FKOL004828#FCHA000571#FCHA003182#FCHA002098#FKOL010158#FKOL001900#FJAM005224#FJAM005191#FJAM005341#FCEB000078#FJAM005877#FJAM004938#FJAM005889#FJAM005464#FJAM005935#FSPI000386#";
		
		if( get_global_access()=='1' || get_dept_folder()=="hr" ||  get_role_dir()=="manager" ||  get_role_dir()=="am") $ret=true;
		else{
			if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
			else $ret=true;
		}
	...
	function is_access_itdeclaration_dump()
	{
		$ret=false;
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="-##FKOL000007#FKOL001730#FKOL012619#FKOL015826#FKOL015826#FKOL016533#FKOL010158#FKOL016640#FKOL016825#FKOL030237#";
	...
	function is_block_add_candidate_requisition($req_id)
	{
		...
		if(strpos($block_ids,"#".$req_id."#") == false) $ret=false;
		else $ret=true;
	...
	function is_access_leave_manage()
	{
		...
		// FCHA000262 – Mangal Singh
		// FKOL011172 - Niladri 
		$access_ids="-##FJAM006076#FJAM005935#FKOL000003#FKOL000004#FKOL001709#FKOL001766#FKOL004086#FKOL006124#FKOL008639#FKOL007911#FKOL008807#FCHA000264#FCHA000262#FKOL010158#FKOL011173#FKOL011172#FKOL001929#FJAM005341#FJAM005224#FJAM005191#FJAM004099#FJAM004503#FKOL007860#FKOL008610#FKOL005386#FKOL007211#FKOL012897#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FJAM005877#FJAM004938#FJAM005889#FJAM005464#FKOL014701#FJAM005941#FJAM005935#FJAM005783#FJAM005674#FJAM005817#FJAM005935#FJAM004938#FKOL002536#FALB000189#FKOL011103#FCEB000093#";
	...
	function is_access_leave_report()
	{
		...
		$access_ids="-##FJAM005935#FCEB000093#FCEB005436#FCEB005426#FKOL001738#FKOL001804#FKOL000001#FKOL000003#FKOL000004#FKOL002198#FKOL004086#FKOL001709#FKOL001766#FJAM005935#FCEB000001#FJAM004503#FKOL001732#FKOL001724#FKOL001747#FBLR000327#FKOL001961#FKOL002523#FKOL006124#FKOL005386#FKOL008142#FCEB004452#FCEB003574#FCEB003628#FCEB000085#FCEB003587#FCEB000744#FCEB000086#FCEB005126#FKOL008639#FKOL007906#FKOL007911#FKOL008807#FCHA000264#FCHA000262#FKOL010158#FKOL011173#FKOL007860#FKOL011477#FJAM005341#FJAM005224#FJAM005191#FJAM004099#FJAM004503#FKOL007211#FKOL012897#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FJAM005935#FKOL014701#FJAM005941#FJAM004938#FJAM004938#FJAM005935#FCEB006490#FCEB000093#";
	...
	function specific_location_leave_approve(){
		...
		//FKOL004086 - mangal
		//FKOL006124 - (Debraj Banerjee 
		$access_ids="-##FKOL004086#FKOL006124#FKOL008639#FKOL007911#FKOL008807#FKOL011173#FKOL007860#FKOL005386#FKOL007211#FCHA000264#FCHA000326#FCHA000279#FCHA000280#FCHA000262#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FKOL014701#FJAM005935#FJAM005877#FJAM006076#FALB000189#FKOL011103#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;
	...
	function team_leave_approve(){
		$ret=false;
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="-##FPAR000001#FALT002231#FHIG000314#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;
	...
	function is_all_dept_access()
	{
		...
		$access_ids="##FHIG000142#FKOL007362#FMAN000343#FCEB000852#FKOL000007#FMAN000342#FKOL001766#FCEB000236#FBLR000017#FCEB000009#FCEB004450#FCAS000204#FKOL005767#FKOL005767#FSPI000004#FCEB000482#FCEB000526#FCEB000502#FCEB005433#FKOL003779#FCHA005112#FHIG000301#FHIG000014#FCHE000633#FKOL001919#FKOL016690#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
		else $ret=true;
		if(get_dept_folder()=="rta" ) $ret=true;
	...
	{		
		...
		*/
		...
		$ses_fusion_id=get_user_fusion_id();
		$acccess = false;				
		$access_ids=" #FKOL001929#FKOL005767#FKOL001961#FKOL003186#FALT000001#FDRA000001#FKOL005795#FALT002231#FKOL007059#FKOL008614#FKOL005854#FKOL015517#";
		if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $acccess=false;
		else $acccess=true;
		return $acccess;
	}
	...
	function is_access_reset_password()
	{
		...
		*/
		...
		$access_ids="##FKOL015739#fkol015865#fkol015979#fkol015980#FKOL014734#FKOL015113#FKOL014689#FKOL014024#FKOL015178#FKOL015102#FKOL000001#FMAN000392#FKOL005767#FCEB000159#FCEB000018#FCEB000015#FCEB000008#FCEB000033#FCEB000004#FCEB000730#FCEB000383#FCEB000288#FCEB003791#FKOL001780#FKOL002501#FKOL001777#FKOL001782#FKOL002505#FKOL001779#FKOL002152#FKOL003040#FKOL004569#FKOL002196#FBLR000213#FBLR000008#FCEB004161#FELS003406#FCHE000227#FELS003419#FCEB000748#FSPI000110#FSPI000266#FSPI000258#FSPI000273#FSPI000117#FSPI000086#FSPI000087#FSPI000127#FSPI000348#FSPI000026#FSPI000020#FSPI000038#FHIG000319#FHIG000323#FHIG000195#FHIG000155#FHIG000239#FHIG000165#FHIG000202#FHIG000211#FHIG000173#FUTA000007#FMIN000011#FMIN000008#FALT002375#FALT002364#FKOL007297#FKOL007405#FKOL007508#FKOL008433#FKOL008627#FCHE000797#FKOL001854#FKOL002245#FKOL004812#FCHA000777#FCHA002102#FCHA002100#FCHA002101#FCHA002018#FCHA002017#FCHA002019#FCHA002016#FCHA001874#FCHA002103#FCHA002095#FELS003414#FELS004207#FKOL009320#FCHA003134#FKOL001900#FCHA002098#FCHA000001#FCAS000657#FSPI000107#FKOL001899#FKOL007158#FKOL015739#FKOL015865#FKOL015979#FKOL015980#FKOL015402#FSPI000005#";
				
		if(get_global_access()=='1' || get_dept_folder()=="hr" || get_dept_folder()=="wfm" || get_role_dir()=="admin" || get_role_dir()=="manager") $ret=true;
        else if(strpos($access_ids,"#".$ses_fusion_id."#") == false) $ret=false;
        else $ret=true;
		
		//Removed access BPO Recruitment, irrespective of designation.
		if(get_dept_id() == '35') $ret=false;
	...
	function is_access_zendesk_crm(){
	
		$ses_fusion_id=get_user_fusion_id();
		
		$access_ids=" #FJAM005935#FJAM005935#FKOL000007#FKOL006487#FKOL015327#FKOL007362#FKOL015989#";
		...
		if(get_dept_folder()=="ba" ) $ret=true;
...
function it_assets_access()
{
	...
	/////Added Date: 20-02-2023

	Narayan Chandra Gorai - FDUR000125
	Vishwanath Prasad - FJMP000105
	Salil Awasthi - FCHA003575
	Omkar Mhetre - FMUM007504
	Ananda Biswas - FKLY000097
	Anand - FCHE010691
	Anil Maurya - FNOI000140
	Pijush Bank - FKOL006366
	Amandeep - FCHA002017
	Debojit Ghosh - FKOL008451
	Arnab Das - FKOL015908
	Biswajit Mukherjee - FKOL015404
	Joel Jacob - FUTA001094
	FKOL008806 / FKOL015949 / FNOI011388 / FKOL013350 / FMON004482
    ...
    $access_ids = array("FMON003209", "FCHA002096", "FALB000003", "FUTA890003", "FBLR000017", "FCHA013994", "FKOL008224", "FKOL011751", "FKOL001917", "FKOL001919", "FKOL011103", "FKOL008610" , "FKOL016690", "FMON004482", "FKOL013350", "FNOI011388", "FKOL015949", "FKOL008806", "FUTA001094", "FKOL015404", "FKOL015908", "FKOL008451", "FCHA002017", "FKOL006366", "FNOI000140", "FCHE010691", "FKLY000097", "FMUM007504", "FCHA003575", "FJMP000105", "FDUR000125","FKOL016824","FKOL007362","FKOL015989");
...
function it_assets_full_access()
{
    ...
    $access_ids = array("FKOL011751", "FKOL001917", "FKOL001919", "FKOL011103", "FKOL008610" ,"FKOL016690");
    if(in_array($ses_fusion_id, $access_ids)) $ret = true;
...
function it_assets_office_email_ids($user_id=null)
{
    ...
    if($user_id!=null){
        ...
        switch($location)
        {
            ...
            case 'KOL':
                $data_arr["to"] = "it.kolkata@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'HWH':
                $data_arr["to"] = "it.howrah@fusionbposervices.com,debojyoti.dutta@fusionbposervices.com, sweta.dalui@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'MUM':
                $data_arr["to"] = "it.mumbai@fusionbposervices.com,Farhaan.John@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'KOC':
                $data_arr["to"] = "it.kochi@fusionbposervices.com,y.vivekananda@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'CHE':
                $data_arr["to"] = "it.chennai@fusionbposervices.com,y.vivekananda@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'CHA':
                $data_arr["to"] = "it.itc1mohali@fusionbposervices.com,it.c157mohali@fusionbposervices.com,samardeep.kaur@fusionbposervices.com,kamaldeep.singh@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'NOI':
                $data_arr["to"] = "it.noida@fusionbposervices.com,radhika.sharma@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'KLY':
                $data_arr["to"] = "it.kalyani@fusionbposervices.com,Kalyani.HRsupport@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;

            case 'DUR':
                $data_arr["to"] = "it.durgapur@fusionbposervices.com,mrinal.kantighosh@fusionbposervices.com,Phul.Das@fusionbposervices.com,HR.MIS@fusionbposervices.com";
                $data_arr["cc"] = "Procurement@fusionbposervices.com,sukhdeep.mangat@fusionbposervices.com";
                break;
	...
	function is_access_as_hr($ses_fusion_id)
	{
		...
		// FCHA002098 – Gautam Rana
		//FELS003414 - Cecilia Carolina Vasquez Posada
		// FALB000064 - Ejona Qerimi 
		
		$access_ids=" #FKOS000001##FKOL007362#FKOL004086#FKOL002198#FKOL001766#FKOL005386#FKOL006124#FKOL008142#FKOL007911#FMON004089#FKOL008639#FKOL007906#FKOL008807#FCHA000264#FCHA000262#FCHA002098#FELS003414#FKOL007860#FKOL007211#FCHA000326#FCHA000279#FCHA000280#FCHA000327#FCHA003519#FCHA000278#FCHA000283#FKOL011173#FKOL014701#FALB000064#FKOL002536#FKOL000001#";
	...
	function is_access_id_card_module(){
		...
		$access_ids=" #FKOL006885#FKOL012897#FKOL006412#FKOL007055#FKOL003663#FNOI000565#FKOL015830#FCHE010744#FKOL016146#FKOL010931#FKOL010535#FKOL006708#";
	...
	function isAccessNaps(){
		...
		$access_ids=" #FKOL008643#FKOL008803#FKOL001854#FKOL002245#FKOL004812#FCHA000327#FCHA000283#FCHA000268#FCHA000328#FCHA000272#FCHA003529#FCHA000326#FKOL001900#FKOL007078#FKOL010158#FKOL005837#FKOL007362#FKOL015989#";
	...
	function isAccessAssetManagement(){
		...
		*/
		...
		$access_ids=" #FKOL002504#FKOL005575#FKOL008262#FKOL008451#FKOL010158#FKOL005837#FKOL011477#FKOL012213#FJAM004503#FKOL012897#FNOI000565#FKOL010888#FKOL008638#FCHA003550#FCHA003562#FCHA002016#FKOL014024#FKOL014734#FKOL014689#FKOL015102#FKOL015113#FKOL015404#FHWH009981#FKOL015827#FKOL016690#FKOL015989#FKOL007362#";
	...
	function isAccessQuestionBank(){
		...
		*/
		...
		$access_ids=" #FCEB000791#FCEB000386#FCEB005369#FKOL008502#FCAS000484#FCEB005596#FCAS000152#FCAS000079#FCAS000484#FCAS000445#FFLO000062#FKOL012215#";
		...
		if(get_dept_folder()=="wfm" || get_dept_folder()=="qa" || get_dept_folder()=="training" || get_dept_folder()=="L&D" || get_dept_folder()=="hr" ||  get_role_dir()=="manager" || get_role_dir()=="admin" || is_approve_requisition()==true ) $ret=true;
	...
	function isAccessAdherenceManagementView(){
		...
		$access_ids=" #FJAM005935#FJAM005935#FCAS000156#";
		...
		if(get_dept_folder()=="wfm" || get_dept_folder()=="hr" ) $ret=true;
		//get_role_dir()=="manager"
	...
	function isAccessAdherenceManagementReport(){
		...
		$access_ids=" #FKOL006359#FJAM005935#FKOL006426#FJAM005935#";
	...
	function isAccessAddPip(){
		...
		if(get_role_dir() == 'tl' || get_role_dir() == 'manager' ) $ret=true;
	...
	function is_disable_module(){
		...
		$ret=false;
		if(get_user_office_id()=="xxxx") $ret=true;
	...
	function is_access_appraisal_module(){
		...
		FKOL000003	Oindrila Banerjee Das
		FKOL005837	Shivika 
		// FKOL012897 - rituparna 
		FKOL010158 - Swapan Kundu
		*/
		
		$ses_fusion_id=get_user_fusion_id();
		
		$access_ids=" #FCHA000283#FKOL000003#FKOL003187#FKOL005837#FKOL007562#FKOL010158#FKOL001929#FKOL012897#FKOL010419#FKOL015827#";
	...
	function is_access_resend_payslip(){
		...
		*/
		...
		$access_ids=" #FKOL000003#FKOL003187#FKOL005837#FKOL007562#FKOL010158#FKOL011477#FKOL011172#FKOL001929#FKOL012897#FKOL015827#";
	...
	function is_access_harhith(){
		...
		$access_ids=" #FKOL007078#FKOL007362#FKOL015989#";
		$ret=false;
		
		if(get_user_office_id()=="CHA") $ret=true;
		...
		if($ret==false){
			 if($client_ids=="") $ret=false;
				 ...
				 if (in_array('239', explode(',',$client_ids)) == true) $ret=true;
			 }
		}
...
///////////Quality Report Access (03/01/2020)////////////
	...
	function data_inserter($_tablename,$_field_array){
		global $CI;
		try{
			if($CI->db->insert($_tablename, $_field_array)) return $CI->db->insert_id();
			else return false;
		}
		catch(Exception $e){
	...
	function data_replace_into($_tablename,$_field_array){
		global $CI;
		try{
			if($CI->db->replace($_tablename, $_field_array)) return $CI->db->insert_id();
			else return false;
		}
		catch(Exception $e){
	...
	function data_batch_inserter($_tablename,$_field_array){
		...
		if($CI->db->insert_batch($_tablename, $_field_array)) return true;
		else return false;
	}
...
function getLocalToEstByMwpId($dttime, $mwpid) {
	...
	function isValidMysqlDate($date, $format = 'Y-m-d'){
	...
	function GetLocalTimeByOffice($loc)
	{
		...
		if(isDayLightSaving($dtTime,$loc) == 0 ) $serverTime = $row->time_zone_str;
		else $serverTime = $row->time_zone_str_ds;
	...
	function GetLocalDate()
	{
		...
		if(isDayLightSaving($dtTime) == 0 ) $serverTime = $row->time_zone_str;
		else $serverTime = $row->time_zone_str_ds;
	...
	function ConvLocalToServer($dtTime)
	{
		...
		if($StartWith == "-" || $StartWith == "+") $serverTime = substr($serverTime, 1);
		if($StartWith == "-") $serverTime = "+ ". $serverTime;
		else $serverTime = "- ". $serverTime;
...
function getIconUrl($filename,$folder="policy"){
	...
	if(in_array($ext, $imgArray)){
		...
		if(file_exists($imgPath)) $imgUrl="/$folder/".$filename;
		else $imgUrl="/uploads/$folder/".$filename;
	}else if( in_array($ext, $docArray)) $imgUrl="/assets/images/doc.png";
	else if(in_array($ext, $excelArray)) $imgUrl="/assets/images/excel.png";
	else if( in_array($ext, $pptArray)) $imgUrl="/assets/images/ppt.png";
	else if( in_array($ext, $pdfArray)) $imgUrl="/assets/images/pdf.png";
	else if( in_array($ext, $txtArray)) $imgUrl="/assets/images/txt.png";	
	else if( in_array($ext, $csvArray)) $imgUrl="/assets/images/csv.png";
	else if( in_array($ext, $videoArray)) $imgUrl="/assets/images/video.png";
	else if( in_array($ext, $htmlArray)) $imgUrl="/assets/images/html.png";
    return $imgUrl;
}
 ...
 function create_birthday_card($bname){
  ...
  function create_birthday_card_url($bname,$fusion_id){
    ...
    $color= imagecolorallocate($jpg_image, 255, 255, 255);
	  ...
	  if (!file_exists($BimagPath)) mkdir($BimagPath, 0777, true);
 ...
 function get_payslip_desc($office_id){
	...
	if( $office_id=="JAM" || $office_id=="ELS" || $office_id=="COL"){
				
		if($currDay>=16 and $currDay <=30){
			$paydesc = $currMonthName."-".$currYear."-1st";
		}else{
			if($currDay>=1 and $currDay <=15){
				if($currMonth=='1'){
					$currMonth = '12';
					$currYear=$currYear-1;
				}
...
function pm_log_record($user_id,$description,$module,$form_data,$logs="",$download='0'){
	...
	if(array_key_exists("client_id",$form_data)) $sql .= ",'".$form_data["client_id"]."'";
	else $sql .= ",''";
	

	if(array_key_exists("process_id",$form_data)) $sql .= ",'".$form_data["process_id"]."'";
	else $sql .= ",''";

	if(array_key_exists("office_id",$form_data)) $sql .= ",'".$form_data["office_id"]."'";
	else $sql .= ",''";
...
/* 		
	...
	function get_profile_pic($fid)
    {
		...
		$row = $query->row_array();		
		$url="";
		if($row["pic_ext"]=="") $url=base_url()."pimgs/blank.png";
			else $url=base_url()."pimgs/$fid/$fid.".$row["pic_ext"];
        return $url;
    }
	...
	function get_client_name_by_id($client_id){
		global $CI;
		if($client_id=="ALL") return $client_id;
		else{
	...
	function get_process_name_by_id($process_id){
		...
		if($process_id=="ALL") return $process_id;
		else{
	...
	function get_timezone_name($GMT){
	      ...
	      if(in_array($GMT,$timezones)) $zonename = $timezones[$GMT];
		  else $zonename ="Asia/Kolkata";
		  return  $zonename;
}
...
function is_access_mckinsey_modules(){
	...
	$access_ids= $access_ids=" #FKOL007187#FHIG000065#FHIG000238#FHIG000301#FKOL001934#FKOL005611#FSPI000099#FSPI000907#FSPI000928#FSPI000931#FSPI000932#FSPI000938#FSPI000958#FSPI000959#FSPI000960#FSPI000924#FSPI000961#FSPI000963#FKOL007362#FKOL015989#";
	$ret=false;
...
function is_access_Cns_modules(){
		
	// CNS		
	$client_ids=get_client_ids();
	$ses_fusion_id=get_user_fusion_id();
	
	$access_ids=""; $access_ids="#FKOL001725#FKOL007187#FKOL001732#FELS003317#FKOL001730#FKOL012895#FKOL001731#FKOL008610#FKOL012753#FKOL001961#FKOL001736#FKOL011684#FKOL008375#FKOL001736#FKOL001917#FKOL000003#FCHA000283#FKOL001709#FKOL012895#FKOL001766#FKOL015828#FALT002231#FKOL007362#FKOL015989#";
	$ret=false;
...
function cns_l2_access(){
	$client_ids=get_client_ids();
	$ses_fusion_id=get_user_fusion_id();
	
	$access_ids= $access_ids=" #FKOL001732#FKOL007187#FELS003317#FKOL001730#FKOL001731#FKOL008610#FKOL012753#FKOL001961#FKOL001736#FKOL011684#FKOL008375#FKOL001736#FKOL001917#FKOL000003#FCHA000283#FKOL001709#FKOL012895#FKOL001766#FKOL015828#FALT002231#";
	$ret=false;
...
function isAccessFNFprocurement() {
    ...
    $ses_fusion_id = get_user_fusion_id();
    $access_ids = " #FKOL002504#FKOL001803#FKOL001780#FKOL001782#FKOL005575#FKOL001774#FKOL001790#FKOL005557#FKOL002505#FKOL001777#FKOL002501#FKOL001779#FKOL007297#FKOL007405#FKOL007232#FKOL007508#FKOL008233#FKOL006963#FCHE000112#FKOL001788#FKOL008638#FKOL009320#FKOL008262#FKOL008451#FKOL008627#FKOL012213#FJAM004503#FDUR000125#FCHE000367#FKOL006366#FKOC000138#FMUM000048#FNOI000140#FJMP000071#FKOL011103#FKOL011751#FKOL001917#FKOL001919#FKOL011103#FKOL008610#FKOL001919#FKOL001917#FKOL016690#FKOL016690#FCEB004473#";
...
function is_access_Call_modules(){
		
	// Call alert		
	$client_ids=get_client_ids();
	$ses_fusion_id=get_user_fusion_id();
	
	$access_ids=""; $access_ids="##FKOL007187#FKOL012753#FKOL011883#FMUM007616#FMUM007619#FMUM007630#FMUM007571#FMUM007577#FMUM007584#FMUM006346#FMUM007509#FMUM007516#FMUM007526#FMUM006114#FMUM006153#FMUM007529#FMUM007641#FMUM007649#FKOL007362#FKOL015989#";
	$ret=false;
...
function call_l2_access(){
	$client_ids=get_client_ids();
	$ses_fusion_id=get_user_fusion_id();
	
	$access_ids="";$access_ids="##FKOL007187#FKOL012753#FKOL011883#FMUM006153#FMUM007529#";
	$ret=false;
...
function isAccessHolidaymaster() {
    $ses_fusion_id = get_user_fusion_id();
    $access_ids = " #FKOS000001#FKOS000054#";
...
/******************************Given access to Level Up User wise -AMIR------------------------------*/
function is_access_level_up(){
		
		$ses_fusion_id=get_user_fusion_id();
		
		$access_ids=" ##FMAN000375#FCEB000013#FKOL006655#FKOL006426#FKOL011685#FKOL002880#FELS000025#FKOL002536#FMON004482#FKOL002536#FKOL001795#FJAM005113#FKOL008602#FKOL009229#FKOL007187#";
		$ret=false;				
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;		
		if(get_global_access()=='1') $ret=true;
...
function get_user_status($status = ""){
    ...
    function is_access_confirmation() {

		$ret = false;

		$ses_fusion_id = get_user_fusion_id();  

		$access_ids = array("FHIG000314");
	...
	function is_access_appointment_letterdownload($brand,$office_id){
		$ret=true;
		$no_app_ack_arr = array(12,1,2);
		if(in_array($brand, $no_app_ack_arr) && isIndiaLocation($office_id))$ret=false;

		$ses_fusion_id=get_user_fusion_id();

		$access_ids = array("FKOL000003","FKOL005837","FKOL010158","FKOL000001");
	...
	function get_def_homepage(){
		global $CI;
		$current_user = get_user_id();
		
		$qSql= "SELECT def_home FROM info_personal WHERE user_id = '$current_user' ";
		$query = $CI->db->query($qSql);
		$res=$query->row_array();
	...
	function get_logo() {
		if (get_user_company_id() == 3) {	
			$logopath = '<img src='.base_url().'assets_home_v3/brand_logo/omind_logo.svg class="logo fusion-logo" alt="Omind Logo">';			
		} else {
			$logopath = '<img src='.base_url().'assets_home_v3/brand_logo/fusion_logo.svg class="logo fusion-logo" alt="Fusion Logo">';			
		}
		return $logopath;
	}
	...
	{
		global $CI;
		$id = get_user_fusion_id();
		$query = $CI->db->query("SELECT pic_ext from signin where fusion_id='$id'");
		$row = $query->row_array();
		$url="";
		if($row["pic_ext"]=="") $url=base_url().'assets_home_v3/images/prof_default.png';
			else $url=base_url()."pimgs/$id/$id.".$row["pic_ext"];
        return $url;
	}


	function critical_data($pfid){
		

		/*
		Oindrila Banerjee (FKOL000003)
		Swapan Kundu (FKOL010158)
		Shivika ( FKOL005837)
		Rohit Sen (FKOL001766)
		Rabi Hazra (FKOL010931)
		
		For below mentioned employees it need to be unmasked.
		Rohit Shaw (FKOL007911)
		Avirup Ghosh (FKOL007211)
		Manoj Moharana (FKOL005386)
		FKOL012897 - ritu
		*/

		$ses_fusion_id=get_user_fusion_id();
		//$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000003#FKOL010158#FCHA000283#FKOL005837#FKOL010931#FKOL001766#FKOL007911#FKOL007211#FKOL005386#FKOL012897#FCEB006490#FCEB000093#FCEB005426#FCEB005848#FCEB006276#FCEB006726#FCEB007653#FMAN002420#FMAN000339#FMAN001634#FCEB000086#FCEB000744#";
		$ret=false;
		
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
		
		if($ret ==true) $ret = critical_data_day_count($pfid);
		if($ses_fusion_id==$pfid) $ret=true;
	...
	function self_data($pfid){
		
		$ses_fusion_id=get_user_fusion_id();
		
		$ret=false;
		if($pfid==$ses_fusion_id) $ret=true;
	
		else $ret=false;
		return $ret;
	}
	
	function critical_data_day_count($pfid){
		global $CI;
		$qSql = "Select doj from signin where fusion_id='$pfid'";
		$query = $CI->db->query($qSql);
		$res = $query->row_array();
		$doj=$res["doj"];
		
		$now = time(); 		$your_date = strtotime($doj);
	...
	function get_assigned_to_by_id($id){
		global $CI;
		 $query = $CI->db->query("SELECT assigned_to from signin where id='$id'");
	...
	function get_help_suport_location_wise_details(){
		$help_support = array();
		$office_id = get_user_office_id();
		global $CI;
		$query = $CI->db->query("SELECT location from office_location where abbr='$office_id'");		
		$row = $query->row_array();

		if(!empty($row['location'])){
			if($row['location'] == 'India'){
				
				$help_support['hr_support_email_1'] = 'hr.grievances@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+91)-9073900798';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'employee.connect@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+91)-8276044428';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Philippines'){

				$help_support['hr_support_email_1'] = 'PhilsEmployee.Connect@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+63) 9278800276';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'accounts.cebu@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+63) 9278800276';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Jamaica'){

				$help_support['hr_support_email_1'] = 'jamhr@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '876-619-8001';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'jamhr@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '876-619-8001';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'El Salvador'){

				$help_support['hr_support_email_1'] = 'hr.esal@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+503)-7472-5070';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'hr.esal@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+503)-7472-5070';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Colombia'){

				$help_support['hr_support_email_1'] = 'hr.col@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+57) 311 4270854';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'hr.col@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+57) 311 4270854';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Albania'){

				$help_support['hr_support_email_1'] = 'Elona.Mekshi@fusionbposervices.com';
				$help_support['hr_support_email_2'] ='TeamEMEA.HR@fusionbposervices.com';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+355) 69 342 8276';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'rohit.sen@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = 'TeamEMEA.PAYROLL@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+919) 875323773';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Morocco'){

				$help_support['hr_support_email_1'] = 'Fatimaezzahra.elkaddioui@fusionbposervices.com';
				$help_support['hr_support_email_2'] = 'sofia.berrada@fusionbposervices.com';
				$help_support['hr_support_email_3'] = 'TeamEMEA.HR@fusionbposervices.com';

				$help_support['hr_support_phone_1'] = '(+212) 771000429';
				$help_support['hr_support_phone_2'] = '(+212)522429080';

				$help_support['payroll_helpdesk_email_1'] = 'assia.idrissi@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = 'meriem.khouirgou@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_3'] = 'TeamEMEA.HR@fusionbposervices.com';

				$help_support['payroll_helpdesk_phone_1'] = '(+212)522429080';
				$help_support['payroll_helpdesk_phone_2'] = '(+212) 771 000 427';

			}elseif($row['location'] == 'Kosovo'){

				$help_support['hr_support_email_1'] = 'donjeta.jakup@fusionbposervices.com';
				$help_support['hr_support_email_2'] = 'TeamEMEA.HR@fusionbposervices.com';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '(+383) 48606002';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'kosovo.hiring@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = 'TeamEMEA.PAYROLL@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '(+383)48606002';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'Montreal'){

				$help_support['hr_support_email_1'] = 'raymond.moreno@fusionbposervices.com';
				$help_support['hr_support_email_2'] = '';
				$help_support['hr_support_email_3'] ='';

				$help_support['hr_support_phone_1'] = '+1 877-251-4646';
				$help_support['hr_support_phone_2'] = '';

				$help_support['payroll_helpdesk_email_1'] = 'leah.he@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';

				$help_support['payroll_helpdesk_phone_1'] = '+1 877-251-4646';
				$help_support['payroll_helpdesk_phone_2'] = '';

			}elseif($row['location'] == 'USA'){

				$help_support['hr_support_email_1'] = 'Kim.massolini@fusionbposervices.com';
				$help_support['hr_support_email_2'] = '';
				$help_support['hr_support_email_3'] ='';
				...
				$help_support['payroll_helpdesk_email_1'] = 'Kim.massolini@fusionbposervices.com';
				$help_support['payroll_helpdesk_email_2'] = '';
				$help_support['payroll_helpdesk_email_3'] = '';
	...
	function is_location_access(){
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000001#FKOL002536#FKOL011883#FKOL000007#";
		$ret=false;
	...
	function power_bi_access(){

		 global $CI; 
		 $user_id = get_user_id();   
		 $ret=false; 
		 $query = $CI->db->query("SELECT pba.access_user_id FROM power_bi_link AS pbl INNER JOIN power_bi_access AS pba ON pba.power_bi_link_id = pbl.id WHERE pbl.status = '1' AND pba.access_user_id = '".$user_id."'");
 		 $res = $query->row_array();
	...
	function power_bi_create_access()
	{
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="";$access_ids="##FKOL008602#FKOL000003#FKOL000004#FKOL015609#FKOL007362#FKOL015989#";
		$ret=false;
		if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
		else $ret=true;
		if(get_global_access()=='1') $ret=true;
	...
	function fnf_philippines_accounting_access() {
	    $ret = false;
	    $ses_fusion_id = get_user_fusion_id();
	    if (get_global_access()==1) $ret = true;
	    $access_ids = array("FCEB004473", "FKOL011103");
	    if(in_array($ses_fusion_id, $access_ids)) $ret = true;

	    return $ret;
	}
	function fnf_philippines_facilities_access() {
	    $ret = false;
	    $ses_fusion_id = get_user_fusion_id();
	...
	function access_job_description(){
		$ses_fusion_id=get_user_fusion_id();
		$access_ids="##FKOL000003#FKOL000005#FKOL010158##FKOL000001#FELS005708#";
		$ret=false;
	...
	function is_disable_all_popup(){ 

        $client_ids=get_client_ids();
        $ses_fusion_id=get_user_fusion_id();
        $access_ids="##FKOL000001#FDRA000001##";
        $ret=false;
        if(strpos($access_ids,"#".$ses_fusion_id."#")== false) $ret=false;
        else $ret=true;
        if(get_global_access()=='1') $ret=true;
    ...
    function get_notice_period_details($office_id, $org_grade_id = "", $org_role_id = "", $user_brand= "",$get_dept_id ="",$is_role_dir ="")
	{
		global $CI;
		$cond = "";
		$notice_period_prod = "0";
		$notice_period_prob = "0";
		$notice_period_ramp_down = "0";
		$notice_period_text = "";
		$check_brand_array = array(1,2,12);
		$current_user = get_user_id();

		if(isIndiaLocation() == true && in_array($user_brand, $check_brand_array) && $office_id != "" && $org_grade_id != "" && $org_role_id != ""  && $user_brand != ""){
			$qSQL = "SELECT CASE WHEN excp_org_role_id in ('$org_role_id') THEN excp_notice_period_day_prod ELSE notice_period_day_prod end as notice_period_prod, CASE WHEN excp_org_role_id = '$org_role_id' THEN excp_notice_period_day_prob ELSE notice_period_day_prob end as notice_period_prob, CASE WHEN excp_org_role_id = '$org_role_id' THEN excp_notice_period_text ELSE notice_period_text end as notice_period_text, CASE WHEN excp_org_role_id in ('$org_role_id') THEN excp_process_ramp_down_day ELSE process_ramp_down_day end as process_ramp_down_day FROM master_notice_period WHERE office_id ='$office_id' and master_grade_id='$org_grade_id' and is_active=1" ;
			$query = $CI->db->query($qSQL);
			$res = $query->row_array();

			$notice_period_prod = $res['notice_period_prod'];
			$notice_period_prob = $res['notice_period_prob'];
			$notice_period_ramp_down = $res['process_ramp_down_day'];

			$notice_period_text = $res['notice_period_text'];
			...
			if($get_dept_id=='6' && $is_role_dir=='agent'){
				...
				$notice_period_prod= $CI->Common_model->get_single_value($qSql);
				...
				$notice_period_ramp_down = $notice_period_prod;
		...
		$noticeArray = array(
			"notice_period_prod" => $notice_period_prod,
			"notice_period_prob" => $notice_period_prob,
			"notice_period_ramp_down" => $notice_period_ramp_down,
			"notice_period_text" => $notice_period_text,
		);
	...
	function get_is_confirmed($user_id)
	{
		global $CI;
		$cond = "";
		$curr_date = date('Y-m-d');

		$qSQL = "SELECT added_date FROM info_confirmation WHERE user_id ='$user_id' and is_confirmed = 1 And added_date <= '$curr_date' limit 1" ;
		$query = $CI->db->query($qSQL);
		if($query->num_rows() > 0){
			return true;
		}else{
	...
	function is_department_restriction() {
		
		$ret = false;
		
		$ses_fusion_id = get_user_fusion_id();
	    $access_ids = array("FHIG000314", "FALT002588");
		if(in_array($ses_fusion_id, $access_ids)) $ret = true;
		
		if (isIndiaLocation()) $ret = true;
		
		return $ret;
	}
	
	function get_sub_user_count($user_id = '', $role = '', $dept = '') {
	...
	function get_l2_id($user_id){
		global $CI;
		$sqll2="Select assigned_to as id from signin where id=(select assigned_to from signin WHERE id='$user_id')";
		$l2_q = $CI->db->query($sqll2);
		$l2_row = $l2_q->row_array();
		if (isset($l2_row)){
				
			$arr['l2_id'] = $l2_row['id'];
			//$arr['l2_name'] = $l2_row['name'];	
	...
	function is_suspended($mwpid)
	{
		global $CI;
		$qSQL = "SELECT status FROM signin WHERE fusion_id ='$mwpid'" ;
		$query = $CI->db->query($qSQL);
		$row = $query->row_array();
		if($row['status'] ==3){
			return true;
	...
	# PMS Start
	#*******************************************************
	// All PMS function move to pms_functions_helper.php

	
	function array_indexed($dataArray = NULL, $column = ""){
		$result = array();
