										...
										?>
											<tr>
												...
												<td>
													...
													<a onclick="sendMail('<?= $token['incident_id']?>');" class="btn btn-info btn-xs "><i class="fa fa-envelope" aria-hidden="true"></i></a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">Send Case Details - HOWARD</h4>
				<input type="hidden" name="email_incident_id" id="email_incident_id" value="">
			</div>
			<div class="modal-body">
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							<strong for="case">E-Mail ID</strong>
							<input type="text" class="form-control" id="report_email_id" placeholder="a@gmail.com,b@gmail.com" value="" name="report_email_id" required>
						</div>
					</div>
				</div>
			</div>
			<div class="modal-footer">
				<!--onclick="return confirm('Are you sure you want send this case details to mail?');"-->
				<button type="button" class="btn btn-success sendMailSubmission" id="crmFormSubmission" name="crmFormSubmission" onclick="send_report_mail();">Send</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
			</div>
...
<script>
  function sendMail(id){
  	$('#email_incident_id').val(id);
    $('#EmailSendModal').modal('show');
  }
  function send_report_mail(){
       //$('#sktPleaseWait').modal('show');
       var baseURL="<?php echo base_url(); ?>"; 
        var email = $('#report_email_id').val();
        var incidentId=$('#email_incident_id').val();
				//alert(email+'***'+incidentId);
       $.ajax({
            url: baseURL+'howard_training/send_single_mail',
            type: 'GET',
            data: 'email_id='+ email+'&incident_id='+incidentId,
            cache: false,
            contentType: false,
            processData: false,
            success: function (data) {
               ...
               setTimeout(function(){
								  send_single_report_mail(email,incidentId);
								}, 10000); 
    ...
    function send_single_report_mail(email,incidentId){
       //$('#sktPleaseWait').modal('show');
       var baseURL="<?php echo base_url(); ?>"; 
        var email = email;
        var incidentId=incidentId;
				//alert(email+'***'+incidentId);
       $.ajax({
            url: baseURL+'howard_training/send_single_report_mail',
            type: 'GET',
            data: 'email_id='+ email+'&incident_id='+incidentId,
            cache: false,
            contentType: false,
