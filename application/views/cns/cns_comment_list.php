    ...
    <div class="col-md-12">
               <div class="card mb-4">
                         ...
                         <thead>
                                ...
                                <?php 
                                $i=1;
                                foreach($cns_list as $key=>$rows){ ?>
                                    <tr>
                                    <td><?php echo $i;?></td>
                                    <td><?php echo $rows['cns_date'];?></td>
                                    <td><?php echo $rows['type'];?></td>
                                    <td><?php echo $rows['location'];?></td>
                                    <td><?php echo $rows['Comment'];?></td>
                                    <td><?php echo $rows['comments'];?></td>
                                    <td><?php echo $rows['final_comment'];?></td>
                                    <td><a href="<?php echo base_url().'uploads/cns/'.$rows['docs'];?>" target="_blank">Documents</a></td>
                                    <td>
                                        <?php if($rows['cns_status']=='P'){ echo 'Pending for approval';}?>
                                        <?php if($rows['cns_status']=='R'){ echo 'Send for review';}?>
                                        <?php if($rows['cns_status']=='A'){ echo 'Approve';}?>
                                    </td>
                                    <td>
                                    <?php /*if($rows['cns_status']=='Completed'){?>
                                    <span style="color:red">Completed</span>
                                    <?php }elseif($rows['cns_status']=='Holding'){ ?>
                                        <a class="btn btn-success btn-xs dropCandidateBtn" onclick="make_pending('<?php echo $rows['cns_id'];?>','Pending');">Pending</a>
                                    <?php }elseif($rows['cns_status']=='Pending'){ ?>
                                        <a class="btn btn-warning btn-xs dropCandidateBtn" onclick="make_pending('<?php echo $rows['cns_id'];?>','Holding');">Holding</a>&nbsp;
                                        <a class="btn btn-danger btn-xs dropCandidateBtn" onclick="make_pending('<?php echo $rows['cns_id'];?>','Completed');">Completed</a>   
                                     <?php } */
                                     //echo'<br>====>'.$rows['approve_status'];echo'<br>*******'.$rows['cns_status'];
                                     if($rows['approve_status']!='A'){
                                        if($rows['cns_status']=='P'){
                                     ?>  
                                       <a class="btn btn-success btn-xs dropCandidateBtn" onclick="change_status('<?php echo $rows['commid'];?>','<?php echo $rows['cns_id'];?>','A');">Approval</a> 
                                    <a class="btn btn-success btn-xs dropCandidateBtn" onclick="change_status('<?php echo $rows['commid'];?>','<?php echo $rows['cns_id'];?>','R');">Send for review</a> 
                                    <?php } }?>
                                    </td>
                                    </tr>
                                 <?php 
...
<div class="modal fade" id="approve_status_form" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="frmdoc" id="frmdoc" action="<?php echo base_url(); ?>'cns/approve_status" data-toggle="validator" method='POST' enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <input type="hidden" id="cns_id" name="cns_id" class="form-control">
    ...
    function change_status(id,cnsid,status){
        ...
        /*$.ajax({
            ...
            data:'id='+id+'&cnsid='+cnsid+'&status='+status,
            success:function(res){
                if(res==1){
                    alert('Data Updated Successfully');
                    location.reload();
