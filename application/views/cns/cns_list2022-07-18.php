               ...
               <div class="card mb-4">
                         ...
                         <thead>
                                ...
                                foreach($cns_list as $key=>$rows){ ?>
                                    ...
                                    <td><?php echo ($rows['cns_status']=='')?'Waiting':(($rows['cns_status']=='Completed')?'Approved':$rows['cns_status']);?></td>
                                    <td>
                                    <?php 
                                        $cnsID=$rows['cns_id'];
                                     if(($rows['cns_status']!='Completed' )&&($rows['approve_status']!='A')){
                                        if(($rows['is_doc_upload']=='0') ||(get_comment_status($cnsID)!='A')){
                                     ?>  
                                       ...
                                       if((get_global_access()=='1')||(get_email_id_off()==$rows['emails1'])){ 
                                           if(($rows['is_doc_upload']=='1')&&(get_comment_status($cnsID)=='P')){
                                           ?>
                                        <a class="btn btn-success btn-xs dropCandidateBtn" href="<?php echo base_url();?>cns/cns_comment_histroy?cns=<?php echo $rows['cns_id'];?>" target="_blank">Approval</a> 
                                      
                                       </a>
                                    <?php }  if(($rows['is_doc_upload']=='0') ||(get_comment_status($cnsID)!='A')){?>
                                    <a class="btn btn-danger btn-xs dropCandidateBtn" onclick="delete_data('<?php echo $rows['cns_id'];?>');">
