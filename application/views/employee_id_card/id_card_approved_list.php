...
<div class="row">
	...
	<div class="col-md-1">
		<div class="form-group">
		<button class="submit-btn"><i class="fa fa-search"></i> Search</button>
		</div>
	</div>
...
<header class="widget-header">
	...
	<?php 
		...
		if($currentPage == "pending" || $currentPage == "approved" || $currentPage == "printing"  || $currentPage == "distribute"  || $currentPage == "handover" ){ 
		if($currentPage == "pending") $btype=2;
		else if($currentPage == "approved") $btype=3;
	...
	?>
		...
		<i class="fa fa-print"></i><?=$btype==2?' Bulk Approve / Reject':' Bulk Move'?></button>
	<?php } ?>
	</div>

	<?php  if(!empty($main)){  if(!empty($download_link_pro)){ ?>
		<div style='float:right; margin-top:0px;' class="col-md-2">
			<!-- <a id="download_pro" href='<?php echo $download_link_pro; ?>'><span style="padding:10px; float:right;border-radius:5px;" class="label label-success">Export Report</span></a> -->
			<a id="download_pro" href='#' onclick="exportIdCardData();"><span style="padding:10px; float:right;border-radius:5px;" class="label label-success">Export Report</span></a>
		</div>
	<?php } }?>
		...
		<tbody>						
			...
			?>
				...
				<tr>
                                    <?php if(($currentPage == "pending" && get_dept_folder()=="hr" && is_hr_permission_id_card()==TRUE )|| $currentPage == "approved" || $currentPage == "printing" || $currentPage == "distribute" || $currentPage == "handover"){ ?>
					<td>					
					<input type="checkbox" class="id_chk" name="employee_id_checkbox[]" value="<?php echo $value['id']; ?>"> &nbsp;&nbsp; 
                                        </td>
                                        <?php } ?> 
                                        <td>
					 <?php echo $counter ?>	
				   ...
				   ?>
					...
					<?php } ?>
					<td><a style="display:block" href="<?php echo base_url() ."employee_id/generateCard/download/".$value['fusion_id'] ?>" class="btn btn-success"><i class="fa fa-download"></i> Download ID Card</a></td>
...
<script>
function exportIdCardData(){	
let checkedVals = $('.id_chk:checkbox:checked').map(function() {
    return this.value;
}).get();
if(checkedVals!=''){
let chk=checkedVals.join(",");
$("#card_id").val(chk);
$("#frmid").attr("action","<?=base_url()?>employee_id_card/export_id_card_data");
$("#frmid").attr("target","_blank");
$("#frmid").submit();
}else{
