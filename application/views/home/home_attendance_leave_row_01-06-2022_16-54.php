...
<?php //echo "<pre>" .print_r($ab_attendanceArray, 1) ."</pre>"; ?>
<?php if(get_user_office_id() == "KOC" && !empty($ab_attendanceArray_others)){ 
...
<div class="col-md-3 abCLBtn">
	 ...
	 <?php if($leave_accessible && !$disable) :?>
		<button class="btn btn-primary btn-sm pull-right myLeaveModalBtn" data-toggle="modal" data-frmdate='<?php echo $mdyabdate;?>' data-todate='<?php echo $mdyabdate;?>' >Apply Leave </button>
	 <?php else: ?>
		<button class="btn btn-primary btn-sm pull-right" disabled data-toggle="modal" data-frmdate='<?php echo $mdyabdate;?>' data-todate='<?php echo $mdyabdate;?>' >Apply Leave </button>
	 <?php endif; ?>
</div>
</div>
<?php } }
...
foreach($ab_attendanceArray as $ab_token){ 
	...
	if($ab_disposition_local == 'Absent' || $ab_disposition_local == 'LOA' || !empty($ab_leave_dtl))
	{
?>
<?php
	 if(get_user_fusion_id() != 'FKOL006361'){
?>
<div class="abItem row">
...
<div class="col-md-3 abCLBtn">
<?php if($ab_disposition_local == 'Absent' || $ab_disposition_local == 'LOA'){ ?>
