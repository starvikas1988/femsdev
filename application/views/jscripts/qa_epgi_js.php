...
<script type="text/javascript">
	function epgi_calc(){
				...
				$(".epgi_point").each(function(index, element){
					if($(element).val()!=""){
						var earned_weightage = parseFloat($(element).children("option:selected").attr('epgi_val'));
						earned_score += earned_weightage;
						var weightage = parseFloat($(element).children("[value='Excellent']").attr('epgi_val'));
						...
						$("#epgi_overall_score").val(parseFloat((earned_score/possible_score)*100).toFixed(2)+"%");
						
					}
				});
			if(!isNaN(earned_score)){
				$("#epgi_earned_score").val(earned_score);
			}
			if(!isNaN(possible_score)){
				$("#epgi_possible_score").val(possible_score);
			}
	}
</script>
...
<script type="text/javascript">
	$(document).on("change", ".epgi_point", function(){
			epgi_calc();
	});
	epgi_calc();
