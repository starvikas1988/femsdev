...
<script type="text/javascript">
		...
		let na_count_park = 0;
		let score_urban_piper_final = 0;
		let scoreable_urban_piper_final = 0;
		let w3_park = 0;

		$('.urban_piper_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Pass'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('urban_piper_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('urban_piper_max'));
				...
				let w1_park = parseFloat($(element).children("option:selected").attr('urban_piper_val'));
				let w3_park = parseFloat($(element).children("option:selected").attr('urban_piper_max'));

				//score = score + w1;
				score_park = score_park + w1_park;
				scoreable_park = scoreable_park + w3_park;
				console.log(scoreable_park);
      ...
      score_urban_piper_final     = (score_park).toFixed(2);
      scoreable_urban_piper_final = (scoreable_park).toFixed(2);

		$('#urban_piper_earned_score').val(score_urban_piper_final);
		$('#urban_piper_possible_score').val(scoreable_urban_piper_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#urban_piper_overall_score').val(quality_score_percent_park+'%');
		}

		
		if($('#urban_piperAF1').val()=='Fail' || $('#urban_piperAF2').val()=='Fail' || $('#urban_piperAF3').val()=='Fail' || $('#urban_piperAF4').val()=='Fail' || $('#urban_piperAF5').val()=='Fail' || $('#urban_piperAF6').val()=='Fail'){
			$('.urban_piperFatal').val(0+'%');
		}else{
			$('.urban_piperFatal').val(quality_score_percent_park+'%');
		}
	}
	
	$(document).on('change','.urban_piper_point',function(){
		urban_piper_calc();
	});
	urban_piper_calc();
</script>
...
<script>
	$(document).ready(function(){
		...
		$("#call_duration").timepicker({timeFormat : 'HH:mm:ss' });
		<?php if($this->uri->segment(2)=="Qa_urban_piper" && $this->uri->segment(4)=="0"){?>
			//console.log("okk");
		<?php }?>
	});
</script>
