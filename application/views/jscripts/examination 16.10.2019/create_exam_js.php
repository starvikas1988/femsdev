...
<script>
	...
	{
		var datas = {'office_id':"<?php echo get_user_office_id(); ?>"};
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				{
					tr += '<tr data-exam_id="'+element.id+'"><td>'+(index+1)+'</td><td>'+element.title+'</td><td>'+element.location+'</td><td>'+element.type+'</td><td><button class="btn btn-xs btn-primary view_sets" title="View Sets" data-exam_id="'+element.id+'"><i class="fa fa-eye" aria-hidden="true"></i></button>&nbsp;<button class="btn btn-xs btn-primary create_sets" title="Create Sets" data-exam_id="'+element.id+'"><i class="fa fa-plus-circle" aria-hidden="true"></i></button></td></tr><tr class="exam_set_container"><td colspan="5"><table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" width="100%" cellspacing="0"><thead><tr class="bg-warning"><th>SL</th><th>Title</th><th>No. of Question</th><th>Action</th></tr></thead><tbody data-exam_set_container="'+element.id+'"></tbody></table></td></tr>';
					//
				});
	...
	function get_sets(exam_id)
	{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				$.each(res.datas,function(index,element)
				{
					tr += '<tr><td>'+(index+1)+'</td><td>'+element.title+'</td><td>'+element.no_of_question+'</td><td style="width:20%"><button class="btn btn-xs btn-primary upload_exam_btn" data-set_id="'+element.id+'" title="Upload Set"><i class="fa fa-file-excel-o" aria-hidden="true"></i></button></td></tr>';
					//<button class="btn btn-xs btn-primary upload_exam_btn" title="Upload Set"><i class="fa fa-file-excel-o" aria-hidden="true"></i></button>
				});
