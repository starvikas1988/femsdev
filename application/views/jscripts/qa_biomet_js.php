...
<script>
 ...
 ///////
	function biomet_calc(){
		...
		$('.biometVal').each(function(index,element){
			var score_type = $(element).val();
					
			if(score_type == 'Yes'){
				var nameVal = $.trim($(element).attr('name'));			
				var weightage = parseFloat($(element).children("option:selected").attr('biomet_val'));
		...
		$('#biometEarned').val(score);
		$('#biometPossible').val(scoreable);
		
		if(!isNaN(quality_score_percent)){
			$('#biometOverallScore').val(quality_score_percent+'%');
		}
		
	/*---- Chat ---*/
		if($('#chatAutof1').val()=='Yes' || $('#chatAutof2').val()=='Yes' || $('#chatAutof3').val()=='Yes' || $('#chatAutof4').val()=='Yes'){
			$('.biometChatFatal').val(0);
			$('#fatalspan3').html("Fatal Error").css("color","red");
		}else{
			$('#fatalspan3').html("Non Fatal").css("color","blue");

			$('.biometChatFatal').val(quality_score_percent+'%');
		}
	/*---- Call ---*/
		if($('#callAutof1').val()=='Yes' || $('#callAutof2').val()=='Yes' || $('#callAutof3').val()=='Yes' || $('#callAutof4').val()=='Yes'){
			$('.biometCallFatal').val(0);
			$('#fatalspan1').html("Fatal Error").css("color","red");

		}else{
			$('#fatalspan1').html("Non Fatal").css("color","blue");
			$('.biometCallFatal').val(quality_score_percent+'%');
		}
	/*---- Call ---*/
		if($('#emailAutof1').val()=='Yes' || $('#emailAutof2').val()=='Yes' || $('#emailAutof3').val()=='Yes' || $('#emailAutof4').val()=='Yes'){
			$('.biometEmailFatal').val(0);
			$('#fatalspan2').html("Fatal Error").css("color","red");
		}else{
			$('#fatalspan2').html("Non Fatal").css("color","blue");
			$('.biometEmailFatal').val(quality_score_percent+'%');
		}
...
/////////////// IT Help Desk ////////////////////////
///////
	function itHelpDesk_cal(){
		var score = 0;
		$('.itHelpDesk').each(function(index,element){
			var weightage = parseFloat($(element).children("option:selected").attr('ithd_val'));
			score = score + weightage;
		});
		
		if(!isNaN(score)){
			$('#itHelpdeskScore').val(score+'%');
		}
	}
...
///////
	function srl_calc(){
		...
		$('.srl_point').each(function(index,element){
			var score_type = $(element).val();
					
			if(score_type == 'Amazing'){		
				var ernwt = parseFloat($(element).children("option:selected").attr('srl_ern'));
				var psblwt = parseFloat($(element).children("option:selected").attr('srl_psbl'));
				score = score + ernwt;
				scoreable = scoreable + psblwt;
			}else if(score_type == 'Miss'){
				var ernwt = parseFloat($(element).children("option:selected").attr('srl_ern'));
				var psblwt = parseFloat($(element).children("option:selected").attr('srl_psbl'));
				score = score + ernwt;
				scoreable = scoreable + psblwt;
			}else if(score_type == 'Needs work'){
				var ernwt = parseFloat($(element).children("option:selected").attr('srl_ern'));
				var psblwt = parseFloat($(element).children("option:selected").attr('srl_psbl'));
				score = score + ernwt;
				scoreable = scoreable + psblwt;
			}else if(score_type == 'Hit the mark'){
				var ernwt = parseFloat($(element).children("option:selected").attr('srl_ern'));
				var psblwt = parseFloat($(element).children("option:selected").attr('srl_psbl'));
				score = score + ernwt;
				scoreable = scoreable + psblwt;
		...
		$('#srlEarned').val(score);
		$('#srlPossible').val(scoreable);
		
		if(!isNaN(quality_score_percent)){
			$('#srlOverallScore').val(quality_score_percent+'%');
		}
		
		if($('#srlAF1').val()=='Yes' || $('#srlAF2').val()=='Yes' || $('#srlAF3').val()=='Yes' || $('#srlAF4').val()=='Yes'){
			$('.srlFatal').val(0);
		}else{
			$('.srlFatal').val(quality_score_percent+'%');
		}
...
////////////////// biomet /////////////////////
	$(document).on('change','.biometVal',function(){
		biomet_calc();
	});
	biomet_calc();
...
////////////////// IT Help Desk /////////////////////
	$(document).on('change','.itHelpDesk',function(){	
		itHelpDesk_cal();		
	});
	itHelpDesk_cal();
	
////////////////// SRL /////////////////////
	$(document).on('change','.srl_point',function(){
		srl_calc();
	});
	srl_calc();
