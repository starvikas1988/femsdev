...
function check_apply_date_range()
{
    ...
    var datas = {'from_date': check_from_date, 'to_date': check_to_date, 'leave_type_id': var_js[$("#form-leave_type").val()][0] };
   ...
   <script>
    $(document).ready(function () {
        $(".approve").click(function () {
            ...
            var leave_crt_id = $(this).attr('leave_crt_id');
            $("#id").val(id);
            $("#user_id").val(user_id);
            $("#leave_crt_id").val(leave_crt_id);
            $('#myModal').modal('show');
        });
    });
</script>
...
<script>
    $("#checkAll").click(function () {
        var checkAllCheckbox = $('input:checkbox').not(this).prop('checked', this.checked);
    ...
    function toggleTransferButton() {
        ...
        if(!anyCheckboxChecked){
            ...
            $(".indiviual_transfer_btn").removeAttr("disabled");
            $("#checkAll").prop("checked", false);
        }else{
            $("#show_all_transfer").show();
            $(".indiviual_transfer_btn").attr("disabled", "disabled");
        }    
    } 

    function checkIndivitual() {
        toggleTransferButton(); 
    }
     

    function multiple_data_transfer(){       
            $('#multiplemyModal').modal('show');
            var checkedValues = [];
            ...
            $("#multitlconfimationclick").click(function () {
                $(".before_success").hide();  
                var tl_muliple_comment = $('#tl_muliple_comment').val();  
                if(tl_muliple_comment!='')
                {
                $('#sktPleaseWait').modal('show');               
                $.post("<?php echo base_url('leave/tl_approve_multiple_compoff'); ?>", {
                    user_ids: checkedValues,
                    tl_muliple_comment:tl_muliple_comment
                }).done(function (res) {  
                    
                    $('#sktPleaseWait').modal('hide'); 
                    $('.after_success').css('display','block');
                    $('.before_success').css('display','none');
                ...
                {
                    $("#tl_muliple_comment").css("border", "1px solid red");
                
                }
            });                  
    }  

    function multiple_data_reject(){
           
            $('#reject_compoff_multiple').modal('show');
            var checkedValues = [];
            $(".checkbox:checked").each(function() {
                var checkboxDataValue = $(this).val(); // Get the value
                checkedValues.push(checkboxDataValue); // Push the data into the array
            ...
            $("#tlmultiplerejectclick").click(function () {
                $(".before_success").hide();  
                var tl_reject_comment_multiple = $('#tl_reject_comment_multiple').val();

               
                if(tl_reject_comment!='')
                {
                $('#sktPleaseWait').modal('show');              
                    $.post("<?php echo base_url('leave/tl_reject_multiple_compoff'); ?>", {
                        user_ids: checkedValues,
                        tl_muliple_comment:tl_reject_comment_multiple
                    }).done(function (res) {  
                ...
                {
                    $("#tl_reject_comment_multiple").css("border", "1px solid red");
...
<script>
     $('#tlconfimationclick').click(function(){
        var tl_comment = $('#tl_comment').val();
        var co_id = $('#id').val();
        var user_id = $('#user_id').val();
        var leave_crt_id = $('#leave_crt_id').val();
        if(tl_comment!='')
        {
            $('#sktPleaseWait').modal('show');
            $.ajax({
			   type: 'POST',    
			   url: baseURL+'leave/tl_approve_compoff',
               data:'tl_comment='+ tl_comment+"&co_id="+co_id+"&user_id="+user_id+"&leave_crt_id="+leave_crt_id,
        ...
        {
            $("#tl_comment").css("border", "1px solid red");
...
<script>
     $('#tlrejectclick').click(function(){
        var tl_reject_comment = $('#tl_reject_comment').val();
        var co_id = $('#id').val();
        ...
        if(tl_reject_comment!='')
        {
            ...
            $.ajax({
			   type: 'POST',    
			   url: baseURL+'leave/tl_reject_compoff',
               data:'tl_reject_comment='+ tl_reject_comment+"&co_id="+co_id,
			   success: function(msg){
                $('#sktPleaseWait').modal('hide');
                 $('.after_success').css('display','block');
                    $('.before_success').css('display','none');
        ...
        {
            $("#tl_reject_comment").css("border", "1px solid red");
