...
<script src="<?php echo base_url('libs/bower/jquery-toast-plugin/js/jquery.toast.js')?>"></script>
<script type="text/javascript">
    $( ".new-parameter-row-position" ).sortable({
        delay: 150,
        stop: function() {
            var selectedData = new Array();
            $('.new-parameter-row-position > tr').each(function() {
...
$(function(){
    ...
    $(document).on('click','.go-to-step-2',function(){
        ...
        if($('#audit_sheet_name,#audit_sheet_client_id,#audit_sheet_process_id').valid()){
            ...
            $("#progressbar").find('li:nth-child(2)').addClass('active');
    ...
    $(document).on('click','.go-to-step-1',function(){
        ...
        next_previous_button(params);
        $("#progressbar").find('li:nth-child(2)').removeClass('active');
        // $(this).hide();
    ...
    $(document).on('click','.back-from-preview',function(){
        ...
        $("#progressbar").find('li:nth-child(3)').removeClass('active');
    });
    ...
    $(document).on('click','.go-to-step-3',function(){
        ...
        if($("#auditSheetCreateForm").valid()){
            ...
            params = {
                btnCount: 2,
                btnText:['<i class="fas fa-edit"></i>&nbsp;Edit','<i class="fas fa-plus"></i>&nbsp;Create Audit Sheet'],
            ...
            $("#progressbar").find('li:nth-child(3)').addClass('active');
        }
    });
                    ...
                    for (const [key, item] of Object.entries(response.data)) {
                        ...
                        html += `<tr> 
                                      ...
                                      <td align="center">
                                            ...
                                            <a class="btn btn-info btn-sm" href="${basePath}audit_sheet_dyn/edit_sheet/${item.dpam_id}" ${item.qa_defect_id == 0 || item.format_id == 0 || item.qa_defect_is_active == 0 ? 'disabled onclick="return false;"' : ''}><i class="fas fa-edit"></i>&nbsp;Edit</a>

                                            <button class="btn btn-${item.qa_defect_is_active == 1 ? 'success' : 'danger'} btn-sm ${item.qa_defect_is_active == 1 ? 'make-in-active-auditsheet' : 'make-active-auditsheet'}" defectId="${item.qa_defect_id}">${item.qa_defect_is_active == 1 ? '<i class="fas fa-check"></i>&nbsp;Active' : '<i class="fas fa-ban"></i>&nbsp;Inactive'}</button>

                                            <a class="btn btn-warning btn-sm" href="${basePath}qa_audit_dyn/add_edit_audit/0/${item.qa_defect_id}"><i class="fas fa-eye"></i>&nbsp;View & Audit</a>
