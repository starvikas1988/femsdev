...
$(".editTraineeNest").click(function(){
	var params=$(this).attr("params");
	var batchid=$(this).attr("batchid");		
	var arrPrams = params.split("#"); 
	$('#t_userid').val(arrPrams[2]);
	$('#t_batchid').val(batchid);
	$('#t_ctrainee').html(arrPrams[1]);
	
	$.ajax({
	   type: 'GET',    
	   url: baseURL+'training/get_trainee_change_nesting',
	   data:'cid=' + arrPrams[4] + '&pid=' + arrPrams[5],
	   success: function(data){
			$('#t_choosetrainee').html(data);
			$('#modelTrainingNestingChange').modal('show');	
		},
		error: function(){
			alert('Fail!');
		}
...
////////////////////////////////////////////////////
	...
	$("#modelNestingResultUpload").on("shown.bs.modal", function () {
		...
		var uUrl=baseURL+'training/uploadNestingResult';
	...
	$(".btnNestingUpload").click(function(){
		...
		var certificateurl = baseURL+'training/nesting_design/' +batch_id;
		$.ajax({
		   type: 'GET',    
		   url:baseURL+'training/getFormatDesignNestingRag',
		   data:'batchid=' + batch_id,
		   success: function(msg){
			 if(msg != 0)
			 {
				var uUrl=baseURL+'training/downloadTrainingNestingHeader' + '?pmdid=' + msg;
				$("#downloadheader1").attr("href", uUrl);
				$("#downloadheader1").show();
			 } else {
				$("#downloadheader1").hide();
				$('#uploadingdivattach').html("<b>No Format Found - </b> <a href='"+certificateurl+"' class='btn btn-primary btn-sm' style='padding:2px 5px'>Create Nesting Design</a>");
			 }
			}
		  });
...
//================================== DESIGN JS ===================================================================//	
	...
	$(".editDesignButton").click(function(){
		...
		var URL=baseURL+'training/getTrainingNestingDesignForm?mdid='+mdid;
...
<script>
	...
	// TRAINING USERS SPLIT
		...
		$(document).on('click','.btnHandOverNesting',function(){		
	...
	// TRAINING USERS MOVE
	$(".moveclasstraineeNesting").click(function(){
		...
		var rURL=baseURL+'training/fetchBatchUsers_nesting';
		var bURL=baseURL+'training/fetchBatchlist_nesting';
	...
	// TRAINING USERS SPLIT
	$(".splitclassbatchNesting").click(function(){
	...
	// TRAINING USERS SPLIT
	$(".newuserbatchNesting").click(function(){
		...
		var rURL=baseURL+'training/fetchBatchTraineeList_Nesting';
		//var bURL=baseURL+'training/fetchBatchlist';
	...
	function getassigned_nesting_batch_details(bid)
	{
		baseURL = "<?php echo base_url(); ?>";
		request_url = baseURL + "/training/nesting_batch_details/" + bid;
		datas = { 'batchid' : bid };
	...
	function batchdetailsnestingclick(a)
	{
	  ...
	  {
		getassigned_nesting_batch_details(batchid);
	  }
	}
