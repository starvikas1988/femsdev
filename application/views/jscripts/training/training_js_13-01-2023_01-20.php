...
$(document).ready(function(){
	...
	$("#terminateUserOth").click(function(){
		...
		if(tuid!="" && terms_date!="" && t_type!="" && sub_t_type!="" && tattrition != "" &&(remarks!="" || resign_remarks!="")){
			...
			if(ans==true){
				...
				if(tbid != "" && tattrition != ""){
				  $.ajax({
				   ...
				   data: { bid : tbid, uid : tuid, trnattrition : tattrition, t_type : t_type, trn_term_day : trn_day_term_oth, term_type_new : term_type_new, terms_date : terms_date },
				   success: function(msg){
			...
			$.ajax({
				...
				success: function(token) {
					if(token==1){
						...
						location.href = "<?php echo base_url()?>training/rejected_candidate";
					}
					else{
