...
/// Nesting Result Add
	...
	$("#modelNestingResultUpload").on("shown.bs.modal", function () {
		...
		var uUrl=baseURL+'training/uploadNestingRagResult';
	...
	$(".btnNestingUpload").click(function(){
		...
		var certificateurl = baseURL+'training/nesting_rag_design/' +batch_id;
		$.ajax({
		   type: 'GET',    
		   url:baseURL+'training/getFormatDesignNesting_Rag',
		   data:'batchid=' + batch_id,
		   success: function(msg){
			 if(msg != 0)
			 {
				var uUrl=baseURL+'training/downloadTrainingNestingRagHeader' + '?pmdid=' + msg;
				$("#downloadheader1").attr("href", uUrl);
				$("#downloadheader1").show();
			 } else {
				$("#downloadheader1").hide();
				$('#uploadingdivattach').html("<b>No Format Found - </b> <a href='"+certificateurl+"' class='btn btn-primary btn-sm' style='padding:2px 5px'>Create Nesting Rag Design</a>");
			 }
			}
		  });
...
//================================== DESIGN JS ===================================================================//	
	...
	$(".editDesignButton").click(function(){
		...
		var URL=baseURL+'training/getTrainingNestingRagDesignForm?mdid='+mdid;
