...
/// Nesting Result Add
	...
	$("#modelNestingResultUpload").on("shown.bs.modal", function () {
		...
		var uUrl=baseURL+'training/uploadRecursiveIncubationResult';
	...
	$(".btnNestingUpload").click(function(){
		...
		var certificateurl = baseURL+'training/recursive_incub_design/' +batch_id;
		$.ajax({
		   type: 'GET',    
		   url:baseURL+'training/getFormatDesignRecursiveIncubation',
		   data:'batchid=' + batch_id,
		   success: function(msg){
			 if(msg != 0)
			 {
				var uUrl=baseURL+'training/downloadTrainingRecursiveIncubationHeader' + '?pmdid=' + msg;
				$("#downloadheader1").attr("href", uUrl);
				$("#downloadheader1").show();
			 } else {
				$("#downloadheader1").hide();
				$('#uploadingdivattach').html("<b>No Format Found - </b> <a href='"+certificateurl+"' class='btn btn-primary btn-sm' style='padding:2px 5px'>Create Upskill Rag Design</a>");
			 }
			}
		  });
...
//================================== DESIGN JS ===================================================================//	
	...
	$('.editPassingMarks').click(function(){
		...
		var mdid = arrPrams[0];
		var pbatch = arrPrams[5];
		var pmarks = arrPrams[6];
		$('.frmUpdatePassingMarks #marks_mdid').val(arrPrams[0]);
		$('.frmUpdatePassingMarks #marks_batchid').val(pbatch);
		$('.frmUpdatePassingMarks #passing_marks').val(pmarks);
	...
	$(".editDesignButton").click(function(){
		...
		var URL=baseURL+'training/get_recursive_Incubation_DesignForm?mdid='+mdid;
