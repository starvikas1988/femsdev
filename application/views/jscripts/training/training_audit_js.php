...
////////////////////////////////////////////////////
	...
	$('#modelAuditResultUpload').on('hidden.bs.modal', function () {
		location.reload();
	});
		
	$("#modelAuditResultUpload").on("shown.bs.modal", function () {
	
		var uUrl=baseURL+'training/uploadAuditResult';
		...
		var settings = {
			...
			{
			   var batch_id=$('#auditbatch_id').val();
			   return {
					'batch_id' : batch_id,
				}
			},
	...
	$(".btnAuditUpload").click(function(){
		
		var batch_id=$(this).attr("batch_id");
		$('#auditbatch_id').val(batch_id);
		$("#downloadheader1").hide();
		var certificateurl = baseURL+'training/audit_design';
		$.ajax({
		   type: 'GET',    
		   url:baseURL+'training/getFormatDesignAuditRag',
		   data:'batchid=' + batch_id,
		   success: function(msg){
			 if(msg != 0)
			 {
				var uUrl=baseURL+'training/downloadTrainingAuditResult' + '?pmdid=' + msg;
		...
		$('#modelAuditResultUpload').modal('show');
...
// NEW DESIGN MODAL
	...
	$("#btn_add_design").click(function(){	
		$('#modalAddTrainingAuditDesign').modal('show');
	});
	...
	// EDIT DESIGN
	$(".editTrainingAuditDesignButton").click(function(){
		...
		var URL=baseURL+'training/get_Audit_DesignForm?mdid='+mdid;
		...
		$.ajax({
		   ...
		   data:'mdid='+mdid,
		   success: function(msg){
			   $('#modalUpdateTrainingAuditDesign').find('#kpi_divs_ed').html(msg);		
			   $('#sktPleaseWait').modal('hide');
			   $('#modalUpdateTrainingAuditDesign').modal('show');	
			},
			error: function(){
				alert('Fail!');
				$('#sktPleaseWait').modal('hide');
