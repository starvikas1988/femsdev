...
$(document).ready(function(){
	...
	var isUpdateEmpList = <?php echo $isUpdateEmpList ?>;
	//alert(isUpdateEmpList);
	if(isUpdateEmpList==1){
		//alert($("#dept_id").val());
...
function get_employee_list(id){
   // if(id!=""){
        $.post("<?php echo base_url()?>leave/get_emlployee_list_based_on_dept", {'id' : id, 'office_id' : $("#location_id").val()}, function(data){
			...
			var selected_emp_arr = selected_emp.split("','");


            $.each(data, function(i,v){  
                sCss = "";
                if(data[i]["id"]== selected_emp) sCss="selected";
				if($.inArray(data[i]["id"],selected_emp_arr) > -1)  sCss="selected"; 
...
function save_reject(){
    ...
    if(reason.trim()!=''){
        ...
        $("#rejectLeaveModal #rejectForm #reject_save").attr('disabled',true);
        ...
        $("#rejectLeaveModal #rejectForm #reject_save").attr('disabled',false);
