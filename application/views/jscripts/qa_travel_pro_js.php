...
////////////////vikas//////////////////////////////
 ...
 <script type="text/javascript">
		...
		$('.travel_pro_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Pass'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('travel_pro_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('travel_pro_max'));
      ...
      scoreable_sea_world_final = (scoreable_park).toFixed(2);
		...
		$('#travel_pro_earned_score').val(score_sea_world_final);
		$('#travel_pro_possible_score').val(scoreable_sea_world_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#travel_pro_overall_score').val(quality_score_percent_park+'%');
		}
	...
	$(document).on('change','.travel_pro_point',function(){
		travel_pro_calc();
	});
	travel_pro_calc();
</script>
