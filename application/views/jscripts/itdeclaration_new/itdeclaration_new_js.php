...
<script type="text/javascript">
 ...
 /*$(function () {
        ...
        $("#month_start_from_1").datepicker({
        	minDate: -3
            maxDate: new Date()
        });
        $("#month_end_to_1").datepicker({
        	minDate: -3
            maxDate: new Date()
        });
...
////////// Lock/Unlock ///////////	
	$(".editItdcl").on('click', function(){
		...
		if(ans==true){
			$.ajax({
			   type: 'POST',    
			   url: '<?php echo base_url(); ?>itdeclaration_new/islockEdit',
			   data:'itd_id='+ itd_id+'&sid='+ sid+'&user_id='+ user_id+'&employee_name='+ employee_name,
			   success: function(msg){
					location.reload();
				}
			});
