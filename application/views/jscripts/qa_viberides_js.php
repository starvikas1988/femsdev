...
///////////////// New QA ////////////////////
	function viberides_calc(){
		var score = 0;
		var scoreable = 0;
		var quality_score_percent = 0;
		
		$('.vibe').each(function(index,element){
			var score_type = $(element).val();
			
			if(score_type == 'Yes'){
				var weightage = parseFloat($(element).children("option:selected").attr('vibe_val'));
				score = score + weightage;
				scoreable = scoreable + weightage;
			}else if(score_type == 'No'){
				var weightage = parseFloat($(element).children("option:selected").attr('vibe_val'));
		...
		quality_score_percent = ((score*100)/scoreable).toFixed(2);
		$('#earnedVibe').val(score);
		$('#possibleVibe').val(scoreable);
		
		if(!isNaN(quality_score_percent)){
			$('#vibeScore').val(quality_score_percent+'%');
		}
	/////////////
		if($('#vibeAF1').val()=="Yes" || $('#vibeAF2').val()=="Yes" || $('#vibeAF3').val()=="Yes" || $('#vibeAF4').val()=="Yes" || $('#vibeAF5').val()=="Yes" || $('#vibeAF6').val()=="Yes" ||$('#vibeAF7').val()=="Yes" || $('#vibeAF8').val()=="Yes" || $('#vibeAF9').val()=="Yes" || $('#vibeAF10').val()=="Yes" || $('#vibeAF11').val()=="Yes" || $('#vibeAF12').val()=="Yes"){
			$('.vibeFatal').val(0);
		}else{
			$('.vibeFatal').val(quality_score_percent+'%');
		}
...
//////// NEW QA ////////
	$(document).on('change','.vibe',function(){
		viberides_calc();
	});
	viberides_calc();
