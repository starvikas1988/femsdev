...
$('#f_body').change(function(){
	val = $(this).val();
	if(val == "custom"){
		$('.customEmailBody').show();
	} else {
		$('.customEmailBody').hide();
	}
...
<?php if(!empty($autoStart)){ ?>
callAutoActionButton();
<?php } ?>

$('.flipswitch-inner').click(function(){
	myval = $('#c_email_auto').val();
	if(myval == '1')
	{
		$('.flipswitch-cb').prop('checked', false);
		$('#c_email_auto').val('0');
	} else {
		$('.flipswitch-cb').prop('checked', true);
		$('#c_email_auto').val('1');
	}
});
