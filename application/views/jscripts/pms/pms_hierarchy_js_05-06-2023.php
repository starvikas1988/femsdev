    ...
    function open_sub(pms_id,user_id){
       ...
       $.ajax({
            ...
            success:function(res){
                ...
                ts+='<tbody><tr><td>';
                if(data['is_under']!='0'){
                ts+='<div class="tooltip_fresh"><a href="javascript:void(0);" onclick="open_sub('+data['pms_id']+','+data['user_id']+')" id="toggle_icon" class="view_btn_circle" data-toggle="collapse" data-target="#demo'+data['pms_id']+'_'+data['user_id']+'"><i class="fa fa-plus" aria-hidden="true"></i><span class="tooltiptext">Expand</span></a></div>';
                }
                ts+='</td><td>'+data['name']+'</td><td>'+data['mwp_id']+'</td><td>'+data['role_name']+'</td><td>'+data['dept_name']+'</td><td>'+data['pms_title']+'</td><td>'+data['pms_status']+'</td><td>'+data['start_date']+'</td><td><span class="badge bg-primary">'+data['status']+'</span></td><td><div class="tooltip_fresh"><a href="'+base_url+'pms_assignment/teams_performance_sheet/'+btoa(data['pms_id'])+'/'+btoa(data['user_id'])+'"><img src="'+base_url+'/assets_home_v3/images/view.svg" alt=""></a><span class="tooltiptext">View</span></div></td></tr><tr><td colspan="12" class="hiddenRow"><div class="accordian-body collapse table_under" id="demo'+data['pms_id']+'_'+data['user_id']+'"></div></td></tr></tbody>';
                });
                ts+='</table>';
