...
<script>
    ...
    $(document).ready(function () {
        ...
        var fieldHTML = '	<div class="row"><div class="col-sm-12 new-col"><div class="form-group new-add-field"><input type="text" autocomplete="off" name="qs_name[]" required class="form-control competency_params fusion_id_exists" id="exampleInputEmail1" aria-describedby="emailHelp"></div><a href="javascript:void(0);" class="remove_button" title="Remove field"><img src="<?php echo base_url() ?>assets_home_v3/images/delete_it_row.svg" alt=""></a></div></div>';
        var x = 1;
        $(addButton).click(function () {
            var arr = [];
            var status_para = '0';
            $(".competency_params").each(function(){
            var value = $(this).val();
            ...
            if (x < maxField) {
                if(status_para != '1'){
                    ...
                    alert('Parameters must not be duplicate');
                }
...
<?php } ?>`;
    ...
    if ($("#active_pms_qs_list").length) {
        ...
        var dataTable = $('#active_pms_qs_list').DataTable({
            ...
            'ajax': {
                ...
                'dataSrc': function (data) {
                    if (data.iTotalRecords === 0) {
                        $('#active_pms_qs_list').DataTable().clear().draw();
            ...
            language: {
              ...
              // You can also customize other language options here if needed.
            }
        });
    }
    ...
    location_wise_pms_list = () => {
        ...
        // Perform an AJAX request to fetch data based on dateRange and userId
        $.ajax({
...
// group_name_submit=()=>{
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (a.start_month != undefined) {
                    ...
                    $('.save_common_btn').removeAttr('disabled');
                    $('.save_common_btn').css('background-color','');
                    ...
                    $('.save_common_btn').prop('disabled','disabled');
                    $('.save_common_btn').css('background-color','gray');
    ...
    get_config_set_list = () => {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#competency_category_min").val(a[0].competency_category_min);
                $("#kra_category_min").val(a[0].kra_category_min);
    ...
    config_update = (val) => {
        ...
        if (val == 'pms_com_kra') {
            ...
            var com_min = $("#competency_category_min").val();
            var kra_min = $("#kra_category_min").val();

            on_off = com + '#' + kra + '#' + com_min + '#' + kra_min;
            // alert(on_off);
