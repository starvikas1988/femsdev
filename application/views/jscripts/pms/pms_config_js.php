    ...
    $(document).ready(function () {
        ...
        switch (pageName) {
            ...
            case("teams_performance_sheet"):
                showTeamPerformanceModal();
                break;
            case("other_teams_performance_sheet"):  
                showTeamPerformanceModal();
                break; 
                ...
                $.post("<?= base_url() ?>Pms_assignment/save_pms_for_team", $("#frm_submit_ques_team").serialize() + '&question_for=' + nos + '&pms_id=' + pms_id).done(function (data) {
                    ...
                    if (data != '') {
                        if (data.trim() == 'exceeded') {
                            popupMessage("Description/Parameter count Not as Per Config Setting", "Goldenrod", "warning");
                        } else {
                            popupMessage("Saved successfully", "green", "success");
                        }
...
<script>
    ...
    function addDescriptionModals(group_id = '', sub_group_id = '', appr_id = '', details = 'Y', is_team = 'N', other_uid = '') {   //add others user id     
        console.log("is_team "+is_team)
        $('#sktPleaseWait').modal('show');
        if (appr_id == '') {
            appr_id = $("#pms_id").val();
        }
        $.post("<?= base_url() ?>pms_assignment/get_description_modals", {group_id: group_id, sub_group_id: sub_group_id, appr_id: appr_id, details: details, is_team: is_team, other_uid: other_uid}).done(function (data) { //add others user id
            $('#sktPleaseWait').modal('hide');
    ...
    function uploadDescriptionModals(group_id = '', sub_group_id = '', appr_id = '', details = 'Y', is_team = 'N', other_uid = '') { //add others user id
        $('#sktPleaseWait').modal('show');
        if (appr_id == '') {
            appr_id = $("#pms_id").val();
        }
        $.post("<?= base_url() ?>pms_assignment/get_upload_description_modals", {group_id: group_id, sub_group_id: sub_group_id, appr_id: appr_id, details: details, is_team: is_team, other_uid: other_uid}).done(function (data) {
            $('#sktPleaseWait').modal('hide');
    ...
    if ($("#pms_qs_list").length) {
            var dataTable = $('#pms_qs_list').DataTable({
                ...
                "columnDefs": [
                    ...
                    {
                        "targets": [4,6,8], // Index of the column you want to hide on the page
                        "visible": false
                    }
                ],
                ...
                'ajax': {
                    complete: function (data) {
                        // Add any custom code after data retrieval if needed
                    },
                    'url': $('#data_url').val(),
                    'data': function (data) {
                        // Add any additional data to send to the server if needed
                    }
                }
            });

            // Initialize DataTables Buttons separately
                ...
                buttons: [
                    {
                        ...
                        exportOptions: {                            
                            columns: ':not(:eq(3)):not(:eq(4)):not(:eq(5)):not(:eq(0)):not(:eq(-1))', // Exclude the 3rd column, include the rest
                        }
                    }
                ]
            });
            ...
            dataTable.buttons().container().appendTo($('#pms_qs_list_wrapper .col-md-6:eq(0)'));
        }
...
//                alert(data);
                if (data) {                    
                    popupMessage(data, "green", "success"); 
                    $("#description_modal").modal("hide");
        ...
        if (!requiredValidation($("#frm_sub_grp"))) {
            ...
            $.post("<?= base_url() ?>Pms_assignment/add_description_master", $("#frm_sub_grp").serialize()).done(function (data) {
                ...
                if (response) {
                    ...
                    addDescriptionModalsMaster(group_id = 1);
                    // $("#defination_modal").modal({backdrop: 'static', keyboard: false}, "show");
                    // $('.modal-backdrop').remove().remove();
                } else {
                    popupMessage("Error ! Please Try Again Later.", "Crimson", "error");
