...
$(document).on('change','.epoch',function(){ epoch_score(); });
$(document).on('change','.communication',function(){ epoch_score(); });
$(document).on('change','.resolution',function(){ epoch_score(); });
$(document).on('change','.zero',function(){ epoch_score(); });
...
<script type="text/javascript">
 ...
 if((phone_no.length <10) || (phone_no.length >12)){
		$("#msg-phone_no").html("<font color=red style='font-size:14px;'>Please enter in number format  at least 10-12 number  inside the Phone Number Text Box </font>");
...
<script>
		function epoch_score(){

var score = 0;
var scoreable = 0;
var overall_score=0;
$('.epoch').each(function(index,element){
	var score_type = $(element).val();
	if(score_type == 'Yes'){
		var w1 = parseFloat($(element).children("option:selected").attr('epoch_val'));
		var w2 = parseFloat($(element).children("option:selected").attr('epoch_max'));
		score = score + w1;
...
scoreable=scoreable.toFixed(2);
	...
	$('#possible_score').val(scoreable);
		...
		if($('#vpace').val()=='Fail' || $('#agent_concern').val()=='Fail' || $('#regularity').val()=='Fail' || $('#manipulation').val()=='Fail'){
			totScore=0;
...
$('.resolution').each(function(index,element){
	...
	if(sc1 == 'Yes'){
		var cw1 = parseFloat($(element).children("option:selected").attr('epoch_val'));
		var cw2 = parseFloat($(element).children("option:selected").attr('epoch_max'));
...
$('.communication').each(function(index,element){
	...
	//alert(sc2);
	if(sc2 == 'Yes'){
		var bw1 = parseFloat($(element).children("option:selected").attr('epoch_val'));
		var bw2 = parseFloat($(element).children("option:selected").attr('epoch_max'));
...
$('.zero').each(function(index,element){
	...
	//alert(sc3);
	if(sc3 == 'Yes'){
		var cpw1 = parseFloat($(element).children("option:selected").attr('epoch_val'));
		var cpw2 = parseFloat($(element).children("option:selected").attr('epoch_max'));
...
$('#category_type').change(function(){
	...
	// alert(id);
     ...
     // AJAX request
     $.ajax({
       url:'<?php echo base_url() ?>qa_epoch/get_sub_category',
       type: "POST",
       data: {id: id},
       dataType: 'json',
       success: function(response){
         ...
         // Remove options
         $('#category_sub_type').find('option').not(':first').remove();

         // Add options
         $.each(response,function(index,data){
		//	alert(2);
           $('#category_sub_type').append('<option value="'+data['id']+'">'+data['name']+'</option>');
         });
       }
    });
	//alert(222);
	...
	// alert(val);
		...
		if(Date.parse(val) < Date.parse(start_date))
		{
			$(".end_date_error").html("To Date must be greater or equal to From Date");
			 $(".epoch-effect").attr("disabled",true);
			 $(".epoch-effect").css('cursor', 'no-drop');
		}
		else{
			 $(".epoch-effect").attr("disabled",false);
			 $(".epoch-effect").css('cursor', 'pointer');
			}
		}
		else{
