...
function get_tls(manager_id){
	...
	start_date = $("#from").val();
	end_date = $("#to").val();
	client_id = $("#fclient_id").val();
	
	
	$("tr.tls_tr").hide();
	
	$.post("<?php echo base_url() ?>Pmetrix_test_management/prepare_tl_row", {'design_id':design_id, 'start_date':start_date, 'end_date':end_date, 'manager_id':manager_id, 'client_id': client_id}, function(data){
...
function call_agents(tl_id){
	...
	$.post("<?php echo base_url()?>Pmetrix_test_tl/call_agents", 
		{"tl_id":tl_id, "design" : design_id, "year": $("#year").val(), "month":$("#month").val(), "start_date":$("#from").val(), "end_date":$("#to").val(), "view_request": $("#view_request").val(), "client_id":$("#fclient_id").val()},
		function(data){
			$('#sktPleaseWait').modal('hide');
			$("tr#tr-tl-"+tl_id).show();
