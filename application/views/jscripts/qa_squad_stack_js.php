...
<script type="text/javascript">
		...
		let na_count_park = 0;
		let score_squad_stack_final = 0;
		let scoreable_squad_stack_final = 0;

		$('.squad_stack_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Yes' || score_type_park == 'Pass'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('squad_stack_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('squad_stack_max'));
			...
			}else if(score_type_park == 'N/A' || score_type_park == 'NA'){
      ...
      score_squad_stack_final     = (score_park).toFixed(2);
      scoreable_squad_stack_final = (scoreable_park).toFixed(2);

		$('#squad_stack_earned_score').val(score_squad_stack_final);
		$('#squad_stack_possible_score').val(scoreable_squad_stack_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#squad_stack_overall_score').val(quality_score_percent_park+'%');
		}

		
		if($('#squadstackAF1').val()=='Fail' || $('#squadstackAF2').val()=='Fail' || $('#squadstackAF3').val()=='Fail' || $('#squadstackAF4').val()=='Fail' || $('#squadstackAF5').val()=='Fail' || $('#squadstackAF6').val()=='Fail' || $('#squadstackAF7').val()=='Fail'){
			$('.squad_stackFatal').val(0+'%');
		}else{
			$('.squad_stackFatal').val(quality_score_percent_park+'%');
		}
	}
	
	$(document).on('change','.squad_stack_point',function(){
		squad_stack_calc();
	});
	squad_stack_calc();
</script>
