...
$('#search_date').datepicker({ dateFormat:'mm/dd/yy', changeMonth: true, changeYear: true, yearRange: '2016:' + new Date().getFullYear().toString(),maxDate: new Date() });
</script>
...
<?php if(!empty($teamOverviewGraph)){ ?>
<script>
...
function drawChart() {
	var data = google.visualization.arrayToDataTable([
	  ['Task', 'Hours per Day'],
	  ["Login, <?php echo $schedule_team['counters']['login']; ?>, (<?php echo $schedule_team['percent']['login']; ?>%)",     <?php echo $schedule_team['percent']['login']; ?>],
	  ["No Login, <?php echo $schedule_team['counters']['nologin']; ?>, (<?php echo $schedule_team['percent']['nologin']; ?>%)",      <?php echo $schedule_team['percent']['nologin']; ?>],
	  ]);

	var options = {
	  title: "Schedule Overview (<?php echo $schedule_team['counters']['scheduled']; ?>) - <?php echo date('d M, Y', strtotime($selected_date)); ?>",
	  is3D: true,
	  colors: [<?php $color=0; for($i=0;$i<3;$i++){ echo  "'" .$colorsPieArray1[$color]; $color++; echo $color < 3 ?"'," : "'"; } ?>],
	};

	var chart = new google.visualization.PieChart(document.getElementById('team_3dpie_schedule_adherence'));
	chart.draw(data, options);
}


google.charts.load("current", {packages:["corechart"]});
google.charts.setOnLoadCallback(drawChartLogin);
function drawChartLogin() {
	var data = google.visualization.arrayToDataTable([
	  ['Task', 'Hours per Day'],
	  ["On Time, <?php echo $schedule_team['counters']['ontime']; ?>, (<?php echo $schedule_team['percent']['ontime']; ?>%)",     <?php echo $schedule_team['percent']['ontime']; ?>],
	  ["Late Login, <?php echo $schedule_team['counters']['latetime']; ?>, (<?php echo $schedule_team['percent']['latetime']; ?>%)",      <?php echo $schedule_team['percent']['latetime']; ?>],
	  ]);

	var options = {
	  title: "Login Overview (<?php echo $schedule_team['counters']['login']; ?>) - <?php echo date('d M, Y', strtotime($selected_date)); ?>",
	  is3D: true,
	  colors: [<?php $color=0; for($i=0;$i<3;$i++){ echo  "'" .$colorsPieArray2[$color]; $color++; echo $color < 3 ?"'," : "'"; } ?>],
	};

	var chart = new google.visualization.PieChart(document.getElementById('team_3dpie_login_adherence'));
	chart.draw(data, options);
}
</script>
<?php } ?>
...
<?php $current_data_set = $schedule_monthly[round($selected_month)]['counters']; ?>
...
function drawChart() {
	var data = google.visualization.arrayToDataTable([
	  ...
	  ["Leave, <?php echo $schedule_monthly[round($selected_month)]['counters']['leavetime']; ?>, (<?php echo $schedule_monthly[round($selected_month)]['percent']['leavetime']; ?>%)",  <?php echo $schedule_monthly[round($selected_month)]['percent']['leavetime']; ?>],
	  ["Absent, <?php echo $schedule_monthly[round($selected_month)]['counters']['absenttime']; ?>, (<?php echo $schedule_monthly[round($selected_month)]['percent']['absenttime']; ?>%)",  <?php echo $schedule_monthly[round($selected_month)]['percent']['absenttime']; ?>],
	]);

	var options = {
	  title: "Monthly Schedule Adherence - <?php echo date('F', strtotime($selected_year .'-'.$selected_month.'-01')) ." " . $selected_year; ?>",
	  is3D: true,
	  colors: [<?php $color=0; for($i=0;$i<5;$i++){ echo  "'" .$colorsPieArray[$color]; $color++; echo $color < 5 ?"'," : "'"; } ?>],
	  chartArea: {
		backgroundColor: {
		  fill: '#FF0000',
		  fillOpacity: 0
	  ...
	  datasets: [
		{
		  data: [<?php $cn = 1;
						...
						{ 
							...
							if(!empty($schedule_monthly[$i])){
								$current_data_set = $schedule_monthly[$i];
								$adherence = !empty($current_data_set['counters']['nonoff']) ? $current_data_set['counters']['ontime']/$current_data_set['counters']['nonoff'] : '0';
