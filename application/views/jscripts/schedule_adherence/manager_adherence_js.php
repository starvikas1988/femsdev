...
<?php if($reportType == "userly"){ ?>
//============================= USERWISE ADHERENCE ============================================

var ctxBAR = document.getElementById("team_2dpie_userwise_adherence");
var myBarChart = new Chart(ctxBAR, {
	type: 'bar',
	data: {
	  labels: ["<?php $checkedNow=0; foreach($team_users as $tokenu){ $tokenuid = $tokenu['id']; $current_data_set = $schedule_team[round($selected_month)][$tokenuid]; if($current_data_set['counters']['total'] > 0){ $checkedNow++; if($checkedNow > 1){ echo '","'; } echo $tokenu['full_name'] .' ('.$tokenu['fusion_id'].')'; } } ?>"],
	  datasets: [
		{
		  label: "Adherence",
		  data: [
		  <?php 
		  $checkedNow=0; foreach($team_users as $tokenu){ 
		  $tokenuid = $tokenu['id']; $current_data_set = $schedule_team[round($selected_month)][$tokenuid]; 
		  if($current_data_set['counters']['total'] > 0){ 
			$checkedNow++; if($checkedNow > 1){ echo ','; } 
			echo $current_data_set['percent']['adherence']; 
		  } } ?>
		  ],
		  backgroundColor: "#30bc06",
		  borderColor: "#30bc06",
		  borderWidth: 1
		},
		{
		  ...
		  if($current_data_set['counters']['total'] > 0){ 
			$checkedNow++; if($checkedNow > 1){ echo ','; } 
			echo $current_data_set['percent']['latetime']; 
		  } } ?>
		  ],
	...
	options: {
	  ...
	  title: {
		display: true,
		lineHeight: 2,
		text: "User Wise Adherence - <?php echo date('F', strtotime($selected_year .'-'.$selected_month.'-01')) ." " . $selected_year; echo !empty($tl['search']['process']) ? " - " .$tl['search']['process'] : '';  ?>"
	  },
	  tooltips: {
		callbacks: {
		   label: function(tooltipItem) {
