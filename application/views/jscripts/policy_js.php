...
$(document).ready(function() {
	...
	$("#btnAddPolicy").click(function(){
		...
		if(aoffice_id!="" && afunction_id!="" && atitle!="" && brand_id!=""){
	...
	$(".editPolicy").click(function(){
		...
		var loc = arrPrams[2].split(",");
		var brands = arrPrams[5].split(",");

		$("#edoffice_id1").multiselect("clearSelection");
		$('#edoffice_id1').multiselect('rebuild');
		$('#edoffice_id1').multiselect('select', loc);
		$('#edoffice_id1').multiselect('refresh');


		$("#ebrand_id").multiselect("clearSelection");
		$('#ebrand_id').multiselect('rebuild');
		$('#ebrand_id').multiselect('select', brands);
		$('#ebrand_id').multiselect('refresh');

		$("#edfunction_id").multiselect("clearSelection");
		$('#edfunction_id').multiselect('rebuild');
		$('#edfunction_id').multiselect('select', arrPrams[3]);
		$('#edfunction_id').multiselect('refresh');
	...
	$("#btnEditPolicy").click(function(){
		...
		var ebrand_id=$('#ebrand_id').val();
		//alert(baseURL+"policy/updatePolicy?"+$('form.frmEditPolicy').serialize());
		
		if(pid!="" && title!="" && office_id!="" && function_id!="" && ebrand_id!="" ){
