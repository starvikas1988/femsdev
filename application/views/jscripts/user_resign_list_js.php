<script>
	...
	////////////////////////Approved Section//////////////////////////	
		...
		$(document).on('click','.userApprovedResign',function(){
			...
			allOffice = '<option value="">-Select-</option><option value="0">0 Days</option><option value="15">15 Days</option><option value="30">30 Days</option><option value="60">60 Days</option><option value="90">90 Days</option>';
		 ...
		 $(document).on('click','#acceptedResignModal',function(){
		...
		$(document).on('change','.frmUserApprovedResign #is_notice',function(){
		...
		// $("#selet_reasons").change(function(){
		$(document).on('change','#selet_reasons',function(){
			var location = "<?php echo get_user_office_id(); ?>";
			var brand = "<?php echo get_user_company_id(); ?>";
			const search_brands = [1, 2, 12];

			const index = search_brands.indexOf(brand);

			var seleted_reason = $(this).val();
			if(seleted_reason == "Process Ramp Down"){
				var resign_period_ramp=<?php echo $notice_period_ramp_down; ?>;
				
				var myDate_release = new Date(new Date().getTime()+(resign_period_ramp*24*60*60*1000));
				final_release = myDate_release.toISOString().slice(0, 10);	
			}else{
				var resign_period_day='<?php echo $resign_period_day; ?>';
				var myDate_release = new Date(new Date().getTime()+(resign_period_day*24*60*60*1000));
		...
		$(document).on('click','.userDeclinedResign',function(){
		...
		$(document).on('click','.readMore',function(){
			...
			var des=$(this).attr("rdes");
			var full_name = $(this).attr("us_name");
			var f_id = $(this).attr("f_id");
			
			
			
			// $('#user_id').val(user_id);
			$('#readMoreDescription #rdes').text(des);
			$('#readMoreDescription #fullName').text(full_name);
			$('#readMoreDescription #fu_id').text(f_id);
			$("#readMoreDescription").modal('show');
			
		});
		
		// $("#declineResignModal").click(function(){
		$(document).on('click','#declineResignModal',function(){
...
//////////////////Accept & Retain Resign (HR section)-16/08/2019//////////////////////	
			...
			$(document).on('click','.termHRAccept',function(){
			...
			$(document).on('click','#userTermHRAccept',function(){
			...
			$(document).on('change','.frmUserTermHRAccept #is_notice',function(){
			...
			$(document).on('click','.termHRRetain',function(){
				var uid=$(this).attr("uid");
			...
			$(document).on('click','#userTermHRretain',function(){
...
/////////////Released Document /////////////
		...
		$(document).on('click','.termReleased',function(){
			var uid=$(this).attr("uid");
		...
		$(document).on('click','#userTermandRelease',function(){
...
///////////////////////////////////////////////////////////////////////////
		...
		$(document).on('submit','#form_resign_user',function(e){
			$('#sktPleaseWait').modal('show');
			//$('#btn_save_row').hide();
		});
		...
		$(document).on('click','.withdrawResign',function(){
