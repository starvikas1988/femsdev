...
function ijp_populate_process_combo(cid,def='',objid='process_id',isAll='N')
{
		...
		$.ajax({
		   ...
		   success: function(pList){
			...
			if(json_obj==""){
				if(isAll=='Y') $('#'+objid).append($('<option></option>').val('0').html('ALL'));
...
$("#client_id").change(function(){
	...
	ijp_populate_process_combo(client_id,'','process_id','N');
});
			...
			if(res.stat == true)
			{
				if(click_type1)
				{
					...
					$.each(res.datas,function(index,element)
					{
						...
						tr += '<tr ><td rowspan="'+element.scores.length+'"> </td><td class="'+trCss+'" rowspan="'+element.scores.length+'">'+element.name+'</td><td rowspan="'+element.scores.length+'">'+element.fusion_id+'</td><td style="color:green">'+element.interviewer_name[0]+'</td><td>'+element.scores[0]+'</td><td width="50%">'+element.remarks[0]+'</td></tr>';
