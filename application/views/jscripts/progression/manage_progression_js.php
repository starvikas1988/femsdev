...
$('#search_requisition_data').submit(function (e) {
	e.preventDefault();
...
$('#create_progression_btn').on('click',function()
{
	...
	$('#create_progression_form [name="hiring_manager_id[]"]').select2();
	$('#create_progression_form #requisition_id').removeAttr('value');
	$('#create_progression_form [name="applicable_location[]"]').select2();
	$('#create_progression_form [name="client_id[]"]').select2();

});

///HD
function ijp_populate_process_combo(cid,def='',objid='process_id',isAll='N')
{
	
		var URL='<?php echo base_url('progression/getProcessList'); ?>';
...
$("#client_id").change(function(){
	...
	ijp_populate_process_combo(client_id,'','process_id','Y');
});
	...
	$('#create_progression_form').on('submit',function(e)
	{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				$('#create_progression_form #client_id').val('');
				var datas = $('#search_requisition_data').serializeArray();
				alert("Success. Please wait for refresh the page");
				get_requisition_result(datas);
...
<script>
	$(document).on('click',".approve_progression",function(e){
		...
		var datas = {'req_code':requisition_id};
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				alert("Approve Done. Please wait for refresh the page");
			}
...
<script>
/*********************************************************/
$('#search_requisition_data_scrap').submit(function (e) {
	e.preventDefault();
	var datas = $(this).serialize();
	get_requisition_result_scrap(datas);
})
$(document).on('click','.schedule_interview_edit',function()
	{
		$('#hr_neutral_edit_form')[0].reset();
		$('#ipd').empty();
		$('#h_managers').empty();
		$('#schedule_interview_container').empty();
		var requisition_id = $(this).attr('data-requisition_id');
		var location = $(this).attr('data-location');
		var hiring_manager_id = $(this).attr('data-hiring_manager_id');
		$('#hr_neutral_edit_form #requisition_id').val(requisition_id);
		$('#hr_neutral_edit_form #location_id').val(location);
		$('#hr_neutral_edit_form #hiring_manager_id').val(hiring_manager_id);
		...
		process_ajax(function(response)
		{
			...
			list_hr += '';
			list_np += '';
			$('#hr_neutral_edit_form #h_managers').html(list.join(' , '));
			//$('#hr_neutral_edit_form #hiring_managers').html(list);
			$('#hr_neutral_edit_form #hr_panel_edit').html(list_hr);
			$('#hr_neutral_edit_form #neutral_panel_edit').html(list_np);
			$('#ipd').html(list1);
			
		},request_url, datas, 'text');


		$('#hr_neutral_edit_modal').modal('show');
	});

	$(document).on('submit',"#hr_neutral_edit_form",function(e){
		e.preventDefault();
		var datas = $(this).serializeArray();
		
			var request_url = "<?php echo base_url('progression/update_hr_neutral'); ?>";
			process_ajax(function(response)
			{
				var res = JSON.parse(response);
				if(res.stat == true)
			...
			$('#hr_neutral_edit_modal').modal('hide');
			//location.reload();
	});

	$(document).on('click',".reopen_progression",function(e){
		e.preventDefault();
		var requisition_id = $(this).attr('data-requisition_id');
		var datas = {'requisition_id':requisition_id};
		
			var request_url = "<?php echo base_url('progression/reopen_progression'); ?>";
			process_ajax(function(response)
			{
				var res = JSON.parse(response);
				if(res.stat == true)
			...
			alert('Progression Reopen');
			location.reload();
	});

	function get_requisition_result_scrap(datas)
	{
		var requisition_id = $('#search_progression_requisition #requisition_id').val();
		var request_url = "<?php echo base_url('progression/get_requisition_result_scrap'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('click',".view_applications",function(e){
		...
		process_ajax(function(response)
		{
			...
			//console.log(res);
			var table = $('#view_application_pop').DataTable({
			lengthChange: false,
			retrieve: true
	...
	$(document).on('click','.schedule_interview',function()
	{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				var table = $('#interview_pop').DataTable({
					lengthChange: false,
				});
				new $.fn.dataTable.Buttons(table, {
					buttons: [{
			...
			if(res.stat == true)
			{
				if(click_type1)
				{
					...
					$.each(res.datas,function(index,element)
					{
						...
						tr += '<tr ><td rowspan="'+element.scores.length+'"> </td><td class="'+trCss+'" rowspan="'+element.scores.length+'">'+element.name+'</td><td rowspan="'+element.scores.length+'">'+element.fusion_id+'</td><td style="color:green">'+element.interviewer_name[0]+'</td><td>'+((element.scores[0]==null) ? "Pending" : element.scores[0])+'</td><td width="50%">'+((element.remarks[0]==null) ? "Pending" : element.remarks[0])+'</td></tr>';
				...
				if(click_type)
				{
					...
					$.each(res.datas,function(index,element)
					{
						tr += '<tr><td rowspan="'+element.scores.length+'"><input type="checkbox" name="select_candidate[]" id="select_candidate" value="'+element.id+'"></td><td rowspan="'+element.scores.length+'">'+element.name+'</td><td rowspan="'+element.scores.length+'">'+element.fusion_id+'</td><td style="color:green">'+element.interviewer_name[0]+'</td><td>'+element.scores[0]+'</td><td width="50%">'+element.remarks[0]+'</td></tr>';
					...
					var table = $('#final_seltn_pop').DataTable({
						lengthChange: false,
					});
					new $.fn.dataTable.Buttons(table, {
						buttons: [{
				...
				if(click_type2)
				{
					...
					$.each(res.datas,function(index,element)
					{
						tr += '<tr><td rowspan="'+element.scores.length+'">'+element.name+'</td><td rowspan="'+element.scores.length+'">'+element.fusion_id+'</td><td style="color:green">'+element.interviewer_name[0]+'</td><td width="50%">'+element.remarks[0]+'</td></tr>';
	...
	$(document).on('click','.schedule_filter_test',function()
	{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				var table = $('#schedule_filter_pop').DataTable({
					lengthChange: false,
					destroy: true,
				});
				new $.fn.dataTable.Buttons(table, {
	...
	$(document).on('change',"#scheduled_exam_time",function(e){
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				var table = $('#filter_test_pop').DataTable({
					lengthChange: false,
				});
				new $.fn.dataTable.Buttons(table, {
					buttons: [{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				$.each(res.datas.user_list,function(index,element)
				{
					...
					var table = $('#atmtc_role_change').DataTable({
						lengthChange: false,
...
<script>
	...
	$(document).on('click',".edit_progression",function(e){
		...
		$('#create_progression_form [name="due_date"]').removeAttr('min');
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
			{
				...
				if(res.datas['requistion_data'].client_id != null)
				{
					...
					$('#create_progression_modal [name="client_id[]"]').select2();
					if(res.datas['requistion_data'].client_id != "" || res.datas['requistion_data'].client_id != null || res.datas['requistion_data'].client_id != "undefined"){

							$('#create_progression_modal [name="client_id[]"]').val(res.datas['requistion_data'].client_id.split(','));
							$('#create_progression_modal [name="client_id[]"]').select2();
						}
				...
				{
					option2 += '<option value="'+element.id+'" >'+element.name+'('+element.office_id+'-'+element.dept_name+')</option>';
				});
				$('#create_progression_modal #hiring_manager_id').html(option2);
...
<script>
	//Search Progress Site Filter (Location wise Site Data)
$("#multiselectwithsearch,#multiselectwithsearch2").change(function(){
	var loc_id=$(this).serializeArray();
	var request_url = "<?php echo base_url('progression/get_requsition_site'); ?>";
	process_ajax(function(response)
	{
		var res = JSON.parse(response);
...
<!-- <script>
	//Search Progress Site Filter (Location wise Site Data)
$("#multiselectwithsearch2").change(function(){
	var loc_id=$(this).serializeArray();
	var request_url = "<?php echo base_url('progression/get_requsition_site'); ?>";
	process_ajax(function(response)
	{
...
<script>
$('.ijp_search').change(function () {

	var datas = $('#search_ijp_code').serializeArray();
	var request_url = "<?php echo base_url('ijp_repot/ijp_code_search'); ?>";

	process_ajax(function(response)
	{
		var res = JSON.parse(response);

		var option = '<option value="">--Select IJP Code---</option>';
		$.each(res,function(index,element)
		{
			option += '<option value="'+element.requisition_id+'" >'+element.requisition_id+'</option>';
		});
		$('.ijp_code').html(option);
	},request_url, datas, 'text');
})
</script>

<!--start custom table for progression-->
<script>
  ...
  if($(this).closest("tr").next("tr").hasClass("hide")){
    ...
    $(this).closest("tr").next("tr").children("td").slideDown(350);
		...
		var request_url = "<?php echo base_url('progression/get_requisition_sub_exploder_data'); ?>";
		var r_id=$(this).attr("data-id-req");
		datas = {id:r_id};    
		...
		process_ajax(function(response)
		{
			...
			{
				//alert(response);
				if (res.datas[0].client_name == null) { var client = "NA"; }
				else { var client = res.datas[0].client_name; }

				if (res.datas[0].process_name == null) { var process_name = "NA"; }
				else { process_name = res.datas[0].process_name; }

				if (res.datas[0].dept_name == null) { var depart = "NA"; }
				else { var depart = res.datas[0].dept_name; }

				if (res.datas[0].sub_dept_name == null) { var sub_depart = "NA"; }
				else { sub_depart = res.datas[0].sub_dept_name; }				

				var req_data = "<td>"+res.datas[0].applicable_location+"</td><td>"+client+"</td><td>"+process_name+"</td><td>"+depart+"</td><td>"+sub_depart+"</td><td>"+res.datas[0].request_reason+"</td><td>"+res.posting_type+"</td><td>"+res.movement_type+"</td><td>"+res.filter_type+"</td><td>"+res.datas[0].no_of_position+"</td><td>"+res.datas[0].total_application+"</td>";
				$('#req_sub_data_'+r_id).html(req_data);
			}
			else
			{
				alert('Try After Some Time.');
...
<!--end custom table for progression-->
