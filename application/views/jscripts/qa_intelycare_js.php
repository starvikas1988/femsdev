...
$(document).on('change','.intelycare',function(){ intelycare_score(); });
$(document).on('change','.customer',function(){ intelycare_score(); });
$(document).on('change','.business',function(){ intelycare_score(); });
$(document).on('change','.compliance',function(){ intelycare_score(); });
...
$( "#agent_id" ).on('change' , function() {
		...
		var URL='<?php echo base_url();?>qa_intelycare/getTLname';
...
<script>
		function intelycare_score(){

var score = 0;
var scoreable = 0;
var overall_score=0;
$('.intelycare').each(function(index,element){
	var score_type = $(element).val();
	if(score_type == 'Yes'){
		var w1 = parseFloat($(element).children("option:selected").attr('intelycare_val'));
		var w2 = parseFloat($(element).children("option:selected").attr('intelycare_max'));
		score = score + w1;
...
totScore = ((score*100)/scoreable).toFixed(2);
	...
	$('#earned_score').val(score);
		...
		if($('#professionalism').val()=='Fail' || $('#critical').val()=='Fail'){
	        $("#overall_score").val(0);
	 }else{
		if(!isNaN(totScore)){
...
$('.customer').each(function(index,element){
	...
	if(sc1 == 'Yes'){
		var cw1 = parseFloat($(element).children("option:selected").attr('intelycare_val'));
		var cw2 = parseFloat($(element).children("option:selected").attr('intelycare_max'));
...
$('.business').each(function(index,element){
	...
	//alert(sc2);
	if(sc2 == 'Yes'){
		var bw1 = parseFloat($(element).children("option:selected").attr('intelycare_val'));
		var bw2 = parseFloat($(element).children("option:selected").attr('intelycare_max'));
...
$('.compliance').each(function(index,element){
	...
	//alert(sc3);
	if(sc3 == 'Yes'){
		var cpw1 = parseFloat($(element).children("option:selected").attr('intelycare_val'));
		var cpw2 = parseFloat($(element).children("option:selected").attr('intelycare_max'));
	...
	// alert(val);
		...
		if(Date.parse(val) < Date.parse(start_date))
		{
			$(".end_date_error").html("To Date must be greater or equal to From Date");
			 $(".intelycare-effect").attr("disabled",true);
			 $(".intelycare-effect").css('cursor', 'no-drop');
		}
		else{
			 $(".intelycare-effect").attr("disabled",false);
			 $(".intelycare-effect").css('cursor', 'pointer');
			}
		}
		else{
