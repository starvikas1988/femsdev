...
<!-- <script type="text/javascript">
	function ayming_calc(){
				...
				var earned_score = 0, possible_score=0, overall_score="", cust_earned=0, cust_possible=0, cust_overall=0,weightage_pass=0,weightage_all=0;
			    var comp_earned=0, comp_possible=0, comp_overall=0, business_earned=0, business_possible=0, business_overall=0;
				$(".ayming_point").each(function(index, element){
					//console.log($(element).val());
					if($(element).val()!=""){
						var earned_weightage = parseInt($(element).children("option:selected").attr('ayming_val'));
						console.log(earned_weightage);
						earned_score += earned_weightage;
						if($(element).val() == 'Yes' || $(element).val() == 'NA'){
							var weightage = parseInt($(element).children("[value='Yes']").attr('ayming_val'));
							possible_score += weightage;
							//weightage_all += (weightage );
						}else if($(element).val() == 'Pass'){
							var weightage_pass = parseInt($(element).children("[value='Pass']").attr('ayming_val'));
							possible_score += weightage_pass;
							//weightage_all += (weightage_pass); 
						}
						console.log(possible_score);
						
						
						// possible_score += weightage_all;
						//console.log(possible_score);
						overall_score=parseInt((earned_score/possible_score)*100);
						...
						if($(this).hasClass("compliance")){
							...
							comp_possible+=(weightage+weightage_pass);
						...
						$("#ayming_overall_score").val(parseFloat((earned_score/possible_score)*100).toFixed(2)+"%");
						
					}
				});
			if(!isNaN(earned_score)){
				$("#ayming_earned_score").val(earned_score);
			}
			if(!isNaN(possible_score)){
				$("#ayming_possible_score").val(possible_score);
			}
	}
</script>
<script type="text/javascript">
