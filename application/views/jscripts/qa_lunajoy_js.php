...
<script type="text/javascript">
		...
		let na_count_park = 0;
		let score_lunajoy_final = 0;
		let scoreable_lunajoy_final = 0;
		let w3_park = 0;

		$('.lunajoy_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Pass'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('lunajoy_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('lunajoy_max'));
				...
				let w1_park = parseFloat($(element).children("option:selected").attr('lunajoy_val'));
				let w3_park = parseFloat($(element).children("option:selected").attr('lunajoy_max'));
      ...
      score_lunajoy_final     = (score_park).toFixed(2);
      scoreable_lunajoy_final = (scoreable_park).toFixed(2);

		$('#lunajoy_earned_score').val(score_lunajoy_final);
		$('#lunajoy_possible_score').val(scoreable_lunajoy_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#lunajoy_overall_score').val(quality_score_percent_park+'%');
		}

		
		if($('#lunajoyAF1').val()=='Fail' || $('#lunajoyAF2').val()=='Fail' || $('#lunajoyAF3').val()=='Fail' || $('#lunajoyAF4').val()=='Fail' || $('#lunajoyAF5').val()=='Fail' || $('#lunajoyAF6').val()=='Fail'){
			$('.lunajoyFatal').val(0+'%');
		}else{
			$('.lunajoyFatal').val(quality_score_percent_park+'%');
		}
	}
	...
	$( document ).ready(function() {
		$(document).on('change','.lunajoy_point',function(){
			lunajoy_calc();
		});
    	lunajoy_calc();
	});
...
<script>
	$(document).ready(function(){
		...
		$("#call_duration").timepicker({timeFormat : 'HH:mm:ss' });
		<?php if($this->uri->segment(2)=="Qa_lunajoy" && $this->uri->segment(4)=="0"){?>
			//console.log("okk");
		<?php }?>
	});
</script>
