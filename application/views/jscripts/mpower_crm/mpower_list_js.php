...
$(document).on('click', '#viewCrmLogs', function(){
	...
	bUrl = '<?php echo base_url(); ?>mpower/crm_logs_list/'+cid;
	$.ajax({
		type: 'POST',
		url: bUrl,
		data: 'cid=' + cid,
...
$(document).on('click', '#viewCrmEmailModal', function(){
    cemail = $(this).attr('cemail');
    cid = $(this).attr('cid');
	$('#crmEmailModal .modal-title').html('Send Email : ' + cid);
	//$('#crmEmailModal .modal-body').html('No Records Found');
	$('#crmEmailModal #form_crm_id').val(cid);
	$('#crmEmailModal #form_email_id').val(cemail);
	$('#crmEmailModal').modal('show');	
});
