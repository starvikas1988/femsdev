...
$(document).ready(function(){
    ...
    $("#location_dt").change(function(){
        localStorage.setItem('location_id',$(this).val());
        if($(this).val()==''){
            $("#loc_cat").empty().html('<option value="ALL">ALL</option>');
        }else{
            $.post("<?php echo base_url(); ?>servicerequest/get_category",{'location_id':$(this).val()}, function(data){
                console.log(data);
                $("#loc_cat").empty().html('<option value="ALL">ALL</option>');
                $("#loc_cat").append(data);
            });
    ...
    $("#location_dt1").change(function(){
        localStorage.setItem('location_id1',$(this).val());
        if($(this).val()==''){
            $("#loc_cat1").empty().html('<option value="ALL">ALL</option>');
        }else{
            $.post("<?php echo base_url(); ?>servicerequest/get_category",{'location_id':$(this).val()}, function(data){
                console.log(data);
                $("#loc_cat1").empty().html('<option value="ALL">ALL</option>');
                $("#loc_cat1").append(data);
            });
        }
    });

    $("#loc_cat").change(function(){
        var location_id=localStorage.getItem('location_id');
        if($(this).val()==''){
            $("#sub_category").empty().html('<option value="ALL">ALL</option>');
        }else{
            $.post("<?php echo base_url(); ?>servicerequest/get_sub_category",{'category_id':$(this).val(),'location_id':location_id}, function(data){
                $("#sub_category").empty().html('<option value="ALL">ALL</option>');
    ...
    $("#loc_cat1").change(function(){
        ...
        if($(this).val()==''){
            $("#sub_category1").empty().html('<option value="">ALL</option>');
        }else{
            $.post("<?php echo base_url(); ?>servicerequest/get_sub_category",{'category_id':$(this).val(),'location_id':location_id}, function(data){
                $("#sub_category1").empty().html('<option value="ALL">ALL</option>');
                $("#sub_category1").append(data);
            });
        }
    });
