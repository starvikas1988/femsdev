...
<script type="text/javascript">
	function checkDecConsumer(el) {
			...
			if(el.value.length >25){
       			//alert("Account number can not be more than 25 digits!");
...
////////////////vikas//////////////////////////////
 ...
 <script type="text/javascript">
		...
		let na_count_park = 0;
		let score_union_pacific_final = 0;
		let scoreable_union_pacific_final = 0;

		$('.union_pacific_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Yes'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('union_pacific_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('union_pacific_max'));
      ...
      score_union_pacific_final     = (score_park).toFixed(2);
      scoreable_union_pacific_final = (scoreable_park).toFixed(2);

		$('#union_pacific_earned_score').val(score_union_pacific_final);
		$('#union_pacific_possible_score').val(scoreable_union_pacific_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#union_pacific_overall_score').val(quality_score_percent_park+'%');
		}

		if($('#union_pacificAF1').val()=='No' || $('#union_pacificAF2').val()=='No' || $('#union_pacificAF3').val()=='No' || $('#union_pacificAF4').val()=='No' || $('#union_pacificAF5').val()=='No' || $('#union_pacificAF6').val()=='No' || $('#union_pacificAF7').val()=='No' || $('#union_pacificAF8').val()=='No' || $('#union_pacificAF9').val()=='No'  || $('#union_pacificAF10').val()=='No' || $('#union_pacificAF11').val()=='No' || $('#union_pacificAF12').val()=='No' || $('#union_pacificAF13').val()=='No' || $('#union_pacificAF14').val()=='No' || $('#union_pacificAF15').val()=='No'){
			console.log($('#union_pacificAF1').val());

			quality_score_percent_park = (0.00).toFixed(2);
			$('.union_pacificFatal').val(quality_score_percent_park+'%');
		}else{
			$('#union_pacific_overall_score').val(quality_score_percent_park+'%');
		}
	}
	
	$(document).on('change','.union_pacific_point',function(){
		union_pacific_calc();
	});
	union_pacific_calc();
</script>
...
//Edited By Samrat 30/9/21
	$(document).ready(function(){
		...
		$("#call_duration").timepicker({timeFormat : 'HH:mm:ss' });
		<?php if($this->uri->segment(2)=="Qa_union_pacific" && $this->uri->segment(4)=="0"){?>
			//console.log("okk");
