...
<script>
	$(document).on('submit','#get_agent_list_form',function(e)
	{
		...
		var datas = {'office_id':office_id, 
						...
						'fusion_id':fusion_id,
						'performance_for_month':performance_for_month,
						'performance_for_year':performance_for_year,
						'user_id':user_id,'others_team':others_team,
						'view_type':view_type,
						'post_period':post_period
					};
		if(fusion_id == 1)
		{
			var request_url = "<?php echo base_url('Pmetrix_test_tl/prepare_row/true'); ?>";
		}
		else
		{
			var request_url = "<?php echo base_url('Pmetrix_test_agent/prepare_row/true'); ?>";
		}
		process_ajax(function(response)
		{
			if(fusion_id == 1)
...
<script>
	$(document).on('click','#pay_period_1_btn,#pay_period_2_btn',function()
	{
		...
		var request_url = "<?php echo base_url('Pmetrix_test_agent/get_bouns_info'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
