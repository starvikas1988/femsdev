...
function docusign_calc(){
		...
		$('#earnedScore').val(quality_score_percent.toFixed(2));
		$('#possibleScore').val(quality_score_percent1.toFixed(2));
		
		if(!isNaN(quality_score_percent2)){
			if($('#deduct_20_perc').val()=="Yes"){
				$('#overallScore').val((quality_score_percent2 - 20)+'%');
			}else{
...
///////////////// Calibration - Auditor Type ///////////////////////	
	...
	$('#sales_call_type').on('change', function(){
		if($(this).val()=='Non-Sale'){
			$('.nonSale_epi').show();
			$('#nonSale').attr('required',true);
			$('#nonSale').prop('disabled',false);
		}else{
			$('.nonSale_epi').hide();
			$('#nonSale').attr('required',false);
			$('#nonSale').prop('disabled',true);
		}
	});	
	...
	$(".custScore").on("change",function(){
		docusign_calc();
	});

	$(".busiScore").on("change",function(){
		docusign_calc();
	});

	$(".compScore").on("change",function(){
		docusign_calc();
	});
	...
	$(".deduct_20_perc").on("change",function(){
		docusign_calc();
	});
