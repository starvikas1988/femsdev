	...
	function get_sets(exam_id)
	{
		...
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				$.each(res.datas,function(index,element)
				{
					tr += '<tr><td>'+(index+1)+'</td><td>'+element.title+'</td><td>'+element.no_of_question+'</td><td style="width:20%"><button class="btn btn-xs btn-warning upload_exam_btn" data-set_id="'+element.id+'" title="Upload Set"><i class="fa fa-file-excel-o" aria-hidden="true"></i></button>&nbsp;<button class="btn btn-xs btn-warning view_question" data-set_id="'+element.id+'" title="View Question"><i class="fa fa-eye" aria-hidden="true"></i></button></td></tr>';
				});
