...
////////////////vikas//////////////////////////////
 ...
 <script type="text/javascript">
		...
		$('.pnb_ob_sal_v1_point').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Good'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('pnb_ob_sal_v1_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('pnb_ob_sal_v1_max'));
				
				score_park = score_park + w1_park;
				scoreable_park = scoreable_park + w2_park;

			}else if(score_type_park == 'Poor'){
				...
				scoreable_park = scoreable_park + w2_park;
				//scoreable = scoreable + weightage;
			}else if(score_type_park == 'Needs Improvement'){
      ...
      scoreable_sea_world_final = (scoreable_park).toFixed(2);
		...
		$('#pnb_ob_sal_v1_earned_score').val(score_sea_world_final);
		$('#pnb_ob_sal_v1_possible_score').val(scoreable_sea_world_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#pnb_ob_sal_v1_overall_score').val(quality_score_percent_park+'%');
		}

		if($('#ob_sales_fatal1').val()=='Fatal' || $('#ob_sales_fatal2').val()=='Fatal' || $('#ob_sales_fatal3').val()=='Fatal'){
			console.log($('#parkAF1').val());

			quality_score_percent_park = (0.00).toFixed(2);
			$('.pnb_outbound_sales_v1Fatal').val(quality_score_percent_park+'%');
		}else{
			$('#pnb_ob_sal_v1_overall_score').val(quality_score_percent_park+'%');
		}
	}
	
	$(document).on('change','.pnb_ob_sal_v1_point',function(){
		pnb_ob_sales_v1_calc();
	});
	pnb_ob_sales_v1_calc();
</script>
