...
<script>
	...
	function get_examinations()
	{
		...
		var request_url = "<?php echo base_url('ld_programs/ld_get_exam_list'); ?>";
	...
	function get_sets(exam_id)
	{
		...
		var request_url = "<?php echo base_url('ld_programs/ld_get_set_list'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
			{
				...
				$.each(res.datas,function(index,element)
				{
				tr += '<tr><td>'+(index+1)+'</td><td>'+element.title+'</td><td>'+element.no_of_question+'</td><td style="width:20%"><button class="btn btn-xs upload_exam_btn no_padding" data-set_id="'+element.id+'" title="Upload Set"><img src="<?php echo base_url();?>assets_home_v3/images/upload_icon.svg" alt=""></button> <button class="btn btn-xs view_question" data-set_id="'+element.id+'" title="View Question"><img src="<?php echo base_url();?>assets_home_v3/images/view.svg" alt=""></button></td></tr>';
				});
...
<script>
	$(document).on('submit','#create_examination_form',function(e)
	{
		...
		var request_url = "<?php echo base_url('ld_programs/ld_create_exam'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('submit','#exam_set_form',function(e)
	{
		...
		var request_url = "<?php echo base_url('ld_programs/ld_upload_exam_set'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('submit','#create_set_form',function(e)
	{
		e.preventDefault();
		var datas = $(this).serializeArray();
		var request_url = "<?php echo base_url('ld_programs/ld_create_set'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('click','.view_question',function(e)
	{
		...
		var request_url = "<?php echo base_url('ld_programs/ld_get_question_to_view'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
