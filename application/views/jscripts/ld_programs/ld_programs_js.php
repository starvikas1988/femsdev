...
// APPEND ALTER
    $("#traineeAdd").click(function () {
        ...
        var addDivTrainee = '<div class="row"><div class="col-md-6"><div class="form-group"><select class="form-control" name="trainee_id_select[]" id="trainee_id_select[]"><option value="' + selectedValue + '">' + selectedName + '</option></select></div></div><div class="col-md-2"><div class="form-group"><button type="button" class="btn remove_bg btn-md" onclick="$(this).parent().parent().parent().remove();"><img src="<?php echo base_url();?>assets_home_v3/images/delete_it_row.svg" alt=""></button></div></div></div>';
        var htmlappend = $('#traineeAddDivFinal').append(addDivTrainee);

    });

    // $('#traineeAddDiv').on('change', 'select[name="batch_external"]', function () {
        $('#batch_external').on('change', function() {
            
        val =$('#batch_external').val();
        if (val == 1) {
            $("#external_trainer_name").prop("readonly", false); 
            ...
            $("#external_trainer_name").prop("readonly", true);
            $("#external_trainer_name").removeAttr('required', 'required');
...
// UPLOAD PERFORMANCE EXCEL
    $('.uploadPerformBtn').click(function () {
        ...
        if (batch_id != "" && design_id != "") {
            ...
            $('#uploadPerformanceModal .linkSampleUploadExcel').html('<a target="_blank" style="cursor:pointer" href="' + downloadLink + '" class="btn btn_padding filter_btn">Download Sample</a>');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i in json_obj) {
                    html += '<tr><td><input type="checkbox" name="userCheckBox[]" value="' + json_obj[i].user_id + '"></td><td>' + json_obj[i].trainee_name + '</td><td>' + json_obj[i].fusion_id + '</td><td>' + json_obj[i].roles + '</td></tr>';
                }
...
// CERTIFICATE UPDATE DETAILS
    $('.certificateuserbatchUpdate').click(function () {
        ...
        if (position_t_id != "") {
            ...
            $('#img_position_thrd_file').html('');
            $('#img_position_snd_file').html('');
            $('#img_position_first_file').html('');
    ...
    function fetchSignature(fid, data) {
        $.ajax({
            ...
            success: function (result) {
                if(result!='')
                {   
                    $('#img_' + data).text('Signature has already been uploaded.('+result+')');
                    $('#img_' + data).css('color','green');                
                    $('#'+data).removeAttr('required'); 
                }else{
                    
                    $('#img_' + data).html('');
                    $('#'+data).addAttr('required'); 
                }
            },
            error: function (result) {
                alert('Something Went Wrong!');
