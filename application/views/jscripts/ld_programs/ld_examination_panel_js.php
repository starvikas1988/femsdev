<script>
	...
	?>
			...
			end.setMinutes ( start.getMinutes() + <?php echo $minutes_to_add; ?> );		
	<?php
		}
	?>
	console.log(end);
	...
	function timeBetweenDates(start,end) {
	  ...
	  if (difference <= 0) {
		...
		var request_url = "<?php echo base_url('ld_programs/submit_exam'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
			{
				window.location.href = "<?php echo base_url('ld_programs/my_examination'); ?>";
			}
			else
			{
				alert('Unable to Submit training. Server Error!');
			}
...
<script>
	$(document).ready(function()
	{
		...
		var request_url = "<?php echo base_url('ld_programs/get_first_question'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('click','#next',function()
	{
		...
		var request_url = "<?php echo base_url('ld_programs/submit_question'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
			{
				...
				var request_url = "<?php echo base_url('ld_programs/get_question'); ?>";
				process_ajax(function(response)
				{
					var res = JSON.parse(response);
					if(res.stat == true)
...
<script>
	$(document).on('click','#question_no_container [data-question_id]',function()
	{
		...
		var exam_schedule_id = $('#question_container').attr('data-exam_schedule_id');
		var datas = {'question_id':question_id,'exam_schedule_id':exam_schedule_id};
		var request_url = "<?php echo base_url('training/get_question'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
