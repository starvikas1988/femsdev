...
var oldRegimeValue = "<?php echo  ( $it_popup_show['old_regime_value']!='')?$it_popup_show['old_regime_value']:0; ?>";
...
var configDateTime = new Date("<?php echo  $it_popup_conf['to_date_time']; ?>");
configDateTime.setHours(23, 59, 59, 0);

var effectedDate = new Date(effectedDateParts[0], effectedDateParts[1] - 1, effectedDateParts[2]);
//alert(currentDate+'******'+configDateTime+'*********'+fromDateTime+'*****'+today);
//alert(configDatediff);
//alert(effectedDate);
...
//alert (isIncrementPromotion);
 ...
 /*if (currentDate - dojDate < 15 * 24 * 60 * 60 * 1000 && grossPay >= oldRegimeValue && isItDeclaration == "0" && it_declaration_is_skip_today == "0" && is_submit=="0") {
  ...
  diffInDays = parseInt(diffInDays)+2;
  //alert(diffInDays);
  if(diffInDays<=15 && grossPay >= oldRegimeValue && isItDeclaration == "0" && it_declaration_is_skip_today == "0" && is_submit=="0"){
	$('#it_declaration_popup').modal('show');
  }
 
  if(diffInDays>=16 && diffInDays<17 && configDateTime < today &&grossPay >= oldRegimeValue && isItDeclaration == "0" && it_declaration_is_skip_today == "0" && is_submit=="0"){
	$('#itdmodelnotification').modal('show');
  }
   //alert(currentDate+'***************'+configDateTime);
  ...
  if (diffInDays >= 17 && fromDateTime <= today && toDateTime >= today && configDateTime >= today && grossPay >= oldRegimeValue && isItDeclaration == "0" && it_declaration_is_skip_today == "0" && is_submit=="0") {
	$('#it_declaration_popup').modal('show');
  }
  //alert(configDateTime);
  var configDateTime1 = new Date(configDateTime);
  var configDateTime2 = new Date(configDateTime);
  configDateTime1.setDate(configDateTime.getDate() + 1);
  configDateTime2.setDate(configDateTime.getDate() + 1);
  configDateTime2.setHours(0, 0, 0, 0);
	//alert(configDateTime1);
  if(configDateTime1>=currentDate && currentDate>=configDateTime2 && grossPay >= oldRegimeValue && isItDeclaration == "0" && it_declaration_is_skip_today == "0" && is_submit=="0") {
	$('#itdmodelnotification').modal('show');
  }
  ...
  var sixteenDaysAfterEffected = new Date(effectedDate);
  sixteenDaysAfterEffected.setDate(sixteenDaysAfterEffected.getDate() + 16);
  if (currentDate >= effectedDate && currentDate > fifteenDaysAfterEffected && currentDate<sixteenDaysAfterEffected){
    $('#itdmodelnotification').modal('show');
  }
