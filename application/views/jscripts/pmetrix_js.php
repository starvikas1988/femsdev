...
$(document).ready(function()
{
		...
		var uUrl=baseURL+'pmetrix/upload';
	...
	$(".onProcessAction").change(function(){
		...
		var URL=baseURL+'pmetrix/get_matrix_week';
		$.ajax({
		   type: 'POST',    
		   url:URL,
		   ...
		   success: function(msg){
				...
				//console.log(data.length);
					if(data.length > 0)
					{
						...
						{
							//console.log(element);
							option += '<option value="'+element.start_date+'#'+element.end_date+'" data-mdid="'+element.mdid+'">'+element.shrange+'</option>';
						});
						option += "</select>";
						//console.log(option);
	...
	$(".editPMButton").click(function(){
		...
		var URL=baseURL+'pmetrix/getDesignForm?mdid='+mdid;
		//alert(URL);
...
<script>
	$(document).on('change','#foffice_id',function()
	{
		...
		$.ajax({
		   type: 'POST',    
		   url:"<?php echo base_url('pmetrix/get_clients');?>",
