...
function comcast_calc(){
		var score = 0;
		var scoreable = 0;
		var quality_score_percent = 0;
		
		$('.comcast_points').each(function(index,element){
			var score_type = $(element).val();
			if(score_type == 'Yes'){
				var weightage = parseFloat($(element).children("option:selected").attr('comcast_val'));
		...
		$('#comcast_earnedScore').val(score);
		$('#comcast_possibleScore').val(scoreable);

		if($('#comcast_AF1').val()=='Yes'){
		$('#comcast_overallScore').val(0);
		}else{
			if(!isNaN(quality_score_percent)){
				$('#comcast_overallScore').val(quality_score_percent+'%');
			}	
		}
	
	}
	
	$(document).on('change','.comcast_points',function(){
		comcast_calc();
	});
	comcast_calc();
