...
$(document).ready(function() {
	...
	//$("#app-navbar").css('pointer-events', 'none');
	...
	?>
			var start = new Date();
			var end   = new Date();
			var allotted_time = "<?php echo $exam_info->allotted_time; ?>";			
			
			end.setMinutes(start.getMinutes() + allotted_time + 1);
	<?php
		}
		else
		{
	...
	?>
			//var start = new Date('<?php echo $exam_info->exam_start_time; ?>');
			var start = new Date('<?php echo GetLocalTime(); ?>');	
			//alert(start);
	...
	function timeBetweenDates(start,end) {
	  ...
	  if (difference <= 0) {
		...
		var request_url = "<?php echo base_url('kat/submit_exam'); ?>";
		process_ajax(function(response)
		{
			...
			if(res.stat == true)
			{
				...
				window.location.href = "<?php echo base_url('kat'); ?>";
			}
			else
			{
				//alert('Unable to Submit kat. Server Error!');
					swal({
				title: "Unable to Submit kat. Server Error!",
				text: "",
				icon: "warning",
				// buttons: true,
				// showCancelButton: false,
		...
		$("#count_down").text(days+':'+hours+':'+minutes+':'+seconds);
	  }
	}
...
<script>
	$(document).ready(function()
	{
		...
		var request_url = "<?php echo base_url('kat/get_first_question'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('click','#next',function()
	{
		...
		var request_url = "<?php echo base_url('kat/submit_question'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
			{
				...
				var request_url = "<?php echo base_url('kat/get_question'); ?>";
				process_ajax(function(response)
				{
					var res = JSON.parse(response);
					if(res.stat == true)
