		   ...
		   data:'pid='+processID +'&oid='+officeID,
			success: function(data){
			  var a = JSON.parse(data);
				...
				$("#select_agent").append('<option value="ALL">-- All Agent --</option>');		  
				$("#select_tl").append('<option value="ALL">-- All TL --</option>');		  
				$("#select_manager").append('<option value="ALL">--- Select Manager ---</option>');		  
				$.each(a, function(index,jsonObject){
...
// TENURE 1
//========================================================================
var ctxBAR = document.getElementById("agent_2dpie_tenure1_count");
var myBarChart = new Chart(ctxBAR, {
	...
	data: {
	  ...
	  datasets: [
		{
		  data: [<?php echo $overview['tenure_1']; ?>,100],
	...
	options: {
	  ...
	  elements: {
		center: {
		  text: "<?php echo $overview['tenure_1_score']; ?>%",
...
// TENURE 2
//========================================================================
var ctxBAR = document.getElementById("agent_2dpie_tenure2_count");
var myBarChart = new Chart(ctxBAR, {
	type: 'doughnut',
	data: {
	  ...
	  datasets: [
		{
		  data: [<?php echo $overview['tenure_2']; ?>,100],
		  backgroundColor: ["#4d1e62","#dbd2e0"],
		  borderColor: ["#4d1e62","#dbd2e0"],
		  borderWidth: 1
		}
	  ]
	},
	...
	options: {
	  ...
	  elements: {
		center: {
		  text: "<?php echo $overview['tenure_2_score']; ?>%",
		  color: '#000000', // Default is #000000
		  fontStyle: 'Arial', // Default is Arial
		  sidePadding: 80, // Default is 20 (as a percentage)
		  minFontSize: 16, // Default is 20 (in px), set to false and text will not wrap.
...
// TENURE 3
//========================================================================
var ctxBAR = document.getElementById("agent_2dpie_tenure3_count");
var myBarChart = new Chart(ctxBAR, {
	type: 'doughnut',
	data: {
	  ...
	  datasets: [
		{
		  data: [<?php echo $overview['tenure_3']; ?>,100],
	...
	options: {
	  ...
	  elements: {
		center: {
		  text: "<?php echo $overview['tenure_3_score']; ?>%",
		  color: '#000000', // Default is #000000
		  fontStyle: 'Arial', // Default is Arial
		  sidePadding: 80, // Default is 20 (as a percentage)
		  minFontSize: 16, // Default is 20 (in px), set to false and text will not wrap.
...
//=================== CQ DEFECT GRAPH ===============================//
var ctxBARDefect = document.getElementById("myDefectScore");
var myBarChartDefect = new Chart(ctxBARDefect, {
	type: 'bar',
	data: {
	  labels: [<?php $sl=0; foreach($cq['params']['count'] as $key => $val){ $sl++; if($sl > 5){ break; } echo '"'.$key; if($sl < 5){ echo '",'; } else { echo '"'; } } ?>],
	  datasets: [
		{
		  label: "CQ Score",
		  data: [<?php $sl=0; foreach($cq['params']['percent'] as $key => $val){ $sl++; if($sl > 5){ break; } echo $val; if($sl < 5){ echo ','; } else { echo ''; } } ?>],
	...
	options: {
	  ...
	  title: {
		display: true,
		lineHeight: 5,
		text: "Top 5 Parameter-wise Defect Percentage"
	  },
	  tooltips: {
		callbacks: {
