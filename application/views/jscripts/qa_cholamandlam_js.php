<script type="text/javascript">
	...
	function smileimpress_calc(){
		var score = 0;
		var scoreable = 0;
		var quality_score_percent = 0;
		
		$('.smileimpress_points').each(function(index,element){
			var score_type = $(element).val();
			if(score_type == 'Yes'){
				var weightage = parseFloat($(element).children("option:selected").attr('smileimpress_val'));
				score = score + weightage;
				scoreable = scoreable + weightage;
			}else if(score_type == 'Fatal'){
				var weightage = parseFloat($(element).children("option:selected").attr('smileimpress_val'));
		...
		$('#smileimpress_earnedScore').val(score);
		$('#smileimpress_possibleScore').val(scoreable);

		if(!isNaN(quality_score_percent)){
				$('#smileimpress_overallScore').val(quality_score_percent+'%');
					}
	}
	
	$(document).on('change','.smileimpress_points',function(){
		smileimpress_calc();
	});
	smileimpress_calc();
</script>
