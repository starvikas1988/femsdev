	...
	$(document).on('change','#view_type',function(e)
	{
		...
		else
		{
			...
			process_ajax(function(response)
			{
				...
				if(res.stat == true)
				{
					var option = '<option value="">--Select--</option><option value="1" data-user_id="0" selected>All Agent</option>';
...
<script>
	$(document).on('submit','#get_agent_list_form',function(e)
	{
		...
		var others_team = $('#others_team').val();
		var view_type = $('#view_type').val();
		var datas = {'process_id':process_id,'fusion_id':fusion_id,'performance_for_month':performance_for_month,'performance_for_year':performance_for_year,'user_id':user_id,'others_team':others_team,'view_type':view_type,'client_id':$('#fclient_id').val(),'office_id':$('#foffice_id').val()};
		if(fusion_id == 1)
		{
		...
		process_ajax(function(response)
		{
			...
			if($('#fprocess_id').val() == 30)
			{
				...
				$('#pd_cheque_amt').text(find_max('.FutChkAmt'));
				$('#pd_cheque_fees').text(find_max('.FutChkFee'));
				$('#pd_card_amt').text(find_max('.FutCardAmt'));
				$('#pd_card_fees').text(find_max('.FutCardFee'));
				
				$('#AcctWorked').text(sum('.AcctWorked'));
			}
		},request_url, datas, 'text');
	});
...
<script>
	$("#fclient_id").change(function(){
		...
		$.when(populate_process_combo(client_id,'<?php echo $pValue; ?>','fprocess_id','Y')).done(function()
		{
			$('#view_type').val('').trigger('change');
			//alert();
			$('#fprocess_id').val('<?php echo $pValue; ?>').trigger('change');
			/* if('<?php echo $rValue ?>'!='tl')
			{
				$('#view_type').val('2').trigger('change');
			}
			else
			{
				$('#view_type').val('1').trigger('change');
			} */
		});
