...
function forward_email_id() {
	...
	if(forward_email_id.length == ""){
		...
		$("#msg-forward_email_id").html("<font color=red> Please Enter some Email ID Text  </font>");
		$("#mailforwardsubmission").attr("disabled",true);
		$("#mailforwardsubmission").css('cursor', 'no-drop');
	}else if(!pattern.test(forward_email_id)){
		
		$("#msg-forward_email_id").html("<font color=red>This Email ID  is not valid </font>");
		 $("#mailforwardsubmission").attr("disabled",true);
		 $("#mailforwardsubmission").css('cursor', 'no-drop');
		//validatesubject();
	}else{
...
function mailing_cc_for_forward_mail() {
	...
	if(mailing_cc_for_forward_mail.length == ""){
		...
		$("#msg-mailing_cc_for_forward_mail").html("<font color=red> Please Enter some CC Email Id Text  </font>");
		$("#mailforwardsubmission").attr("disabled",true);
		$("#mailforwardsubmission").css('cursor', 'no-drop');
	}else if(!pattern.test(mailing_cc_for_forward_mail)){
		
		$("#msg-mailing_cc_for_forward_mail").html("<font color=red>This CC Email Id is not valid </font>");
		 $("#mailforwardsubmission").attr("disabled",true);
		 $("#mailforwardsubmission").css('cursor', 'no-drop');
		//validatesubject();
	}else{
...
function validatemailing_subject(mailing_subject) {
	...
	if(mailing_subject.length == ""){
		$("#msg-mailing_subject").html("<font color=red> Please Enter some Text  </font>");
		$(".btn_widget").attr("disabled",true);
		$(".btn_widget").css('cursor', 'no-drop');
	}else if(!pattern.test(mailing_subject)){
		//var pattern ='/^[a-z\d\-_\s]+$/i';
		$("#msg-mailing_subject").html("<font color=red>This subject is not valid </font>");
		$(".btn_widget").attr("disabled",true);
		$(".btn_widget").css('cursor', 'no-drop');
	}else{
...
function mytextarea_newws(mytextarea_newws) {
	
    var mytextarea_newws=$("#mytextarea_newws").val();
	//var pattern = /^[A-Za-z0-9]+$/;
	//var matches = subject.match('/[^a-zA-Z0-9]/');
	//alert(mytextarea_newws);
	var pattern =/\w+|\.\s|[!"#€%&/()=?`´^¨*'-_;:.,]/g; // search 
	if(mytextarea_newws.length == ""){
		$("#msg-mytextarea_neww").html("<font color=red> Please Enter some Text  </font>");
		$("#mailNoteSubmission").attr("disabled",true);
		$("#mailNoteSubmission").css('cursor', 'no-drop');
	}else if(!pattern.test(mytextarea_newws)){
		//var pattern ='/^[a-z\d\-_\s]+$/i';
		$("#msg-mytextarea_neww").html("<font color=red>This subject is not valid </font>");
		$("#mailNoteSubmission").attr("disabled",true);
		$("#mailNoteSubmission").css('cursor', 'no-drop');
	}else{
