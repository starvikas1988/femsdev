...
$(document).ready(function(){	
	...
	$('.sent_button_required').click(function(){
		//alert(22);
		ticket_no = $(this).attr('data-ticket_no');
		// ticket_no = $('.reply_form input[name="ticket_no"]').val();
		// alert(ticket_no);
		$("#ticket_no_hidden").val(ticket_no);
	...
	sent_approval_submit=()=>{
	    ticket_no = $('#modalsentapproval #ticket_no_hidden').val();
		...
		if (email != "") {  
			...
			$.ajax({
				...
				success: function(token) {
					...
					notReply = 1;
					$('.loader_area').modal('hide');
					alert(link_generated);
					$("#modalsentapproval").modal('hide');
			...
			alert('Please Fill Up the Email ID!');
		}
	}
	...
	$('.dropdown').on('change', 'select[name="ticket_list_dropdown"]', function(){
		pageType = $(this).val();
		emailID = "<?php echo !empty($email_id) ? bin2hex($email_id) : ''; ?>";
	...
	// $('#start_date').datepicker({ dateFormat:'mm/dd/yy', changeMonth: true, changeYear: true, yearRange: '2021:' + new Date().getFullYear().toString(), max: maxDate });
	// $('#end_date').datepicker({ dateFormat:'mm/dd/yy', changeMonth: true, changeYear: true, yearRange: '2021:' + new Date().getFullYear().toString(), max: maxDate });

	// $("#attachments_work").change(function() {
	...
	$('.submit_btn_for_portal').click(function(){
		baseURL = "<?php echo base_url(); ?>";
		 ...
		 let side_bar_view='all';
		if (start_date > end_date){
			alert("From Date can not be greater than To Date");
		}else{
			$.ajax({
				url: "<?php echo base_url('legal_manual/ticket_portal_search'); ?>",
				type: "POST",
				...
				success : function(json_obj){
					...
					for(var i in alrt.queryTicket){
						var bookmarkClr = '';
						var diff = '';
						var date_added = new Date(alrt.queryTicket[i].date_added);
						var current_date = new Date('<?php echo CurrMySqlDate();?>');
						var millisBetween = current_date.getTime() - date_added.getTime();
						...
						if(alrt.queryTicket[i].is_reply_mail == 1){
							var checkedTicket="<i class='fa fa-check green' aria-hidden='true'></i>";
						}else{
							var checkedTicket="<i class='fa fa-bookmark "+bookmarkClr+"' aria-hidden='true'></i>";
						}
						if(alrt.queryTicket[i].fullname!=null){
							var assigned_to=alrt.queryTicket[i].fullname+` (`+alrt.queryTicket[i].fusion_id+`)`;
						}
						else{
							var assigned_to="<span class='red'>Unassigned</span>";
							
						}
						html+=`<tr >
								<td>
									<div class="book_mark">`+checkedTicket+`</div>
									<span><b>`+alrt.queryTicket[i].ticket_no+`</b></span> : `+alrt.queryTicket[i].ticket_subject+`
								</td>`;
								html+=`<td>`+alrt.queryTicket[i].ticket_category+`</td>
								<td>`+assigned_to+`</td>`;
								if(alrt.queryTicket[i].total_time!=null){
									var tot_time=`<span class="green">`+alrt.queryTicket[i].total_time+`</span>`;
								}
								else{
									var tot_time='<b>n/a</b>';
...
// this is for validation purpose by sougata start 
function search_subjectFunction(search_subject)
{
var httpxml;
try
{
...
if(httpxml.responseText!=1){
    $(".ticket_finder").attr("disabled",true);
    $(".ticket_finder").css('cursor', 'no-drop');
    document.getElementById("msg-search_subject").innerHTML=httpxml.responseText;
    //document.getElementById("msg-references_no").innerHTML=httpxml.responseText;
 }
 else{
    $("#msg-search_subject").html("");
    $(".ticket_finder").attr("disabled",false);
    $(".ticket_finder").css('cursor', 'pointer');
...
var url="<?php echo base_url('legal_executive/search_subject'); ?>";

//url=url+"?to_date="+to_date+"?from_date="+from_date;

url=url+"?search_subject="+search_subject;
...
// this is for validation purpose by sougata end 
</script>
...
<script type="text/javascript">
// this is for validation purpose by sougata start 
function email_fromFunction(email_from)
{
var httpxml;
try
{
...
if(httpxml.readyState==4)
{
document.getElementById("msg-email_from").innerHTML=httpxml.responseText;
}
}
var url="<?php echo base_url('legal_executive/email_from'); ?>";

//url=url+"?to_date="+to_date+"?from_date="+from_date;

url=url+"?email_from="+email_from;
//url=url+"?from_date="+from_date;
...
<script type="text/javascript">
// this is for validation purpose by sougata start 
function ticket_idFunction(ticket_id)
{
var httpxml;
try
{
...
if(httpxml.readyState==4)
{
//document.getElementById("msg-ticket_id").innerHTML=httpxml.responseText;



if(httpxml.responseText!=1){

    $("#submit_btn_for_assigned btn btn-primary").attr("disabled",true);
    $("#submit_btn_for_assigned btn btn-primary").css('cursor', 'no-drop');

    document.getElementById("msg-ticket_id").innerHTML=httpxml.responseText;
 }
 else{
    $("#msg-ticket_id").html("");
    $("#submit_btn_for_assigned btn btn-primary").attr("disabled",false);
    $("#submit_btn_for_assigned btn btn-primary").css('cursor', 'pointer');
     
    
 }
}
}
var url="<?php echo base_url('legal_executive/ticket_id'); ?>";
...
<script type="text/javascript">
// this is for validation purpose by sougata start 
// this portion is correctly functioning 
function references_noFunction(references_no)
{
var httpxml;
try
{
...
var url="<?php echo base_url('legal_executive/references_no'); ?>";

//url=url+"?to_date="+to_date+"?from_date="+from_date;

url=url+"?references_no="+references_no;
//url=url+"?from_date="+from_date;
...
<script type="text/javascript">
// this is for validation purpose by sougata start 
function remarksRFunction(remarks)
{
var httpxml;
try
{
...
if(httpxml.responseText!=1){
    ...
    $(".add_remarks").css('cursor', 'no-drop');
    //document.getElementById("msg-remarksR").innerHTML=httpxml.responseText;
    document.getElementById("msg-remarksR").innerHTML=httpxml.responseText;
 }
 else{
    $("#msg-remarksR").html("");
...
var url="<?php echo base_url('legal_executive/remarks'); ?>";
...
<script type="text/javascript">
// this is for validation purpose by sougata start 
function remarksFunction(remarks)
{
var httpxml;
try
{
...
if(httpxml.responseText!=1){
    ...
    $(".add_remarks").css('cursor', 'no-drop');
    //document.getElementById("msg-remarksR").innerHTML=httpxml.responseText;
    document.getElementById("msg-remarks").innerHTML=httpxml.responseText;
 }
 else{
    $("#msg-remarks").html("");
    $(".add_remarks").attr("disabled",false);
...
<script type="text/javascript">
// this is sfor validation of legal/ticket_raise validation of subject 
...
if(httpxml.responseText!=1){
   $(".ticket_raise").attr("disabled",true);
   $(".ticket_raise").css('cursor', 'no-drop');
    document.getElementById("msg-remarksR").innerHTML=httpxml.responseText;
    document.getElementById("msg-subject").innerHTML=httpxml.responseText;
 }
 else{
   $("#msg-subject").html("");
   $(".ticket_raise").attr("disabled",false);
    $(".ticket_raise").css('cursor', 'pointer');
...
var url="<?php echo base_url('legal_executive/subject'); ?>";
...
function validatesubject(subject) {
	...
	//alert(matches);
	if(subject.length == ""){
		$("#msg-subject").html("<font color=red style='font-size:14px;'> Please Enter some Text  </font>");
		$(".ticket_raise").attr("disabled",true);
		$(".ticket_raise").css('cursor', 'no-drop');
	}else if(!pattern.test(subject)){
		//var pattern ='/^[a-z\d\-_\s]+$/i';
		$("#msg-subject").html("<font color=red style='font-size:14px;'>This subject is not valid </font>");
		$(".ticket_raise").attr("disabled",true);
		$(".ticket_raise").css('cursor', 'no-drop');
	}else{
		$("#msg-subject").html("");
