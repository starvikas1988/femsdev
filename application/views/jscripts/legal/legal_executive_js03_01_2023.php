...
$(document).ready(function(){	
	...
	$('#status_flag_approve').click(function(){
	
		$("#modalstatus_flag_approve").modal('show'); 

	});
	$('#status_flag_reject').click(function(){
	
	$("#modalstatus_flag_reject").modal('show'); 
...
// This code shall working for view->legal->administrative_approcal.php page 


administrative_approval_submit_by_admin=(val)=>{
	
		if(val=='accept'){
			
			type=val;
			remarks = $('#modalstatus_flag_approve #remarks').val();
			
		}
		else{
			type=val;
			//alert(type);
			remarks = $('#modalstatus_flag_reject #remarks').val();
...
//alert(selected);
			...
			$.ajax({
				url: "<?php echo base_url() ?>legal_executive/administrative_status",
				type: "POST",
				data: {
					remarks:remarks,
					status_flag:val,	
				...
				//	alert(token);
					if(token==1){
						location.href = "<?php echo base_url()?>legal_executive/show_request";
					}
					else{
						alert('Please Recify Error ');
					}
...
// This code shall working for view->legal->administrative_approcal.php page
	...
	category_wise_agent =(val) =>{
		// alert(val);
		$('.loader_area').show();
		$('.loader_area').addClass('loader_center');
		$('.loader_area').html("<img src='<?php echo base_url()."assets/images/loader.png"; ?>' >");
		...
		$.ajax({
			...
			success : function(json_obj){
				...
				// $(".onload_tr_value").hide();
				$('.loader_area').hide();
				$('.loader_area').removeClass('loader_center');
