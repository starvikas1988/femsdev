...
<script>
	function add_comment(id)
	{
		// console.log(id);
		$('#online_accept').hide();
		var pay_period = $('#pay_prd'+id).val();
		var fusion_id = $('#fem_id'+id).val();
		var post_period = $('#post_prd'+id).val();
		var datas = {'pay_period':pay_period,'fusion_id':fusion_id,'post_period':post_period};
		var request_url = "<?php echo base_url('pmetrix_v2/get_bouns_info'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			// console.log(res);
			if(!jQuery.isEmptyObject(res.datas[0]))
			{
				...
				$('#supervisor_comment').attr('readonly','readonly');
				// console.log(fusion_id);
				$('#vrs_bonous_form').modal('show');
			}
			else{
				alert('No Data Found');
...
<script>
	$(document).on('click','#pay_period_1sup_btn,#pay_period_2sup_btn',function()
	{
		$('#online_accept').hide();
		var pay_period = $(this).val();
		var fusion_id = $('#final_summary #fusion_id').text();
		var post_period = $('#post_period').val();
		var datas = {'pay_period':pay_period,'fusion_id':fusion_id,'post_period':post_period};
		var request_url = "<?php echo base_url('Pmetrix_v2_tl/get_bouns_info'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			// console.log(res);
			if(!jQuery.isEmptyObject(res.datas[0]))
			{
				...
				$('#om_comment').attr('readonly','readonly');
				$('#vrs_bonous_form').modal('show');
			}
			else{
				alert('No Data Found');
...
<script>
	$(document).on('click','#online_accept',function()
	{
		...
		var sum = $('#pay_period').text();
		var pay = sum.replace ( /[^\d.]/g, '' );
		var pay_period = pay / 1;
		...
		datas.push({name: 'pay_period', value: pay_period});
		
		var request_url = "<?php echo base_url('Pmetrix_v2_tl/process_bonus'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
...
<script>
	$(document).on('click','#submit_comment',function()
	{
		...
		var pay = sum.replace ( /[^\d.]/g, '' );
		var pay_period = pay / 1;
		var sum3 = $('#fems_id').text();
		var fusion_id = sum3.replace ("undefined", '' );
		var pay_period = pay / 1;
		var sum2 = $('#post_period').text();
		var post = sum2.replace ( /[^\d.]/g, '' );
		var post_period = post / 1;
		var datas = $('#vrs_bonous_form_form').serializeArray();
		datas.push({name: 'fusion_id', value: fusion_id});
		datas.push({name: 'post_period', value: post_period});
		datas.push({name: 'pay_period', value: pay_period});
		
		var request_url = "<?php echo base_url('pmetrix_v2/process_bonus'); ?>";
		process_ajax(function(response)
		{
			var res = JSON.parse(response);
			if(res.stat == true)
