...
//$('.model_add_client_id').on('change', function() {
    ...
    let selected_2 = $(selected_elemnt).closest('td').next('td').next('td').find('select:first');

    $(selected_2).find('option').prop('selected', false);

    let request_url = "<?=base_url('allocation/get_client_process_list')?>";
    process_ajax(function(response)
    {   
        var res = JSON.parse(response);
        if (res.stat == true) 
        {   
            $.each(res.datas,function(index,element) { process_ids += '<option value="'+element.id+'">'+element.name+'</option>'; });

            selected.html(process_ids);
        }
        else { selected.html(process_ids); }
...
$(document).on('change','.model_add_cost_location',function(){
    ...
    let check_deiplicate_arr = [];
    let check_deiplicate = false;

    for (let i = 0; i < selected_cost_location.length; i++) {

        if (check_deiplicate_arr.includes(selected_cost_location[i]+','+selected_process_id[i])) {
            check_deiplicate = true;
            break;
        }
        check_deiplicate_arr.push(selected_cost_location[i]+','+selected_process_id[i]);
    }
    ...
    if (check_required_felds) {
        ...
        if (check_deiplicate) {
            this.value='';
            $("#add_btn_error").text("**Same Process can not be added within same Cost Location.");
        }
        else {
            ...
            let datas = {
                ...
                "month_name": month_name, 
                "type": type,
                "selected_cost_location_id": selected_cost_location,
                "selected_process_id": selected_process_id
            };
            ...
            {   
                ...
                if (res.stat == true)
                {
                    if(res.check_duplicate == true) {
                         $(selected).find('option').prop('selected', false);
                        $("#add_btn_error").text("**Same Process can not be added within same Cost Location.");
                    }
                    else {

                        let index = $(selected).closest('tr').index();
    ...
    else { this.value=''; $("#add_btn_error").text("**Please select Client, Process & Cost Location"); }
