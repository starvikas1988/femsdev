...
function indivitual_data_transfer_cogs(user_id,name){
            $(".after_success").hide();
            $(".modal-footer").show();
            $(".before_success").show();          
            $('#myModal').modal('show');
            $('#emp_name').text(name);
            $('#emp_name_st').text(name);
            // Iterate through all checkboxes with the "checkbox" class
            var checkedValues = [];            
            checkedValues.push(user_id); // Push the data into the array
            
            $("#confimationclickcogs").click(function () {
                
                $(".before_success").hide();                
                $.post("<?php echo base_url('allocation/insert_transfer_for_cogs_non_cogs'); ?>", {
                    user_ids: checkedValues
                }).done(function (res) {      
                                            
                    $(".after_success").show();                 
                        if(res != ''){
                            get_employee_data_transfer();
                            checkedValues = [];
                        } 
                });              
                $(".modal-footer").hide();
...
function get_employee_data_transfer(){
        ...
        if(fusion_id != ''){
            ...
            $.post("<?php echo base_url('allocation/check_fusion_id_for_cogs_non_cogs'); ?>", {
                fusion_id: fusion_id
                }).done(function (res) {
                    $('#sktPleaseWait').modal('hide');
...
function clear_row_cogs(remove_id){        
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('allocation/clear_cogs_non_cogs_indiviual_row'); ?>",
            data: { input_string: fusion_id, remove_id: remove_id },
            success: function(response) { 
                $('#sktPleaseWait').modal('hide');
                if(response.trim() != 0){                       
    ...
    function multiple_data_transfer_cogs(){
            $(".after_success").hide();
            $(".modal-footer").show();
            $(".before_success").show();          
            $('#myModal').modal('show');
