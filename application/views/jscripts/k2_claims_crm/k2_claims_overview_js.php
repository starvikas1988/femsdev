...
var myLineChart = new Chart(ctxPIE, {
  ...
  data: {
    labels: [
	<?php $ij=0; foreach($tableArray as $token){ $ij++; echo  "'" .$token['name'] ." (".$today[$token['table']].")"; echo $ij< count($tableArray)?"'," : "'"; } ?>
	],
    datasets: [
	 { 
        data: [
		<?php $ij=0; foreach($tableArray as $token){ $ij++; echo  $today[$token['table']]; echo $ij< count($tableArray)? "," : ""; } ?>
		],
