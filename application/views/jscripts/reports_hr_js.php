...
$('#start_dates, #end_dates').datepicker({
	changeMonth: true,
	changeYear: true,
...
<script>
	$(document).ready(function(){
		...
		/*$("#fclient_id").on('change',function() {
			...
			var URL='<?php echo base_url();?>reports_hr/getClientTitle';
			$('#sktPleaseWait').modal('show');
			$.ajax({
			   type: 'POST',    
			   url:URL,
		...
		*/
		...
		$("#fprocess_id").on('change',function() {
			var pid = this.value;
			var office_id = $('#foffice_id').val();
			var cid = $('#pfclient_id').val();
			//alert(cid);
			//alert(office_id);
			if(cid=="") alert("Please Select Client");
			if(pid=="") alert("Please Select Process");
			var URL='<?php echo base_url();?>reports_hr/getProcessTitle';
			$('#sktPleaseWait').modal('show');
			$.ajax({
			   type: 'POST',    
			   url:URL,
			   data:'pid='+pid+'&office_id='+office_id+'&cid='+cid,
			   success: function(cList){
				   //alert(pList);
					var json_obj = $.parseJSON(cList);//parse JSON
...
// FOR MASTER DATABASE REPORT - CAS
$('.masterDBDiv select[name="language"]').select2();
$('.masterDBDiv').on('change', 'select[name="office_id"]', function(){
	curVal = $(this).val();
	$('.masterDBDiv select[name="language"]').val('');
	$('.masterDBDiv select[name="exam"]').val('');
	$('.masterDBDiv select[name="cin"]').val('');
	$('.masterDBDiv select[name="expiry_date"]').val('');
	$('.masterDBDiv select[name="expiry_search"]').val('No');
	if(curVal == "CAS"){
		$('.masterDBDiv .languageDIV').show();
		$('.masterDBDiv .examDIV').show();		
		$('.masterDBDiv .cinDIV').show();		
		$('.masterDBDiv .expiryDIV').show();		
		$('.masterDBDiv .expirySearchDIV').show();		
	} else {
		$('.masterDBDiv .languageDIV').hide();
		$('.masterDBDiv .examDIV').hide();
		$('.masterDBDiv .cinDIV').hide();
		$('.masterDBDiv .expiryDIV').hide();
		$('.masterDBDiv .expirySearchDIV').hide();
	}
	$('.masterDBDiv select[name="language"]').select2();
});
