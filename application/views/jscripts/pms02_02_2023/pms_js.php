...
$('.editStatus').click(function(){
	...
	get_start_end_month=(val)=>{
		...
		$.ajax({
						...
						$("#start_month").val(a.start_month);
						$("#end_month").val(a.end_month);
						$("#start_month").prop('readonly',false);
						$("#start_month").prop('required',true);
						$("#end_month").prop('readonly',false);
	...
	baseURL = "<?php echo base_url(); ?>";
				...
				// if(on_off=='OFF'){
				// var text = "Do you confirm to ON this";

				// }
				// else{
				// var text = "Do you confirm to OFF this";

				// }	


						if (swal_confirm(text) == true) {
							//text = "You pressed OK!";
				...
				// alert(baseURL);
					$.ajax({
						...
						success: function(token) {
							...
							if(token==1){
								if(val=='pms_com_kra'){
									alert('Competency/KRA category count successfully updated');
								}
								else{
							alert('Status Changed Successfully Done');
								}
      						get_config_set_list();
 ...
 function swal_confirm(title_val){
	...
	Swal.fire({ title: title_val, text: "", icon: 'info', showCancelButton: true, confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Confirm' }).then((result) => { if (result.isConfirmed) { 
