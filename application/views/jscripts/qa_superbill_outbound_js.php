...
////////////////vikas//////////////////////////////
 ...
 <script type="text/javascript">
		...
		let na_count_park = 0;
		let score_superbill_ob_final = 0;
		let scoreable_superbill_ob_final = 0;

		$('.superbill_ob_points').each(function(index,element){
			let score_type_park = $(element).val();
			
			if(score_type_park == 'Pass'){
				pass_count_park = pass_count_park + 1;
				let w1_park = parseFloat($(element).children("option:selected").attr('superbill_ob_val'));
				let w2_park = parseFloat($(element).children("option:selected").attr('superbill_ob_max'));
      ...
      score_superbill_ob_final     = (score_park).toFixed(2);
      scoreable_superbill_ob_final = (scoreable_park).toFixed(2);

		$('#superbill_ob_earned_score').val(score_superbill_ob_final);
		$('#superbill_ob_possible_score').val(scoreable_superbill_ob_final);
		
		if(!isNaN(quality_score_percent_park)){
			$('#superbill_ob_overall_score').val(quality_score_percent_park+'%');
		}

		if($('#superbillAF1').val()=='Fail' || $('#superbillAF2').val()=='Fail' || $('#superbillAF3').val()=='Fail'){
			console.log($('#superbillAF1').val());

			quality_score_percent_park = (0.00).toFixed(2);
			$('.superFatal').val(quality_score_percent_park+'%');
		}else{
			$('#superbill_ob_overall_score').val(quality_score_percent_park+'%');
		}
	}
	
	$(document).on('change','.superbill_ob_points',function(){
		superbill_ob_calc();
	});
	superbill_ob_calc();
</script>
...
//Edited By Samrat 30/9/21
	$(document).ready(function(){
		...
		$("#call_duration").timepicker({timeFormat : 'HH:mm:ss' });
		<?php if($this->uri->segment(2)=="Qa_superbill_outbound" && $this->uri->segment(4)=="0"){?>
			//console.log("okk");
