...
//                }
//            });
//            $(".datepicker5").datepicker($.extend({}, datepickersOpt));
//            
...
//////
        ...
        $('#marital_status').on('change', function () {
            ...
            if (this.value === "Unmarried") {
                $('.conditional_marital_status').removeClass('fa fa-asterisk');
            } else {
                $('.conditional_marital_status').addClass('fa fa-asterisk');
            }
        ...
        // $(document).on('click', '#pan_no', function () {
            $(document).on('focusout', '#pan_no', function () {
                ...
                if (isIndiaLocation(location) == true){
                    ...
                    if(location == "JAM"){
                        if (!(pan_no.match(numregEx))) {
                            alert("Invalid TRN No.");
                            $('#pan_no').val('');
                            $('#pan_no').css('border-color', 'red');
                            return false;
                        }
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo base_url(); ?>profile/validate_existing_pan',
                    data: 'pan_no=' + pan_no,
        ...
        // $(document).on('click', '#email_id_per', function () {
        $(document).on('focusout', '#email_id_per', function ()
        {
            $('#email_id_per').css('border-color', '#ddd');
            var email = $(this).val();
            if (email.length > 0)
            {
                ...
                if (!(email.toLowerCase().match(validRegex))) {
                    ...
                    $('#email_id_per').val('');
                    $('#email_id_per').attr('style', 'border-color: red !important');
                    return false;
                } else {
                    return true;
                }
        ...
        $(document).on('click', '#email_id_off', function () {
            ...
            {
                $('#email_id_off').css('border-color', '#ddd');
                var email = $(this).val();
                if (email.length > 0)
                {
                    var validRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@(fusionbposervices.com|vitalsolutions.net|xplore-tech.com|omindtech.com|travelomind.com|corp.fusionbposervices.com)$/;
                    if (!(email.toLowerCase().match(validRegex))) {
                        alert("Invalid email address");
                        $('#email_id_off').val('');
                        $('#email_id_off').attr('style', 'border-color: red !important');
                        return false;
                    } else {
...
<script>
    ...
    function check_size_five(id, chkFile = 'N') {
        dt = $(id).val();
        var size = id.files[0].size;
        if (size > 5242880) {
            alert('The File size should not be greater than 5 MB ');
            $(id).val('');
        }
        if (chkFile == 'Y') {
            var fileExtension = ['jpeg', 'jpg', 'png', 'pdf'];
            if ($.inArray($(id).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
                alert("We support only : JPEG, JPG, PNG, and PDF formats");
                $(id).val('');
            }
    }
