					...
					<div class="widget-body">
					 	<div class="table-responsive">
							<table class="table table-striped skt-table" width="100%">
								<tbody>
									<tr style="background-color:#114150"><td id="theader" style="color:white" colspan="6"><?php echo $sheet_name ?></td></tr>
									...
									if($process_id!=''){
										...
										<td <?php echo $td_visibility ?>><input type="<?php echo $h_detail['input_type'] ?>" class="form-control" id="<?php echo $h_detail['id_string'] ?>" name="data[<?php echo $h_detail['id_string'] ?>]" value="<?php echo $$field_val; ?>" <?php echo $required ?> readonly></td>
										<?php }else{ ?>
											<td <?php echo $td_visibility ?>><input type="<?php echo $h_detail['input_type'] ?>" class="form-control" id="<?php echo $h_detail['id_string'] ?>" value="<?php echo $$field_val; ?>" <?php echo $required ?> readonly></td>
											<?php } ?>
										<?php }elseif($h_detail['field_type']=='dropdown'){ 
											...
											<select class="form-control" id="<?php echo $h_detail['id_string'] ?>" name="data[<?php echo $h_detail['id_string'] ?>]" <?php echo $required ?> disabled>
										...
										foreach($parameter_details as $param_data){ 
											...
											<td>
												<select <?php echo $class ?> name="data[<?php echo $param_data['short_name'] ?>]" required disabled>
											...
											<td colspan=2><input type="text" class="form-control" name="data[<?php echo $comm_var ?>]" value="<?php echo $audit_data[$comm_var] ?>" readonly></td>
											<?php }elseif($param_data['comment_type']==1){ 
												//$sen_name = $param_data['short_name'].'_scenario';
												$sen_arr = explode(',', $param_data['scenario_data']);
												?>
											<td colspan=2>
											<select class="form-control" name="data[<?php echo $comm_var ?>]" disabled>
										...
										<tr>
											<td>Call Summary:</td>
											<td colspan=2><textarea class="form-control" name="data[call_summary]" disabled><?php echo $audit_data['call_summary'] ?></textarea></td>
											<td>Feedback:</td>
											<td colspan=2><textarea class="form-control" name="data[feedback]" disabled><?php echo $audit_data['feedback'] ?></textarea></td>
										</tr>
										...
										<tr style="background-color:#F5B7B1">
											...
											<?php if($audit_data['attach_img_file']!=''){ ?>
												<td colspan="4">
													<?php $attach_img_file = explode(",",$audit_data['attach_img_file']);
													 foreach($attach_img_file as $mp){ ?>
														<button class="btn btn-info"><a href="<?php echo base_url(); ?>qa_files/<?php echo $p_name;?>/images/<?php echo $mp; ?>"><?php echo $mp; ?></a></button>
													 <?php } ?>
												</td>
											<?php }else{
									...
									<?php 
										echo '<tr><td style="font-size:16px">Operation TL/Manager Review:</td> <td colspan="5" style="text-align:left">'.$audit_data['mgnt_rvw_note'].'</td></tr>';
										echo '<tr><td style="font-size:16px">QA Rebuttal:</td><td style="text-align:left">'.$audit_data['qa_rebuttal'].'</td><td style="font-size:16px">Rebuttal Comment:</td><td colspan="3" style="text-align:left">'.$audit_data['qa_rebuttal_comment'].'</td></tr>';
										echo '<tr><td style="font-size:16px">QA TL/Manager Rebuttal:</td><td style="text-align:left">'.$audit_data['qa_mgnt_rebuttal'].'</td><td style="font-size:16px">Rebuttal Comment:</td><td colspan="3" style="text-align:left">'.$audit_data['qa_mgnt_rebuttal_comment'].'</td></tr>';
									?>
									...
									<tr>
										<td colspan=2 style="font-size:16px">Agent Feedback Acceptance</td>
										<td colspan=2>
											<select class="form-control" name="agnt_fd_acpt" required>
												<option value="">-Select-</option>
												<option <?php echo $audit_data['agnt_fd_acpt']=='Accepted'?"selected":""; ?> value="Accepted">Accepted</option>
												<option <?php echo $audit_data['agnt_fd_acpt']=='Not Accepted'?"selected":""; ?> value="Not Accepted">Not Accepted</option>
											</select>
										</td>
									</tr>
									<tr>
										<td colspan=2 style="font-size:16px">Your Review</td>
										<td colspan=4><textarea class="form-control" name="agent_rvw_note" required><?php echo $audit_data['agent_rvw_note'] ?></textarea></td>
									</tr>
									
									<?php 
									if($audit_data['qa_rebuttal']!="" && $audit_data['qa_mgnt_rebuttal']==""){
										$entry_date=$audit_data['qa_rebuttal_date'];
										$acpt_time='48';
									}else if($audit_data['qa_mgnt_rebuttal']!=""){
										$entry_date=$audit_data['qa_mgnt_rebuttal_date'];
										$acpt_time='48';
									}else{
										$entry_date=$audit_data['entry_date'];
										$acpt_time='72';
									}
									...
									if(is_access_qa_agent_module()==true){
										...
										<tr>
											<?php if($audit_data['agnt_fd_acpt']!='Accepted'){ ?>
