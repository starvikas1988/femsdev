					...
					<div class="row">
						<div class="col-sm-4">
							<div class="form-group filter-widget">
								...
								<input name="start_date" type="text" value="<?=$start_date?>" class="form-control datepicker due_date-cal" id="start_date" readonly>
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group filter-widget">
								<label>End date </label>
								<input type="text" name="end_date" value="<?=$end_date?>" class="form-control datepicker due_date-cal" id="end_date" readonly>
							</div>
						</div>
						<div class="col-sm-4 form-group">
							<div class="new-select filter-widget">
								<label>Location</label>
								<select name="office_id[]" id="office_id" class="form-control multiselect_options" multiple>
						...
						<div class="col-sm-4 form-group">
							<div class="new-select filter-widget">
								...
								<select name="dept_search[]" id="dept_search" class="form-control multiselect_options" multiple>
									<?php foreach ($department_data as $key => $value) {
										$sel = "";
										if (in_array($value['id'],$dept_search)) $sel = "selected";												?>
						...
						<div class="col-sm-4 form-group">
							<div class="new-select filter-widget">
								...
								<select name="assets_id[]" id="assets_id" class="form-control multiselect_options" multiple>
									<?php
						...
						<div class="col-sm-4 form-group" id="declined_reason_container">
									<div class="filter-widget new-select">
										<label>Declined Reason</label>
										<select name="declined_reason[]" id="declined_reason" class="multiselect_options form-control" multiple>
											<?php
											foreach ($declined_list as $value) {
												$sel = "";
												if (in_array($value['id'],$declined_reason)) $sel = "selected";
											?>
													<option value="<?= $value['id'] ?>" <?= $sel ?>><?= $value['name'] ?></option>
										<?php } ?>
										</select>
...
<script>
	...
	if ($('#assign_stat').find(":selected").val() == 2) {
		$("#declined_reason_container").css("display", "block");
	} else {
		$("#declined_reason").val([]);
		$("#declined_reason_container").css("display", "none");
	}
