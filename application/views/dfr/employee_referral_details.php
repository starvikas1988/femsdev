  ...
  <div class="panel-body"> 
	<div class="row">
	  <div class="col-md-4">
		  ...
		  <b>Location :</b> <span id="loc"><?php echo $referral_details['location']; ?></span> <br/>
		  <b>Company :</b> <span><?php echo $referral_details['company_brand']; ?></span> <br/>
		  <?php if($referral_details['location'] =='CHA'){?>
		  <b>Site :</b> <span><?php echo $referral_details['site_name']; ?></span> <span id="site_id" style="display:none"><?php echo $referral_details['site_id']; ?></span><br/>
		  <?php } ?>
		  
		  <b>Phone :</b> <?php echo $referral_details['phone']; ?> <br/>
		  <b>Email :</b> <?php echo $referral_details['email']; ?> <br/> 
		  <b>Position Referred To :</b> <span><?php echo !empty($referral_details['position']) ? ($referral_details['position'] == "agent" ? "CCE" : ucwords($referral_details['position'])) : "-"; ?></span> <br/> 
		  <b>CV :</b> 
	...
	<div class="col-md-6">
		<p>
		  ...
		  <?php if(!empty($referral_details['ref_candidate_id']) && !empty($referral_details['requisition_id'])){ ?>
		   ...
		   <a target="_blank" href="<?php echo base_url('dfr/view_requisition/'.$referral_details['ref_requisition_id']); ?>" class="btn btn_padding filter_btn btn-xs"> View Requisition</a>
		   
		   <br/><br/>
		   
		  <?php if($referral_details['candidate_status'] !='E' && $referral_details['requisition_status']=='A'){ ?>
		   
		  <a onclick="return confirm('Are you sure you want to resend apply link ?')" hrefURL="<?php echo base_url('dfr/resend_basic_link?r_id='.$referral_details['ref_requisition_id'].'&c_id='.$referral_details['ref_candidate_id']); ?>" class="btn btn_padding filter_btn btn-xs ref_sendMyLink"> Resend Apply Link</a>
		  
		  <a onclick="return confirm('Are you sure you want to resend document link ?')"  hrefURL="<?php echo base_url('dfr/resend_doc_link?r_id='.$referral_details['ref_requisition_id'].'&c_id='.$referral_details['ref_candidate_id'].'&requisition_id='.$referral_details['requisition_id']); ?>" class="btn btn_padding filter_btn btn-xs ref_sendMyLink"> Send Document Link</a>
  ...
  <div class="panel-body"> 
	...
	<div class="col-md-4">
		<div class="form-group">
		 ...
		 <textarea name="selection_comments" class="form-control" style="min-height: 34px;" required></textarea>		 
		</div>
	</div>
	<?php
	if($referral_details['location'] =='CHA'){
	?>
	<div class="row">
		<div class="col-md-12">
			<div class="form-group">
				<label>Site</label>
				<select name="site" class="form-control site" required>
	...
	<div class="col-md-4">
		<div class="form-group">
		 <button type="button" name="save" class="btn btn_padding filter_btn_blue save_common_btn rf_updateReferralStatus">Update</button>
		</div>
	</div>
	</div>
	</form>
 ...
 <div class="panel panel-default">
  ...
  <div class="panel-body"> 
	...
	<form id="rf_updateReferralMoveFrm" method="POST">
		<input type="hidden" name="hiring_source" value="<?=$referral_details['referral_fusionid']?>">
		<input type="hidden" name="ref_name" value="<?=$referral_details['added_name']?>">
		<input type="hidden" name="ref_dept" value="<?=$referral_details['department']?>">
	<div class="row">	
	<div class="col-md-4">
		<div class="form-group">
		 <label for="case">Location</label>
		 <input type="hidden" name="selection_id" value="<?php echo $referral_details['id']; ?>">
		  <select class="form-control" name="selection_location" id="location">
			<option value="">-- Select --</option>
			<?php
