			...
			<div class="col-md-12">
				<div class="white_widget padding_3">
					<div class="avail_widget_br">
						<h2 class="avail_title_heading">Pending Candidate List</h2>
					</div>
						...
						<div class="common_table_widget report_hirarchy_new table_export new_fixed_widget">
							<table id="default-datatable" data-plugin="DataTable" class="table table-bordered table-striped skt-table">
								...
								<tbody>
									...
									foreach ($pending_candidate as $cd) :
										...
										<tr class="smart_search_tr">
											...
											?>
												<td class="leave_columns_fixed action_column_right table_width1">
													...
													$params = $cd['requisition_id'] . "#" . $cd['fname'] . "#" . $cd['lname'] . "#" . $cd['hiring_source'] . "#" . $cd['d_o_b'] . "#" . $cd['email'] . "#" . $cd['phone'] . "#" . $cd['last_qualification'] . "#" . $cd['skill_set'] . "#" . $cd['total_work_exp'] . "#" . $cd['country'] . "#" . $cd['state'] . "#" . $cd['city'] . "#" . $cd['postcode'] . "#" . $cd['address'] . "#" . $cd['summary'] . "#" . $cd['attachment'] . "#" . $cd['gender'] . "#" . $cd['ref_name'];
													...
													if ($c_id != "") {
														echo '<a class="btn action_btn_hover1 btn-xs d_inline viewCandidate" href="' . base_url() . 'dfr/view_candidate_details/' . $c_id . '" target="_blank"  c_id="' . $c_id . '" r_id="' . $r_id . '" title="Click to View Candidate Details"><img src="' . base_url() . 'assets_home_v3/images/view.svg" alt=""></a>';
														if ($cd['requisition_status'] != 'CL') {
															if ($req_no_position > $filled_no_position) {
																if ($c_status != 'R' && $c_status != 'CS') {
																	if (get_dept_folder() == "hr" || $is_role_dir == "admin" || $is_role_dir == "super" || $is_global_access == 1) {
																		echo '<a class="btn d_inline action_btn_hover1 btn-xs editCandidate" c_id="' . $c_id . '" r_id="' . $r_id . '" params="' . $params . '" title="Click to Edit Candidate"><img src="' . base_url() . 'assets_home_v3/images/edit_icon.svg" alt=""></a>';
																	}
																}
															} else {
																//echo "<span class='action_btn_hover1 btn_left' title='Filled Position'><img src='".base_url()."assets_home_v3/images/filled_position.svg' alt=''></span>";
															}
														}
													}
													?>
																	...
																	if ($req_no_position > $filled_no_position) {
																		...
																		if ($c_status == 'P' || $c_status == 'IP') {
																			...
																			if (get_dept_folder() == "hr" || $is_role_dir == "admin" || $is_role_dir == "manager" || $is_role_dir == "super" || $is_global_access == 1) {
																				...
																				echo '<li><a class="btn  btn-xs candidateApproval" r_id="' . $r_id . '" c_id="' . $c_id . '" c_status="' . $c_status . '" requisition_id="' . $requisition_id . '" title="Approval to Final Selection (Joining Date)" ><img src="' . base_url() . 'assets_home_v3/images/add.svg" alt=""> Approval to Final Selection (Joining Date)</a></li>';
																			...
																			if (get_dept_folder() == "hr" || $is_role_dir == "super" || $is_global_access == 1) {
																				if ($c_status != 'E') {
																					echo '<li><a class="btn btn-xs finalSelection" r_id="' . $r_id . '" c_id="' . $c_id . '" cparams="' . $cparams . '" title="" ><img src="' . base_url() . 'assets_home_v3/images/add_employee.svg" alt=""> Add as Employee</a></li>';
																				} else {
																	...
																	?>
																		...
																		<tr>
																				...
																				<td width="100px">
																					...
																					if ($c_status != 'R' && $c_status != 'CS') {
																						...
																						if ($sh_status == "P") {
																							...
																							echo '<a class="btn btn-xs candidateAddInterview" title="Add Interview" r_id="' . $r_id . '" c_id="' . $c_id . '" sch_id="' . $sch_id . '" schType="' . $interview_type . '" schSite="' . $interview_site . '" schAssin="' . $assign_interviewer . '" sh_status="' . $sh_status . '" sch_date="' . $sch_date . '"><img src="' . base_url() . 'assets_home_v3/images/add_employee.svg" alt=""></a>';
																							/* }else{
																			echo '<a class="btn btn-success btn-xs" disabled="true" title="Button will be active on '.$sch_date.' " style="font-size:12px"><i class="fa fa-plus"></i></a>';
																							...
																							echo '<a class="btn btn-xs cancelSchedule" r_id="' . $r_id . '" c_id="' . $c_id . '" sch_id="' . $sch_id . '" params1="' . $params1 . '" title="Cancel Interview Schedule"><img src="' . base_url() . 'assets_home_v3/images/terminate_user.svg" alt=""></a>';
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-3">
							<div class="form-group">
								<label>Phone</label>
								<input type="text" id="phone" name="phone" class="form-control" value="" onkeydown="return ( event.ctrlKey || event.altKey
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id="from_date_exp" name="from_date" class="form-control due_date-cal datepicker" required autocomplete='off' readonly>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								<label>To Date</label>
								<input type="text" id="to_date_exp" name="to_date" class="form-control due_date-cal datepicker" required autocomplete='off' readonly>
							</div>
						</div>
					</div>
					<div class="row">
...
<script>
	...
	$(document).ready(function() {
        ...
        $('#from_date_exp').on('change', function() {
            validateDates();
        });
        $('#to_date_exp').on('change', function() {
            validateDates();
        });
        function validateDates() {
            var startDate = new Date($('#from_date_exp').val());
            var endDate = new Date($('#to_date_exp').val());
            if (startDate && endDate && endDate < startDate) {
                alert("End date cannot be a previous date of start date.");
                $('#to_date_exp').val('');
            }
            var today = new Date().toISOString().split('T')[0];
            $('#to_date_exp').attr('min', today);
        }
    });
</script>
