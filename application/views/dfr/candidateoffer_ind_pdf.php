...
if($location=="CHA" || $brand){
    $bonus = get_bonus_cha($skill_set_slab, "CHA");
    $medical_allowance = get_medical_allowance($gross_pay, $basic, $hra, $conveyance, $bonus, "CHA");
}
...
$noticePeriodDetails = get_notice_period_details($location, $org_grade_id, $org_role, $brand,''); 
//End WORKPLACE-619//
...
<div style="margin:5px;">
    ...
    <div id="body1" style='width:100%;'>
        <br /><br /><br /><br />
			...
			<P style='text-align:right;'>Date: <?php $brand_arr = array(12,1,2);
                        if((in_array($brand, $brand_arr) && isIndiaLocation($location) == true && !empty($approved_on))){

                    echo date('d-F-Y', strtotime($approved_on)); } else { echo $approved_on;}?></P>
		...
		if((in_array($brand, $brand_arr) && isIndiaLocation($location) == true )){ ?>
		    <br>
		    <strong>Address:</strong> <?php echo $can_dtl_row['address']; ?> <?php if($can_dtl_row['postcode'] != ''){ echo ' ,'.$can_dtl_row['postcode']; }?>
		    <br>
		    <?php if($can_dtl_row['email'] != ''){ echo '<strong>Email ID:</strong> '.$can_dtl_row['email']; }?>
		<?php } else { ?>
            <br>
        ...
        <br>
        <br>
        <span>We are pleased to offer you the position of “<?php echo $can_dtl_row['position_name']; ?>” <?php if(isIndiaLocation($location) == true ){?> <?php echo  '('.@$can_dtl_row['position_name']; ?> - <?php echo  @$can_dtl_row['department_name']; ?> <?php echo (@$can_dtl_row['sub_department'] == '') ? ')' : ''; ?> <?php echo (@$can_dtl_row['sub_department'] != '') ? '('.@$can_dtl_row['sub_department'].'))' : ''; ?> <?php } ?> for <?php echo $for_comp; ?> and its group of companies.</span>
        <P style='text-align:justify;'>This offer is contingent upon proof of employment eligibility, background and reference check, and confirmation that you are not bound by any contractual agreements that restrict your ability to perform your duties for For <?php echo $for_comp; ?>, and any of its subsidiary companies. </p>
		...
		$brand_arr = array(12,1,2);
		if((in_array($brand, $brand_arr) && isIndiaLocation($location) == true )){ ?>			
            <span>We look forward to have you onboard with us by <?php if($can_dtl_row['doj'] != '') { echo date('d-F-Y', strtotime($can_dtl_row['doj'])); } ?></span>
        <?php } else { ?>
        ...
        <?php if ($joining_bonus > 0) { ?> <!--$incentive_amt > 0 || -->
            ...
            <span> <?php
                //if ($incentive_amt > 0)
                    //echo "$incentive_period Incentive Amount: Rs. $incentive_amt ";
                if ($joining_bonus > 0)
                    echo " Joining Bonus: Rs. $joining_bonus";
                ?></span>
        <?php } ?>
        ...
        <br /><br /><br /><br />
        <?php if($noticePeriodDetails['notice_period_text'] != ''){ ?> <P style='text-align:justify;'> <span style='font-size:15px; font-weight:bold; '>SEPARATION AND NOTICE PERIOD: </span><br /><?php echo $noticePeriodDetails['notice_period_text']; ?></p><?php } ?>
        ...
        <table cellpadding='0' cellspacing="0" border='0' align='center' style='font-weight:bold; width:100%;padding-top:120px;'>
            <tr>
                <td>
        ...
        <table cellpadding='2' cellspacing="0" border='1' align='center' style='font-size:12px; text-align:center; width:99%;'>
            ...
            <?php if($location =="CHA"){ ?>
            <tr bgcolor="#D3D3D3">
                <td style="text-align:left;">Cost to Company (Excl PLI)</td>
                <td><?php echo round($ctc - $pli_incentive); ?></td>
                <td><?php echo round($ctc_year  - $pli_incentive_year); ?></td>
            </tr>
            <?php } ?>
            <tr bgcolor="#D3D3D3">
                <td style="text-align:left;"><?php if($location =="CHA"){ ?> Take Home Salary Incl 100% PLI (E) = (A - C + D) <?php } else { ?> Take Home Salary (F) = (A + D - C) <?php } ?></td>
                <td><?php echo $tk_home; ?></td>
                <td><?php echo $tk_home_year; ?></td>
            </tr>
            <?php if($location =="CHA"){ ?>
            <tr bgcolor="#D3D3D3">
                <td style="text-align:left;">Take Home Salary Excl PLI (F) = (A - C)</td>
                <td><?php echo round($tk_home - $pli_incentive); ?></td>
                <td><?php echo round($tk_home_year - $pli_incentive_year); ?></td>
            </tr>
            <?php } ?>
