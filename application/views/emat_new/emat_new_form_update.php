...
<div class="main-content page-content">
  ...
  strtotime($crm_details['trans_flight_date'])); ?>" placeholder="" min="<?= date('Y-m-d') ?>">
									</div>
								</div>
								<div class="col-sm-3">
									<div class="form-group">
								...
								<div class="col-sm-6 new" >
									<div class="form-group">
										...
										foreach($action_taken as $action){
											$selection = "";

											$action_val=explode(",",$crm_details['action_taken_id']);
echo $action_val;
											if (in_array($action['id'], $action_val)){ $selection = "selected"; }	
											?>
											<option value='<?php echo $action['id']; ?>' <?php echo $selection; ?>><?php echo $action['name']; ?></option>
											<?php }?>
										</select> 
									</div>
								</div>
								<div class="col-sm-6 new">
									<div class="form-group">
										...
										<?php 
										foreach($action_pending as $action){
											$selection = "";

											$action_val=explode(",",$crm_details['pending_items_id']);
