			...
			<div class="col-md-12">
				<div class="widget">
					<header class="widget-header">
						<h4 class="widget-title" style="display:inline-block; line-height:30px">Leave</h4>                        
					...
					<div class="widget-body clearfix">
                         <center><span style="color:red;font-size: 15px;">*This window needs to be used if leave is applied for one or more days   </span> </center> 
                    <?php 
                     /*$userid = get_user_id();
                    ...
                    <div class="col-md-1" style='margin-top:22px;'>
                        <div class="form-group">
                            <?php 
                                ...
                                ?>    
                                <a href="<?php echo base_url();?>/diy/leave_master_csv<?php echo $search_from_date.$search_to_date;?>"><button class="btn btn-primary waves-effect" type="button" >DOWNLOAD</button></a>
                                <?php } else { 
                                     $search_from_date = ($search_from) ?"&search_from='$search_from'" : "";
                                     $search_to_date = ($search_to) ?"&search_to='$search_to'" : "";
                                    ?>
                                <a href="<?php echo base_url();?>/diy/leave_master_csv?id=<?php echo $userid; ?><?php echo $search_from_date.$search_to_date;?>"><button class="btn btn-primary waves-effect" type="button" >DOWNLOAD</button></a>
                            <?php } ?>
                        </div>
                    </div>
					<div class="widget-body clearfix">
						<div class="row">
                            ...
                            <thead>
                                <tr class="text-center bg-primary">
                                    ...
                                    <th width="140px" style="font-size:85%;border-bottom-width: 0px; border: 1px solid #ddd;" class="text-center">From</th>
                                    <th width="140px" style="font-size:85%;border-bottom-width: 0px; border: 1px solid #ddd;" class="text-center">To</th>
                                    <th width="100px" style="font-size:85%;border-bottom-width: 0px; border: 1px solid #ddd;" class="text-center">No. Of Day(s)</th>  
                                    ...
                                    <th style="font-size:85%;border-bottom-width: 0px; border: 1px solid #ddd;" class="text-center">Reason</th>									
                            ...
                            foreach($leave_master as $key=>$leave): ?>
                                <tr style="font-size:85%">
                                    ...
                                    <td style="font-size:80%;" class="text-center"><?php echo $leave['from_date']; ?></td>  
                                    <td style="font-size:80%" class="text-center"><?php echo $leave['to_date'] ?></td>
                                    <td style="font-size:80%" class="text-center">
									<?php 
										if($leave['no_of_days'] != null){
											echo $leave['no_of_days'].' days';
										} else{
											echo $leave['no_of_days'];
										}                                           
									?>
                                    </td>
                                    ...
                                    <?php 
                                        $status="";
                                        if($leave['status']=='applied')$status="Applied";
                                        if($leave['status']=='approved')$status="Approved";
                                        if($leave['status']=='disapproved')$status="Unapproved";
                                        /*$leave['status']*/
                                    ?>  
                                    <td style="font-size:80%"><?php echo strtoupper($status); ?></td>  
                                    <td style="font-size:100%" class="text-center">
                                     <?php  echo $leave['reason']; ?> 
                                    </td>    
									<?php if((get_login_type() == "client" && get_role() != 'teacher') || get_global_access()){ ?>
										<td style="font-size:80%" class="text-center">
                                            <select name="leave_status" id="leave_status<?php echo $leave['id'];?>" onchange="change_leavestatus('<?php echo $leave['id'];?>',this)">
                                                <option value="applied" <?php ($leave['status']=='applied')?"selected='selected'":"";?> >Applied</option>
                                                <option value="approved" <?php ($leave['status']=='approved')?"selected='selected'":"";?>>Approved</option>
                                                <option value="disapproved" <?php ($leave['status']=='disapproved')?"selected='selected'":"";?>>Rejected</option>
                                            </select>
										</td>
									<?php } ?>
                                </tr>
        ...
        <div class="modal-content">
            <form method="post" action="leave_applied" id="leave_form" onsubmit="submit_data();" autocomplete="off">
                <div class="modal-header" style="padding:8px 15px;">
                    ...
                    <h4 class="modal-title">Leave Application</h4><br>
                    <center><span style="color:red;font-size: 14px;">The leave is subject to approval by the admin and it depends on the Guidelines laid by diy</span></center><br>
                </div>
