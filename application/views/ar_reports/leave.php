		...
		<div class="row">
			<div class="col-md-12">
				<div class="white_widget padding_3">
					<h2 class="avail_title_heading">Leave</h2>
...
<script>
	function get_location_leaves(elem) {
		var location_abbr = $(elem).val();
		if (location_abbr == "All") {
			location_abbr = "";
		}
		$.get("<?php echo base_url() ?>leave/get_leaves_location", {
			"loc_id": location_abbr
		}, function(data) {
			console.log(data);
			data = $.parseJSON(data);
			htm = '<option value="">--select--</option><option value="">All</option>';
			$.each(data, function(i, v) {
				htm += '<option value="' + i + '">' + v + '</option>';
			});
			$("#leave_type").empty().append(htm);
		});
	}
</script>