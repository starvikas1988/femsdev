						...
						<div class="row">
							...
							<div class="col-sm-1 col-1 pull-right">
								<?php if($gant_access == true){ ?>
									<a href="<?php echo base_url();?>course" type="button" class="btn  btn-success" title="Navigate Back"><i class="glyphicon glyphicon-arrow-left"></i> Back </a>
								<?php }elseif($gant_access == false){  ?>
									<a href="<?php echo base_url();?>home" type="button" class="btn  btn-success" title="Navigate Back"><i class="glyphicon glyphicon-arrow-left"></i> Back </a>
								<?php }		  ?>
							</div>
						<?php if($gant_access == true){  ?>
							<div class="col-sm-1 col-1 pull-right">
					...
					<div class="widget-body">
						...
						<table class="table table-responsive">
						  ...
						  <?php foreach($course_details as $course) :?> 
							...
							<?php }else{ ?> 
							  <td><a href="<?php echo base_url();?>course/view_courses/<?php echo $course['categories_id']; ?>/<?php echo $course['course_id']; ?>"  title="<?php echo  $gant_access == true ? "Click here to view Upload ".$course['course_name'] : "Click here to View ".$course['course_name']; ?>"><span class="fa fa-book" style="font-size:55px"  id='imageid1'/></span></a></td>
							<?php } ?>  
						  ...
						  <!--<td class="pull-right"><form method="post" action="<?php echo base_url();?>course/remove_global" ><input type="hidden" name="cat_id" value="<?php echo $course['categories_id']; ?>"><input type="hidden" name="c_id" value="<?php echo $course['course_id']; ?>" ><button type="submit"  class="btn btn-success set_rule" title="<?php echo $course['is_global'] == 0 ? 'Mark Global' : 'Unmark Global'?>" ><span style="<?php echo $course['is_global'] == 0 ? 'color:white' : 'color:red'?>" class="<?php echo $course['is_global'] == 0 ? 'fa fa-map' : 'fa fa-asterisk '?>" /></button></form></td>-->
							  ...
							  <?php } ?>
							  <td class="pull-right"><button type="button" id="trash_course<?php echo $course['course_id']; ?>" data-trashid="<?php echo $course['categories_id']; ?>" value="<?php echo $course['course_id']; ?>" class="btn btn-success trash_course"  title="Delete Course"><span class="fa fa-trash" /></button></td>
							  ...
							  <td class="pull-right"><button type="button" data-categoryid="<?php echo $course['categories_id']; ?>" id="setrule_<?php echo $course['course_id']; ?>" value="<?php echo $course['course_id']; ?>" class="btn btn-success set_rule" data-toggle="modal" data-target="#setRuleModal" title="Set Rules"><span class="fa fa-recycle" /></button></td>
						  ...
						  <?php }else{  ?>
							...
							<?php }else{  ?>
								...
								<?php if($course['is_complete'] == 1){?>
										<td class="pull-right"><input type="hidden" name="course_id" value="<?php echo $course['course_id']; ?>" ><input type="hidden" name="categories_id" value="<?php echo $course['categories_id']; ?>"> <button type="submit" title="Proceed to Exam" id="view_<?php echo $course['id']; ?>" value="<?php echo $course['id']; ?>" class="btn btn-success view_course"><span class="fa fa-graduation-cap" /></button></td>	
								<?php }   ?>
							<?php }   ?>	
						  <?php }   ?>	 
						 <?php endforeach; ?>
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
				<div class="container"> 
					...
					<div class="row" style="padding-bottom:10px">
						<div class="col-sm-1">
							<label for="inputdescription">Is Global:</label> 
						</div>
						<div class="col-sm-4">
							<select class="form-control" name="global_field" id="global_field">
			...
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
				...
				<form method="post" action="<?php echo base_url();?>course/set_rules">
					<input type="text" id="cid_rules"  name="cid_rules" value="" >
					<input type="text" id="catid_rules"  name="catid_rules" value="" >
													...
													<div class="col-md-4" id="expiry_rules" >
														<div class="form-group" >
															...
															<select class="form-control" name="expiry" id="expiry" >
																<option value=''>--Select--</option>
																<option value='N'>No Expiry Period</option>
																<option value='M'>Expiry Period</option>
																<option value='C'>Before Confirmation</option>
																<option value='P'>Prelim period</option>
															</select>
														</div>
													</div>
		...
		<div class="modal-content">
			...
			<form id="create_dipcheck_form"  action="<?php echo base_url(); ?>course/savedipcheck" method='POST'>
				<div class="modal-body">
					...
					<!--<input type="text" id="categories_id"  name="catid" value="" >
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
