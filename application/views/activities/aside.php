<!-- APP ASIDE ==========-->
<style>
.app-aside.light .aside-menu .active{
	color:#188ae2;
}

aside{
	scrollbar-width: thin;
	overflow: auto;
}
</style>
<?php
$pmenu="";
$act1="";
$act2="";
$act3="";
$act4="";
$act5="";
$act6="";
if(!isset($_SESSION['pmenu'])){
   $pmenu="1";
} else {
    $pmenu=$_SESSION['pmenu'];
}
if ($pmenu=='1') $act1="active";
else if ($pmenu=='2') $act2="active";
else if ($pmenu=='3') $act3="active";
else if ($pmenu=='4') $act4="active";
else if ($pmenu=='5') $act5="active";
else if ($pmenu=='6') $act6="active";

?>
<aside id="app-aside" class="app-aside left light">
	<header class="aside-header">
		<div class="animated">
			<a href="<?php echo base_url()?>home" id="app-brand" class="app-brand">
				<span id="brand-icon" class="brand-icon"></span>
				<span id="brand-name" class="brand-icon foldable"><img src="<?php echo base_url() ?>assets/images/fusion-bpo.png" border="0" alt="Fusion BPO"></span>
			</a>
		</div>
	</header><!-- #sidebar-header -->
	<div style="padding:20px 0; clear:both;"></div>
	<div class="aside-scroll">
		<div id="aside-scroll-inner" class="aside-scroll-inner">
			<ul class="aside-menu aside-left-menu">
				<!--
				<li class="menu-item">
					<a href="<?php echo base_url('activities')."/dashboard" ?>" class="menu-link <?php echo $act6;?>">
						<span class="menu-icon"><i class="fa fa-dashboard"></i></span>
						<span class="">Dashboard</span>
					</a>
				</li>
				--->
				
				<li class="menu-item">
					<a href="<?php echo base_url('activities')?>" class="menu-link <?php echo $act1;?>">
						<span class="menu-icon"><i class="zmdi zmdi-accounts-outline zmdi-hc-lg"></i></span>
						<span class="">My Activity</span>
					</a>
				</li>
				
				<li class="menu-item">
					<a href="<?php echo base_url('activities/myteam')?>" class="menu-link <?php echo $act2;?>">
						<span class="menu-icon"><i class="zmdi zmdi-accounts-outline zmdi-hc-lg"></i></span>
						<span class="">My Team</span>
					</a>
				</li>
				
				<!--
				<li class="menu-item">
					<a href="<?php //echo base_url('activities/myteam2')?>" class="menu-link <?php echo $act3;?>">
						<span class="menu-icon"><i class="zmdi zmdi-accounts-outline zmdi-hc-lg"></i></span>
						<span class="">My Team 2</span>
					</a>
				</li>
				-->
				
				<li class="menu-item">
					<a href="<?php echo base_url('activities/myteam3')?>" class="menu-link <?php echo $act4;?>">
						<span class="menu-icon"><i class="fa fa-bar-chart"></i></span>
						<span class="">Realtime Activities</span>
					</a>
				</li>
				
				<li class="menu-item">
					<a href="<?php echo base_url('activities/config')?>" class="menu-link <?php echo $act5;?>">
						<span class="menu-icon"><i class="fa fa-cog"></i></span>
						<span class="">Config</span>
					</a>
				</li>
				
			</ul>
			</footer><!-- #sidebar-footer -->
		</div><!-- .aside-scroll-inner -->
	</div><!-- #aside-scroll -->
</aside>
<!--========== END app aside -->