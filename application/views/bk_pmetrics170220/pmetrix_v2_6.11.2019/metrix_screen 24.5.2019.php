			...
			<div class="form-group">
				...
				<select class="form-control" name="client_id" id="fclient_id" >
					...
					<?php foreach($client_list as $client): ?>
						...
						if(in_array($client['id'],explode(',',$client_ids)))
						{
							$sCss="";
									...
									<tr id="last_row">
										...
										<?php
											foreach($total[$mpid] as $key=>$v){
												$kpi_value = round($v['summary'],1);
												if($v['type'] == 1 ) $kpiDispVal = $kpi_value;
												else if($v['type'] == 7 ) $kpiDispVal = round($kpi_value,2);
												else if($v['type'] == 8 ) $kpiDispVal = sprintf("%1.2f",round(($kpi_value),2));
												else if($v['type'] == 9 ) $kpiDispVal = gmdate("H:i:s", $kpi_value);
												else $kpiDispVal = sprintf("%1.2f",round(($kpi_value*100.0),2)) . "%";
