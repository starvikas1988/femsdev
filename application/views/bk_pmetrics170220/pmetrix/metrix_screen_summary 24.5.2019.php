						...
						<table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" cellspacing="0" width="100%">
								...
								<?php
									...
									foreach($ind_user_summary as $key=>$value)
									{
										...
										foreach($value as $k=>$v)
										{
											$kpi_value = round($v['value'],1);
											if($v['kpi_type'] == 1 ) $kpiDispVal = $kpi_value;
											else if($v['kpi_type'] == 7 ) $kpiDispVal = round($kpi_value,2);
											else if($v['kpi_type'] == 8 ) $kpiDispVal = sprintf("%1.2f",round(($kpi_value),2));
											else if($v['kpi_type'] == 9 ) $kpiDispVal = gmdate("H:i:s", $kpi_value);
											else $kpiDispVal = sprintf("%1.2f",round(($kpi_value),2)) . "%";
									...
									<tr id="last_row">
										...
										<?php
											foreach($overall_summary as $key=>$value_array){
												...
												$kpi_value = round($value_array['value'],1);
												if($value_array['kpi_type'] == 1 ) $kpiDispVal = $kpi_value;
												else if($value_array['kpi_type'] == 7 ) $kpiDispVal = round($kpi_value,2);
												else if($value_array['kpi_type'] == 8 ) $kpiDispVal = sprintf("%1.2f",round(($kpi_value),2));
												else if($value_array['kpi_type'] == 9 ) $kpiDispVal = gmdate("H:i:s", $kpi_value);
												else $kpiDispVal = sprintf("%1.2f",round(($kpi_value),2)) . "%";
