...
<?php
	function generate_all($pmAllArray,$mpid)
	{
		$row = generate_indiv_date_table_row($pmAllArray,$mpid);
		$fianl = generate_final_table_row($row,$mpid);
		$summary = generate_indv_user_summary($fianl,$mpid);
		$all[] = $summary;
		return $all;
	}
	...
	function generate_indiv_date_table_row($pmAllArray,$mpid)
	{
		...
		foreach($table_row[$mpid] as $key=>$value)
		{
			foreach($value as $ke=>$va)
			{
				...
				{
					$table_row[$mpid][$key][$ke][$v['id'].' ## '.$v['kpi_name'].' ## '.$v['kpi_type'].' ## '.$v['summ_type'].' ## '.$v['summ_formula']] = '';
				}
			}
		}
	...
	function value_calculation($kpi_type,$kpi_value,$return=false)
	{
		if($kpi_value != '')
		{
			...
			else if($kpi_type == 9 ) $kpiDispVal = gmdate("H:i:s", $kpi_value);
				...
				<div class="widget">
					...
					<div class="widget-body" >
						...
						<table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" cellspacing="0" width="100%">
							<thead>
								<tr class='bg-info text-center'>
									<td colspan="<?php echo count($pmAllArray[$mpid.'kpi']); ?>">Your Target</td>
								</tr>
								<tr class='bg-info'>
									<?php
						...
						<table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" cellspacing="0" width="100%">
							<thead>
								<tr class='bg-info text-center'>
									<td colspan="<?php echo count($pmAllArray[$mpid.'kpi']); ?>">Best Scores</td>
								</tr>
								<tr class='bg-info'>
									<?php
										foreach($pmAllArray[$mpid.'kpi'] as $key=>$value)
								...
								<tr>
									<?php
										foreach($pmAllArray[$mpid.'kpi'] as $key=>$value)
										{
											echo '<td>';
												$val = '-';
											foreach($best_values as $ke=>$va)
											{
											//echo $ke;
												$kpi_infos = explode(' ## ',$ke);
												//print_r($kpi_infos);
												if($value['kpi_name'] == $kpi_infos[0])
												{
													$val = value_calculation($kpi_infos[1],$va);
												}
											}
												echo $val;
											echo '</td>';
