			...
			foreach($valu as $keyy=>$value)
			{
				...
				foreach($value['data'] as $ke=>$va)
				{
					...
					if($formula == 3)
					{
						...
						if($condition != '')
						{
							$cond_array = explode('=',$condition);
							$cond_col = $cond_array[0];
							$cond_val = $cond_array[1];
							foreach($to_search_data as $k=>$v)
							{
								$k_info = explode(' ## ',$k);
								if($k_info[1] == $cond_col)
								{
									if($cond_val == $v)
									{
										if($va != '')
						...
						if($condition != '')
						{
							...
							foreach($to_search_data as $k=>$v)
							{
								...
								if($k_info[1] == $cond_col)
								{
									...
									{
										$summary[$mpid][$value['fusion_id']][$ke] = $summary[$mpid][$value['fusion_id']][$ke] + 0;
									}
								}
							}
						}
							...
							<tbody>
								...
								<?php
									...
									foreach($summary[$mpid] as $key=>$value)
									{
										...
										{
											echo '<tr>';
												echo '<td colspan="1">MTD '.$cur_month_year.'</td>';
											foreach($value as $ke=>$va)
											{
												if($ke !== 'fusion_id')
												{
													...
													if($hide_col[trim($kpi_type[1])]['tl_view'] == 1)
													{
														echo '<td>'.value_calculation($kpi_type[2],$va,false).'</td>';
													}
												}
											}
											echo '</tr>';
