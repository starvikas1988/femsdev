

<aside id="app-aside" class="app-aside left light">
	<header class="aside-header">
		<div class="animated">
			<a href="<?php echo base_url()?>home" id="app-brand" class="app-brand">
				<span id="brand-icon" class="brand-icon"></span>
				<span id="brand-name" class="brand-icon foldable"><img src="<?php echo base_url() ?>assets/images/fusion-bpo.png" border="0" alt="Fusion BPO"></span>
			</a>
		</div>
	</header><!-- #sidebar-header -->
	
	<div style="padding:20px 0; clear:both;"></div>
	<div class="aside-scroll">
		<div id="aside-scroll-inner" class="aside-scroll-inner">
			<ul class="aside-menu aside-left-menu">
						
				<li class="menu-item">
					<a href="<?php echo fdcrm_url('dashboard'); ?>" class="menu-link">
						<span class="menu-icon"><i class="zmdi zmdi-view-dashboard zmdi-hc-lg"></i></span>
						<span class="">Dashboard</span>
					</a>
				</li>
				
				<li class="menu-item open">
					
					<a href="" class="menu-link submenu-toggle">
						<span class="menu-icon"><i class="zmdi zmdi-email-open zmdi-hc-lg"></i></span>
						<span class="menu-text foldable"><?php echo fdcrm_title(); ?></span>
						<span class="menu-caret foldable"><i class="zmdi zmdi-hc-sm zmdi-chevron-right"></i></span>
					</a>
					
					<ul class="submenu" style="display:block">
						
						<?php if(!is_crm_readonly_access_mindfaq()){ ?>
						<li>
							<a  href="<?php echo fdcrm_url('newCustomer'); ?>">
								<span class="menu-icon"><i class="zmdi zmdi-assignment-check zmdi-hc-lg"></i></span>
								<span class="">New Customer</span>
							</a>
						</li>
						<?php } ?>
						
						<li>
							<a  href="<?php echo fdcrm_url('customerSearch'); ?>">
								<span class="menu-icon"><i class="zmdi zmdi-assignment-check zmdi-hc-lg"></i></span>
								<span class="">Search Customer</span>
							</a>
						</li>
						
						<li>
							<a  href="<?php echo fdcrm_url('customerList'); ?>">
								<span class="menu-icon"><i class="zmdi zmdi-assignment-check zmdi-hc-lg"></i></span>
								<span class="">Customer List</span>
							</a>
						</li>
						
					</ul>
						
				</li>
				
				
				<li class="menu-item">
					<a href="<?php echo fdcrm_url('customerReport'); ?>" class="menu-link">
						<span class="menu-icon"><i class="fa fa-download"></i></span>
						<span class="">Customer Report</span>
					</a>
				</li>
				
				<li class="menu-item">
				<a href="<?php echo fdcrm_url('feedback_reports')?>" class="menu-link">
					<span class="menu-icon"><i class="fa fa-smile-o zmdi-hc-lg"></i></span>
					<span class=""><?php echo fdcrm_title(1); ?> Report</span>
				</a>
			    </li>
				
										
				
				</ul>
			</footer><!-- #sidebar-footer -->
		</div><!-- .aside-scroll-inner -->
	</div><!-- #aside-scroll -->
</aside>