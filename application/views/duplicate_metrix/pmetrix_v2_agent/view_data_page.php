<?php
	...
	function prepare_row_indv_user_data($design_id,$kpi_col,$kpi_data,$qa_data)
	{
		...
		foreach($kpi_data[$design_id] as $k=>$v)
		{
			...
			if(in_array($v->kpi_name, $available_kpi_name))
			{
				...
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['kpi_type'] = $kpi_col[$design_id][$col_counter-1]->kpi_type;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['summ_type'] = $kpi_col[$design_id][$col_counter-1]->summ_type;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['summ_formula'] = $kpi_col[$design_id][$col_counter-1]->summ_formula;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['agent_view'] = $kpi_col[$design_id][$col_counter-1]->agent_view;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['tl_view'] = $kpi_col[$design_id][$col_counter-1]->tl_view;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['start_date'] = $v->start_date;
				$indv_user_data[$v->fusion_id]['row'.$row_no]['col'.$col_counter]['weightage'] = $v->weightage;
	...
	function prepare_row_every_indv_user_data($design_id,$kpi_col,$kpi_data,$qa_data)
	{
		$available_col_count = count($kpi_col[$design_id]);
		$indv_user_data = array();
		$row_no = 1;
...
foreach($kpi_design as $key=>$kpi_design_value)
{
	...
	$everyone_indv_user_raw_data_row = prepare_row_every_indv_user_data($kpi_design_value->design_id,$kpi_col,$everyone_kpi_data,$qa_data);
							...
							<table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" width="100%" cellspacing="0" style="text-align:center;">
								<thead>
									<tr style="background-color: #35b8e0;color: #fff; border:1px solid black;">
										<?php
										foreach($final_score[$fusion_id] as $type=>$score)
										{
											if($type != 'total_score')
											{
												echo '<th style="text-align:center;border: 1px solid black;">'.$type.'</th>';
											}
										}
								...
								<tbody>
									<?php
										...
										foreach($final_score[$fusion_id] as $type=>$score)
										{
											if($type != 'total_score')
											{
												echo '<td style="text-align:center;border: 1px solid black;">'.number_format($score,2).'</td>';
											}
										}
										echo '<td style="text-align:center;border: 1px solid black;">'.number_format($final_score[$fusion_id]['total_score'],2).'</td>';
										foreach($grade as $key=>$value)
										{
											if($value->grade_start <= $final_score[$fusion_id]['total_score'] && $value->grade_end >= $final_score[$fusion_id]['total_score'])
											{
												echo '<td style="text-align:center;color:red; border:1px solid black;">'.$value->grade.'</td>';
											}
										}
										echo '</tr>';
									?>
					...
					<div class="widget-body">
						<div class="table-responsive">
							<table id="default-datatable" data-plugin="DataTable" class="table table-striped skt-table" width="100%" cellspacing="0">
								<thead>
									<tr style="background-color: #35b8e0;color: #fff;">
										<?php
											$cout = 0;
											...
											echo '<th style="text-align:center;border: 1px solid black;" colspan="'.($cout-1).'">Your Target</th>';
										?>
										
									</tr>
									<tr  style="background-color: #35b8e0;color: #fff;">
										<?php
											foreach($target[$kpi_design_value->design_id] as $key=>$value)
											{
									...
									<?php
										foreach($target[$kpi_design_value->design_id] as $key=>$value)
										{
											...
											if($value->agent_view == 1)
											{
												...
												{
													echo '<td style="text-align:center;border: 1px solid black;">'.value_calculation($value->kpi_type,$value->target,false).'</td>';
												}
											}
										}
									?>
								...
								<thead>
									...
									foreach($kpi_col as $key=>$value)
									{
										foreach($value as $ke=>$val)
										{
											if($val->agent_view == 1)
											{
												echo '<th style="text-align:center;border: 1px solid black;">'.$val->kpi_name.'</th>';
											}
										}
									}
									?>
