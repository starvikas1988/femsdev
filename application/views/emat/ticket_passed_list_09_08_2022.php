...
<div class="email-white1">
	<div class="tab-content">
		<div id="category1" class="tab-pane active">
			<h2 class="heading-title"><?php echo !empty($catInfo) ? $catInfo[0]['category_name'] : "All"; ?> Passed List</h2>
				...
				<div class="col-sm-3">							
					<div class="left-side2">
						...
						?>
							...
							<a href="<?php echo base_url()?>emat/ticket_agents_passed/<?php echo bin2hex($email_id); ?>/<?php echo $agentInfo['id']; ?>/all">
								<div class="all-main">									
									...
									<div class="all-widget" style="width:30%;font-size:10px">
										<?php echo sprintf('%.2f', $agentPassPerArrayAll[$agentInfo['id']]); ?>%
									</div>									
							...
							<div class="body-widget">
								<div class="row">
									...
									<div class="col-md-8 dropdownCapture">								
										...
										<?php if(e_show_category($email_id)){ ?>
											<select class="form-control pull-right" style="width:100px;height: 22px;margin-bottom: 1px;margin-top: -2px;margin-right:5px" name="sort_list_category">
											<option value="all" selected="selected">ALL</option>
											<?php foreach($ticketCategory as $key=>$cat){ ?>	
											<option value="<?php echo $cat['category_code'];?>" <?php echo e_sort_email_type_group($email_id) == $cat['category_code'] ? 'selected' : ''; ?> ><?php echo $cat['category_name'];?></option>
										<?php } ?>
										</select>
										<?php } ?>
									...
									<table class="table table-bordered table-striped">
										<tbody>
										  <tr>
											...
											<td  style="width:<?php echo $_t4; ?>%;font-size:11px;padding-left:10px;"><span class="red"><?php echo e_display_aging_time(e_aging_ticket($token['date_added'])); ?></span></td>
