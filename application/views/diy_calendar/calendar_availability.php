...
<div class="row">
<?php //echo $my_gmt_timezone;?>

<div class="col-md-7" <?php if(get_role() == 'teacher' || get_role() == 'Teacher'){ echo 'style="display:none"'; } ?>>
<form method="GET">
<div class="row techer_selection_row">
	...
	<?php //echo $my_gmt_timezone;
	//echo "<br>";
	$my_gmt_timezoneval=($my_gmt_timezone=='+5:30')?'GMT+05:30':$my_gmt_timezone;
	?>
	<div class="col-md-5">	
		<select name="timezone_id" class="form-control" required>
		<?php foreach($gmtList as $token){	
		$arr = explode("#",$token['GMT_offset']);
		echo $arr[0];
		$selection = "";
		if($timezone_id == $token['id']){ $selection = "selected"; }
		elseif($my_gmt_timezoneval == $token['GMT_offset']){ $selection = "selected"; }
		?>
