...
class Pmetrix_v2_gm extends CI_Controller {
    ...
    public function index()
    {
        if(check_logged_in()){
			...
			$data["aside_template"] = "pmetrix_v2_gm/aside.php";
			...
			$data["content_template"] = "pmetrix_v2_gm/matrix_screen_tl_wise.php";
			$data["content_js"] = "pmetrix_v2_gm/tl_js.php";
	...
	public function prepare_tl_row()
	{
		...
		//echo "</pre>";
				...
				$data['processs_qa_data'][$value->design_id] = $this->get_qa_data_same_process($value->design_id,$start_date,$end_date,$current_user);
		...
		$current_user = get_user_id();
		log_record($current_user,'pm_management_view','pm',$form_data);
		pm_log_record($current_user,'pm_management_view','pm',$form_data);

		if($form_data['process_id'] == 30)
		{
			echo $this->load->view('pmetrix_v2_gm/view_data_page_management_vrs',$data,true);
		}
		else
		{
			echo $this->load->view('pmetrix_v2_gm/view_data_page_management',$data,true);
		}
	}
	...
	private function get_qa_data_same_process($design_id,$start_date,$end_date,$tl_id)
	{
		...
		WHERE pm_design_v2.id="'.$design_id.'" AND signin.assigned_to !='.$tl_id.' AND audit_date >= "'.$start_date.'" AND audit_date <= "'.$end_date.'" ORDER by audit_date');
		if($query)
		{
			return $query->result_object();
		}
