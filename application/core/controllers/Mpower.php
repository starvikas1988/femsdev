...
class Mpower extends CI_Controller {
...
///=========================== ALPHA GAS CRM  ================================///
	...
	{		 		
		redirect(base_url()."mpower/dashboard");	 
	}
	...
	{
		$sql = "SELECT count(*) as value from crm_mpower ORDER by cid DESC LIMIT 1";
		$lastid = $this->Common_model->get_single_value($sql);
		$new_crm_id = "MPW" .mt_rand(11,99) .sprintf('%06d', $lastid + 1);
		return $new_crm_id;
	}
	...
	public function dashboard()
	{
		...
		$sql = "SELECT count(*) as value from crm_mpower as c WHERE 1 $extraFilter";
		$data['todays_records'] = $todays_records = $this->Common_model->get_single_value($sql);
		
		$sql = "SELECT count(*) as counter, c.c_status from crm_mpower as c WHERE 1 $extraFilter GROUP BY c.c_status";
		...
		$sqlDatas = "SELECT c.*, CONCAT(s.fname, ' ' ,s.lname) as fullname, s.fusion_id 
		             from crm_mpower as c LEFT JOIN signin as s ON c.agent_id = s.id 
					 WHERE 1 $extraFilter";
		...
		//-------- TOTAL COUNT
		$sql = "SELECT count(*) as value from crm_mpower as c WHERE 1 $extraTotal";
		...
		$data["content_template"] = "mpower_crm/mpower_dashboard.php";				
		$data["content_js"] = "mpower_crm/mpower_graph_js.php";				
	...
	public function newCustomer()
	{
		...
		$data["aside_template"] = "mpower_crm/aside.php";
		$data["content_template"] = "mpower_crm/mpower_form.php";				
		$data["content_js"] = "mpower_crm/mpower_js.php";				
	...
	public function updateCustomer()
	{
		...
		$crm_query = "SELECT * from crm_mpower WHERE crm_id = '$crmid'";
		$data["crm_details"] = $this->Common_model->get_query_row_array($crm_query);
		
		$data["aside_template"] = "mpower_crm/aside.php";
		$data["content_template"] = "mpower_crm/mpower_form_update.php";				
		$data["content_js"] = "mpower_crm/mpower_js.php";				
	...
	public function CustomerList()
	{
		...
		$data["aside_template"] = "mpower_crm/aside.php";
		$data["content_template"] = "mpower_crm/mpower_list.php";
		$data["content_js"] = "mpower_crm/mpower_list_js.php";
		...
		$sqlcase = "SELECT c.*, concat(s.fname,' ', s.lname) as added_by_name from crm_mpower as c LEFT JOIN signin as s ON s.id = c.added_by 
	...
	public function CustomerSearch()
	{
		...
		$data["aside_template"] = "mpower_crm/aside.php";
		$data["content_template"] = "mpower_crm/mpower_search.php";
		$data["content_js"] = "mpower_crm/mpower_list_js.php";
		...
		//--------  SEARCH NAME
		$s_name = "";
		if(!empty($this->input->get('s_ref')))
		{	
			$search_terms = 1;
			$s_crm_ref = $this->input->get('s_ref');
			$extraFilter .= " AND ( ";
			$extraFilter .= " (c.c_fname LIKE '%$s_crm_ref%' OR c.c_lname LIKE '%$s_crm_ref%') ";
			$extraFilter .= " OR (c.c_phone_no = '$s_crm_ref') ";
			$extraFilter .= " OR (c.c_email = '$s_crm_ref') ";
			$extraFilter .= " OR (c.c_call_ref = '$s_crm_ref') ";
			$extraFilter .= " OR (c.crm_id = '$s_crm_ref') ";
			$extraFilter .= " ) ";
		}
		$data['s_crm_ref'] = $s_crm_ref;
	...
	public function CustomerReport()
	{
		...
		$data["aside_template"] = "mpower_crm/aside.php";
		$data["content_template"] = "mpower_crm/mpower_reports.php";
		$data["content_js"] = "mpower_crm/mpower_list_js.php";
	...
	public function crm_logs_list()
	{
		...
		$this->load->view('mpower_crm/mpower_logs',$data);
	}
	
	public function crm_logs_record_details($cid)
	{
		$sql = "SELECT c.*, concat(s.fname,' ', s.lname) as added_by_name from crm_mpower_logs as c 
	...
	public function generate_crm_reports($from_date ='', $to_date='', $call_type='')
	{
		...
		$sqlcase = "SELECT c.*, concat(s.fname,' ', s.lname) as added_by_name, concat(l1.fname,' ', l1.lname) as l1_supervisor, r.name as designation from crm_mpower as c 
		...
		$title = "MPOWER";
		$sheet_title = "MPOWER - CUSTOMER RECORDS (" .date('Y-m-d',strtotime($startDateFull)) ." - " .date('Y-m-d',strtotime($endDateFull)).")";
		$file_name = "Mpower_Records_".date('Y-m-d',strtotime($from_date));
		...
		$i++; $this->objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i,2, "Email");
		...
		{	
			...
			$j++;  $this->objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($j,$i+2, $wv["c_email"]);
	...
	public function submit_mpower()
	{
		...
		$c_email_auto = $this->input->post('c_email_auto');
		...
		$c_reference           = $this->input->post('c_reference');
		...
		{			
			...
			data_inserter('crm_mpower', $case_array);			
			$flag = "insert";
		}
		...
		if(!empty($crm_id) && !empty($c_email_auto)){
			 //$this->load->library('../controllers/Feedback.php');
			 $this->Feedback_email_model->mpower_send_feedback($crm_id, $c_email, '', 2);
			 $updateData = [ 'c_feedback_mail' => 1 ];
			 $this->db->where('crm_id', $crm_id);
			 $this->db->update('crm_mpower', $updateData);
		}
		
		if($flag=="insert"){ redirect(base_url()."mpower/newCustomer"); }
		if($flag=="update"){ redirect(base_url()."mpower/updateCustomer/".$crm_id); }
		if($flag=="error"){ redirect(base_url()."mpower/newCustomer"); }
	}
	...
	public function crm_entry_delete()
	{
		...
		if(!empty($crmdeletID))
		{
			...
			{			
				...
				$this->db->delete('crm_mpower');
				
				$this->db->where('crm_id', $crmdeletID);
				$this->db->delete('crm_mpower_logs');
				echo "DELETED";
