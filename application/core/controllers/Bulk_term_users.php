...
class Bulk_term_users extends CI_Controller {
				...
				try{
					...
					foreach ($objPHPExcel->getWorksheetIterator() as $worksheet){
						...
						for ($col = 0; $col <= $highestColumnIndex; $col++) {
							...
							}else if(strtolower($val) =="term_date"){
								$col_indexs .=",".$col;
								$termdate_index=$col;
							}
							...
							for ($row = 2; $row <= $highestRow; $row++) {
								...
								try{
									...
									if($status==1 || $status==7 ){
										...
										if($term_date=="") $term_date=$evt_date;
										else $term_date = $term_date . " 16:16:16";
										
										$start_date = mysqlDt2yymmddDate($term_date);
										$end_date = $start_date;
										//$lwd = $start_date;
										
										$qSql="select date(max(login_time)) as last_login_time, date(max(logout_time)) as last_logout_time from signin a left join logged_in_details b on a.id=b.user_id where fusion_id='$fusion_id'";
			
										$LDQuery = $this->db->query($qSql);
										$ldRow=$LDQuery->row_array();
										$lwd=$ldRow["last_login_time"];
										$last_logout_time=$ldRow["last_logout_time"];
										...
										$this->updateFurloughWhenTerm($user_id,$start_date,"Update by Term Process");
										$this->updateBenchWhenTerm($user_id,$start_date,"Update by Term Process");
