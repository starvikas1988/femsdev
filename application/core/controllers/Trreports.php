...
class Trreports extends CI_Controller {
	...
	public function download_assmnt_dtls(){
		if(check_logged_in()){
				...
				$this->generate_assessment_xls($exam_id,$asmnt_name,$assmnt_id,$examDtls);
			}	
		}
	}
	
	
	private function generate_assessment_xls($exam_id, $heading, $assmnt_id, $set_count ){
		...
		$style_header = array(
			...
			//$objWorksheet->getStyle("A1:P1")->applyFromArray($style_header);
			...
			$sheet->mergeCells('A1:J1');
					...
					$row1=2; 	
						...
						$SQLtxt="Select fusion_id,fname,lname,(Select CONCAT(fname,' ' ,lname) from signin where signin.id=s.assigned_to) as asign_tl, user_id,set_id,exam_id,exam_schedule_id, ua.ques_id, no_of_question, ans_id, QA_O.text AS User_answer, QA_O.correct_answer from lt_exam_schedule es 
						inner JOIN lt_questions y on y.set_id = es.allotted_set_id
						inner JOIN lt_user_exam_answer ua on ua.exam_schedule_id = es.id and ua.ques_id = y.id
						LEFT JOIN lt_questions_ans_options QA_O ON QA_O.id=ua.ans_id
						left JOIN signin s on s.id = es.user_id
						Where exam_id =". $exam_id ." and allotted_set_id = ". $no_set['allotted_set_id'] . " 
						AND module_id = '$assmnt_id' AND module_type = 'TR'
						...
						$cnt=1; 
							...
							//$this->objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($style_def);
							 ...
							 if($recRow['User_answer'] =='') $recRow['User_answer'] ="NA";
								...
								if($recRow['fusion_id'] == $old_fusion_id ){
									...
									if($recRow['correct_answer']==1){
										//$this->objPHPExcel->getActiveSheet()->getStyle($cell)->applyFromArray($style_green);
									}
