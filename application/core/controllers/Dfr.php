...
//////////////////////////////////End PDF Part///////////////////////////////////////////	
	...
	public function view_requisition($id)
	{
		if(check_logged_in())
		{
			...
			if(get_role_dir()!="super" && get_global_access()!=1){
				...
				if($user_office_id=="CEB" || $user_office_id=="MAN") $tl_cnd=" and ( office_id in ('CEB','MAN') ) ";
			...
			// $data['location'] = $user_office_id;			
			$data["qualification_list"] = $qual_quary->result_object();
			
			$qSqlcomp = "Select * from master_company order by name asc";
			$data["company_list"] = $this->Common_model->get_query_result_array($qSqlcomp);
	...
	public function pending_candidate(){
		if(check_logged_in()){
			...
			if($req_status=="") $req_status = trim($this->input->get('req_status'));
			if($req_status=="") $req_status=0;
			if($requisition_id != "") $req_status=2;
			
			if($req_status==0) $_filterCond .=" and requisition_status in ('A')";
			else if($req_status==1) $_filterCond .=" and requisition_status='CL'";
	...
	public function selected_candidate(){
		if(check_logged_in()){
			...
			$bank = "Select bank_name,branch_name,acc_type, bank_acc_no, ifsc_code from dfr_candidate_details as dcd where id=2375";
			$data["bank_data"] = $this->Common_model->get_query_result_array($bank);
			// print_r($data["bank_data"]);
			// exit();
...
public function view_uploaded_docs(){
		...
		if(isIndiaLocation($location)==true) $view_page="dfr/documents_popup_ind";
		else if(isUSLocation($location)==true) $view_page="dfr/documents_popup_us";
		else if($location == 'CEB' || $location == 'MAN') $view_page="dfr/documents_popup_phil";
		else if($location == 'JAM') $view_page="dfr/documents_popup_jam";
		else if($location == 'ELS') $view_page="dfr/documents_popup_els";
		else $view_page="dfr/documents_popup";
...
public function verifyDocuments(){
		...
		if(check_logged_in()){
			...
			$r_id = trim($this->input->post('r_id'));
			$is_verify_doc = trim($this->input->post('is_verify_doc'));
			$current_user = get_user_id();
			...
			if($c_id!=""){
				...
				$field_array = array(
					"is_verify_doc" => $is_verify_doc,
					"doc_verify_by" => $current_user,
					"doc_verify_on" => $added_date
				);
			...
			$referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : "dfr/selected_candidate";
			redirect($referer);	
...
/////////////////All Candidates List///////////////////////
	...
	public function process_upload()
	{
        ...
        // print_r($this->upload_error);
		if(empty($this->upload_error)) $pan = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_pan.".$ext;

		$path       = $_FILES['adhar']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		if(empty($this->upload_error)) $adhar = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_adhar.".$ext;

		$path       = $_FILES['adhar_back']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		if(empty($this->upload_error)) $adhar_back = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_adhar_back.".$ext;

		$path       = $_FILES['photo']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		if(empty($this->upload_error)) $photo = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_photo.".$ext;
		...
		if($data){
					$uparray = array(
						...
						"attachment_adhar_back" => $adhar_back,
						"attachment_pan" => $pan,
	...
	private function upload_pan($infos)
	{
		...
		$path       = $_FILES['pan']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_pan.".$ext;
        $config['overwrite']      = 0;
        $config['max_size']       = 0;
        $config['max_width']      = 0;
        $config['max_height']     = 0;
	...
	private function upload_photo($infos)
	{
		...
		$path       = $_FILES['photo']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_photo.".$ext;
        $config['overwrite']      = 0;
        $config['max_size']       = 0;
        $config['max_width']      = 0;
        $config['max_height']     = 0;
	...
	private function upload_adhar($infos)
	{
		...
		$path       = $_FILES['adhar']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_adhar.".$ext;
        $config['overwrite']      = 0;
        $config['max_size']       = 0;
        $config['max_width']      = 0;
        $config['max_height']     = 0;
	...
	private function upload_adhar_back($infos)
	{
		// print_r($infos);
		// exit;
		...
		$path       = $_FILES['adhar_back']['name'];
        $ext        = pathinfo($path, PATHINFO_EXTENSION);
		$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_adhar_back.".$ext;
        $config['overwrite']      = 0;
        $config['max_size']       = 0;
        $config['max_width']      = 0;
        $config['max_height']     = 0;
	...
	private function upload_expfile($infos)
	{
		...
		foreach ($_FILES['exp']['name'] as $key => $value) {
			...
			$path       = $_FILES['exp']['name'][$key];
	        $ext        = pathinfo($path, PATHINFO_EXTENSION);
			$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_experience".$key.".".$ext;
	        $config['overwrite']      = 0;
	        $config['max_size']       = 0;
	        $config['max_width']      = 0;
	        $config['max_height']     = 0;
	...
	private function upload_edufile($infos)
	{
		foreach ($_FILES['edu']['name'] as $key => $value) {
			...
			$path       = $_FILES['edu']['name'][$key];
	        $ext        = pathinfo($path, PATHINFO_EXTENSION);
			$config['file_name']      = $infos[0]['fname']."_".$infos[0]['lname']."_".$c_id."_education".$key.".".$ext;
	        $config['overwrite']      = 0;
	        $config['max_size']       = 0;
	        $config['max_width']      = 0;
	        $config['max_height']     = 0;
...
//////////////////////////////////////////////////////////////////////////////	
	...
	public function send_basic_link()
	{
		if(check_logged_in())
		{
			...
			$onboarding_type = trim($this->input->post('onboarding_typ'));
		...
		if(check_logged_in())
		{
			...
			if($fname!=""){
				...
				$field_array = array(
					...
					"company" => trim($this->input->post('company')),
					"onboarding_type" => trim($this->input->post('onboarding_type')),
					"candidate_status" => 'P',
					"added_by" => $current_user,
					"added_date" => $added_date,
					"log" => $log
				...
				if($this->input->post('onboarding_type')=='NAPS'){
					$nps_array=array(
						'enroll_state'=> $this->input->post('onboarding_type'),
						'dfr_id'=>$rowid,
						'nap_stat'=>0
					);
					data_inserter('dfr_naps_candidate_details',$nps_array);
				}
				

				//data_inserter('dfr_candidate_details',$field_array);
                ...
                {	
					...
					if(isValidMysqlDate($doj,'Y-m-d') == true){
						...
						if($user_id!==FALSE)
						{
							...
							if(isIndiaLocation()==true ){
								$bank_array = array(
									...
									"ifsc_code" => $ifsc_code,
									"is_accept" => 1,
									"accepted_time" => CurrMySqlDate(),
									// "bank_doc" => $candidate_documents[0]['attachment_bank'],
									"upl_bank_info" => $candidate_documents[0]['attachment_bank'],
									"log" => $log
								);
								$rowid= data_inserter('info_bank',$bank_array);
						    }
						...
						if(!empty($candidate_documents[0]['attachment_pan'])){
							rename($BaseRealPath.'/uploads/pan/'.$candidate_documents[0]['attachment_pan'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_pan']);
						}
						if(!empty($candidate_documents[0]['attachment_birth_certificate'])){
							rename($BaseRealPath.'/uploads/candidate_birth/'.$candidate_documents[0]['attachment_birth_certificate'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_birth_certificate']);
						}
						if(!empty($candidate_documents[0]['attachment_health_insurence'])){
							rename($BaseRealPath.'/uploads/candidate_insurence/'.$candidate_documents[0]['attachment_health_insurence'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_health_insurence']);
						}
						if(!empty($candidate_documents[0]['attachment_nbi_clearence'])){
							rename($BaseRealPath.'/uploads/candidate_nbi_clearence/'.$candidate_documents[0]['attachment_nbi_clearence'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_nbi_clearence']);
						}
						if(!empty($candidate_documents[0]['attachment_sss'])){
							rename($BaseRealPath.'/uploads/candidate_aadhar/'.$candidate_documents[0]['attachment_sss'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_sss']);
						}
						if(!empty($candidate_documents[0]['attachment_tin'])){
							rename($BaseRealPath.'/uploads/candidate_pan/'.$candidate_documents[0]['attachment_tin'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_tin']);
						}
						if(!empty($candidate_documents[0]['attachment_tax'])){
							rename($BaseRealPath.'/uploads/candidate_pan/'.$candidate_documents[0]['attachment_tax'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_tax']);
						}
						if(!empty($candidate_documents[0]['attachment_nis'])){
							rename($BaseRealPath.'/uploads/candidate_insurence/'.$candidate_documents[0]['attachment_nis'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_nis']);
						}
						if(!empty($candidate_documents[0]['attachment_adhar'])){
							rename($BaseRealPath.'/uploads/candidate_aadhar/'.$candidate_documents[0]['attachment_adhar'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_adhar']);
						}
						if(!empty($candidate_documents[0]['attachment_adhar_back'])){
							rename($BaseRealPath.'/uploads/candidate_aadhar_back/'.$candidate_documents[0]['attachment_adhar_back'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_adhar_back']);
						}
						if(!empty($candidate_documents[0]['attachment_bank'])){
							rename($BaseRealPath.'/uploads/dfr_bank/'.$candidate_documents[0]['attachment_bank'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['attachment_bank']);
						}
						if(!empty($candidate_documents[0]['photo'])){
							rename($BaseRealPath.'/uploads/photo/'.$candidate_documents[0]['photo'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['photo']);
						}
						...
						if(!empty($candidate_documents[0]['experience_doc'])){
							rename($BaseRealPath.'/uploads/experience_doc/'.$candidate_documents[0]['experience_doc'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['experience_doc']);
						}
						// print_r($candidate_documents);
						...
						if(!empty($candidate_documents[0]['education_doc'])){
							rename($BaseRealPath.'/uploads/education_doc/'.$candidate_documents[0]['education_doc'], $destination_path.'/'.$fusion_id.'/'.$candidate_documents[0]['education_doc']);
						}
						// exit;
						

						$iSql="Insert into info_document_upload (user_id,pan_doc,aadhar_doc,aadhar_doc_back,nis_doc,birth_certi_doc,photograph,sss_no_doc,tin_no_doc,philhealth_no_doc,nbi_clearance_doc) select '$user_id' as user_id, attachment_pan,attachment_adhar,attachment_adhar_back,attachment_nis,attachment_birth_certificate,photo,attachment_sss,attachment_tin,attachment_health_insurence,attachment_nbi_clearence from dfr_candidate_details where id = '$c_id'";
						$this->db->query($iSql);
...
////////////////////////////
	...
	public function pool_requisition()
	{
		if(check_logged_in())
		{
			...
			$onboarding_typ = $this->input->get('onboarding_typ');
			...
			if($oValue=="") $oValue = trim($this->input->get('office_id'));
			// $cStatus = 'P&IP';
			$cStatus = trim($this->input->get('candidate_status'));
			if($cStatus=="") $cStatus = trim($this->input->get('candidate_status'));
			...
			if($cStatus!="")
			{
				switch($cStatus){
					...
					break;
					case 'SLCS' : 
						$a = "c.candidate_status IN ('SL','CS')";
					break;
					case 'RD' : 
						$a = "c.candidate_status IN ('R','D') ";
					break;
				}
				 $_filterCond .= " And $a";
				//  var_dump($_filterCond);
			}

			
			
			$data['onboarding_typ']=$onboarding_typ;
			if(get_role_dir()=="super" || get_global_access()==1){
				$data['location_list'] = $this->Common_model->get_office_location_list();
			}else{
				$data['location_list'] = $this->Common_model->get_office_location_session_all($current_user);
			...
			$qSql= "Select c.*, c.id as can_id, DATE_FORMAT(dob,'%m/%d/%Y') as d_o_b, 
			       DATE_FORMAT(doj,'%m/%d/%Y') as d_o_j,  (select concat(fname, ' ', lname)
				    as name from signin x where x.id=c.added_by) as added_name from
					 dfr_candidate_details c where  r_id IS NULL  $_filterCond  
					 ORDER BY FIELD(c.candidate_status, 'P', 'IP', 'SL', 'CS', 'E', 'R') ";
					//  var_dump($qSql);
	...
	public function getRefConsultant()
	{
		if(check_logged_in()){
			...
			if($refname=='Job Portal'){
				...
				//$qSql="select concat(fname,' ',lname) as full_name from signin where dept_id=3 and status=1 and office_id in('KOL', 'HWH', 'BLR', 'CHE', 'NOI','MUM')";
				
				$qSql="select concat(fname,' ',lname) as full_name from signin where dept_id=3 and status=1 ";
	...
	public function bench_user_list(){
		if(check_logged_in()){
			...
			if( $is_global_access!=1){
				$_filterCond .=" where (office_id='$user_office_id' OR '$user_oth_office' like CONCAT('%',office_id,'%') )";
			}else{
				if($off_id=='All'){
					$_filterCond ="";
				}else{
					$_filterCond .=" where office_id='$off_id'";
				}
			}
			...
			$qSql="SELECT * from user_bench as u
				   ...
				   LEFT JOIN (SELECT abbr as shortform, office_name,location from office_location) as o ON o.shortform = s.office_id
				   ...
				   LEFT JOIN (SELECT id as roid, name as roleorgname from role_organization) as rg ON rg.roid = s.org_role_id $_filterCond";
	...
	{	
		...
		if(isIndiaLocation($loc) == true){
			...
			$personalSql ="SELECT * FROM dfr_candidate_details where id='$c_id'";
			$candidate_details = $this->Common_model->get_query_result_array($personalSql);
			// $candidate_details ="";
			
			if($candidate_details){
				
				$email = $candidate_details[0]['email'];
				$emailArr=explode("@",$email);
				$phone = $candidate_details[0]['phone'];
				$alter_phone = $candidate_details[0]['alter_phone'];
				$adhar = $candidate_details[0]['adhar'];
				$search_by="Search By:: <b>";
				
				$qSql ="SELECT s.id as exuser_id, s.fusion_id,s.fname,s.lname,s.doj,infop.social_security_no, infop.email_id_off, infop.email_id_per,infop.phone, infop.phone_emar,t.*, (select  name from master_term_type mt where mt.id=t.t_type ) as term_type, date(terms_date) as usr_term_date  FROM info_personal infop ";
				$qSql .= " left join signin s on s.id= infop.user_id ";
				$qSql .= " left join terminate_users t on t.user_id=infop.user_id where is_term_complete ='Y' and (";
				
				if($email != "" && strtoupper($emailArr[0]) !="NA"){
					$qSql .= " email_id_off like '%$email%' or email_id_per Like '%$email%' ";
					$search_by .= " email_id: $email";
				}
				if($phone != ""){
						$qSql .= " or phone Like '%$phone%'";
						$search_by .= " OR phone: $phone";
				}
				if($alter_phone != ""){
					$qSql .= " or phone_emar Like '%$alter_phone%'";
					$search_by .= " OR alt phone: $alter_phone";
				}
				
				if($adhar != ""){
					$qSql .= " or social_security_no like '%$adhar%'";
					$search_by .= " OR adhar: $adhar";
				}
				...
				$exUserInfo = $this->Common_model->get_query_result_array($qSql);
				// print_r($exUserInfo);
				if(!empty($exUserInfo)){
					
					$html= $search_by."<br><table id='default-datatable' data-plugin='DataTable' class='skt-table' cellspacing='0' cellpadding='2' width='100%'>";
					$html .="<tr class='bg-info'> <th>FEMS ID</th> <th>Name</th><th>DOJ</th><th>Rehire</th><th>Term Type</th><th>Term Date</th> <th>Term Comment</th></tr>";
					$html .="<tbody>";
					foreach($exUserInfo as $row){
						
						$html .="<tr>";
						$html .="<td>".$row['fusion_id']."</td>";
						$html .="<td>".$row['fname']. " ". $row['lname']."</td>";
						$html .="<td>".$row['doj']."</td>";
						$html .="<td>".$row['is_rehire']."</td>";
						$html .="<td>".$row['term_type']."</td>";
						$html .="<td>".$row['usr_term_date']."</td>";
					...
					$html="<b>No Previous Records Found</b>";	
				}
				
			}else{
				$html="<b>Candidate info not found</b>";	
			}
		}else{
			$html="hide";
		}
