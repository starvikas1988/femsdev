		...
		foreach($req_result as $key=>$value)
		{
			...
			else if($value->application_status == 'FilterScheCompl')
			{
				if((strtotime(date('Y-m-d H:i:s')) > strtotime(ConvLocalToServer($value->exam_end_time))) && $value->exam_status == 0)
				{
					...
					$response['datas'] .= '<td data-filter_test_time="'.$value->filter_test_scheduled.'" data-filter_test_end_time="'.$value->exam_end_time.'" data-exam_schedule_id="'.$value->exam_schedule_id.'"><span class="label label-success">Filter Test Scheduled</span></td>';
