...
//////////////////////////////	
		...
		public function index(){
			if(check_logged_in())
			{
				...
				if(get_role_dir()=="super" || get_role_dir()=="manager"){
					$qSql="SELECT * from (Select requisition_id FROM training) xx left join (Select id as rid, job_title as batch_code FROM dfr_requisition) yy ON (xx.requisition_id=yy.rid) group by batch_code";
					$data["get_assigned_batch"] = $this->Common_model->get_query_result_array($qSql);
				}else{
					$qSql="SELECT * from (Select requisition_id FROM training where trainer_id='$current_user') xx left join (Select id as rid, job_title as batch_code FROM dfr_requisition) yy ON (xx.requisition_id=yy.rid) group by batch_code";
					$data["get_assigned_batch"] = $this->Common_model->get_query_result_array($qSql);
				}
				...
				if($this->input->get('show')=='Show')
				{
					...
					if(get_role_dir()=="super" || get_role_dir()=="manager"){
						$qSql="SELECT * from (Select * FROM training where phase_status=2) xx left join 
						...
						$qSql="SELECT * from (Select * FROM training where phase_status=2 and trainer_id='$current_user') xx left join 
								(Select id as rid, job_title as batch FROM dfr_requisition) yy ON (xx.requisition_id=yy.rid) left join
								(select id, fusion_id, fname, lname, role_id, (select name from role r where r.id=role_id) as rolename from signin) zz ON (xx.user_id=zz.id) left join
								(select user_id as ip_userid, email_id_off, phone from info_personal) aa on (xx.user_id=aa.ip_userid)
								 $cond ";
		...
		public function add_assessment(){
			if(check_logged_in())
			{
				...
				$data["aside_template"] = "training/aside.php";
				$data["content_template"] = "training/add_assessment.php";
			...
			////////////////////
				...
				if($this->input->post('submit')=='SAVE'){
					...
					foreach ($uid_array as $key => $value){
						...
						$userid= data_inserter('training_assessment',$train_assess_Arr);
		...
		public function certification(){
			if(check_logged_in())
			{
				...
				$data["aside_template"] = "training/aside.php";
				$data["content_template"] = "training/certification.php";
				...
				if($this->input->get('show')=='Show')
				{
					...
					if(get_role_dir()=="super" || get_role_dir()=="manager"){
						$qSql="SELECT * from (Select * FROM training where phase_status=2) xx left join 
								...
								(select user_id as user_score, asmnt_name, asmnt_date, score from training_assessment) bb On (xx.user_id=bb.user_score)
			...
			////////////////////
				...
				if($this->input->post('submit')=='SAVE'){
					...
					foreach ($uid_array as $key => $value){	
						...
						$this->db->update('training', $userArr);
						
						$i++;
					}
					
					redirect('training/certification');
				}
			/////////////////	
	...
	/////////////////////////////////////////////////////////////////////
		public function assmnt_score(){
			if(check_logged_in()){
				...
				$qSql="Select * FROM training_assessment where user_id='$assmntUid'";
	...
	//////////////////////////////////////////////////
		...
		public function move_production(){
			if(check_logged_in())
			{
				...
				$data["aside_template"] = "training/aside.php";
				$data["content_template"] = "training/move_production.php";
			...
			////////////////////
				...
				if($this->input->post('submit')=='Move To'){
					...
					foreach ($uid_array as $key => $value){	
						...
						$this->db->where('user_id', $value);
						$this->db->update('training_assessment', $userArr);
					...
					redirect('training/move_production');
				}
			//////////////	
...
/*-------------------Manager View------------------------------*/		
		public function training_manager_view(){
			if(check_logged_in())
			{
				...
				$data["aside_template"] = "training/aside.php";
				$data["content_template"] = "training/trn_manager_view.php";
				
				$qSql = "SELECT * from (Select trainer_id, (select concat(fname, ' ', lname) as name from signin s where s.id=trainer_id) as trn_name from training group by trainer_id) xx left join (Select id, fusion_id, office_id from signin) yy on (xx.trainer_id=yy.id) where office_id='$user_office_id'";
				...
				foreach($assn_trainer as $row):
					...
					$QSql="SELECT * from (Select *, (select concat(fname, ' ', lname) as name from signin x where x.id=trainer_id) as trn_name FROM training where 
			...
			///////////	
				...
				if($this->input->post('submit')=='Assign Trainer & Batch'){
					...
					redirect('training/training_manager_view');
				}
			/////////	
				$this->load->view('dashboard',$data);
			}
