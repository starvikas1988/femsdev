...
class Client_qa_dipcheck extends CI_Controller {
	...
	public function index()
	{
		if(check_logged_in()){
			...
			$qSQL="SELECT * FROM client where id IN ($ses_client_id) AND is_active=1 ORDER BY shname";
			$query = $this->db->query($qSQL);
			$data['client_list'] = $query->result();
			
			$qSQL="SELECT id,name FROM process WHERE id IN ($ses_process_id) AND is_active=1 ORDER BY name";
			$query = $this->db->query($qSQL);
			$data['process_list'] = $query->result();
			
			$_filterCond="";
			$_filterCond.=" AND qa_dipcheck.process_id IN ($ses_process_id)";
	...
	public function dipcheck_report(){
		if(check_logged_in()){
			...
			$SQLtxt ="SELECT * FROM qa_dipcheck WHERE qa_dipcheck.process_id IN ($ses_process_id) ORDER BY id ASC";
