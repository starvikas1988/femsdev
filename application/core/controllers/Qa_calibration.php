...
class Qa_calibration extends CI_Controller {
...
/*----------------------------------------------------*/
	...
	private function parameter_score($table,$params_columns,$process_name,$from_date,$to_date){
		...
		$qSql='Select (select concat(fname, " ", lname) as name from signin s where s.id=entry_by) as entry_name, date(audit_date) as audit_date, auditor_type, '.$params_columns.', overall_score from '.$table.' where (date(audit_date)>="'.$from_date.'" and date(audit_date)<="'.$to_date.'") and audit_type="Calibration" '.$cond.' order by audit_date,auditor_type asc';
		$query = $this->db->query($qSql);
		return $query->result_array();
	}
