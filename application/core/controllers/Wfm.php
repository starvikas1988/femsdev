...
class Wfm extends CI_Controller {
    ...
    public function index()
    {
		...
		if(check_logged_in())
        {
			...
			if(get_dept_folder()!="wfm" && get_dept_folder()!="rta" ) redirect(base_url().get_role_dir()."/dashboard","refresh");
            			
			$data["aside_template"] = get_aside_template();
			$data["content_template"] = "wfm/dashboard.php";
			...
			if($filter_id==1){
				...
				$qSql="select * from (select * from (SELECT id,fusion_id,office_id,xpoid,dept_id,omuid,fname,lname,role_id,site_id,status,assigned_to, get_client_names(b.id) as client_name ,(Select shname from department s where s.id=b.dept_id) as dept_name,(Select name from sub_department sd where sd.id=b.sub_dept_id) as sub_dept_name,(Select name from event_master k where k.id=b.disposition_id) as disp_name, get_process_names(b.id) as process_name,(Select name from sub_process l where l.id=b.sub_process_id) as sub_process_name, (Select name from site z  where z.id=b.site_id) as site_name, (Select CONCAT(fname,' ' ,lname) from signin x where x.id=b.assigned_to) as asign_tl, (Select name from role a  where a.id=b.role_id) as role_name FROM signin b where b.status = 2)xx , (select t_type, sub_t_type, lwd, terms_date,comments, user_id as duid, (Select CONCAT(fname,' ' ,lname) from signin ts where ts.id=t.terms_by) as raised_by, (Select name from master_term_type mtt where mtt.id=t.t_type) as t_type_name,
