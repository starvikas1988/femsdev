...
class Leave extends CI_Controller {
	...
	public function index(){
		if(check_logged_in()){
			...
			$_ind_location = ['KOL', 'HWH', 'BLR', 'NOI', 'CHE'];
			$_jam_location = ['JAM'];
			$_cebu_location = ['CEB', 'MAN'];
			...
			foreach($leaves_available as $key => $leave){		
				...
				$_access = $leave->leave_access_allowed;
				
				if(($user_office_id == $_ind_location[0]) && leave_probation_extended() && $leave->extra_config_query=="" && $leave->show_after_days !=1){
					$_access = 0;
					
					$_l_struct[$key]["leave_access_allowed"] = 0;
				}
				

				if(in_array($user_office_id,$_cebu_location)){

					// Birthday Leave Access
	
					if($leave->leave_criteria_id == 38){
						$q = $this->db->query("select isApplyBDLeave(".$_user_id.") as leave_accessible");
						$res = $q->row();
						$_access = $res->leave_accessible;
					}
				}
				...
				$aa[] = array(
					...
					"leave_details" => array(
						...
						"leave_allowed" => $_access,
					)
				);	

				/*
				if(in_array($user_office_id, $_jam_location)){
					if($leave->leave_criteria_id == 30 && $aa["leave_details"]["leave_present"] > 9.9){
						$aa["leave_details"]["leave_present"] = $leave->leaves_present;
					}
				}
				*/
				...
				if($user_office_id == 'JAM'){
					...
					$n = $leave->leaves_present - (int)$leave->leaves_present;
					
					if($n == 0.96){
						$leave_present = round($leave->leaves_present);
					}else{
						if((int)$leave->leaves_present < 1){ 
							$leave_present = floor($leave->leaves_present);
						}else{
							$leave_present = round($leave->leaves_present);
						}
					}
					
					$var_js[$leave->leave_criteria_id] = array($leave->leave_type_id, $leave_present, $_leave_accessible, $leave->criteria, $leave->can_also_use_leave_criteria, $leave->has_sub_category, $rr);
				}else{
			...
			if($this->input->post()){
				...
				if($this->input->post('from_date')!="" && $this->input->post('to_date')!=""){
					...
					if($diff <= $var_js[$this->input->post('leave_criteria_id')][1] && $diff>=0){
						...
						if($this->input->post('sub_sections')!=""){
							$_field_array["sub_section_id"] = $this->input->post('sub_sections');
							$diff = $var_js[$this->input->post('leave_criteria_id')][6][$this->input->post('sub_sections')]["sub_deduction"];
						} 
						else $_field_array["sub_section_id"] = null;
					...
					$referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : "leave";
					redirect($referer);
	...
	*/
	...
	public function leave_approval(){
		if(check_logged_in()){
			...
			$data["leave_accessible"] = $this->Leave_model->get_leaves_accessible($_office_id);
	...
	public function set_leave_status(){
		if(check_logged_in()){
			...
			if($leave_status == '2'){
				...
				if($leave_criteria->id == 40){
					$_is_paid = $this->input->post("is_paid");
					$_no_of_days = $this->input->post("no_of_days");

					if($_is_paid !="" && $_no_of_days !=""){
						$data["is_paid"] = $_is_paid;
						$data["approve_for_no_of_days"] = $_no_of_days;
					}
				}
				
				if(get_assigned_to() == '8' || get_assigned_to() == '9'){
					$this->Email_model->self_leave_approval_emails(get_user_id(), $leave_id);
				}
	...
	public function leave_balance(){
		if(check_logged_in()){
			...
			if(is_access_leave_manage()==true || leave_approvers_location_specify("JAM", FALSE, ['FJAM004129','FJAM002694','FJAM004503'])){
				...
				else if(is_access_leave_manage()) $data["location_list"] = $this->Common_model->get_office_location_session_all($current_user);
	...
	*/
			...
			if($this->input->post('from_date') !="" && $this->input->post('to_date') !="" && $this->input->post('downloadReport')=='Download CSV'){
				...
				foreach($_date_range as $_date){
					...
					foreach($reports as $row){
						...
						else if($row->status == '3') $_status = 'Cancelled';
	...
	public function forlough(){
		if(check_logged_in()){
			$data["aside_template"] = "leave/aside.php";
			$data["content_template"] = "leave/forlough.php";
	...
	public function cebu_extra_leaves(){
		if(check_logged_in()){
			$data["user_office_id"] = $user_office_id = get_user_office_id();
			...
			$data["cebu_user_list"] = $this->Leave_model->get_cebu_users();

			$data["aside_template"] = "leave/aside.php";
			$data["content_template"] = "leave/cebu_extra_leaves.php";
			$this->load->view('dashboard',$data);
			...
			if($this->input->post()){
				...
				$leaves_present = $this->input->post('leaves_present');
				
				if($user_id!="" && $leaves_present > 0){
					
					$this->Leave_model->update_cebu_additional_leaves($leaves_present, $user_id);
				}
				
				redirect(base_url()."leave/cebu_extra_leaves", "refresh");
			}
	...
	public function deactivate_cebu_additional_leaves($user_id){
		if(check_logged_in()){
			$this->Leave_model->deactivate_cebu_additional_leaves($user_id);
			redirect(base_url()."leave/cebu_extra_leaves", "refresh");
		}
	}
