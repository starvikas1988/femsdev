...
class Homecha extends CI_Controller {
    ...
    public function index()
    {
		...
		///////////Birth Notification News//////////
			...
			if( get_update_pswd() == "Y") $data["content_template"] = "home/homecha.php";
		...
		//////////////////////////////////	
			...
			$sql_covid_consent = "SELECT * from covid19_screening_checkup WHERE date_added >= '$covid_start_date' AND 
			date_added <= '$covid_end_date' AND user_id = '$covid_current_user' ORDER by id DESC LIMIT 1";
			...
			if(get_update_pswd() == "N"){
			...
			}else if((empty($covid_consent_details['id']) && isIndiaLocation($user_office_id)==true) || (!empty($covid_consent_details['id']) && $covid_consent_details['employee_consent'] == 0 && isIndiaLocation($user_office_id)==true)){
				
				$data["content_template"] = "home/covid_checkup.php";
				...
				if($role_id > 0 ){
					...
					$data["content_template"] = "home/homecha.php";
				}
...
///////////////	
	...
	public function saveYourMood(){
		if(check_logged_in()){
			...
			$rowid = data_inserter('user_mood',$field_array);
