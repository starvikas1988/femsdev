...
class Pmetrix extends CI_Controller {
    ...
    public function index()
    {
        if(check_logged_in()){
			...
			if (get_role_dir() == "agent") $data["content_template"] = "pmetrix/metrix_screen_agent.php";
			else
			{
		...
		if (get_role_dir() == "agent"){
			...
			foreach($pm_design as $row){
				...
				//$qSql = "Select id,fusion_id,office_id, fname, lname, (Select name from role a  where a.id=b.role_id) as role_name, get_client_names(b.id) as client_name, get_process_names(b.id) as process_name from signin b where status=1 and id in (Select user_id from pm_data where mdid = $mp_id )  $condP  $cond ";
				...
				foreach($userlist as $urow){
					...
					if (get_role_dir() == "agent"){
						$qSql = "Select start_date from pm_data where user_id = $user_id group by start_date";
					}else{
						if($this->input->get('showReports') == 'Show')
						{
							$qSql = "Select start_date from pm_data where user_id = $user_id and start_date='$shMonDate' and end_date='$shSunDate'";
						}
						else
						{
							$qSql = "Select start_date from pm_data where user_id = $user_id and start_date >= '".date('Y-m-01')."' AND end_date <= '".date('Y-m-t')."'";
						}
					}
					...
					foreach($DateArray as $dtrow){
						...
						$get_all_data = "SELECT pm_data.*,get_client_names(pm_data.user_id) as client_name ,pm_design_kpi.kpi_name,pm_design_kpi.kpi_type FROM `pm_data`
							left join signin on pm_data.user_id=signin.id
							left join pm_design_kpi on pm_design_kpi.id=pm_data.kpi_id
							where signin.office_id='KOL' and get_client_ids(pm_data.user_id)='".$client_id."' and start_date >= '".date('Y-m-01')."' AND end_date <= '".date('Y-m-t')."'" ;
						$data['all_data']=$this->Common_model->get_query_result_array($get_all_data);
			...
			*/
			...
			$qSql="Select start_date,end_date,CONCAT(DATE_FORMAT(start_date,'%m-%d-%Y'),' To ' ,DATE_FORMAT(end_date,'%m-%d-%Y')) as shrange from pm_data group by start_date order by start_date desc";
   ...
   public function getDesignForm()
{
	if(check_logged_in())
    {
		...
		$html .= "<div class='col-md-12 kpi_input_row'>";
			...
			$html .= "<div class='col-md-2'><select class='form-control' name='summ_type[]' > ";
			...
			$html .= "<div class='col-md-2'><input type='text' value='". $kpiRow['target'] ."' class='form-control' placeholder='Target' name='target[]'></div>";
			$html .= "<div class='col-md-2'><input type='number' min='0' step='1' pattern='\d+' value='". $kpiRow['weightage'] ."' class='form-control' placeholder='Integer' name='weightage[]'></div>";
