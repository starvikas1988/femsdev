...
/////////////////VRS Agent part//////////////////////////	
	...
	public function agent_vrs_feedback()
	{
		...
		////////////
			...
			$qSql="Select count(id) as value from qa_vrs_lm_feedback where id  not in (select fd_id from qa_vrs_lm_agent_rvw) and agent_id='$current_user'";
			...
			// RP ANALYSIS //
			/* $qSql="Select count(id) as value from qa_vrs_rp_analysis_feedback where agent_id='$current_user'";
			$data["tot_agent_rp_analysis_feedback"] =  $this->Common_model->get_single_value($qSql);
			
			$qSql="Select count(id) as value from qa_vrs_rp_analysis_feedback where id  not in (select fd_id from qa_vrs_rp_analysis_agent_rvw) and agent_id='$current_user'";
			$data["tot_agent_rp_analysis_yet_rvw"] =  $this->Common_model->get_single_value($qSql);
				 */
			$from_date = '';
			$to_date = '';
			...
			if($this->input->get('btnView')=='View')
			{
				...
				$data["agent_lm_review_list"] = $this->Qa_vrs_model->get_agent_lm_review_data($field_array);
				...
				//$data["agent_rp_analysis_review_list"] = $this->Qa_vrs_model->get_agent_rp_analysis_review_data($field_array);
			////////////////////	
				if($from_date !="" && $to_date!=="" )  $cond= " Where (audit_date >= '$from_date' and audit_date <= '$to_date' ) ";
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_vrs_feedback $cond and agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit')) xx Left Join
				...
				//$data["agent_rp_analysis_review_list"] = $this->Qa_vrs_model->get_agent_not_rp_analysis_review_data($current_user);		
			//////////
				$qSql="SELECT * from
				(Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name,
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
...
////////////////////VRS Left Message////////////////////////	
	public function agent_vrs_lm_feedback_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_vrs/agent_vrs_lm_feedback_rvw.php";
			$data["agentUrl"] = "qa_vrs/agent_vrs_feedback";
