<?php 
 ...
 class Qa_alteras extends CI_Controller{
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$qSql="SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder ='agent') and dept_id=6 and  is_assign_client (id,167) or is_assign_client (id,168) or is_assign_client (id,192) or is_assign_process (id,354) or is_assign_process (id,355) or is_assign_process (id,356) or is_assign_process (id,357) or is_assign_process (id,358) or is_assign_process (id,387)  and status=1  order by name";
