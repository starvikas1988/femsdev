...
class Pmetrix_test_tl extends CI_Controller {
    ...
    public function index()
    {
        if(check_logged_in()){
			...
			if(strpos($_SERVER['HTTP_REFERER'], 'Pmetrix_v2_management') !== false)
				...
				$data["aside_template"] = "pmetrix_test_tl/aside.php";
			...
			$SQLtxt = "SELECT * from client where id in (". get_client_ids() .")";
			$data['client_list'] = $this->Common_model->get_query_result_array($SQLtxt);
			
			$SQLtxt = "SELECT * from process where id in (". get_process_ids() .")";
			$fields = $this->db->query($SQLtxt);
			$data['process_list'] = $fields->result();
			...
			$data["content_template"] = "pmetrix_test_tl/matrix_screen_tl_wise.php";
	...
	public function prepare_row($json=false){
		if(check_logged_in()){
			...
			$_form_data = $this->input->post();
			
			$_process_id = $_form_data['process_id'];
			$_client_id = $_form_data['client_id'];
			$_office_id = $_form_data['office_id'];
			$_current_user = get_user_id();
			$_user_id = $_form_data['user_id'];
			$_all_tls = $_form_data['others_team'];
			
			
			if($_all_tls !=0) $_current_user = $_all_tls;
			
			$_design = $this->Pmetrix_model->get_available_design($_client_id, $_process_id, $_office_id);
			
			if($_form_data['process_id'] == 30){
				$month = substr($_form_data['post_period'],4);
				$year = substr($_form_data['post_period'],0,4);
				
				$start_date = $year.'-'.$month.'-01';
			
				$end_date = date("Y-m-t", strtotime($start_date));
				
				$_form_data['performance_for_month'] = $start_date;
				$_form_data['performance_for_year'] = $end_date;
			}
			$start_date = $_form_data['performance_for_month'];
			$end_date = $_form_data['performance_for_year'];
			$start_date_array = explode('-',$end_date);
			
			$data["design"] = $_design;
			$data["year"] = $start_date_array[0];
			$data["month"] = $start_date_array[1];
				
			$data["agent_list"] = $this->Pmetrix_model->get_available_agent_list_ids($_design,$start_date,$end_date,$current_user);
			$data['kpi_col_new'] = $this->Pmetrix_model->get_kpi_column($_design);	
			$data['grade_new'] = $this->Pmetrix_model->get_grade_bucket($_design, $start_date_array[1], $start_date_array[0]);
						
			$data["all_users_kpi_data"] = $this->Pmetrix_model->get_kpi_data($_design, $start_date,$end_date, false, true);
											
			echo $this->load->view('pmetrix_v2_agent/more_functions_page',$data,true);
		}
	}
	...
	public function prepare_tl_row(){
		...
		else
		{
			...
			foreach($available_design as $key=>$value)
			{
				...
				$data["design"] = $value->design_id;
				$data["year"] = $start_date_array[0];
				$data["month"] = $start_date_array[1];
				
				$data["agent_list"] = $this->Pmetrix_model->get_available_agent_list_ids($value->design_id, $start_date, $end_date, $current_user);
				...
				$data['kpi_data_new'] = $this->Pmetrix_model->get_kpi_data_same_process($value->design_id, $start_date, $end_date,$current_user);
				
				/*
				$_main_data = set_all_kpi_data($data['kpi_col_new'], $data['kpi_data_new'], $data["agent_list"], $data['grade_new'], $value->design_id, $data["year"], $data["month"]);
				*/
				...
				//print_r($data['tl_kpi_data'][$value->design_id]);
		...
		//pm_log_record($current_user,'pm_tl_tl_view','pm',$form_data);
		
		if($form_data['process_id'] == 30)
		{
			echo $this->load->view('pmetrix_v2_tl/view_data_page_tl_vrs_test',$data,true);
		}
		else
		{
	...
	public function get_process_tl()
	{
		$form_data = $this->input->post();
		if($form_data['process_id'] == 30){
			$qSql="SELECT DISTINCT kpi_value,signin.fname,signin.lname,signin.id FROM `pm_data_v2`
				LEFT JOIN signin ON signin.fusion_id=kpi_value
				LEFT JOIN pm_design_kpi_v2 ON pm_design_kpi_v2.id=pm_data_v2.kpi_id
				LEFT JOIN pm_design_v2 ON pm_design_v2.id=pm_design_kpi_v2.did
				WHERE pm_design_kpi_v2.kpi_name = 'Sup_FEMS_ID' AND signin.fname != '' AND pm_design_v2.process_id = '".$form_data['process_id']."' AND pm_design_v2.office_id='".$form_data['office_id']."'";
		}
		else{
	...
	public function get_agent_list()
	{
		...
		if($form_data['process_id'] == 30)
		{
			$qSql = 'SELECT agent.id,agent.fusion_id,agent.fname,agent.lname FROM `pm_data_v2`
				LEFT JOIN pm_design_kpi_v2 ON pm_design_kpi_v2.id=pm_data_v2.kpi_id
				LEFT JOIN pm_design_v2 ON pm_design_kpi_v2.did=pm_design_v2.id
				LEFT JOIN signin ON signin.fusion_id=pm_data_v2.kpi_value
				LEFT JOIN signin AS agent ON agent.id=pm_data_v2.user_id
				WHERE pm_design_kpi_v2.kpi_name="Sup_FEMS_ID" AND pm_design_v2.office_id="'.$form_data['office_id'].'" AND pm_design_v2.process_id="'.$form_data['process_id'].'"'; 
				
				if($form_data['others_team'] !='0') $qSql .= 'AND signin.id="'.$form_data['others_team'].'"';
				$qSql .= ' GROUP BY user_id';
		}
		else
		{
			$qSql = 'SELECT signin.id,signin.fname,signin.lname,signin.fusion_id FROM `signin` LEFT JOIN info_assign_process ON info_assign_process.user_id=signin.id LEFT JOIN role_organization ON role_organization.id=signin.org_role_id WHERE office_id="'.$form_data['office_id'].'" AND info_assign_process.process_id="'.$form_data['process_id'].'" AND role_organization.controller="agent" AND signin.status=1';
			
			if(count($query_array)>0) $qSql .=" AND ";
			$qSql .=implode(' AND ',$query_array);
		}
	...
	private function get_available_design($form_data=false)
	{
		...
		$query = $this->db->query('SELECT pm_design_v2.id AS design_id,pm_design_v2.mp_type FROM `pm_design_v2`
			WHERE process_id="'.$form_data['process_id'].'" AND client_id="'.$form_data['client_id'].'" AND office_id="'.$form_data['office_id'].'" 
			AND is_active=1');
	...
	private function get_grade_bucket($design_id,$year,$month)
	{
		...
		//print 'SELECT * FROM `pm_grade_v2` WHERE did="'.$design_id.'" AND month="'.$month.'" AND year="'.$year.'"';
	...
	{
		$query = 'SELECT user_id,signin.fusion_id,signin.fname,signin.lname FROM pm_data_v2 LEFT JOIN signin ON signin.id=pm_data_v2.user_id WHERE pm_data_v2.mdid="'.$did.'" AND start_date >= "'.$start_date.'" AND end_date <="'.$end_date.'" and signin.assigned_to="'.$current_user.'" group by user_id';
		
		if($current_user==0) {
			$query = 'SELECT user_id,signin.fusion_id,signin.fname,signin.lname FROM pm_data_v2 LEFT JOIN signin ON signin.id=pm_data_v2.user_id WHERE pm_data_v2.mdid="'.$did.'" AND start_date >= "'.$start_date.'" AND end_date <="'.$end_date.'" group by user_id';
		}
	...
	{
		$sql = 'SELECT tl.fusion_id as tl_fusion_id,tl.fname as tl_fname,tl.lname as tl_lname,signin.fusion_id, 
		pm_design_kpi_v2.kpi_name,pm_data_v2.kpi_value,
		pm_data_v2.start_date,pm_design_kpi_v2.weightage,pm_design_kpi_v2.summ_formula,signin.fname as agent_fname,
		signin.lname as agent_lname,pm_design_kpi_v2.kpi_type,pm_design_kpi_v2.summ_type,pm_design_kpi_v2.agent_view,
		pm_design_kpi_v2.tl_view,pm_design_kpi_v2.management_view,pm_design_kpi_v2.weightage_comp ,pm_design_kpi_v2.currency
		FROM `pm_data_v2` LEFT JOIN pm_design_kpi_v2 ON pm_design_kpi_v2.id=pm_data_v2.kpi_id
		LEFT JOIN signin ON signin.id=pm_data_v2.user_id
		LEFT JOIN signin as tl ON tl.id=signin.assigned_to
		WHERE mdid="'.$design_id.'" AND start_date >= "'.$start_date.'" 
		AND end_date <= "'.$end_date.'"';
		
		if($tl_id!="1"){
			$sql .= ' AND signin.assigned_to='.$tl_id;
			
		}else{
			$sql .= ' AND signin.assigned_to=16382';
		}
	...
	public function get_post_period()
	{
