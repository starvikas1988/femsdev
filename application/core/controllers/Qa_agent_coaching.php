 ...
 class Qa_agent_coaching extends CI_Controller{
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$data['agent_name']=$this->Common_model->get_query_result_array("SELECT DISTINCT Q.agent_id, CONCAT(S.fname, ' ', S.lname) as agent_name from qa_coaching_feedback Q
						inner join  signin S on Q.agent_id = S.id ORDER BY agent_name ASC");		
			...
			if($from_date !="" && $to_date!="" ){ 
				if(get_global_access()=='1' || $client_id==0){
					$cond = " where  (audit_date >= '$from_date' and audit_date <= '$to_date')";
				}else if(get_dept_folder()=="qa"){
					$cond = " where  (Q.audit_date >= '$from_date' and Q.audit_date <= '$to_date') and Q.entry_by='$current_user'";
				}else{
					$cond= " where  (Q.audit_date >= '$from_date' and Q.audit_date <= '$to_date') and (Q.agent_id in (SELECT id FROM signin WHERE (assigned_to='$current_user' OR (assigned_to IN (SELECT id FROM signin WHERE  assigned_to ='$current_user')) OR (assigned_to IN (SELECT id FROM signin WHERE assigned_to IN (SELECT id FROM signin WHERE  assigned_to ='$current_user' ))))))";
				}
			}else{
				if(get_global_access()=='1' || $client_id==0){
					$cond = "where  1";
				}else if(get_dept_folder()=="qa"){
					$cond = " where Q.entry_by='$current_user'";
				}else{
					$cond=" where (Q.agent_id in (SELECT id FROM signin WHERE (assigned_to='$current_user' OR (assigned_to IN (SELECT id FROM signin WHERE  assigned_to ='$current_user')) OR (assigned_to IN (SELECT id FROM signin WHERE assigned_to IN (SELECT id FROM signin WHERE  assigned_to ='$current_user' ))))))";
				}
			}

				//$qSql="SELECT id,(select concat(fname, ' ', lname) as name from signin s where s.id=qa_coaching_feedback.entry_by) as auditor_name,audit_date,(select concat(fname, ' ', lname) from signin s where s.id=qa_coaching_feedback.agent_id) as agent_name,(select concat(fname, ' ', lname) from signin s where s.id=(SELECT `assigned_to` FROM `signin` WHERE id=qa_coaching_feedback.agent_id)) as tl_name,(select name from process p where p.id=(SELECT `process_id` FROM info_assign_process as iap WHERE iap.user_id=qa_coaching_feedback.agent_id)) as process_name,(select comment from qa_coaching_agent_rvw qag where qag.cf_id=qa_coaching_feedback.id) as agent_feedback from qa_coaching_feedback $cond";
				
				/* $qSql="SELECT qa_coaching_feedback.id,(SELECT CONCAT(fname, ' ', lname) AS NAME FROM signin s WHERE s.id=qa_coaching_feedback.entry_by) AS auditor_name,audit_date,
						(SELECT CONCAT(fname, ' ', lname) FROM signin z WHERE z.id=qa_coaching_feedback.agent_id) AS agent_name,
						(SELECT CONCAT(fname, ' ', lname) FROM signin l WHERE l.id=(SELECT `assigned_to` FROM `signin` m WHERE m.id=qa_coaching_feedback.agent_id)) AS tl_name,
						get_process_names(qa_coaching_feedback.agent_id) AS process_name,
						(SELECT COMMENT FROM qa_coaching_agent_rvw qag WHERE qag.cf_id=qa_coaching_feedback.id) AS agent_feedback
						FROM qa_coaching_feedback $cond"; */
				...
				$qSql="SELECT Q.*, CONCAT(S.fname, ' ', S.lname) as agent_name, CONCAT(T.fname, ' ', T.lname) as tl_name, get_process_names(Q.agent_id) AS process_name,
						QC.comment as agent_feedback, CONCAT(A.fname, ' ', A.lname)  AS auditor_name,audit_date
						...
						$cond AND Q.agent_id $par $agent_name";
						
			$data["qa_agent_coaching_data"] = $this->Common_model->get_query_result_array($qSql);

			$tcnt=count($data["qa_agent_coaching_data"]);
			$total_data=array();
			$ack_data=array();
			$pending_data=array();

			for ($i=0; $i <$tcnt ; $i++) { 				
					$total_data[]=$data["qa_agent_coaching_data"][$i];
				if($data["qa_agent_coaching_data"][$i]['agent_feedback']!=""){
					 $ack_data[]=$data["qa_agent_coaching_data"][$i];
				}else{
					$pending_data[]=$data["qa_agent_coaching_data"][$i];
				}				
			}

			$data['total']=count($total_data);
			$data['ack_count']=count($ack_data);
			$data['NotAck_count']=count($pending_data);
			
				if(isset($_GET['agent_feedback'])){
					
				if ($_GET['agent_feedback']=="all") {
					$data['sel']=$this->input->get('agent_feedback');
					$data["qa_agent_coaching_data"]=$total_data;
				}elseif ($_GET['agent_feedback']=="ack") {
					$data['sel']=$this->input->get('agent_feedback');
					$data["qa_agent_coaching_data"]=$ack_data;
				}elseif ($_GET['agent_feedback']=="NotAck") {
					$data['sel']=$this->input->get('agent_feedback');
					$data["qa_agent_coaching_data"]=$pending_data;
				}else{
					$data["qa_agent_coaching_data"]=$pending_data;	
				}
			}else{
			...
			$data["agent_name_show"] = $agent_name;
	...
	public function add_feedback(){
		if(check_logged_in())
		{
			...
			if(!empty($this->input->post('call_date'))){
				// echo $this->input->post('call_date');
				// die;
				$date=explode(" ",$this->input->post('call_date'));
				$date=mmddyy2mysql($date[0])." ".$date[1];	
			}
			...
			if($this->input->post('agent_id')){
				...
				//data_inserter('qa_coaching_agent_rvw',$field_array1);
	...
	public function mgnt_agent_coaching_rvw($id){
		if(check_logged_in()){
			...
			(select office_abbr from site where site.id=(SELECT site_id from signin sst where sst.id=Q.agent_id)) as site_name
				from qa_coaching_feedback Q
				inner join  signin S on Q.agent_id = S.id
				inner join signin T on T.id = S.assigned_to
				inner join signin A on Q.entry_by = A.id
...
/////////////////////////Agent part/////////////////////////////////	
	...
	public function agent_coaching_feedback()
	{
		if(check_logged_in())
		{
			...
			$from_date = $this->input->get('from_date');
			$to_date = $this->input->get('to_date');
			//$cond="Where qa_coaching_feedback.agent_id=$current_user";
			
			if($from_date!="") $from_date = mmddyy2mysql($from_date);
			if($to_date!="") $to_date = mmddyy2mysql($to_date);
			
			if($from_date !="" && $to_date!=="" )  $cond= " Where (Q.audit_date>='$from_date' and Q.audit_date<='$to_date' and Q.agent_id='$current_user')";
