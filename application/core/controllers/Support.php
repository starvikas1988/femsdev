...
class Support extends CI_Controller {
    ...
    public function index()
    {
		...
		if(check_logged_in())
        {
			...
			if(get_role_dir()!="support") redirect(base_url().get_role_dir()."/dashboard","refresh");
			$data["aside_template"] = get_aside_template();
            $data["content_template"] = "support/dashboard.php";
			...
			$cond= " where status in  (1,4) and role_id>1 ".$_filterCond;
			$data['total_agent'] = $this->Common_model->get_total("signin b", $cond );
			
			$cond= " where status in  (1,4) and is_logged_in=0 and role_id>1 ".$_filterCond;
			$data['total_offline'] = $this->Common_model->get_total("signin b",$cond);
			
			$cond= " where status in  (1,4) and is_logged_in=1 and role_id>1".$_filterCond ;
			$data['total_online'] = $this->Common_model->get_total("signin b",$cond);
			
			$cond= " where status in  (1,4) and is_logged_in=0 and disposition_id in(2,3,4,5) and role_id>1 " . $_filterCond ;
			...
			if($filter_id==1){
				$mCond=" and role_id>1 ".$_filterCond;
				...
				$qSql="select * from (select * from (SELECT id,fusion_id,office_id,dept_id,omuid,fname,lname,role_id,site_id,status,assigned_to, get_client_names(b.id) as client_name ,(Select shname from department s where s.id=b.dept_id) as dept_name,(Select name from sub_department sd where sd.id=b.sub_dept_id) as sub_dept_name,(Select name from event_master k where k.id=b.disposition_id) as disp_name, get_process_names(b.id) as process_name,(Select name from sub_process l where l.id=b.sub_process_id) as sub_process_name, (Select name from site z  where z.id=b.site_id) as site_name, (Select CONCAT(fname,' ' ,lname) from signin x where x.id=b.assigned_to) as asign_tl, (Select name from role a  where a.id=b.role_id) as role_name FROM signin b where b.status = 2)xx , (select t_type, sub_t_type, lwd, terms_date,comments, user_id as duid, (Select name from master_term_type mtt where mtt.id=t.t_type) as t_type_name,
				(Select name from master_sub_term_type mstt where mstt.id=t.sub_t_type) as sub_t_type_name from terminate_users t where is_term_complete='N') d  where xx.id=d.duid  $_filterCond ) yy LEFT JOIN ( select max(logout_time) as llogout_time,user_id from logged_in_details group by user_id) c on (yy.id=c.user_id)";
				...
				$mCond=" and role_id>1 and disposition_id =1 ".$_filterCond;
