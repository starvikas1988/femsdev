...
class Message extends CI_Controller {
	...
	{
		$form_data = $this->input->post();
		if(!isset($form_data['specific_user']))
		{
			$form_data['specific_user'] = 0;
			$form_data['specific_users'] = array();
		}
		else
		{
			$form_data['client'] = '';
		...
		{
			$query = $this->db->query('INSERT INTO `faq_messageboard`(`subject`, `message_body`, `location_id`, `client_id`, `process_id`, `department_id`,`individual`,`individual_ids`) VALUES ("'.$form_data['name'].'",'.$this->db->escape($form_data['text']).',"'.$form_data['location'].'","'.$form_data['client'].'","'.$form_data['process'].'","'.$form_data['department'].'","'.$form_data['specific_user'].'","'.implode(',',$form_data['specific_users']).'")');
		}
		else
		{
			$query = $this->db->query('UPDATE `faq_messageboard` SET `subject`="'.$form_data['name'].'",`message_body`='.$this->db->escape($form_data['text']).',`location_id`="'.$form_data['location'].'",`client_id`="'.$form_data['client'].'",`process_id`="'.$form_data['process'].'",`department_id`="'.$form_data['department'].'",individual="'.$form_data['specific_user'].'",individual_ids="'.implode(',',$form_data['specific_users']).'" WHERE id="'.$form_data['faq_messageboard_id'].'"');
		}
...
(SELECT faq_messageboard.id as message_id,faq_messageboard.*,GROUP_CONCAT(faq_msg_attach.file_name SEPARATOR ",") AS file_name,GROUP_CONCAT(faq_msg_attach.id SEPARATOR ",") AS attached_img_id FROM `faq_messageboard` LEFT JOIN faq_message_read_stat ON faq_message_read_stat.message_id=faq_messageboard.id  LEFT JOIN faq_msg_attach ON faq_msg_attach.msgid=faq_messageboard.id WHERE faq_messageboard.id NOT IN(SELECT faq_messageboard.id as message_id FROM `faq_messageboard` INNER JOIN faq_message_read_stat ON faq_message_read_stat.message_id=faq_messageboard.id AND faq_message_read_stat.user_id="'.$current_user.'") AND '.implode(' AND ',$query).' AND individual = 1 AND find_in_set('.$current_user.',individual_ids) GROUP BY faq_messageboard.id ORDER BY faq_messageboard.id DESC)';
   ...
   public function read_msg()
   {
	...
	public function get_users()
	{
		$form_data = $this->input->post();
		$current_user = get_user_id();
		$query = $this->db->query('SELECT signin.id,CONCAT(signin.fname," ",signin.lname) AS name,signin.fusion_id,department.shname  FROM signin LEFT JOIN department ON department.id=signin.dept_id WHERE signin.office_id="'.$form_data['location'].'" AND signin.id != '.$current_user.'');
		if($query)
		{
			$response['stat'] = true;
