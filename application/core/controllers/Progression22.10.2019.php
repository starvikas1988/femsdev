...
class Progression extends CI_Controller {
	...
	public function get_requisition_result()
	{
		...
		{
			if(strtotime($value->due_date) < strtotime(GetLocalTimeByOffice($value->location_id)))
			{
		...
		foreach($result as $key=>$value)
		{
			$response['datas'] .= '<tr>';
				...
				$response['datas'] .= $this->get_access_button(get_dept_folder(),$value->life_cycle,$value->requisition_id,$value->total_application,$value->filter_type,$value->location_id,$value->no_of_position,$value->hiring_manager_id);
	...
	public function get_access_button($dept,$life_cycle,$requisition_id,$total_application,$filter_test,$location,$no_of_position,$hiring_manager_id)
	{
		$current_user = get_user_id();
		$is_global_access = get_global_access();
		$hiring_manager_id_array = explode(',',$hiring_manager_id);
		...
		if($dept=="wfm")
		{
			...
			else if($life_cycle == 'HRV')
			{
				...
				{
					$response .= '<td>in Progress &nbsp; <button class="btn btn-xs btn-primary view_applications" data-requisition_id="'.$requisition_id.'" title="View Applications">View Applications</button></td>';
				}
			}
		...
		else if($is_global_access==1)
		{
			...
			{
				$response .= '<td><button class="btn btn-xs btn-primary view_applications" data-requisition_id="'.$requisition_id.'" title="View Applications">Applications</button>&nbsp;<button class="btn btn-xs btn-warning start_filter_test" data-requisition_id="'.$requisition_id.'" title="Start Filter Test">Start Test</button></td>';
			}
			else if($life_cycle == 'FTD')
			{
				$response .= '<td><button class="btn btn-xs btn-primary view_applications" data-requisition_id="'.$requisition_id.'" title="View Applications">Applications</button>&nbsp;<button class="btn btn-xs btn-warning select_threshold" data-requisition_id="'.$requisition_id.'" title="Select Threshold">Threshold</button></td>';
		...
		else if($hirging_manager == true)
		{
			if($life_cycle == 'OPN')
			{
				$response .= '<td>Progression Opened Pending For WFM Approval</td>';
			}
			else if($life_cycle == 'APR')
			{
				$response .= '<td><button class="btn btn-xs btn-danger close_progression_btn" data-requisition_id="'.$requisition_id.'" title="Close Progression">Close Progression</button></td>';
			...
			{
				$response .= '<td>Pending HR Scrutiny</td>';
			}
			else if($life_cycle == 'AC')
			{
				if($total_application != 0)
	...
	public function get_hiring_manager()
	{
		...
		if($office_id=="KOL") $office_id = "'KOL','HWH','BLR'";
		...
		$hiring_manager_query = $this->db->query('SELECT signin.id, CONCAT(signin.fname," ",signin.lname) AS name , office_id, (Select shname from department where department.id=signin.dept_id) as dept_name FROM `signin`
			...
			WHERE role.folder in ("manager","admin") AND role.is_active=1 AND signin.office_id in ('. $office_id . ')');
	...
	{
		$current_user = get_user_id();
		$query = $this->db->query('SELECT DATEDIFF(CURDATE(),doj) AS tenure FROM `signin` WHERE id='.$current_user.'');
		$row = $query->row();
		return $row->tenure;
	}
	...
	public function get_apply_ijp()
	{
		...
		if($this->get_user_tenure() < 180)
		{
			$response['datas'] .= '<tr><td colspan="14">You\'ll be eligible to Apply After 6 month From Date of Joining.</td></tr>';
			$response['available_requisition'] .= '0';
		}
		else
		{
			if($role_id != 0)
			...
			if($location != 'ALL')
			{
				...
				WHERE ijp_requisitions.requisition_id NOT IN(SELECT requisition_id FROM ijp_requisition_applications WHERE user_id="'.$current_user.'")  AND life_cycle !="OPN" '.$search_query.' AND due_date > "'.GetLocalTime().'" ORDER BY id DESC');
			...
			else
			{
				...
				WHERE ijp_requisitions.requisition_id NOT IN(SELECT requisition_id FROM ijp_requisition_applications WHERE user_id="'.$current_user.'")  AND life_cycle !="OPN" AND due_date > "'.GetLocalTime().'" ORDER BY id DESC');
			}
							...
							$response['datas'] .= '<form class="process_user_credential_form" enctype="multipart/form-data">';
								$response['datas'] .= '<table width="100%">';
									$response['datas'] .= '<tbody>';
										...
										$response['datas'] .= '</tr>';
										$response['datas'] .= '<tr>';
											$response['datas'] .= '<td>Is your teniority morethen 6 month?</td>';
											$response['datas'] .= '<td style="padding-left: 20px;">';
										...
										$response['datas'] .= '</tr>';
										$response['datas'] .= '<tr>';
											$response['datas'] .= '<td>Do you have any unauthorize leave in last 6 month?</td>';
											$response['datas'] .= '<td style="padding-left: 20px;">';
	...
	public function applied_progression()
	{
		...
		foreach($req_result as $key=>$value)
		{
			...
			{
				$response['datas'] .= '<td><span class="label label-success">HR Approved You Application</span></td>';
				$response['datas'] .= '<td></td>';
			}
			else if($value->application_status == 'Approved1')
			{
				$response['datas'] .= '<td><span class="label label-success">Shortlisted For Final Interview</span></td>';
				$response['datas'] .= '<td></td>';
			}
			else if($value->application_status == 'Rejected')
			{
				$response['datas'] .= '<td><span class="label label-danger">Your Application Rejected</span></td>';
				$response['datas'] .= '<td></td>';
			}
	...
	public function get_application()
	{
		...
		foreach($rows as $key=>$value)
		{
			...
			$response .= '<tr>';
				...
				{
					$response .= '<td>Application Auto Approved</td>';
				}
				else if($value->status == 'Approved1' && (strtotime($value->due_date) < strtotime("now")))// if approved & due date < current date
				{
					$response .= '<td>Waiting For Final Interview</td>';
				...
				{
					$response .= '<td>Filter Test Scheduled at <b>'.$value->exam_start_time.'</b></td>';
				}
	...
	{
		$form_data = $this->input->post();
		$query = $this->db->query('SELECT id,exam_start_time FROM `lt_exam_schedule` WHERE module_id="'.$form_data['requisition_id'].'" GROUP BY exam_start_time');
		if($query)
		{
			$response['stat'] = true;
			$response['datas'] = $query->result_object();
