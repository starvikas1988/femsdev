...
class Servicerequest extends CI_Controller {
	...
	public function index()
	{
		if(check_logged_in())
		{
			...
			$data["content_template"] = "servicerequest/index.php";
			
			$qSql="Select * from signin where status=1 and office_id='$user_office_id' and role_id > 0 order by fname";
			...
			$data["priority_dropdown"] = $this->Servicerequest_model->get_priority_list_dropdown();
			$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown();
			$data["category_dropdown_form"] = $this->Servicerequest_model->get_category_dropdown_form($current_user);
			$data["sub_category_dropdown"] = $this->Servicerequest_model->get_subcategory_dropdown();

			$data["priority_list"] = $this->Servicerequest_model->get_priority_list_active();
			$data["status_list"] = $this->Servicerequest_model->status_list_active();
			...
			if(trim($this->input->get('sr_category')!="")){
				...
				if($sr_category != "" && $sr_category >= 1) {
					$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown("",$sr_category, $current_user);
				}	
			}
			...
			if(trim($this->input->get('sr_sub_category')!="")){
				...
				if($sr_sub_category != "" && $sr_sub_category >=1) {
					$data["sub_category_dropdown"] = $this->Servicerequest_model->get_subcategory_dropdown($sr_category,"",$sr_sub_category);
				}else{
					$data["sub_category_dropdown"] = $this->Servicerequest_model->get_subcategory_dropdown($sr_category);
				}	
			}
			...
			if(trim($this->input->get("ticket_view")!="")){
				...
				if($my_tickets == '1') $data["tickets"] = $this->Servicerequest_model->get_all_my_tickets($search, $status, $priority, $transferred, $sr_category);
				else{
					if($is_global_access) $data["tickets"] = $this->Servicerequest_model->get_all_tickets($search, $status, $priority, $transferred, $sr_category, $office_id);
					else $data["tickets"] = $this->Servicerequest_model->get_all_tickets_access_based($search, $status, $priority, $transferred, $sr_category,$office_id);
				}
			}else{				
				if($is_global_access) $data["tickets"] = $this->Servicerequest_model->get_all_tickets($search, $status, $priority, $transferred, $sr_category,$office_id);
				else $data["tickets"] = $this->Servicerequest_model->get_all_tickets_access_based($search, $status, $priority, $transferred, $sr_category,$office_id);
	...
	public function add_ticket(){
		if(check_logged_in()){
			...
			$record = data_inserter('sr_tickets_tbl', $_field_arr);
			...
			if($record){
				...
				if($total > 0){
					if(!is_dir($this->config->item('ServiceResquestPath') ."/".$record)){
						mkdir($this->config->item('ServiceResquestPath') ."/".$record, 0777);
					}
					...
					for( $i=0 ; $i < $total ; $i++ ) {
						...
						if ($tmpFilePath != ""){
							...
							$newFilePath = $this->config->item('ServiceResquestPath') ."/".$record."/".$static_file_path; 
							...
							if(move_uploaded_file($tmpFilePath, $newFilePath)) {
								...
								if(data_inserter('sr_tickets_attachments_tbl', $_field_arr)===false){
	...
	public function edit(){
		if(check_logged_in()){
			...
			$data["ticket"] = $this->Servicerequest_model->get_ticket($this->uri->segment('4'));
			$data["ticket_attachments"] = $this->Servicerequest_model->ticket_attachments($this->uri->segment('4'));

			$data["ticket_id"] = $this->uri->segment('4');

			$data["priority_dropdown"] = $this->Servicerequest_model->get_priority_list_dropdown($data["ticket_id"]);
			$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown($data["ticket_id"]);
			$data["sub_category_dropdown"] = $this->Servicerequest_model->get_subcategory_dropdown("", $data["ticket_id"]);
			...
			if($this->input->post()){
				...
				$this->db->update('sr_tickets_tbl', $_field_arr);
					...
					for( $i=0 ; $i < $total ; $i++ ) {
						...
						if ($tmpFilePath != ""){	
							...
							if(move_uploaded_file($tmpFilePath, $newFilePath)) {
								...
								data_inserter('sr_tickets_attachments_tbl', $_field_arr);
							}
						}
					}
				}
				redirect(base_url()."servicerequest/ticket/".$record);
			}
	...
	public function get_category(){
		if(check_logged_in()){
			print $this->Servicerequest_model->get_category_location_dropdown($this->input->post('location_id'));
		}
	}

	public function get_sub_category(){
		if(check_logged_in()){
			print $this->Servicerequest_model->get_subcategory_location_dropdown($this->input->post('category_id'),$this->input->post('location_id'));
		}
	}
	...
	public function user_list(){
		if(check_logged_in()){
			$result = $this->Servicerequest_model->get_assign_list_all($this->input->post('office_id'));
	...
	public function ticket($_id){
		if(check_logged_in()){
			...
			$data["is_role_dir"] = get_role_dir();
			$data["aside_template"] = "servicerequest/aside.php";
			$data["content_template"] = "servicerequest/ticket.php";
			...
			$data["ticket"] = $this->Servicerequest_model->get_ticket($_id);
			$data["ticket_id"] = $data["ticket"][0]->id;
			//$data["ticket_replies"] = $this->Servicerequest_model->get_ticket_replies($_id);

			$data["ticket_me"] = $this->Servicerequest_model->ticket_assigned_user($_id);
			//$data["assigned_user_list"] = $this->Servicerequest_model->get_assigned_users($_id);
			
			$data["assigned_users"] = $this->Servicerequest_model->get_assign_list_all($_id);
			$data["pre_assign_users"] = $this->Servicerequest_model->get_pre_assign_list($_id);
			
			$data["is_ticket_creator"] = $this->Servicerequest_model->ticket_creator($_id);
			
			$data["ticket_replies"] = $this->Servicerequest_model->get_ticket_replies($_id);
			$data["ticket_attachments"] = $this->Servicerequest_model->ticket_attachments($_id);

			$data["priority_dropdown"] = $this->Servicerequest_model->get_priority_list_dropdown();
			$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown();

			$data["transfer_tickets"] = $this->Servicerequest_model->transfer_ticket_details($_id);
			$data["ticket_status_logs"] = $this->Servicerequest_model->ticket_status_logs($_id);
	...
	public function takeover_ticket(){
		if(check_logged_in()){
			if(trim($this->input->post('id'))!="" && trim($this->input->post('created_by'))!=""){
				...
				$this->db->update('sr_tickets_tbl', $data);
				
				
				$ticket_id = $this->input->post('id');
				$evt_date = CurrMySqlDate();
				
				//$this->Servicerequest_model->delete_assigned_users($this->input->post('id'));
				
				$rSql= "REPLACE INTO  sr_tickets_assignment_tbl(ticket_id,assigned_by,assigned_to,taken_over,taken_over_on)values(".$ticket_id.",'".$current_user."','".$current_user."','1','".$evt_date."')";
	...
	public function status_inprocess(){
		if(check_logged_in()){
			if(trim($this->input->post('id'))!=""){
				...
				data_inserter('sr_status_changes_tbl', $_field_arr);
				...
				$this->db->where($ts);
				$this->db->update('sr_tickets_assignment_tbl', $taken_over);
				*/
	...
	public function ticket_replies(){
		if(check_logged_in()){
			if(trim($this->input->post('ticket_id'))!=""){
				...
				$_id = data_inserter('sr_tickets_reply_tbl', $_field_arr);
				...
				$ticket_id = $this->input->post('ticket_id');
				$evt_date = CurrMySqlDate();
				//$this->Servicerequest_model->delete_assigned_users($this->input->post('ticket_id'));
				$rSql= "REPLACE INTO  sr_tickets_assignment_tbl(ticket_id,assigned_by,assigned_to,taken_over,taken_over_on)values(".$ticket_id.",'".$current_user."','".$current_user."','1','".$evt_date."')";
				...
				redirect(base_url()."servicerequest/ticket/".$this->input->post('ticket_id'));
			}
		}
	}
	...
	public function delete_attachments(){
		if(check_logged_in()){
			if(trim($this->input->post('id'))!=""){
				$this->Servicerequest_model->delete_reply_attachments($this->input->post('id'));
			}
			print 1;
		}
	}
	...
	public function transfer_ticket(){
		if(check_logged_in()){
			if(trim($this->input->post('id'))!=""){
				...
				$ticket = $this->Servicerequest_model->get_ticket($this->input->post('id'));
				...
				data_inserter("sr_tickets_transfer_tbl", $_field_arr);
				...
				redirect(base_url()."servicerequest/ticket/".$this->input->post('id'));
			}
		}
	}
	...
	public function reports(){
		if(check_logged_in()){
			...
			$data["is_role_dir"] = get_role_dir();
			$data["aside_template"] = "servicerequest/aside.php";
			$data["content_template"] = "servicerequest/reports.php";

			$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown();
			$data["status_dropdown"] = $this->Servicerequest_model->status_list_active();
			...
			if($this->input->post()){
				...
				$data["category_dropdown"] = $this->Servicerequest_model->get_category_dropdown("", $this->input->post('category'));
				$data["sub_category_dropdown"] = $this->Servicerequest_model->get_subcategory_dropdown($this->input->post('category'),"", $sub_category);
				
				$data["ticket_reports"] = $this->Servicerequest_model->ticket_reports($office_id,$category, $sub_category, $status);
	...
	public function download(){
		...
		if(check_logged_in()){
			if($this->input->post()){
				$currDate=date("Y-m-d");
				$newfile="servicerequest_report-".$currDate.".xlsx";

				$office_id = $this->input->post('office_id');
				// if($office_id=="All")  $office_id=$user_office_id;
				...
				$no_of_reply_cols = 3 * $this->Servicerequest_model->get_max_no_reply_columns();
				...
				foreach($data["ticket_reports"] as $ticket){
					...
					$replies = $this->Servicerequest_model->get_ticket_replies($ticket->id);
