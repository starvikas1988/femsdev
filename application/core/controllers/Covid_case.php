...
class Covid_case extends CI_Controller {
...
///=========================== COVID CASE PART  ================================///
     public function index(){
		...
		redirect(base_url()."covid_case/form/");
	...
	public function case_list()
	{
		...
		$data["content_template"] = "covid19_case/covid_case_form_list.php";
		
		$sqlcase = "SELECT c.*, concat(s.fname,' ', s.lname) as added_by_name from covid19_case as c LEFT JOIN signin as s ON s.id = c.added_by 
		            WHERE c.case_source <> 'individual'";
		$data['case_list'] = $querycase = $this->Common_model->get_query_result_array($sqlcase);
	...
	public function case_individual_list()
	{
		...
		$data['is_role_dir'] = $is_role_dir;
		$data["aside_template"] = "covid19_case/aside.php";
		$data["content_template"] = "covid19_case/covid_case_form_individual_list.php";
		
		$data['formBaseUrl'] = $this->get_form_public_url();
	...
	public function form()
	{
		...
		$data['is_role_dir'] = $is_role_dir;
		$data["aside_template"] = "covid19_case/aside.php";
		$data["content_template"] = "covid19_case/covid_case_form.php";
		$data['uri'] = $this->uri->segment(4);
		$data['mysections'] = array('personal', 'administrative', 'demographics', 'clinical', 'treatment', 'notes', 
		                            'exposure', 'risk', 'aftercase');
		...
		if($this->uri->segment(4) == 'administrative'){
			...
			$data["content_template"] = "covid19_case/covid_case_form_administrative.php";
		}
		...
		if($this->uri->segment(4) == 'demographics'){
			...
			$data["crmdetails"] = $this->covid_case_details($crmid);
			$data['crm_treatment'] = $this->check_treatment($crmid);	
			$data["content_template"] = "covid19_case/covid_case_form_demographics.php";
		}
		...
		if($this->uri->segment(4) == 'clinical'){
			...
			$data["crmdetails"] = $this->covid_case_details($crmid);
			$data['crm_treatment'] = $this->check_treatment($crmid);	
			$data["content_template"] = "covid19_case/covid_case_form_clinical.php";
		}
		...
		if($this->uri->segment(4) == 'treatment'){
			...
			$data["crmdetails"] = $this->covid_case_details($crmid);
			$data['crm_treatment'] = $this->check_treatment($crmid);	
			$data["content_template"] = "covid19_case/covid_case_form_treatment.php";
		}
		...
		if($this->uri->segment(4) == 'notes'){
			...
			$data['crm_treatment'] = $this->check_treatment($crmid);
			$data['ai_address'] = $this->ai_check_address($crmid);	
			$data["content_template"] = "covid19_case/covid_case_form_notes.php";
		}
		...
		if($this->uri->segment(4) == 'exposure'){
			...
			$data["content_template"] = "covid19_case/covid_case_form_exposure.php";
		}
		...
		if($this->uri->segment(4) == 'risk'){
			...
			$data["crmdetails"] = $this->covid_case_details($crmid);
			$data['crm_treatment'] = $this->check_treatment($crmid);	
			$data["content_template"] = "covid19_case/covid_case_form_risk.php";
		}
		...
		if($this->uri->segment(4) == 'transmission'){
			...
			$data["content_template"] = "covid19_case/covid_case_form_transmission.php";
		}
		...
		if($this->uri->segment(4) == 'aftercase'){
			...
			$data['crm_treatment'] = $this->check_treatment($crmid);	
			$data["crmdetails"] = $this->covid_case_details($crmid);
			$data["content_template"] = "covid19_case/covid_case_form_aftercase.php";
		}
		...
		if($this->uri->segment(4) == 'investigation'){
			...
			$data["content_template"] = "covid19_case/covid_case_form_investigation_notes.php";
		}
		...
		if($this->uri->segment(4) == 'contacts'){
			...
			$data["content_template"] = "covid19_case/covid_case_form_contact_actions.php";
		}
	...
	public function sendtoform($formid = 1, $crmid='', $type='success')
	{
		if($formid == 1)
		{
			redirect('covid_case/form/'.$crmid.'/personal/'.$type);
		}
		
		if($formid == 2)
		{
			redirect('covid_case/form/'.$crmid.'/administrative/'.$type);
		}
		
		if($formid == 3)
		{
			redirect('covid_case/form/'.$crmid.'/demographics/'.$type);
		}
		
		if($formid == 4)
		{
			redirect('covid_case/form/'.$crmid.'/clinical/'.$type);
		}
		
		if($formid == 5)
		{
			redirect('covid_case/form/'.$crmid.'/treatment/'.$type);
		}
		
		if($formid == 6)
		{
			redirect('covid_case/form/'.$crmid.'/notes/'.$type);
		}
		
		if($formid == 7)
		{
			redirect('covid_case/form/'.$crmid.'/exposure/'.$type);
		}
		
		if($formid == 8)
		{
			redirect('covid_case/form/'.$crmid.'/transmission/'.$type);
		}
		
		if($formid == 9)
		{
			redirect('covid_case/form/'.$crmid.'/risk/'.$type);
		}
		
		if($formid == 10)
		{
			redirect('covid_case/form/'.$crmid.'/aftercase/'.$type);
		}
		
		if($formid == 11)
		{
			redirect('covid_case/form/'.$crmid.'/investigation/'.$type);
		}
		
		if($formid == 12)
		{
			redirect('covid_case/form/'.$crmid.'/contacts/'.$type);
		}
	}
	...
	public function check_calendar()
	{
		$data['is_role_dir'] = $is_role_dir;
		$data["aside_template"] = "covid19_case/aside.php";
		$data["content_template"] = "covid19_case/covid_case_calendar.php";
	...
	public function check_logs()
	{
		$data['is_role_dir'] = $is_role_dir;
		$data["aside_template"] = "covid19_case/aside.php";
		$data["content_template"] = "covid19_case/covid_case_tracker.php";
		
		$crm_id = $this->uri->segment(3);
		$data["crmdetails"] = $this->covid_case_details($crm_id);
		$sql = "SELECT l.*, CONCAT(s.fname, ' ', s.lname) as full_name from covid19_case_logs as l LEFT JOIN signin as s ON l.cl_added_by = s.id WHERE l.crm_id = '$crm_id' ORDER by l.cl_date_added DESC";
		$data['logdetails'] = $logsdetails = $this->Common_model->get_query_result_array($sql);
		
		$sql = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(cl_interval))) as value from covid19_case_logs WHERE crm_id = '$crm_id'";
	...
	//--------- INVESTIGATION NOTES
	public function submit_investigation()
	{
		$current_user = get_user_id();
		$crm_id = $this->input->post('crm_id_check');
		$time_interval = $this->input->post('time_interval');		
		$flag = "error";
		...
		if($querycheck > 0){ 
			...
			$inv_info_1 = $this->input->post('inv_info_1');
			
			if(count($inv_info_1) > 0){
				$this -> db -> where('crm_id', $crm_id);
				$this -> db -> delete('covid19_investigation_notes');
				
				for($i=0; $i<count($inv_info_1); $i++)
				{
					$data_array = [
						'inv_info_1' => $this->input->post('inv_info_1')[$i],
						'inv_info_2' => $this->input->post('inv_info_2')[$i],
						'inv_info_3' => $this->input->post('inv_info_3')[$i],
						'inv_info_5' => $this->input->post('inv_info_5')[$i],
						'inv_info_6' => $this->input->post('inv_info_6')[$i],
						'inv_info_7' => $this->input->post('inv_info_7')[$i],
						'inv_info_8' => $this->input->post('inv_info_8')[$i],
						'inv_info_9' => $this->input->post('inv_info_9')[$i],
						'inv_info_10' => $this->input->post('inv_info_10')[$i],
					];
					
					if(!empty($this->input->post('inv_info_4')[$i])){ $data_array += ['inv_info_4' => $this->input->post('inv_info_4')[$i], ]; }
					$data_array += [ 'crm_id' => $crm_id ];
					data_inserter('covid19_investigation_notes', $data_array);
				}
		...
		$cl_type = 'investigation';
		$cl_disposition = $this->input->post('cl_disposition');
		$cl_comments = $this->input->post('cl_comments');
		$this->covid_case_logs($crm_id, $cl_disposition, $cl_comments, $cl_type, $time_interval);
		
		if($flag == "update"){ $this->sendtoform('12',$crm_id, 'update'); }
		if($flag == "insert"){ $this->sendtoform('12',$crm_id, 'success'); }
		if($flag == "critical"){ $this->sendtoform('12',$crm_id, 'error'); }
		if($flag == "error"){ redirect('covid_case/updatecase'); }
	...
	//--------- CONTACT ACTION NOTES
	public function submit_contact_action()
	{
		$current_user = get_user_id();
		$crm_id = $this->input->post('crm_id_check');	
		$time_interval = $this->input->post('time_interval');
		...
		if($querycheck > 0){ 
			...
			$ca_info_1 = $this->input->post('ca_info_1');
			
			if(count($ca_info_1) > 0){
				$this -> db -> where('crm_id', $crm_id);
				$this -> db -> delete('covid19_contact_action');
				
				for($i=0; $i<count($ca_info_1); $i++)
				{
					$data_array = [
						'ca_info_1' => $this->input->post('ca_info_1')[$i],
						'ca_info_2' => $this->input->post('ca_info_2')[$i],
						'ca_info_3' => $this->input->post('ca_info_3')[$i],
						'ca_info_5' => $this->input->post('ca_info_5')[$i],
						'ca_info_6' => $this->input->post('ca_info_6')[$i],
						'ca_info_7' => $this->input->post('ca_info_7')[$i],
						'ca_info_8' => $this->input->post('ca_info_8')[$i],
					];
					
					if(!empty($this->input->post('ca_info_4')[$i])){ $data_array += ['ca_info_4' => $this->input->post('ca_info_4')[$i], ]; }
					$data_array += [ 'crm_id' => $crm_id ];
					data_inserter('covid19_contact_action', $data_array);
				}
	...
	//--------- CONTACT CASE CLOSURE
	public function submit_case_closure()
	{
		$current_user = get_user_id();
		$crm_id = $this->input->post('crm_id_check');	
		$time_interval = $this->input->post('time_interval');
	...
	public function send_email()
	{
		...
		$this->send_email_contact_tracing($form_crm_id, $form_email_id, $current_user);
		$this->covid_case_move_next($crm_id);	
		
	}
	
	
	public function send_email_contact_tracing($crmid, $emailid, $uid)
	{
		$eto="";
		$ecc="";
		$nbody="";
		...
		if(count($contactQ) > 0)
		{
			...
			$formBaseUrl = $this->get_form_public_url();
			$encodedUrl = urlencode(base64_encode($crmid.'/personal'));
			$SendFullURL= $formBaseUrl.$encodedUrl;
			
			$nbody='Dear <b>'.$contactQ["fname"] .' ' .$contactQ["lname"].',</b></br></br>
					...
					<table border="1" width="80%" cellpadding="0" cellspacing="0">
							...
							<tr>
								<td style="background-color:powderblue;padding:2px 0px 2px 8px">Case Name:</td>
								<td style="padding:2px 0px 2px 8px">'.$contactQ["fname"] .' ' .$contactQ["lname"].'</td>
							</tr>
							<tr>
								<td style="background-color:powderblue;padding:2px 0px 2px 8px">Case Phone:</td>
								<td style="padding:2px 0px 2px 8px">'.$contactQ["p_phone"].'</td>
							</tr>
							<tr>
								<td style="background-color:powderblue;padding:2px 0px 2px 8px">Case Email:</td>
								<td style="padding:2px 0px 2px 8px">'.$contactQ["p_email"].'</td>
							</tr>
					</table>
					
					<br/><br/>
					<a target="_blank" href="'.$SendFullURL.'" style="background-color:#F15664;color:#fff;padding:4px 8px; border-radius:2px;text-decoration:none">
					Click to Open Form</a>
