<?php 
 ...
 class Qa_hra extends CI_Controller{
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$data["content_template"] = "qa_hra/qa_amd_episource_feedback.php";
...
//////////////////////HRA ADD Audit//////////////////////////////
		...
		if(check_logged_in())
		{
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_hra/add_amd_episource_new.php";
	...
	private function mt_upload_files($files){
        $config['upload_path'] = './qa_files/qa_hra';
...
///////////////////HRA Manegment review///////////////////////////

	public function mgnt_hra_rvw($id){
		...
		if(check_logged_in()){
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_hra/mgnt_amd_episource_rvw.php";
...
// ///////////////////////////////////////////////////////////////////////////////////////////
	...
	public function qa_hra_report(){
		...
		if(check_logged_in()){
			...
			$data["content_template"] = "qa_hra/qa_amd_episource_report.php";
			...
			if($this->input->get('show')=='Show')
			{
				...
				if($office_id != "") $cond1 .= " and office_id='$office_id'";
					...
					$dn_link = base_url()."qa_hra/download_qa_hra_new_CSV";
			}
...
/////////////////HRA Agent part//////////////////////////	
	...
	public function agent_hra_feedback()
	{
		if(check_logged_in())
		{
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_hra/agent_amd_episource_feedback.php";
			$data["agentUrl"] = "qa_hra/agent_hra_feedback";
			
			$from_date = '';
			$to_date = '';
			$cond="";
			
				$qSql="Select count(id) as value from qa_amd_episource_feedback where agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit') and qa_type in ('regular')";
				$data["tot_feedback"] =  $this->Common_model->get_single_value($qSql);
				
				$qSql="Select count(id) as value from qa_amd_episource_feedback where agent_rvw_date is null and agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit')  and qa_type in ('regular')";
				...
				if($this->input->get('btnView')=='View')
				{
					...
					$qSql="SELECT * from (Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name, (select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name, (select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_name from qa_amd_episource_feedback $cond) xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to from signin) yy on (xx.agent_id=yy.sid) order by audit_date";
	...
	public function agent_hra_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_hra/agent_amd_episource_rvw.php";
			$data["agentUrl"] = "qa_hra/agent_hra_feedback";
			
			
			$qSql="SELECT * from (Select *, (select concat(fname, ' ', lname) as name from signin s where s.id=entry_by) as auditor_name, (select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name, (select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_name,agent_rvw_note as agent_note,mgnt_rvw_note as mgnt_note from qa_amd_episource_feedback where id=$id) xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to from signin) yy on (xx.agent_id=yy.sid) order by audit_date";
			...
			if($this->input->post('pnid'))
			{
				...
				redirect('qa_hra/agent_hra_feedback');
