...
class Reports extends CI_Controller {
        ...
        if(check_logged_in())
        {
			...
			$data["role_dir"]=$role_dir;
				...
				if($this->input->get('showReports')=='Show')
				{
					...
					if(get_role_dir()=="trainer" || get_role_dir()=="tl") $filterArray["assigned_to"]=$current_user;
	...
	public function create_CSV($rr,$fArray)
	{
		...
		if($filter_key=="OfflineList" || $filter_value=="2,3,4"){
			$header = array("Date", "Fusion ID", "OM ID", "Agent Name","Dept","Client Name", "Site", "Process", "Login Time", "Logout Time", "Logged In Hours","Other Break Time","Lunch/Dinner Break Time", "Disposition", "Ticket No" , "Comments");
		}else{
			$header = array("Date", "Fusion ID", "OM ID", "Agent Name","Dept","Client Name", "Site", "Process", "Login Time", "Logout Time", "Logged In Hours","Other Break Time","Lunch/Dinner Break Time", "Disposition", "Comments");
		}
		...
		{	
			...
			if($logged_in_hours!="0"){
				//if($disposition!="") $row .= '"P & '.$disposition.'",'; 
				//else 
...
/*-------------------Resignation Report-----------------------------*/	
	...
	public function create_Resign_CSV($rr)
	{
		$filename = "./assets/reports/Report".get_user_id().".csv";
		$fopen = fopen($filename,"w+");
		$header = array("Name", "Fusion ID", "XPOID", "OMUID", "Role", "Location", "Resign Date", "Released Date", "User Remarks", "Current Resign Date/Time", "Accepted Name", "Accepted Remarks", "Current Accepted Date/Time", "Original Released Date", "Status");
...
/*-------------------Requisition Report-----------------------------*/	
	...
	public function create_Requisition_CSV($rr)
	{
		$filename = "./assets/reports/Report".get_user_id().".csv";
		$fopen = fopen($filename,"w+");
		$header = array("Requisition_id", "Location", "Due Date", "Department", "Role", "Req Qualification", "Req Exp Range", "Position Required", "Position Filled", "Total Applied", "Total Shortlisted", "Job Title", "Job Desc", "Additional Info", "Raised By", "Raised Date/Time", "Status", "Update By", "Update Date/Time", "Update Remarks");
		...
		{	
			...
			$row .= '"'.$status.'",';
			$row .= '"'.$user['update_name'].'",';
			$row .= '"'.$user['update_date'].'",';
			$row .= '"'. str_replace('"',"'",str_replace($searches, "", $user['update_comment'])).'",';
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public function create_Candidate_CSV($rr)
	{
		$filename = "./assets/reports/Report".get_user_id().".csv";
		$fopen = fopen($filename,"w+");
		$header = array("Requisition ID", "Location", "Due Date", "Position Applied For", "Hiring Source", "Source Name", "Candidate Name", "DOB", "Email", "Gender", "Phone", "Qualification", "Skill", "Total Exp", "Address", "Country", "State", "City", "Postcode", "Attachment", "Summary", "Added By", "Candidate Status", "Added Date", "Approved By", "Approved Comment", "DOJ");
	...
	public function dfr_candidate_info()
	{
		if(check_logged_in()){
			...
			if($this->input->get('showReports')=='Show')
			{
				...
				if($office_id == 'ALL')
				{
					$query = $this->db->query("SELECT dfr_candidate_details.*,dfr_requisition.raised_date FROM dfr_candidate_details LEFT JOIN dfr_requisition ON dfr_requisition.id=dfr_candidate_details.r_id WHERE (DATE_FORMAT(dfr_candidate_details.added_date,'%m/%d/%Y') BETWEEN '".$start_date."' AND '".$end_date."')");
				}
				else
				{
					$query = $this->db->query("SELECT dfr_candidate_details.*,dfr_requisition.raised_date FROM dfr_candidate_details LEFT JOIN dfr_requisition ON dfr_requisition.id=dfr_candidate_details.r_id WHERE dfr_requisition.location='KOL' AND (DATE_FORMAT(dfr_candidate_details.added_date,'%m/%d/%Y') BETWEEN '".$start_date."' AND '".$end_date."')");
				}
