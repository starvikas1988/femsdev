<?php 
 ...
 class Qa_k2_claims extends CI_Controller{
	...
	private function k2_upload_files($files,$path){
	...
	public function index(){
		if(check_logged_in())
		{
			...
			$data["content_template"] = "qa_k2_claims/qa_k2_claims_feedback.php";
			$data["content_js"] = "qa_kabbage_js.php";
			
			$qSql="SELECT id, concat(fname, ' ', lname) as name, assigned_to, fusion_id FROM `signin` where role_id in (select id from role where folder ='agent') and dept_id=6 and is_assign_client (id,142) and is_assign_process (id,288) and status=1  order by name";
			...
			$qSql = "SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_k2_claims_feedback $cond) xx Left Join
				(Select id as sid, fname, lname, fusion_id, get_process_names(id) as campaign, assigned_to from signin) yy on (xx.agent_id=yy.sid) $ops_cond order by audit_date";
			$data["k2_claims"] = $this->Common_model->get_query_result_array($qSql);
	...
	public function add_edit_k2_claims($k2_id){
		if(check_logged_in())
		{
			$current_user=get_user_id();
			...
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_k2_claims/add_edit_k2_claims.php";
			$data["content_js"] = "qa_kabbage_js.php";
			$data['k2_id']=$k2_id;
			$tl_mgnt_cond='';
			...
			$qSql = "SELECT * from
				...
				from qa_k2_claims_feedback where id='$k2_id') xx Left Join (Select id as sid, fname, lname, fusion_id, office_id, assigned_to, get_process_names(id) as process from signin) yy on (xx.agent_id=yy.sid)";
			$data["k2_claims"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($this->input->post('agent_id')){
				...
				$field_array=array(
					...
					"communication_skill" => $this->input->post('communication_skill'),
					"voice_delivary" => $this->input->post('voice_delivary'),
					"closing" => $this->input->post('closing'),
					"FNOL_from" => $this->input->post('FNOL_from'),
					"customer_service" => $this->input->post('customer_service'),
					"process_procedure" => $this->input->post('process_procedure'),
				...
				if($k2_id==0){
					...
					$a = $this->k2_upload_files($_FILES['attach_file'], $path='./qa_files/qa_k2_claims/');
					$field_array["attach_file"] = implode(',',$a);
					$rowid= data_inserter('qa_k2_claims_feedback',$field_array);
					/////////
					$field_array2 = array(
						"audit_date" => CurrDate(),
					...
					$this->db->update('qa_k2_claims_feedback',$field_array2);
					///////////
					...
					$this->db->where('id', $rowid);
					$this->db->update('qa_k2_claims_feedback',$field_array1);
					
				}else{
					
					$this->db->where('id', $k2_id);
					$this->db->update('qa_k2_claims_feedback',$field_array);
					/////////
					if(get_login_type()=="client"){
						$field_array1 = array(
...
/////////////////////////Agent part/////////////////////////////////	
	...
	public function agent_k2_claims_feedback(){
		if(check_logged_in()){
			$user_site_id= get_user_site_id();
			$role_id= get_role_id();
			$current_user = get_user_id();
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_k2_claims/agent_k2_claims_feedback.php";
			$data["content_js"] = "qa_kabbage_js.php";
			$data["agentUrl"] = "qa_k2_claims/agent_k2_claims_feedback";
			
			
			$qSql="Select count(id) as value from qa_k2_claims_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit')";
			$data["tot_feedback"] =  $this->Common_model->get_single_value($qSql);
			
			$qSql="Select count(id) as value from qa_k2_claims_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit', 'Operation Audit', 'Trainer Audit') and agent_rvw_date is Null";
			...
			if($this->input->get('btnView')=='View')
			{
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_k2_claims_feedback where agent_id='$current_user' And audit_type in ('CQ Audit', 'BQ Audit')) xx Left Join
	...
	public function agent_k2_claims_rvw($id){
		if(check_logged_in()){
			$current_user=get_user_id();
			$user_office_id=get_user_office_id();
			$data["aside_template"] = "qa/aside.php";
			$data["content_template"] = "qa_k2_claims/agent_k2_claims_rvw.php";
			$data["content_js"] = "qa_kabbage_js.php";
			$data["agentUrl"] = "qa_k2_claims/agent_k2_claims_feedback";
			...
			$qSql="SELECT * from
				...
				(select concat(fname, ' ', lname) as name from signin_client sc where sc.id=client_entryby) as client_name,
				(select concat(fname, ' ', lname) as name from signin s where s.id=tl_id) as tl_name,
				(select concat(fname, ' ', lname) as name from signin sx where sx.id=mgnt_rvw_by) as mgnt_rvw_name from qa_k2_claims_feedback where id='$id') xx Left Join
				(Select id as sid, fname, lname, fusion_id, assigned_to, get_process_names(id) as campaign from signin) yy on (xx.agent_id=yy.sid)";
			$data["k2_claims"] = $this->Common_model->get_query_row_array($qSql);
			...
			if($this->input->post('pnid'))
			{
				...
				redirect('Qa_k2_claims/agent_k2_claims_feedback');
...
///////////////////////////////// QA K2 Claims REPORT ///////////////////////////////////	
	...
	public function qa_k2_claims_report(){
		if(check_logged_in()){
			...
			$data["content_template"] = "qa_k2_claims/qa_k2_claims_report.php";
			$data["content_js"] = "qa_kabbage_js.php";
			...
			if($this->input->get('show')=='Show')
			{
				...
				$dn_link = base_url()."qa_k2_claims/download_qa_k2_claims_CSV";
			}
