...
WHERE pm_design_kpi_v2.kpi_name = 'Sup_FEMS_ID' AND signin.fname != '' AND pm_design_v2.process_id = '".$form_data['process_id']."' AND pm_design_v2.office_id='".$form_data['office_id']."'";
	...
	public function prepare_row($json=false)
	{
		...
		pm_log_record($current_user,'pm_tl_agent_view','pm',$form_data);
	}
	...
	public function supervisor()
	{
		if(check_logged_in()){
			...
			//$data['table'] = $this->prepare_row();
			$data["content_template"] = "pmetrix_v2_tl/matrix_screen_sup_wise.php";
			$data["content_js"] = "pmetrix_v2_tl/tl_js.php";
			$this->load->view('dashboard',$data);
		}
	}
	public function get_bouns_info()
	{
		...
		$form_data = $this->input->post();
		$query = $this->db->query('SELECT * FROM pm_vrs_bonus_sup WHERE pay_period="'.$form_data['pay_period'].'" AND  fems_id="'.$fusion_id.'" AND post_period="'.$form_data['post_period'].'"');
		if($query)
		{
			$response['stat'] = true;
			$response['datas'] = $query->result_object();
	...
	public function get_bonus_sup()
	{
		if(check_logged_in()){
			...
			$vrs_sup_query = $this->db->query('SELECT * FROM `pm_vrs_bonus_sup` WHERE post_period="'.$post_period.'" AND fems_id="'.$fusion_id.'" ');
			// echo $current_user; exit;
			if($vrs_sup_query)
			{
				$data['vrs_sup'] = $vrs_sup_query->row();
			}
	...
	public function process_bonus()
	{
		...
		$query = $this->db->query('UPDATE pm_vrs_bonus_sup SET supervisor_comment="'.$form_data['supervisor_comment'].'", om_comment="'.$form_data['om_comment'].'", accepted_status="1", accepted_on=now() WHERE pay_period="'.$form_data['pay_period'].'" AND  fems_id="'.$form_data['fusion_id'].'" AND post_period="'.$form_data['post_period'].'"');
		if($query)
		{
			$response['stat'] = true;
		}
	...
	public function prepare_tl_row()
	{
		...
		if($form_data['process_id'] == 30)
		{
			echo $this->load->view('pmetrix_v2_tl/view_data_page_tl_vrs',$data,true);
		}
		else
		{
