    ...
    public function index()
    {
		...
		if (get_role_dir() == "agent" and get_dept_folder() !="wfm" and get_dept_folder() !="rta" and get_dept_folder() !="mis"  ){
			...
			foreach($pm_design as $row){
				...
				//$qSql = "Select id,fusion_id,office_id, fname, lname, (Select name from role a  where a.id=b.role_id) as role_name, get_client_names(b.id) as client_name, get_process_names(b.id) as process_name from signin b where status=1 and id in (Select user_id from pm_data_v2 where mdid = $mp_id )  $condP  and id = 158 or id=1271 ";
				...
				foreach($userlist as $urow){
					...
					//$qSql = "Select * from pm_data_v2 where user_id = $user_id and start_date='$shMonDate' and end_date='$shSunDate'";
					
					if (get_role_dir() == "agent" and get_dept_folder() !="wfm" and get_dept_folder() !="rta" and get_dept_folder() !="mis"  ){
						
						$qSql = "Select start_date, end_date from pm_data_v2 where user_id = $user_id group by start_date";
        ...
        if(check_logged_in())
        {
			...
			if($sdate!="" && $edate!=""){
				...
				if(!is_array($_FILES["myfile"]["name"])) //single file
				{
					...
					$ret[]= $this->Import_excel_file($fileName,'','',$mpid, $mptype);
						...
						foreach($kpiValuesArry as $kpiID => $kpiVal){
							...
							try{
								if($sdate == '')
								{
									...
									$end_date = gmdate("Y-m-d", $UNIX_DATE);
								}
								else
								{
									$start_date = $sdate;
								...
								$iSql="REPLACE INTO pm_data_v2 (user_id,added_by,added_date,start_date,end_date,mdid,kpi_id, kpi_value ) Values($user_id,'$current_user', now() ,'$start_date', '$end_date', $mpid, $kpiID,'$kpiVal');";
								...
								log_message('FEMS',  'Caught v2 exception: '.  $ex->getMessage());
								log_message('FEMS',  'Error : v2  -> '. $fusion_id. " >> " . " $user_id >> :: " .$iSql );
