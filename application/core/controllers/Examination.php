...
class Examination extends CI_Controller {
	...
	public function index()
	{
		if(check_logged_in()) 
		{
			...
			if(isAccessQuestionBank()==true )
			{
				
				$ctid = $this->input->get('ctid');
				$cid = $this->input->get('cid');
				
				if(isset($ctid) && isset($cid)){
					$data['set_nav'] = array('nav'=>true,
											 'cat_id'=>$ctid,
											 'cid'=>$cid);
				}else{
					$data['set_nav'] = array('nav'=>false,
											 'cat_id'=>0,
											 'cid'=>0);
				}
	...
	public function get_exam_list()
	{
		...
		if($is_global_access==1){
			...
			$qSql = "SELECT * FROM lt_examination WHERE  (location IN ('".$form_office_id."','ALL') OR '$user_oth_office' like CONCAT('%',location,'%'))  ".$addextrasql." AND status=1 ORDER BY id DESC";
	...
	public function read_excel($file_name,$set_id)
	{
		...
		for($row = 2; $row <= $highestRow; $row++)
		{
			...
			if($this->dataTypeForValue($objWorksheet->getCell('C'.$row)->getValue()) == 'b'){
				 if($objWorksheet->getCell('C'.$row)->getValue() == 1){
					 $type = 'TRUE';
				 }else{
					 $type = 'FALSE';
				 }
			}else{
				$type = $objWorksheet->getCell('C'.$row)->getValue();
			}
			
			$type=htmlspecialchars($type);
			
			if($this->dataTypeForValue($objWorksheet->getCell('D'.$row)->getValue()) == 'b'){
				 if($objWorksheet->getCell('D'.$row)->getValue() == 1){
					 $type1 = 'TRUE';
				 }else{
					 $type1 = 'FALSE';
				 }
			}else{
				$type1 = $objWorksheet->getCell('D'.$row)->getValue();
			}
			$type1=htmlspecialchars($type1);
			
			if($this->dataTypeForValue($objWorksheet->getCell('E'.$row)->getValue()) == 'b'){
				 if($objWorksheet->getCell('E'.$row)->getValue() == 1){
					 $type2 = 'TRUE';
				 }else{
					 $type2 = 'FALSE';
				 }
			}else{
				$type2 = $objWorksheet->getCell('E'.$row)->getValue();
			}
			
			$type2=htmlspecialchars($type2);
			
			if($this->dataTypeForValue($objWorksheet->getCell('F'.$row)->getValue()) == 'b'){
				 if($objWorksheet->getCell('F'.$row)->getValue() == 1){
					 $type3 = 'TRUE';
				 }else{
					 $type3 = 'FALSE';
				 }
			}else{
				$type3 = $objWorksheet->getCell('F'.$row)->getValue();
			}
			
			$type3=htmlspecialchars($type3);
			
			
			$question_ans['option1'][] = $type;
			$question_ans['option2'][] = $type1;
			$question_ans['option3'][] = $type2;
			$question_ans['option4'][] = $type3;
	...
	public function reject_exam_all()
	{
		$current_user = get_user_id();
		$form_data = $this->input->post();
		$requisition_id=$form_data['requisition_id'];
		//$query = false;
		
		$ApplnUpdateSql='UPDATE ijp_requisition_applications SET status="MisFiltTest"  WHERE requisition_id="'.$requisition_id.'" AND status="FilterScheCompl" ';
		
		$this->db->query($ApplnUpdateSql);
		
		//$SchUpdateSql='UPDATE lt_exam_schedule SET exam_status="4"  WHERE module_id="'.$requisition_id.'" And module_type="IJP" AND user_id IN (SELECT user_id  FROM ijp_requisition_applications WHERE requisition_id="'.$requisition_id.'" AND status="MisFiltTest")';
		
		$SchUpdateSql='UPDATE lt_exam_schedule SET exam_status="4"  WHERE module_id="'.$requisition_id.'" And module_type="IJP" AND exam_status NOT IN (1,4)';
		
		$this->db->query($SchUpdateSql);
				
		//check if any more exam left
		$query1 = $this->db->query('SELECT COUNT(*) as available_exam FROM lt_exam_schedule WHERE module_id="'.$requisition_id.'" And module_type="IJP" AND exam_status NOT IN (1,4)');
		
		$results = $query1->row();
		if($results->available_exam == 0)
		{
			$this->db->query('UPDATE ijp_requisitions SET life_cycle="FTD"  WHERE requisition_id="'.$requisition_id.'"');
		}
