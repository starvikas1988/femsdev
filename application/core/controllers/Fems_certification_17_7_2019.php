  ...
  class Fems_certification extends CI_Controller{
		...
		public function pendingexam($attempt_id)
		{
			if(check_logged_in())
			{
				...
				setcookie("remtime", ($time_per_ques * ($TotQuesDisplay - $ques_counter) ) );
	...
	public function startexam(){
			...
			$pass_mark = $this->config->item('fems_certificate_pass_marks');
			echo $TotQuesDisplay =  $this->config->item('fems_certificate_total_question');
			echo $time_per_ques =   $this->config->item('fems_certificate_time_per_ques');
			...
			if($is_complete_exam > 0){
				...
				//echo ($time_per_ques * $TotQuesDisplay) ."HHHHH ";
				
				$this->session->set_userdata('exam_rem_time', ($time_per_ques * $TotQuesDisplay) );
				 
				setcookie("remtime", ($time_per_ques * $TotQuesDisplay) , time()+3600) or die('unable to create cookie');
