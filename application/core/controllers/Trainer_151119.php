	   ...
	   if(check_logged_in())
        {
			...
			*/
			...
			if(get_role_dir()!="trainer") redirect(base_url().get_role_dir()."/dashboard","refresh");
			$data["aside_template"] = get_aside_template();
            $data["content_template"] = "trainer/dashboard.php";
			...
			$cond= " where status = 1 And ". $assToCond ;
			...
			$cond= " where status = 1 and is_logged_in=0 And ". $assToCond ;
			...
			$cond= " where status = 1 and is_logged_in=1 And ". $assToCond ;
			...
			$cond= " where status = 1 and is_logged_in=0 and disposition_id in(2,3,4,5) And ". $assToCond ;
			...
			if($filter_id==1) $data['user_list'] = $this->Common_model->get_user_list("All",$mCond);	
				...
				(Select name from master_sub_term_type mstt where mstt.id=t.sub_t_type) as sub_t_type_name from terminate_users t where is_term_complete='N') d  where xx.id=d.duid  $mCond ) yy LEFT JOIN ( select max(logout_time) as llogout_time,user_id from logged_in_details group by user_id) c on (yy.id=c.user_id)";
				...
				$mCond=" and disposition_id =1 And ".$assToCond." ".$_filterCond;
