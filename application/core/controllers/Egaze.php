...
class Egaze extends CI_Controller {
	...
	function dashboard()
	{
		...
		if(check_logged_in()){	
			...
			// TOP PRODUCTIVE APPS
			$sql_topproductive = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as totaltimespent, app_name FROM egaze_activities_details $innrJoin WHERE start_datetime_local >= '$filter_date_start' AND start_datetime_local <= '$filter_date_end'  $AssCond 
			...
			$sql_tounpproductive = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as totaltimespent, app_name  FROM egaze_activities_details $innrJoin WHERE start_datetime_local >= '$filter_date_start' AND start_datetime_local <= '$filter_date_end' 
			$AssCond
			...
			$sql_topproductive_emp = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as totaltimespent FROM egaze_activities_details WHERE start_datetime_local >= '$filter_date_start' AND start_datetime_local <= '$filter_date_end' $AssCond 
			  AND (app_name RLIKE '$productiveApps')
			...
			$sql_topunproductive_emp = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as totaltimespent FROM egaze_activities_details WHERE start_datetime_local >= '$filter_date_start' AND start_datetime_local <= '$filter_date_end' $AssCond
			 AND (app_name RLIKE '$noncomplianceApps')
			 GROUP BY user_id ORDER BY totaltimespent DESC LIMIT 10";			
			...
			*/
			...
			$sql_topproductive_emp = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as totaltimespent, user_id FROM egaze_activities_details $innrJoin WHERE start_datetime_local >= '$filter_date_start' AND start_datetime_local <= '$filter_date_end' $AssCond
			GROUP BY user_id ORDER BY totaltimespent DESC LIMIT 10";
	...
	function reports()
	{
		if(check_logged_in()){	
			...
			$sql = "SELECT e.user_id, concat(s.fname, ' ', s.lname) as fullname, s.is_logged_in, 
					...
					FROM egaze_event_details WHERE date(end_dt_local) = '$current_date'
					GROUP BY user_id) as i ON i.user_id = e.user_id
					LEFT JOIN signin as s ON s.id = e.user_id
					WHERE 
					(start_datetime_local >= '$current_start_date' AND start_datetime_local <= '$current_end_date')
			...
			foreach($activities as $tokenm){
				...
				$sqlProductive = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as value 
					FROM egaze_activities_details WHERE (start_datetime_local >= '$current_start_date' AND start_datetime_local <= '$current_end_date') AND
				...
				$sqlUnProductive = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as value 
					FROM egaze_activities_details WHERE (start_datetime_local >= '$current_start_date' AND start_datetime_local <= '$current_end_date') $extraFilter AND user_id = '$e_userid'";
	...
	function timeline()
	{
		if(check_logged_in()){	
			...
			$sql = "SELECT e.user_id, concat(s.fname, ' ', s.lname) as fullname, s.is_logged_in, 
					...
					LEFT JOIN 
					(SELECT user_id, SUM(TIME_TO_SEC(total_time)) as idletime 
					FROM egaze_event_details WHERE start_dt_local >= '$current_start_date' AND start_dt_local <= '$current_end_date'
					GROUP BY user_id) as i ON i.user_id = e.user_id
					LEFT JOIN signin as s ON s.id = e.user_id
					WHERE 
					date(start_datetime_local) = '$current_date' 
					AND e.user_id IN ($myteam)
					GROUP BY e.user_id
					ORDER BY fullname";
	...
	function realtime_23()
	{
		...
		if(check_logged_in()){	
			...
			foreach($myteamd as $tokend)
			{
				...
				{
					$s_date = date('d', strtotime($tokenr['start_datetime_local']));
					$s_month = date('m', strtotime($tokenr['start_datetime_local']));
					$s_year = date('Y', strtotime($tokenr['start_datetime_local']));
					$s_hour = date('H', strtotime($tokenr['start_datetime_local']));
					$s_min = date('i', strtotime($tokenr['start_datetime_local']));
					$s_sec = date('s', strtotime($tokenr['start_datetime_local']));
					
					$e_date = date('d', strtotime($tokenr['end_datetime_local']));
					$e_month = date('m', strtotime($tokenr['end_datetime_local']));
					$e_year = date('Y', strtotime($tokenr['end_datetime_local']));
					$e_hour = date('H', strtotime($tokenr['end_datetime_local']));
					$e_min = date('i', strtotime($tokenr['end_datetime_local']));
					$e_sec = date('s', strtotime($tokenr['end_datetime_local']));
					...
					if($tokenr['start_datetime_local'] > $tokenr['end_datetime_local']){ $errorsFound .= "ERROR : egaze_activities_details - id(" .$tokenr['id'] .") - ".$tokenr['app_name'] ." | " .$tokend['fullname'] ." - " .$uid ." - " .$fid ." | Start Date : " .$tokenr['start_datetime_local'] ." - End Date : " .$tokenr['end_datetime_local'] ."<br/>"; }
					
					if($tokenr['start_datetime_local'] <= $tokenr['end_datetime_local']){ 
				...
				{
					$s_date = date('d', strtotime($tokenr['start_dt_local']));
					$s_month = date('m', strtotime($tokenr['start_dt_local']));
					$s_year = date('Y', strtotime($tokenr['start_dt_local']));
					$s_hour = date('H', strtotime($tokenr['start_dt_local']));
					$s_min = date('i', strtotime($tokenr['start_dt_local']));
					$s_sec = date('s', strtotime($tokenr['start_dt_local']));
					
					$e_date = date('d', strtotime($tokenr['end_dt_local']));
					$e_month = date('m', strtotime($tokenr['end_dt_local']));
					$e_year = date('Y', strtotime($tokenr['end_dt_local']));
					$e_hour = date('H', strtotime($tokenr['end_dt_local']));
					$e_min = date('i', strtotime($tokenr['end_dt_local']));
					$e_sec = date('s', strtotime($tokenr['end_dt_local']));
					
					//GET ACTIVITY TYPE
					$colorchange = "color:#000000";
					
					if($tokenr['start_dt_local'] > $tokenr['end_dt_local']){ $errorsFound .= "ERROR : egaze_event_details - id(" .$tokenr['id'] .") -".$tokenr['event_name'] ." | " .$tokend['fullname'] ." - " .$uid ." - " .$fid ." | Start Date : " .$tokenr['start_dt_local'] ." - End Date : " .$tokenr['end_dt_local'] ."<br/>"; }
					
					if($tokenr['start_dt_local'] <= $tokenr['end_dt_local']){
				...
				$sqltotal = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(total_time))) as value from egaze_event_details WHERE user_id = '$uid' AND (start_dt_local >= '$start_date' AND start_dt_local <= '$end_date')";
	...
	function individual()
	{
		...
		if(check_logged_in()){	
			...
			//======== ACTIVITIES DETAILS 			
			$sqltotal = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(total_time))) as value from egaze_event_details WHERE user_id = '$current_user' AND (start_dt_local >= '$current_start_date' AND start_dt_local <= '$current_end_date')";
			...
			WHERE user_id = '$current_user' AND (start_datetime_local >= '$current_start_date' AND start_datetime_local <= '$current_end_date')";
			...
			for($i=0;$i<7;$i++)
			{
				...
				$sqldate = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(total_time_spent))) as total_time, SUM(TIME_TO_SEC(total_time_spent)) as total_seconds from egaze_activities_details WHERE user_id IN ($current_user) AND (start_datetime_local >= '$start_date_week_start' AND start_datetime_local <= '$start_date_week_end')";
				...
				WHERE user_id IN ($current_user) AND (start_datetime_local >= '$start_date_week_start' AND start_datetime_local <= '$start_date_week_end') 
				AND ((app_name RLIKE '$productiveFilter'))";
		...
		if(check_logged_in()){	
			...
			if($report_type == 1)
			{
				$sqlReport = "SELECT s.fusion_id as Fusion_ID, CONCAT(s.fname, ' ', s.lname) as Name, CONCAT(ls.fname, ' ', ls.lname) as L1_Supervisor,
				              ...
				              WHERE (e.start_datetime_local >= '$sdate_start' AND e.start_datetime_local <= '$sdate_end') $_filterCond";
			...
			if($report_type == 2)
			{
				$sqlReport = "SELECT s.fusion_id as Fusion_ID, CONCAT(s.fname, ' ', s.lname) as Name, CONCAT(ls.fname, ' ', ls.lname) as L1_Supervisor,
				              ...
				              LEFT JOIN signin as s ON e.user_id = s.id
							  LEFT JOIN signin as ls ON ls.id = s.assigned_to
				              WHERE (e.end_dt_local >= '$sdate_start' AND e.end_dt_local <= '$sdate_end') $_filterCond";
				$queryReport = $this->Common_model->get_query_result_array($sqlReport);
	...
	public function activities_details_info()
	{
		...
		$sqlData = "SELECT * from egaze_activities_details  WHERE start_datetime_local >= '$startDate' AND end_datetime_local <= '$endDate' AND user_id = '$sid' AND app_name = '$app'";
	...
	private function get_activities_records($uid, $count = '0', $startdate = "", $enddate = "", $type = "")
	{
		...
		$sql = "SELECT * from egaze_activities_details WHERE total_time_spent>0 and user_id = '$uid' AND (start_datetime_local >= '$start_date' AND start_datetime_local <= '$end_date') ORDER by id DESC $exralimit";
	...
	private function get_activities_records_all($uid = '', $count = '0', $startdate = "", $enddate = "", $type = "")
	{
		...
		if($count > 0){ $exralimit = " LIMIT " .$count; }
		if($uid == ''){ $uid = 0; }
		$sql = "SELECT * from egaze_activities_details WHERE total_time_spent>0 and user_id IN ($uid) AND (start_datetime_local >= '$start_date' AND start_datetime_local <= '$end_date') ORDER by id DESC $exralimit";
		$query = $this->Common_model->get_query_result_array($sql);
	...
	private function get_activities_records_classified($uid, $count = '0', $startdate = "", $enddate = "", $type = "")
	{
		...
		$sql = "SELECT SUM(TIME_TO_SEC(total_time_spent)) as total_time, app_name, user_id, count(id) as count_records from egaze_activities_details 
		        WHERE total_time_spent>0 and user_id = '$uid' AND (start_datetime_local >= '$start_date' AND start_datetime_local <= '$end_date') 
	...
	private function get_events_records($uid, $count = '0', $startdate = "", $enddate = "", $type = "")
	{
		...
		$sql = "SELECT * from egaze_event_details WHERE user_id = '$uid' AND (start_dt_local >= '$start_date' AND start_dt_local <= '$end_date') ORDER by id DESC $exralimit";
		$query = $this->Common_model->get_query_result_array($sql);
	...
	private function get_events_records_all($uid = '', $count = '0', $startdate = "", $enddate = "", $type = "")
	{
		...
		if($count > 0){ $exralimit = " LIMIT " .$count; }
		if($uid == ''){ $uid = 0; }
		$sql = "SELECT * from egaze_event_details WHERE user_id IN ($uid) AND (start_dt_local >= '$start_date' AND start_dt_local <= '$end_date') ORDER by id DESC $exralimit";
		$query = $this->Common_model->get_query_result_array($sql);
