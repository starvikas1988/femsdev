...
class Super extends CI_Controller {
    ...
    public function index()
    {
		...
		if(check_logged_in())
        {
			...
			$qSQL="SELECT count(user_id) as total FROM suspended_users, signin where user_id=signin.id and is_complete = 'N' $_filterCond ";
			...
			if(date('D', strtotime($cDate)) == "Sat") $shSatDate=$cDate;
			else $shSatDate=date('Y-m-d',strtotime($cDate.' +1 Saturday'));
			
			$qSQL="Select count(user_id) as total from user_shift_schedule, signin where user_id=signin.id and start_date='$shSunDate' and end_date='$shSatDate' and shday='$currDay' and in_time not like '%OFF%' and in_time not like '%Leave%' $_filterCond ";
			...
			if($filter_id==1){
				...
				$qSql="select * from (select * from (SELECT id,fusion_id,office_id,dept_id,omuid,fname,lname,role_id,site_id,process_id,status,assigned_to, get_client_names(b.id) as client_name,(Select shname from department s where s.id=b.dept_id) as dept_name,(Select name from sub_department sd where sd.id=b.sub_dept_id) as sub_dept_name,(Select name from event_master k where k.id=b.disposition_id) as disp_name, get_process_names(b.id) as process_name,(Select name from sub_process l where l.id=b.sub_process_id) as sub_process_name, (Select name from site z  where z.id=b.site_id) as site_name, (Select CONCAT(fname,' ' ,lname) from signin x where x.id=b.assigned_to) as asign_tl, (Select name from role a  where a.id=b.role_id) as role_name FROM signin b where b.status = 2)xx , (select terms_date,comments,user_id as duid from terminate_users where is_term_complete='N') d  where xx.id=d.duid  $_filterCond ) yy LEFT JOIN ( select max(logout_time) as llogout_time,user_id from logged_in_details group by user_id) c on (yy.id=c.user_id)";
				...
				$qSql="SELECT a.*,fusion_id,office_id,fname,lname,role_id,omuid,is_logged_in,status,assigned_to,(Select shname from department s where s.id=b.dept_id) as dept_name,(Select name from process y where y.id=b.process_id) as process_name,(Select name from site z  where z.id=b.site_id) as site_name, (Select CONCAT(fname,' ' ,lname) from signin x where x.id=b.assigned_to) as asign_tl, (Select name from role k  where k.id=b.role_id) as role_name FROM suspended_users a , signin b where a.user_id=b.id and a.is_complete = 'N' $_filterCond ";
