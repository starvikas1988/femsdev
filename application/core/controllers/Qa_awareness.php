<?php 
 ...
 class Qa_awareness extends CI_Controller{
    ...
    {
        $config['upload_path'] = './qa_files/qa_awareness/'.$table;
    ...
    public function scoreVal(){
    	return $arrayName = array("val1"=>1,"val3"=>3,"val5"=>5);
    }
	 ...
	 public function agent_process_feedback()
	{
		if(check_logged_in())
		{
			...
			$cond="";
				...
				$qSql="Select count(id) as value from qa_".$page."_email_feedback where agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit')";
				$data["tot_feedback_sa"] =  $this->Common_model->get_single_value($qSql);
				
				$qSql="Select count(id) as value from qa_".$page."_email_feedback where agent_rvw_date is null and agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit') ";
				$data["yet_rvw_sa"] =  $this->Common_model->get_single_value($qSql);

				$qSql="Select count(id) as value from qa_".$page."_call_feedback where agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit')";
				$data["tot_feedback_ta"] =  $this->Common_model->get_single_value($qSql);
				
				$qSql="Select count(id) as value from qa_".$page."_call_feedback where agent_rvw_date is null and agent_id='$current_user' and audit_type in ('CQ Audit', 'BQ Audit') ";
