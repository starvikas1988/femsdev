...
//================= ACCEPTANCE DASHBOARD =================================================//
	...
	{ 
		if(check_logged_in())
		{
			...
			$select_start_date = $this->input->get('select_start_date');
			$select_end_date = $this->input->get('select_end_date');
			...
			if($tableFound){
				...
				if($type == "column"){
					...
					$sqlAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as qa
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw <> '' AND qa.$agent_column_rvw IS NOT NULL) OR qa.overall_score = 100) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					...
					$sqlNotAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as qa
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((qa.$agent_column_rvw = '' OR qa.$agent_column_rvw IS NULL) AND qa.overall_score != 100) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
				...
				if($type == "table"){
					...
					$sqlAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as fd
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw <> '' AND fd.$agent_column_rvw IS NOT NULL) OR qa.overall_score = 100) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countAccepted = $this->Common_model->get_single_value($sqlAccepted);
					...
					$sqlNotAccepted = "SELECT qa.agent_id, count(qa.agent_id) as feedback_count from $agentTable as fd
									...
									WHERE (DATE(qa.audit_date) >= '".$start_date."' AND DATE(qa.audit_date) <='".$end_date."') 
									AND s.office_id = '$search_office' AND qa.audit_type = 'CQ Audit'
									AND ((fd.$agent_column_rvw = '' OR fd.$agent_column_rvw IS NULL) AND qa.overall_score != 100) AND qa.agent_id IN ($signinIds) GROUP BY qa.agent_id";
					$countNotAccepted = $this->Common_model->get_single_value($sqlNotAccepted);
...
/////////============= CALCULATE AVG NPS SCORE PROCESS WISE  ==========================///////////////////////////////////
    ...
    {		 
        ...
        $data["content_template"] = "qa_graph/qa_changes.php";
		...
		if(!empty($teacherID))
		{		
			$sqlFile = "SELECT * from kyt_teacher_availability WHERE teacher_id = '$teacherID'";
			$queryFile = $this->Common_model->get_query_result_array($sqlFile);
			$uniqueArray = $this->array_indexed($queryFile, 'group_id');
			
			$daysArrayData = array('MON','TUE','WED','THU','FRI','SAT','SUN');
			$availableDayData = array();
			foreach($daysArrayData as $tokendata)
			{
				foreach($uniqueArray as $groupData){
					$groupName = $groupData['group_id'];
					$availableDayDataFilter = array_filter($queryFile, function($tokencheck) use ($tokendata, $groupName){
						if($tokencheck['day'] == $tokendata && $tokencheck['group_id'] == $groupName){
							return $tokencheck;
						}
					});
					$availableDayData[$tokendata][$groupName] = $availableDayDataFilter;
				}
				
			}
			
			$data['result'] = array();
			foreach($uniqueArray as $token){
				$groupID = $token['group_id'];
				$startDate = $token['from_date'];
				$endDate = $token['to_date'];
				if(strtotime($endDate) >= strtotime($startDate)){				
					...
					while(strtotime($currentDay) <= strtotime($endDate))
					{
						...
						if(!empty($availableDayData[$currentDayName][$groupID]))
						{ 	
							//echo "hi";
							foreach($availableDayData[$currentDayName][$groupID] as $calendarData){
								//echo "hi";
								$currentSlot = $calendarData['time_slots'];
								$timesArray = explode(',', $currentSlot);
								foreach($timesArray as $token){
									$statTime = $currentDay ." " .$token;
									$endTime = $currentDay ." " .date('H:i:s', strtotime('+1hour', $statTime));
									$data['result'][] = [ 
										'title' => date('h:i A', strtotime($statTime)),
										'start' => $statTime,
										'end' => $endTime,
        ...
        //echo "<pre>".print_r($uniqueArray, 1)."</pre>";
       // echo "<pre>".print_r($availableDayData, 1)."</pre>";
       // echo "<pre>".print_r($data['result'], 1)."</pre>";
       // die();
	...
	{
		//$scheduleStartDate = $e_info['start_time'];
		//$scheduleEndDate = $e_info['end_time'];			
		//$subject = "New Class has been Scheduled | " .$e_info['course_name'];						
				
		#-- Vcard INFO
		$v_Summary = 'Course';
		$v_From = "support@kyt.com";
		$v_Organiser = "KYT";
		$v_Domain = "kyt.com";
		$v_Location = "KYT ACADEMY";
		
		#-- Email Message
		$message = "Dear ".$e_to_name.",<br><br>";
		$message .= "Kindly add this event to your calendar for meeting alerts in your google email.<br><br>";
		$message .= "<b>Regards<br/> KYT Academy </b><br/><hr/><br/>";
		
		$scheduleStartDate = "2021-02-24 10:30:00";
		$scheduleEndDate = "2021-02-24 11:30:00";
		$subject = "TEST - New Class has been Scheduled";
		$e_to = 'sachinkrpaswan17@gmail.com';
		//$e_to = 'manashkundu2@gmail.com';
		//$e_cc = "sachinkrpaswan17@gmail.com";
		...
		'END:VTIMEZONE' . "\r\n" .  
		'BEGIN:VEVENT' . "\r\n" .
		'ORGANIZER;CN="'.$v_Organiser.'":MAILTO:'.$v_From. "\r\n" .
		'ATTENDEE;CN="'.$e_to_name.'";ROLE=REQ-PARTICIPANT;RSVP=TRUE:MAILTO:'.$e_to. "\r\n" .
		'LAST-MODIFIED:' . date("Ymd\TGis") . "\r\n" .
		'UID:12'."\r\n" .
		'DTSTAMP:'.date("Ymd\TGis"). "\r\n" .
		'DTSTART;TZID="Asia/Kolkata":'.date("Ymd\THis", strtotime($scheduleStartDate)). "\r\n" .
		'DTEND;TZID="Asia/Kolkata":'.date("Ymd\THis", strtotime($scheduleEndDate)). "\r\n" .
		'TRANSP:OPAQUE'. "\r\n" .
		'SEQUENCE:1'. "\r\n" .
		'SUMMARY:' . $v_Summary . "\r\n" .
		'LOCATION:' . $v_Location . "\r\n" .
		...
		$fp = fopen(FCPATH .'uploads/kyt_calendar/kyt_calendar_cancel.ics', 'w');
		fwrite($fp, $ical);
		fclose($fp);
				
		$e_from_name = 'Fusion FEMS';
		$e_from = 'noreply.fems@fusionbposervices.com';
		$e_Body = $message;
		$e_Subject = $subject;
		...
		$this->email->set_newline("\r\n");
	 	$this->email->from($e_from, $e_form_name);
        $this->email->to($e_to);
		$this->email->cc($e_cc);
        $this->email->subject($e_Subject);
        $this->email->message($e_Body);  
		$this->email->attach(FCPATH .'uploads/kyt_calendar/kyt_calendar_cancel.ics');
        if($this->email->send()){
			return true;
		} else {
			return false;
	 ...
	 function availability_calendar()
	{
		...
		foreach($availabilityList as $token)
		{
			...
			if(!empty($allLeaveDates[$currentTeacher]) && in_array($currentDate, $allLeaveDates[$currentTeacher])){
				$checkColor = $leaveColor; $extraAdd=" (Abs)";
			}
			...
			$linkURL = base_url('qa_graph/teacher_calendar_details/'.$currentAvailID);
	...
	function courses_calendar()
	{
		...
		$sqlAvailability = "SELECT k.*, CONCAT(c.fname, ' ', c.lname) as teacher_name, mc.name as course_name from kyt_availability_records as k 
		                    LEFT JOIN signin_client as c ON c.id = k.teacher_id 
		                    LEFT JOIN kyt_course_master as mc ON mc.id = k.course_id
							WHERE k.course_id = '$courseID' ORDER BY k.avail_date, k.avail_start_time";
		$data['availabilityList'] = $availabilityList = $this->Common_model->get_query_result_array($sqlAvailability);
		...
		foreach($availabilityList as $token)
		{
			...
			$flagLeave = false;
			if(!empty($allLeaveDates[$currentTeacher]) && in_array($currentDate, $allLeaveDates[$currentTeacher])){
				$checkColor = $leaveColor; $extraAdd=" (Abs)"; $flagLeave = true;
			}
			if(!empty($allOffDates[$currentTeacher]) && in_array($currentDate, $allOffDates[$currentTeacher])){
		...
		foreach($data['calendar_result_timeup'] as $token)
		{
			...
			$linkURL = base_url('qa_graph/course_calendar_details?date='.$dateGot.'&startTime='.$startTime.'&endTime='.$endTime);
			$data['calendar_result'][] = [ 
