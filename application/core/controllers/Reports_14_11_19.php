...
/////////////////////////////////////////////////////////////////////
	...
	public function process_update_acceptance_report_list(){
		if(check_logged_in()){
			...
			$qSql="Select client_id as id, (select shname from client c where c.id=process_updates.client_id) as shname from process_updates order by shname asc";
			...
			if($this->input->get('show')=='Show')
			{
				...
				if($pValue=='OYO IBD'){
					...
					if($lob=='IB/OB'){
						...
						$fullAray = $this->reports_model->qa_oyo_report_model($field_array);
						$data["qa_oyo_list"] = $fullAray;
						$this->create_qa_oyolife_followup_CSV($fullAray);	
						$dn_link = base_url()."reports/download_qa_oyolife_followup_CSV";
					}
...
/*----------------OYO LIFE FOLLOW UP CSS Part Start--------------------*/	
	public function download_qa_oyolife_followup_CSV()
	{
		$currDate=date("Y-m-d");
		$filename = "./assets/reports/Report".get_user_id().".csv";
