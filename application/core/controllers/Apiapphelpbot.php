...
class Apiapphelpbot extends CI_Controller {
	...
	public function putsearch()
    {
		...
		if($user_id !="" && $fid !=""){
			...
			log_record($user_id,'putsearch','API Apphelp Bot',$form_data,$cip);
			...
			try{
				...
				$ret = data_inserter('bot_apphelp_search',$_field_array);
				if($ret!==false){		
				...
				log_record($user_id,'putsearch','API Apphelp Bot',$form_data,$logs);
	...
	public function putfeedback()
    {
		...
		if($user_id !="" && $fid !=""){
			...
			log_record($user_id,'putfeedback','API Apphelp Bot',$form_data,$cip);
			...
			if($comment=="") $comment=trim($this->input->get('comment'));
					...
					try{
						...
						$ret = data_inserter('bot_apphelp_feedback',$_field_array);
						...
						$logs= json_encode($_field_array);
						log_record($user_id,'putfeedback','API Apphelp Bot',$form_data,$logs);
