...
class Apibrightwaybot extends CI_Controller {
	...
	public function putsearch()
    {
		...
		if($user_id !="" && $fid !=""){
			...
			log_record($user_id,'putsearch','API brightway Bot',$form_data,$cip);
			...
			try{
				...
				$ret = data_inserter('bot_brightway_search',$_field_array);
				if($ret!==false){		
				...
				log_record($user_id,'putsearch','API brightway Bot',$form_data,$logs);
	...
	public function putfeedback()
    {
		...
		if($user_id !="" && $fid !=""){
			...
			log_record($user_id,'putfeedback','API brightway Bot',$form_data,$cip);
			...
			if($promptreason=="") $promptreason=trim($this->input->get('promptreason'));
					...
					try{
						...
						$ret = data_inserter('bot_brightway_feedback',$_field_array);
						...
						$logs= json_encode($_field_array);
						log_record($user_id,'putfeedback','API brightway Bot',$form_data,$logs);
