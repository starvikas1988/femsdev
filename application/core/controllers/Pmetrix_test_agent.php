...
class Pmetrix_test_agent extends CI_Controller {
    ...
    public function index()
    {
        if(check_logged_in()){
			...
			$data["content_template"] = "pmetrix_test_agent/matrix_screen_agent.php";
			$data["content_js"] = "pmetrix_test_agent/js.php";
	...
	public function prepare_row($json=false){
		...
		if(isset($form_data['fusion_id']))
		{
			...
			$row = $query->row();
			if($row) $form_data['user_id'] = $row->id;
			else $form_data['user_id'] = False;
		}
				
		//
		// GET DEIGN ID
		$data['kpi_design'] = $this->Pmetrix_model->get_available_design($form_data['client_id'], $form_data['process_id'], $form_data['office_id']);	
		 
		if($data['kpi_design']){
			$data['stat'] = true;
			
			$data['kpi_col_new'] = $this->Pmetrix_model->get_kpi_column($data['kpi_design']);	
			
			if(isset($form_data['performance_for_year'])){
				
				$date_array = explode('-',$data["end_date"]);
				
				$data['kpi_data_new'] = $this->Pmetrix_model->get_kpi_data($data['kpi_design'], $data["start_date"], $data["end_date"], $form_data['user_id']);	
				$data['target_new'] = $this->Pmetrix_model->get_target($data['kpi_design'], $date_array[1], $date_array[0], $form_data['user_id']);
				
				$data['grade_new'] = $this->Pmetrix_model->get_grade_bucket($data['kpi_design'], $date_array[1], $date_array[0]);
				
			}else{
				$data['kpi_data_new'] = $this->Pmetrix_model->get_kpi_data($data['kpi_design'], date('Y-m-01'), date('Y-m-t'), $form_data['user_id']);	
				$data['target_new'] = $this->Pmetrix_model->get_target(date('m'), date('Y'), $data['kpi_design'], $form_data['user_id']);
				$data['grade_new'] = $this->Pmetrix_model->get_target->get_grade_bucket($data['kpi_design'], date('m'), date('Y'));
			}
			//print_r($data['kpi_data_new']);
		}
	...
	//
	//
	public function prepare_row_main($json=false)
	{
		$form_data = $this->input->post();
		...
		if($json == false)
		{
			...
			{
				return $this->load->view('pmetrix_v2_agent/more_functions',$data,true);
			}
		}
		else
		{
	...
	private function get_target($month,$year,$did,$user_id=false)
	{
		...
		$row = $query1->row();
		//echo $row->tenure;
		//echo 'SELECT pm_target_v2.target,pm_design_kpi_v2.kpi_name,pm_design_kpi_v2.kpi_type,pm_design_kpi_v2.agent_view,pm_design_kpi_v2.tl_view FROM `pm_target_v2` LEFT JOIN pm_design_kpi_v2 ON pm_design_kpi_v2.id=pm_target_v2.pm_design_kpi_id WHERE month="'.$month.'" AND year="'.$year.'" AND tenure_bucket_start <= '.$row->tenure.' AND tenure_bucket_end >= '.$row->tenure.' AND pm_target_v2.did="'.$did.'"';
		if($row->tenure)
		{
		...
		//$query = $this->db->query('SELECT pm_target_v2.did,pm_target_v2.year,pm_target_v2.month,pm_target_v2.target,pm_design_kpi_v2.kpi_name,pm_design_kpi_v2.kpi_type,pm_design_kpi_v2.agent_view,pm_design_kpi_v2.tl_view,pm_target_v2.tenure_bucket_start,pm_target_v2.tenure_bucket_end FROM `pm_target_v2` LEFT JOIN pm_design_kpi_v2 ON pm_design_kpi_v2.id=pm_target_v2.pm_design_kpi_id');
		if($query)
		{
