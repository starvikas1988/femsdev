...
class Letter extends CI_Controller{
	...
	public function view_release_letter(){
			...
			$SQLtxt = "SELECT f.*,f.id as fnfid, s.doj, s.fusion_id, s.xpoid, s.fname, s.lname, s.office_id, r.user_email, r.user_phone, r.resign_date, r.accepted_released_date, date(t.terms_date) as terms_date, t.lwd, (select email_id_off from info_personal WHERE info_personal.user_id = s.id )as office_email,(select email_id_per from info_personal WHERE info_personal.user_id = s.id )as personal_email,(select name from role where role.id= s.role_id) as designation,(select location from office_location WHERE s.office_id = office_location.abbr )as location
			 ...
			 WHERE office_id in ('KOL','HWH','BLR','CHE','NOI') AND s.id = '$user_id' ";
				...
				$html = $this->load->view('letters/release_letter', $data, TRUE); 
					...
					if($send_mail=="Y"){
						...
						if(file_exists($attachment_path)){
							...
							$is_send=$this->Email_model->send_email_sox($crow['id'],implode(',',$eto),implode(',',$ecc),'Please find your attached Release Letter',$esubj,$attachment_path);
	...
	public function job_release_letter_not_req(){
			$filename = ''; 
			
			$SQLtxt = "select *, user_fnf.id as fnfid, (select email_id_off from info_personal WHERE info_personal.user_id = signin.id )as office_email,(select email_id_per from info_personal WHERE info_personal.user_id = signin.id )as personal_email,(select name from role where role.id= signin.role_id) as designation,(select location from office_location WHERE signin.office_id = office_location.abbr )as location,IF(accepted_released_date = '0000-00-00'|| accepted_released_date is null ,0,(DATEDIFF(now(), accepted_released_date))) as no_of_days from user_resign INNER JOIN signin on signin.id = user_resign.user_id  INNER JOIN user_fnf ON signin.id =  user_fnf.user_id   WHERE office_id in ('KOL','HWH','BLR','CHE','NOI') AND fnf_status='C' and is_resign_email_sent='N' HAVING no_of_days>=46 and no_of_days<=48";
			
			//user_resign.resign_status in('AC') AND   
			
			$data['release_letter']  = $this->Common_model->get_query_result_array($SQLtxt);
			
			foreach($data['release_letter'] as $released_candidate){
				
				$fname= $released_candidate['fname'];
				$lname= $released_candidate['lname'];
				$filename = 'Release_letter_'.$fname.'_'.$lname.'_'.$released_candidate['fusion_id'];
				$esubj='Release Letter of '.$fname.'_'.$lname.'_'.$released_candidate['fusion_id'];
				$data['individual_user'] = $released_candidate;
				
				$html = $this->load->view('letters/release_letter', $data, TRUE);
				
				
				$attachment_path = $this->generate_pdf_files($html,$filename,$released_candidate,1,'Y');
				sleep(1);
				if(file_exists($attachment_path)){
					
					$is_send=$this->Email_model->send_email_sox($released_candidate['user_id'],$released_candidate['personal_email'],$released_candidate['office_email'],'Please find your attached Release Letter',$esubj,$attachment_path);
	...
	public function job_appointment_letter(){
			...
			$SQLtxt = "SELECT *,info_personal.user_id,(select email_id_off from info_personal WHERE info_personal.user_id = signin.id )as office_email,(select email_id_per from info_personal WHERE info_personal.user_id = signin.id )as personal_email,(select name from role where role.id= signin.role_id) as designation,(select location from office_location WHERE signin.office_id = office_location.abbr )as location, signin.id, fusion_id, concat(fname,' ',lname)as fullname,
					   ...
					   LEFT JOIN info_personal on info_personal.user_id = signin.id
					   LEFT JOIN info_payroll on info_payroll.user_id = signin.id
					   WHERE office_id in('KOL','HWH','BLR','CHE','NOI') and status in (1,7) and is_sent_appoint_letter='N' and gross_pay>0
