...
class Dfr_demo extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('Email_model_demo');
...
///////////////////////////////////////////////////////////////////////
	...
	public function add_requisition()
	{
		if(check_logged_in())
		{
			...
			if($location!=""){
				...
				$this->Email_model_demo->send_email_requisition_raised($current_user,$field_array,$user_office_id);
	...
	public function view_requisition($id)
	{
		...
		/////////////
			...
			//$qSql= "Select yy.*, ii.*,ii.id as sch_id from (Select c.*, c.id as can_id, DATE_FORMAT(dob,'%m/%d/%Y') as d_o_b, (select concat(fname, ' ', lname) as name from signin x where x.id=c.added_by) as added_name from dfr_candidate_details c where c.candidate_status not in ('CS', 'R') ) yy Left Join (Select dis.* from dfr_interview_schedules dis where dis.id = (select max(id) from dfr_interview_schedules where c_id = dis.c_id)) ii ON (ii.c_id=yy.id) where r_id='$id' ";
	...
	public function approved_requisition(){
		if(check_logged_in()){
			...
			if($r_id!=""){
				...
				$this->db->where('id', $r_id);
				$this->db->update('dfr_requisition', $field_array);
				if($this->email_model->hiring_requis_approve($current_user,$r_id,$field_array))
				{
					$ans="done";
				}
				else
	...
	public function add_candidate_interview(){
		...
		if(check_logged_in()){
			...
			if($interview_status=="CNR"){
				$this->Email_model_demo->send_email_candidate_further_review($current_user,$field_array,$user_office_id,$r_id,$c_id);
			}
	...
	public function candidate_final_approval(){
		if(check_logged_in()){
			...
			if($c_id!=""){
				...
				$this->Email_model_demo->send_email_candidate_final_selection($current_user,$field_array,$user_office_id,$r_id,$c_id);
