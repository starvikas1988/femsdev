...
class IMap
{
	 ...
	 * @var string
	 */
	protected $attachments_dir = 'emat_files/attachments/';
	...
	public function __construct(array $config = [])
	{
		...
		if (! empty($config))
		{
		    ...
		    $this->connect();
			if(!$this->stream) {
				echo "Could not connect to iMap Server. Output: " . imap_last_error();
	...
	public function connect(array $config = [])
	{
		...
		if(isset($config['shared']) && isset($config['authuser']))
		{
			$enc .= '/authuser='.$config['authuser'].'/user='.$config['shared'];
		}
		
		$folder_check = "";
		if(isset($config['folder']) && !empty($config['folder']))
		{
			$folder_check = $config['folder'];
		}

		$this->mailbox = '{' . $config['host'] . $enc . '}' .$folder_check;
		$this->stream  = imap_open($this->mailbox, $config['username'], $config['password']);
	
		//show_error($this->get_last_error());
		//print_r($this->get_last_error());
	...
	public function append_mail($mimeMSG, $authHost, $folder)
	{
		imap_append($this->stream, $authHost, $mimeMSG, $folder);
		return true;
	}
	...
	protected function _get_attachments(int $uid, $structure, string $part_number = '',	int $index = null)
	{
		...
		else
		{
			...
			if (isset($structure->dparameters[0]))
			{
				...
				$fileNameText = !empty($decoded_name[0]->text) ? $decoded_name[0]->text : "unknown_".strtotime("now");
				$filename     = $this->convert_to_utf8($fileNameText);
				$content      = imap_fetchbody($this->stream, $id, $part_number);
				if(!empty($content)){
					$content      = (string)$this->struc_decoding($content, $bodystruct->encoding);
					
					$currentInfo = $this->config;
					$uploadDir = $this->attachments_dir;
					$currentMailBox = $currentInfo['username'];
					$currentMailBoxDir = $uploadDir.$currentMailBox."/";
					$currentMailBoxFullDir = FCPATH.$currentMailBoxDir;
					
					$fileUploadName = $uid."_".strtotime("now")."_".preg_replace("/[^a-zA-Z0-9._]+/", "", $filename);
					if(!file_exists($currentMailBoxFullDir)){
						mkdir($currentMailBoxFullDir, 0777, true);
					}
					file_put_contents($currentMailBoxFullDir.$fileUploadName, $content );
					...
					$attachment = [
						...
						'content'      => $fileUploadName,
						'content_dir'  => $currentMailBoxDir.$fileUploadName,
	...
	protected function embed_images(array $email)
	{
		foreach ($email['attachments'] as $key => $attachment)
		{
			if ($attachment['disposition'] === 'inline' && ! empty($attachment['reference']))
			{
				...
				$img_content = file_get_contents($attachment['content_dir']);
				//$img_embed = 'data:image/' . $attachment['type'] . ';base64,' . base64_encode($img_content);
				$img_embed = base_url() .$attachment['content_dir'];
	...
	public function search_imap($type)
	{
		return imap_search($this->stream,$type);
	}
