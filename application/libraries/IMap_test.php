...
class IMap_test {
	...
	public function get_mailboxes($host) 
	{
		try {
			$results = imap_list($this->connection,"{" .$host. "}", '*');
		} catch(Exception $e) {
			$this->CI->template->error("Invalid Search: " . $e->getMessage());
		}
		return $results;
	...
	public function move_to_folder($msg, $folder) 
	{
		try {
			$results = imap_mail_move($this->connection,$msg,$folder);
